  ...
  return `<section class="book-detail-private" data-book-detail-panel role="dialog" aria-modal="true" aria-labelledby="book-detail-heading" tabindex="-1">
    ...
    <nav class="book-detail-tabs" aria-label="书籍内容入口">
      ...
      <div class="book-detail-tablist" role="tablist" aria-label="标注内容">
        ...
        <button type="button" role="tab" id="book-detail-tab-notes" data-book-detail-tab="notes" aria-controls="book-detail-panel-notes" aria-selected="${String(!highlightsSelected)}" tabindex="${highlightsSelected ? "-1" : "0"}">老己笔记</button>
      </div>
      <span aria-disabled="true">PPT作品</span>
    </nav>
    <div class="book-detail-flow">
      ...
      <section class="book-detail-section book-detail-tabpanel" id="book-detail-panel-notes" role="tabpanel" aria-labelledby="book-detail-tab-notes"${highlightsSelected ? " hidden" : ""}><h2 id="book-detail-notes-title">老己笔记</h2>${renderDraft(snapshot)}${snapshot.notes.length ? snapshot.notes.map(renderNote).join("") : `<p class="book-detail-empty">还没有老己笔记。</p>`}</section>
