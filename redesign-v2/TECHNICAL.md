...
微信读书书架与批注同步以账户内 requestId 幂等的持久化 run 驱动；排队、运行、完成、暂停与失败状态写入 PostgreSQL，进程中断的运行项可回到队列并由单一领取者恢复。成功页原子替换当前可见快照；暂停或失败只更新运行状态和安全错误信息，继续返回上一次完整书架或单书批注。连接被替换后，旧 run 的迟到结果不得覆盖新连接快照。该数据门仍不代表 worker 已启动、HTTP runtime 已注入或真实腾讯调用已经完成。
