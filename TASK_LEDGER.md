...
- **Goal**：当前有终点里程碑为“Desktop `M1-F4-A` 免费体验领取在当前 main 完成候选审查、真实 Chrome / DB Case、必要集成与状态收口；并行将 Mini Drawer v2 在唯一 `f312` 推进至 writer 自验与非作者 EARLY verdict”。`M1-F3B-A2` responder contract 是本 Goal 的新增范围：只推进 development fake / 无配置 fail-closed 合同、非作者审查与 main 组合接缝，不把 fake 冒充真实自由聊天。唯一项目总控任务仍为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`；Mini 环境不得阻塞 Desktop。
- 当前执行波次：`GOV-9967BB8-RULE-LOAD`（原子控制事务；仅正式规则加载、loaded ACK、控制指针与治理 lint，不实施、不集成、不操作共享 GUI）
- 当前活动项：无
- 协调下一动作：本收据后立即 yield；下一短事件恢复两个互斥续作点：冻结 responder 4文件组合 checkpoint 并完成 main 双模式 Case；原 Mini writer按静态预审 `REWORK` 修正 panel-bottom / slot 几何与4项失败测试、补三背景可审合成证据，未通过前不得进入唯一 `f312`。
- **已加载规则基线**：本控制事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@51d967cecb4ff6f65d8beea136fcf4b4ed741e57` 复核；main另有总控独占的4个tracked responder组合接缝checkpoint，9组受保护未跟踪路径仍在。总控已从真实全局路径完整重载 `adaptive-delivery` 正式提交 `9967bb8a492a905799c6b5692625c454acf2e8ef` 的 `SKILL.md`、`references/long-task-governance.md` 与 `references/methods.md`；远端 `refs/heads/main` 为该提交，三份远端文件与全局文件逐字节一致，SHA-256 依次为 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`、`a0f6abe6298e9063a151b3527dbf0636a66598144701921f79d28506a5cf09c5`、`109f5fabddb39dad44dd238624842baa1c094111cbc4818ebd16b7225a021e59`。主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。正式规则 delta 仅改变控制事件、四证 ACK 恢复、规则 loaded ACK、未集成候选、唯一 GUI 与换总控门，不改变产品 / 视觉事实。
- **活动实现 / 复验线**：Desktop quota仍 `DONE@ed489a8`。Responder两提交已非作者复审并顺序进入 `main@2ba58fd + 66aecec`；总控在 `.env.example`、`conversation-responder{,.test}.ts`、`index.ts` 保留显式development opt-in / 默认fail-closed的RED→GREEN未提交checkpoint，本控制事件暂停实现、提交与运行验收，故整体仍 `VERIFY`。Mini writer在 `mdrv2@d17bf52` 已形成三源码 + 新v3 asset首个runnable未提交diff并按停止门结束；只读静态预审判 `REWORK`，`f312`仍 `23d6371`、尚未集成。规则发布时受影响 live 清单为唯一总控 `/root` 与 `/root/mini_drawer_early_review`：两者均已回 `LOADED ACK 9967bb8a492a905799c6b5692625c454acf2e8ef`；notified=`mini_drawer_early_review`，ACK pending=`0`，loaded=`/root, mini_drawer_early_review`。`mini_drawer_rework_writer` 与 `conversation_responder_review` 在发布前均已完成 / 停止，不是 live 任务；本事件中断数 `0`。收据时仅总控 `1 / 4`、可用 `3`。
- **READY 重算**：当前 Goal内下一事件有两项互不冲突的 `READY` 续作：总控冻结4文件 responder组合checkpoint并完成main fail-closed / 显式fake API与Chrome Case；原 Mini writer按 `REWORK` 修正既有四文件checkpoint并先闭合定向测试 / 三背景证据，再由总控决定是否顺序进入唯一f312。本规则发布回合按用户明确停止条件不顺手实施、集成或派发，故两个READY均在本收据后事件触发；DeepSeek本机安全密钥入口与provider seam仍为后续 Goal候选，不静默扩入当前Goal。
- **下一用户可见检查点**：下一短事件分别闭合 responder组合checkpoint的范围单一提交与main双模式Case，以及Mini v3修正后的定向绿灯 / 三背景证据；只有后者通过静态门才进入唯一f312的320同态checkpoint。二者均不冒充真实自由聊天；当前Goal收口后，后续Goal再启动DeepSeek本机安全Key入口与provider seam。
- **真实阻塞 / 授权门**：真实文本模型验收供应商已由用户确认为 `DeepSeek`；精确模型、当前官方计费、Key与调用上限仍 `MODEL / CREDENTIAL / COST PENDING`，当前未核官方目录、未调用。真实调用前必须只查DeepSeek官方当前目录并先声明模型、预计次数与费用上限；Key不得要求贴入聊天，也不得进入Git、命令参数、日志、截图或台账，优先产品内加密配置，未完成前只可用不回显本机输入 / 临时环境注入。该门不阻塞contract / fake / 无凭证fail-closed，但不能宣称自由聊天完成。
- **当前可见入口 / revision**：事件复核主线为 `51d967c`；Web `127.0.0.1:4276` 与API `127.0.0.1:4212` 的既有入口保留，API已恢复为 `CONVERSATION_RESPONDER_MODE` 未设置的默认 fail-closed 进程且 live / ready，未把健康检查扩大为新业务验收，quota既有真实收据保持。当前对话仍不是真实DeepSeek自由聊天。Mini唯一常驻DevTools路径仍为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、HEAD `23d6371`；recent-project只保留f312，禁止writer另建入口。
- **受保护现场**：main 的短暂 clean 是旧 handoff 搬移造成的假象，禁止写入结论；现已恢复并重验为 9 组未跟踪路径：3 个 Word 锁文件 SHA-256 均为 `abc74d95f4f537ceb8bdab4421e4c1bae6be3a899423649df3be61b3be38013e`，J02 为 `a78426a2b3807e44a7a2a79fc79eb3534fa04eaa8550057a1b22bb6d3dd60bcc`，J03 为 `0eed6879e60f51dc71479db756626ba88491c7fba964837199348dde2a78d0a9`，长期记忆 DOCX 为 `176bebe713fd223642dbf394974053d9ea53a2deb544832fc42e7d91497baea3`；`mobile-drawer` / `local-cover-candidates` / `shared-shell` 目录清单 SHA-256 分别为 `0ff73069ffc1f409a2d70a03b3026245832156880418c62eab7a899b408ba49b`、`30fa25870af4108fe47a50e85114c56000807d98da0cfd16318460df41707233`、`8e5b5bb1bfaceeba72e9d7f2843d904ee76329a4cdfda31ed4298625e831c6f9`；均与冻结 `a233` 副本逐字节一致，不暂存、不修改、不删除。
...
| `M2` 微信小程序与双端闭环 | `VERIFY` | 客户端可独立的运行壳、状态与视觉先行；真实身份、书籍、阅读持久化、会话、额度和 PPTX 按上游能力逐项接入 | 当前无active writer；Drawer v3为未集成checkpoint，完整双端V1仍依赖对应M1能力、真实开发者工具 / 真机与外部授权 |
...
| `M1-F2-D` | `ACTIVE` / 项目总控 | 正文标注、想法、手工老己笔记、引用到会话、书籍详情和作品入口父闭环；不含 AI 整理笔记 | 按文本、PDF、入口真实依赖拆分；子项未全部通过前父项不得 `DONE` | 总控独占共享 contracts / schema / route / entry；子项互斥写入 | 文本入口、reader 768与当前书PPT handoff均已main scoped FINAL；`D-PDF` 等 C，真实删除 / 200% / DPR2及引用到会话等边界仍开放 |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3A-A` | `PENDING` · `NEXT-GOAL CANDIDATE / DEEPSEEK TEST PROVIDER CONFIRMED` / 待分配 | 四家固定文本模型目录、API Key 信封加密、检测后原子替换、撤销、脱敏和桌面设置 UI 的完整闭环；当前最小真实验收先走DeepSeek，不开放自定义端点 | `M1-F1-V1`；DeepSeek精确模型 / 官方计费 / Key / 调用上限pending；实施或真实调用前重验DeepSeek官方当前目录 | CORE、API、DB、WEB、QA、DOC | 本机安全Key入口与DeepSeek provider seam在当前Goal后展开；密钥不回传 / 不进聊天 / Git / 命令 / 日志 / 截图 / 台账，失败保留旧配置和输入，成功不自动重放；候选`3e4b020`仅作种子 |
...
| `M1-F3A-V1` | `PENDING` / 待分配 | 完成模型配置本地闭环，并以DeepSeek取得至少一次脱敏H4最小真实对话收据 | A、B；用户已授权SelfAlone最小真实对话验收且供应商确定为DeepSeek，精确模型 / Key / 官方计费 / 调用上限仍pending | QA、DOC | 假适配器证明保存 / 失败 / 替换 / 撤销；真实Key只经产品内加密配置或不回显本机注入；调用前先声明模型、次数与费用上限，H4仅记供应商 / 模型 / 次数 / 结果，不记录密钥 |
...
| `M1-F3B-A2-RESPONDER-CONTRACT` | `VERIFY` · `MAIN MERGED 2ba58fd + 66aecec / COMPOSITION CHECKPOINT UNCOMMITTED` / 项目总控 | 窄 `ChatResponderPort`、完整顺序上下文、非回显development fake，以及无配置 / 空回复 / 调用失败fail-closed合同已进main；不实现真实供应商、凭证、成本或额度 | 非作者复审Critical / Important均无；总控组合checkpoint要求默认无mode失败关闭、只有显式development + development环境启用fake。4文件尚未提交 / 主线Case未跑，故不DONE，也不能替代DeepSeek或真实自由聊天 | main两提交改responder / store四文件；当前总控独占 `.env.example`、`conversation-responder{,.test}.ts`、`index.ts` 未提交diff | 候选定向15及非作者复审通过；组合RED为缺少显式mode resolver，当前定向16 + Server typecheck已GREEN。下一事件审实际diff、build / diff-check、范围单一提交、main无mode 503保留与显式fake上下文API / Chrome Case |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `mdrv2 RUNNABLE DIFF / NOT IN f312` / 项目总控 | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；320旧候选EARLY FAIL保持，v3仅是未集成runnable checkpoint，不冒充视觉通过 | writer在 `mdrv2@d17bf52` 形成nav-drawer三源码修改 + 新v3 asset未提交diff并按首个runnable停止；新治理两SHA已LOADED ACK、diff保留；唯一f312仍`23d6371`且不直接写 | 下一事件由总控审实际diff / 资产三背景证据，顺序放入唯一f312后只先做320 writer自验与新EARLY；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| --- | --- | --- | --- |
| `nav-drawer` | `11-mobile-conversation-drawer.png`（`9e764e60189845b6f46eb1181c97bbc622a363c067274ae479cfb7a15c68c36e`） | 打开 / 关闭、长会话、底部导航、iOS / Android 安全区、各宽高与命中 / 溢出 | `NORMAL COVERED / VISUAL FAIL REOPEN`：用户同态截图 `1af1637…35f4` 证明运行时仍有横向矩形接缝，山亭位置 / 尺度 / 上缘渐隐与列表 / 底栏节奏偏离；旧 hasAlpha / 三宽 PASS 失效。新门为非ImageGen多背景合成、320 / 360 / 390 / 430同状态、五态 / 长列表、console / overflow、EARLY后FINAL；键盘仍独立未覆 |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `STATIC PRE-REVIEW REWORK / WRITER STOPPED / NOT IN f312` / 项目总控 | ref11与Drawer专用山亭asset同态返工；旧320硬FAIL与禁止项保持，当前v3 checkpoint尚无真实端结论 | `mdrv2@d17bf52`有三源码 + 新 `mobile-drawer-landscape-transparent-cropped-v3.png` 未提交diff；非作者静态预审发现 panel-bottom / slot 几何未闭合、定向17项中4项失败且缺少三背景可审合成图，判 `REWORK`；未导入DevTools、未写f312 / dist / config / lock / QA | 下一事件由原writer在既有四文件边界修正几何 / 测试并补三背景证据；静态门通过后才可冻结候选并顺序集成唯一f312，仅320通过writer自验才交非作者新EARLY，不在结构 / Alpha门提前PASS |
...
| `M2-F0-A` 当前候选 | main已含`35bad75`长列表与`14b69dc` Settings；唯一`f312@23d6371`仍是v2 EARLY FAIL现场；`mdrv2@d17bf52`另有三源码 + 新v3 asset未提交runnable diff | Settings / 长列表旧scoped FINAL仍`FAIL / NO-GO`。新治理loaded ACK闭合，writer已按首个runnable停止且未导入DevTools；下一事件由总控审查 / 冻结 / 顺序集成，保护config / lock与既有QA不动。 |
...
| 当前 Mini 候选波次 | Reader / Conversation既有限定结论保持；长列表`35bad75`与Settings`14b69dc`已在main但scoped FINAL为NO-GO；唯一f312仍是`23d6371`旧EARLY FAIL，v3只在mdrv2未提交checkpoint | recent-project清理`CLOSED`、原生菜单唯一、query 18项已送达；writer规则loaded ACK后停止，保护现场与历史源码/worktree均未删除。 |
| 当前 Desktop main 预览 | `RUNNING / EVENT BASELINE 66aecec / QUOTA H3 PASS / RESPONDER VERIFY / FREE CHAT NOT WIRED` | responder contract两提交已进main，4文件显式fake / 默认fail-closed组合checkpoint未提交；Web / API health ready不代表该checkpoint业务验收。DeepSeek仅确认供应商，模型 / Key / 计费pending。 |
| 开发基线 | 本台账事件父为`main@66aecec90f7bdd326951b715b38c45c38c588d65`；本轮控制提交只写`TASK_LEDGER.md` | main总控独占4个tracked responder checkpoint保持未提交；6个受保护DOCX / Word锁文件与3组视觉候选目录仍与冻结a233副本同字节；Mini唯一f312保护修改 / QA未改变。 |
...
- 当前外部授权门：验证合格后的本地 `main` 合并已授权；用户已授权SelfAlone当前开发 / 验收的最小真实文本模型对话Case并确认供应商为DeepSeek，但精确模型、官方当前计费、预计次数 / 费用上限与安全Key录入未闭合前不得调用，也不扩展为生产、其他项目或无限调用。远端push、PR、发布、部署、真实邮件 / 微信、微信读书 / Presenton及其他付费调用和小程序审核均未授权。外部图片生成仅限已授权的20次总额度并受上表逐次记录约束。
