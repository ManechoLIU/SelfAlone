...
- 当前执行波次：`CONTROL-RESET-1`；Web `47db65b` 的非作者终验已判 `FAIL / REWORK`，原作者已恢复并在 4 个限定 Web 文件中修正 1024px 覆盖、生成中锁定文案与纸白 / 青瓷密度，总控全仓 24 files / 133 tests、typecheck / build / diff-check 全绿，但真实 Chrome 与冻结提交尚缺。D-TEXT 原作者也已恢复，13 个私有 Web 文件的定向 41 项与全仓 30 files / 145 tests、typecheck / build / diff-check 全绿，仍需同一真实 Chrome Case 与提交。小程序已推进到 `a952a49`，其中 `79c9d8b` 关闭 PPT failure→retry 局部恢复，`e4d94b4` 补四尺寸机械矩阵，`a952a49` 关闭 Library direct-failure 保留书架与 retry；父项保持 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 为 D-TEXT 私有唯一写入者，状态 `ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING`，共享入口继续排队。原 Web 作者 `/root/desktop_shell_m0_fix` 已恢复为 `running`，4 文件 WIP 状态 `ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING`；非作者仍正常 idle 等待冻结候选。小程序同一 writer 已提交 `79c9d8b → e4d94b4 → a952a49`；总控复核最新 7 文件范围、保护哈希、12 files / 63 tests、typecheck / build / diff-check 与 320 / 360 DevTools 收据通过。UI/UX 保持 `QUEUED / CANDIDATE MOVING`，两个受保护差异保持未提交。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4 → a952a49` 已形成十四个局部恢复点；`a952a49` 仅关闭 Library direct-failure 保留 3 本已知书、原位 alert 与唯一 retry | 总控核对 `a952a49` 的 7 文件范围、保护哈希、commit diff-check、12 files / 63 tests、typecheck / build 与 320 / 360 DevTools 收据；390 / 430 未重复取证且所有图仍 keyboard=0。PPT 预览仍为开发适配器且不冒充真实产物；drawer 正式山亭仍 ASSET BLOCKED；父项与 H3 均不升级 |
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@e2a6ac4`、`codex/m1-f2-d-text-web`；非作者已完成独立真实 TXT / EPUB、Selection、持久化、失败 / 并发、主题 / 专注、AX 与两视口复验并清理环境 | 当前 13 个私有 Web 文件 WIP 持续收口；总控定向 5 files / 41 tests、全仓 30 files / 145 tests、typecheck / build / diff-check 通过。原作者已恢复 `running`，停止条件仍为同一真实 Chrome Case 重验 + scoped commit；全局路由在 Web shell 与私有候选分别通过后由总控顺序接入 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main 旧 M0；`6afd@47db65b` 非作者已覆盖 requirements / outline / template / 真实 generating / failed / completed、四宽、路由 / 草稿 / scroll / focus / retry / AX / console 并清理环境；原作者当前只改 conversation view / test、desktop-shell test 与共享 styles 4 文件 | 4 文件 WIP 持续收口，已处理 1024px compact、生成中精确锁定文案与纸白 / 青瓷密度；总控全仓 24 files / 133 tests、typecheck / build / diff-check 通过。原作者已恢复 `running`，仍须隔离端口真实 Chrome 四宽×844、各阶段输入可见和首次进入 / 往返同源 Case、scoped commit，再由非作者按 02 终验；禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / FOURTEEN CHECKPOINTS COMMITTED / DESIGN-READY / STATE-MATRIX REWORK / LIBRARY FAILURE CHECKPOINT / REVERIFY QUEUED / CANDIDATE MOVING` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `79c9d8b → e4d94b4 → a952a49` 依次关闭 PPT failure→retry 局部逻辑、四尺寸机械矩阵与 Library direct-failure 书架保留 / retry；它们都不是完整系统冻结候选。旧 `598e34f` 已过期，UI/UX 保持 `QUEUED / CANDIDATE MOVING`；软件键盘仍未实显，正式山亭与真实 PPT 预览缺失，全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4 → a952a49`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 十四个局部点已冻结；`a952a49` 只关闭 Library direct-failure 保留书架与 retry。软件键盘、正式山亭 / PPT 资产、其余状态与最终全路由视觉均未完成；既有受保护差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4 → a952a49` 是十四个局部恢复点；Reader / Library / PPT failure checkpoints 与机械矩阵不替代全小程序六页 / 共享层四宽四高和状态矩阵，真实软件键盘、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
