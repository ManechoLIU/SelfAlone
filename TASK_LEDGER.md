...
- 下一可见检查点：`M1-F4-B-REWORK-03` 已形成 clean 候选 `f47a99044693210d738126aeffee8dd30675d3cc`；总控独立复验 migration `12/12`、cost-ledger store `4/4`、Server typecheck、diff-check 全 PASS。旧 Kimi 非作者审查语义上 `REVIEW_PASS` 且完成多组反例攻击，但首个 repo tool 之前缺 delivered ACK，因此仅保留为辅助证据、不得作为新规则 required review 收据。当前显式 `M1-F4-B-REWORK-03-REVIEW-TDD` 进入 READY：对同一 candidate 做 ACK-first 的 TDD 因果证据审查，绑定修复前同案 RED、精确 candidate、修复后同案 GREEN、counterexample 与 PASS/FAIL；通过后才允许 main 集成。
...
| `M1-F4-B` | `VERIFY / CANDIDATE f47a990 / CONTROLLER GREEN / NONAUTHOR REVIEW REQUIRED / CURRENT GOAL` / 项目总控 | 新候选 `f47a990` 仅含 migration 两 owned files；总控独立 migration 12/12、store 4/4、Server typecheck、diff-check 全绿。历史 `f53c930` 已被新 HEAD supersede，不再作为可集成候选 | 必须新的非作者 reviewer；PASS 后才允许集成 main，FAIL 则返同一 Assignment 修复 |
| `M1-F4-B-REWORK-03` | `VERIFY / CANDIDATE f47a990 / CONTROLLER GREEN / TDD REVIEW REQUIRED` | 同一 Grok 4.6 writer/session 完成两文件 TDD rework并形成 clean HEAD `f47a99044693210d738126aeffee8dd30675d3cc`；总控独立 migration 12/12、store 4/4、Server typecheck、diff-check PASS；旧 Kimi review 语义 PASS 但 ACK 顺序不合新 required-review 收据，只作辅助证据 | 冻结 candidate；由显式 `M1-F4-B-REWORK-03-REVIEW-TDD` 对同一 revision 做 ACK-first TDD causal review；PASS 后由总控集成并跑 current-main 回归 |
| `M1-F4-B-REWORK-03-REVIEW-TDD` | `ACTIVE / NONAUTHOR TDD CAUSAL REVIEW / Kimi K3 / candidate f47a990 / DELIVERED ACK PASS` | ACK-only 回合严格 `tool_call=0` 后送达完整 delivered ACK；新 revision Assignment lease 已按 `primary_goal / success_criteria / owned_scope / forbidden_scope / parallelizable` 全字段 PASS。same-case 现场：pre-fix `f53c930` + exact candidate test SHA `d0496518…508d0` = migration `2 FAIL / 10 PASS`；candidate `f47a990` 同一 test = `12/12 PASS`。reviewer 必须独立核 exact revision/test SHA、RED→GREEN 同案因果及至少一组 counterexample，输出 formal PASS/FAIL receipt | 只读审查进行中；不得修改 candidate；FAIL 返同一实现包，PASS 才进入 main integration |
