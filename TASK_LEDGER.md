...
| `M1-WEB-SHELL-01` | `VERIFY / FROZEN 9638d12 / NONAUTHOR FINAL ACTIVE / NOT IN MAIN` / `/root/desktop_shell_unify` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | `6afd@9638d12`（父 `221d5bb`）为范围单一 5 文件 `+107/-98`：conversation / library / settings 复用 `renderDesktopRail` 单一 DOM / active / token；仅既有 `?? data/` 未跟踪。与 main merge-base 为 `0969c33`，禁止盲目 cherry-pick | 作者收据：`pnpm verify` 137 tests、typecheck / build 通过；Chrome 1440 / 1200 / 1024 / 768×844 覆盖 fresh、切换、刷新、前后退、`?stage=`，单 rail、输入可见、无横溢、console 0。API 未监听，因此仅证明缓存 / 断网保留态；UI/UX 正在对当前 HEAD 做 FINAL。verdict 后由总控审查完整差异 / 冲突并决定安全集成，通过前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY / VISUAL REWORK FROZEN a0e98f9 / NONAUTHOR FINAL ACTIVE / H3 BLOCKED / NOT IN MAIN` / UI/UX + `/root/mini_visual_rework` | `f312@a0e98f9`（父 `f81b463`）冻结 13 文件 `+81/-30`；Reader 标题三档 `28/36`、`26/34`、`22/28`、`max-width:12em`，drawer 唯一列表滚动、纸白开放行、140px 无图 scenery slot。保护差异仍未提交，台账与保护文件禁止触碰 | 作者收据：定向 14 / 14、miniapp 65 / 65、typecheck / build / diff-check 与 DevTools 320 / 360 / 390 / 430 + font23 通过；同一 UI/UX 正在 FINAL。正式山亭、软件键盘、游客凭证 / console、真实 PPT preview 与 H3 均未闭合；完整父链集成前不得升级父项 |
...
| `M1-WEB-SHELL-01` 当前候选 | `0969c33 … 221d5bb → 9638d12`，状态 `VERIFY / NONAUTHOR FINAL ACTIVE / NOT IN MAIN` | 末提交范围单一但分支与 main merge-base 为 `0969c33`；必须审完整候选差异与 main 冲突，不能盲目 cherry-pick。作者真实 Chrome 的 API 未监听，仅证明缓存 / 断网保留态；UI/UX verdict 后再决定集成。 |
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4 → a952a49 → f81b463 → a0e98f9`，状态 `VERIFY / NONAUTHOR FINAL ACTIVE / H3 BLOCKED / NOT IN MAIN` | `a0e98f9` 是当前冻结视觉修正点，但 main merge-base 仍为 `ff4767b`；集成必须覆盖整条链并保护 project config / lock 未提交差异。正式山亭、软件键盘、PPT 资产 / 真实预览仍缺；`IMG-04` 未实装。 |
