...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复、北京时间双时段计费、真实 Store 并发硬上限与失败后重试本地门已分别在 `main@b637023+81f5f1f+df32890+ba70ced` 闭合。真实 Provider H4 继续等待三项外部条件；项目级旁路 `M1-F5-A1` 原候选 `8725581` 经非作者实证 PostgreSQL `int4` 边界 Important FAIL，clean 修复候选 `301026d` 已由 `M1-F5-A1-INT4-VERIFY-02` 核验 GREEN，现由 `M1-F5-A1-INT4-NONAUTHOR-03` 复审；不得重做或并行写。
- 当前阻塞：`M1-F4-V1` 的本地实现与验证已清空，真实成功路径仍缺外部 H4。无泄密检查已确认当前 shell、监听 4322 的 Server 与仓库可见普通环境文件（仅 `.env.example`）均未提供平台专用 Key 或价格变量，既有用户模型 Key 授权不静默扩展为平台凭证；唯一唤醒条件是安全注入独立平台 Key、与官方当前两时段一致的六项人民币微元价格快照，并取得本次真实付费 Provider 调用授权，之后才验证真实领取→Provider→成本流水→硬上限→浏览器恢复。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
...
| `M1-F4-V1` | `VERIFY / LOCAL GATES CLOSED / MAIN b637023+81f5f1f+df32890+ba70ced / EXTERNAL H4` / 项目总控 | 领取、AI / PPT消耗、失败恢复和并发硬上限 Case；正式 Server 环境组合、fake HTTP usage 结算、真实 PostgreSQL ACK lost 恢复、北京时间双时段计费、非法配置拒绝、真实 capability + Store 并发硬上限与失败释放后重试均已有 current-main 代码门 | 本地可完成项已清空；真实 Provider H4、真实成本流水与浏览器成功路径只在独立平台 Key、六项当次价格快照和明确付费调用授权同时到位后唤醒。等待期转项目级无冲突 READY 旁路，不把 fake / 单测升级为 H4 |
...
| `M1-F5-A` | `VERIFY / A1 BACKEND CANDIDATE` / 项目总控 | 对话 / 书籍双入口、单书来源、范围 / 需求与任务工作区 | `M1-F5-A1` 的初版候选已取得非作者 FAIL，窄修复候选已完成 Writer 核验，当前只待非作者复审；通过后再接 Web 真实入口。已闭合 Mini handoff 不重复开发、不触碰 f312 现场 |
| `M1-F5-A1` | `VERIFY / CANDIDATE 301026d / NONAUTHOR REVIEW ACTIVE` / 项目总控 | 真实账号 + 已发送用户消息门；当前会话内创建独立 PPT 草稿；来源关系按集合建模且 MVP 恰好一本书；固定字段仅用途、受众、页数范围、补充要求；换书保留通用要求 | 原 clean 候选 `8725581` 经非作者确认 Important：JS safe integer 会穿透 PostgreSQL `int4` 并被映射500；真实 RED 为 routes 10过1败、migration/store 6过1败。clean `301026d` 仅改 routes/store 四文件，补 `2_147_483_647` 边界；Writer 核验定向13、Server270、typecheck、diff-check均PASS。`M1-F5-A1-INT4-NONAUTHOR-03` 已 delivered ACK，当前复审；禁止模型、联网、模板、生成、下载、端侧视觉及并行重做 |
...
| `M1-F5-A1-INT4-NONAUTHOR-03` | `ACTIVE / DELIVERED ACK / READ-ONLY` / `/root/pricing_schedule_review` | 只读复审 `8725581..301026d` 四文件，验证上一唯一 Important 是否关闭 | stop=给出 Critical / Important / Minor 与 PASS/FAIL 后立即 TERMINAL；不得写入、派发或集成 |
