...
- 下一可见检查点：`M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` 已显式任务化并进入 READY；立即派 Kimi K3 Writer 从 current main 隔离 worktree 做6文件 adaptation。必须先给 delivered ACK，再以 current-main 旧 v1 pre-send taskId / immediate-sheet 行为形成 RED，保留现有 hydration / persistence / conversation API 语义；禁止整支 f312 合入。并行 `M1-F4-B` 继续保持 BLOCKED。每次状态变化后继续项目级重算 READY。
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY / F312 cf6a8c5 / SELF PASS / NONAUTHOR PASS / MAIN ADAPT READY` | 累计 `0fa1447 + cf6a8c5` 语义由独立 Kimi reviewer 判定符合 SPEC 143–145：当前会话复用、pre-send 无 task/sheet/auto-send、可编辑预填、失败/刷新恢复与 stale fencing 均通过只读审查；current-main 现有3文件基线测试59/59 PASS | current-main adaptation 由 `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` 驱动；不得整支 f312 合入或由总控手改实现 |
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` | `READY / Kimi K3 WRITER / CURRENT-MAIN 6-FILE ADAPT` / 待派 | 仅拥有 `apps/miniapp/src/core/ppt-intent.ts`、`ppt-intent.test.ts`、`pages/conversation/index.ts`、`index.test.ts`、`pages/reader/index.ts`、`page-state.test.ts`；以 reviewer PASS 的 `0fa1447 + cf6a8c5` 语义与 f312 `f6a08f9` 仅作参考，禁止整支 cherry-pick/merge | 从派发时 current main 建隔离 worktree；先 RED 锁住旧 v1 在书籍入口提前 taskId/awaiting-confirmation/sheet 的缺口，再 TDD 重织 `onShow/beginSend/persistLocalState` 并保留当前 hydration/persistence/API 行为；禁止 install、lock/config/visual 修改。候选需定向测试、Mini 全量、typecheck、build、diff-check 后交总控，Writer 不集成 |
