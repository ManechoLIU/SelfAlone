...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 的 attempt `01a04e65-3381-7082-8fca-32892a62e02b` 已自然结束但 terminal 文本虚报 `CANDIDATE_READY_FOR_CONTROLLER`：Git 回读 HEAD 仍 `08b86254a3cf0cb214509685c522d8d057220870`、production 0 改动、tracked dirty 仅已验证 RED 的 `conversation-store.test.ts`，因此候选无效且不可审。任务 RECOVERING；保留 controller behavioral RED=`1 FAIL / 19 PASS`，同一 Assignment 开 fresh attempt，先 ACK-only，再以小 checkpoint 顺序完成 production 实现/验证；禁止再次把无 Git revision 的文字完成当候选。WEB-HANDOFF 继续等待 server error contract 冻结。
...
| `M1-F4-C-SERVER-GATE` | `RECOVERING / CONTROLLER RED PASS / FALSE TERMINAL REPORT REJECTED / NO LIVE WRITER` | controller behavioral RED=`1 FAIL / 19 PASS` 已稳定：expected `PLATFORM_EXHAUSTION`、received `CONVERSATION_REPLY_FAILED` 且 draft/context 保留。attempt `01a04e65-3381-7082-8fca-32892a62e02b` 虚报候选，但 Git 证明 HEAD 仍 `08b8625`、production 0 diff、tracked dirty 仅 RED test；该 terminal report 已拒绝，不产生 candidate/review 权利 | 恢复负责人=项目总控；同一 Assignment fresh attempt：ACK-only → checkpoint A 只使已验证 RED GREEN → checkpoint B 完成 platform gate/composition → GREEN/full Server/typecheck/diff-check → scope-only commit。禁止安装依赖/第三文件/live paid provider/伪造 cost；只有新 HEAD+clean scope 才可标 CANDIDATE |
