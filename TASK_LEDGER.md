...
- 下一可见检查点：`M1-F2-WEREAD-WEB-REWORK-01` 已完整 ACK并从 clean `dfb0574` 恢复；先交 fresh disconnected、账户 A/B 隔离、unknown local bookId、不显示常态手动同步、成功空快照与同 requestId 重试 RED。旁路 `M2-F2@73a0f11` 已经 root actual-diff判 FAIL，恢复 Assignment 也已完整 ACK并从 clean candidate开始行为 RED；唯一 f312 仍为 `4cdce97` 与既有保护现场，未写入候选。
