...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / SYSTEM VISUAL REWORK / SIZE PASS 3fed4c7 / H3 BLOCKED / NOT IN MAIN` | 用户最新 430×932 证据否决 conversation + drawer 整体视觉；旧 Reviewer 的 Reader / Alpha / 四宽机械 PASS 只保留为局部证据。`3fed4c7` 仅把 runtime 从 865,820 B 降至 165,998 B，并以 13 files / 69 tests、typecheck / build、dist=1552 KiB 关闭机械包体门；保护 project config / lock 与旧 EARLY 图未纳入 | `/root/mini_visual_final_review` 只读重开 DESIGN-READY；conversation 私有 writer 等 brief 后释放。软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION+DRAWER DESIGN-READY ACK PENDING / IMG-04 ASSET ALPHA VERIFY` | 以 `main@3199945`、13 / 30–32 修正 conversation 的选择层→轻摘要→单一阶段入口、正常模式内部文案、消息 / 任务层级、空白和输入趴宠；按 11 修正 drawer 全层级尺度 / 留白 / 山亭关系 | `IMG-04` runtime 的真实 Alpha与 size `3fed4c7` 可复用，但整体实装视觉已被用户推翻。UI/UX reviewer 已恢复但尚无真实 ACK；conversation writer 不提前派发。drawer 与 conversation 保持单一 owner、互斥文件边界；320 / 360 / 390 / 430、font23、safe-area、EARLY / FINAL 必须由非作者真实 DevTools 复验 |
