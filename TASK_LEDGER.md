...
| `M2-F2` | `ACTIVE / M2-F2-WEREAD-MINI-REWORK-02 / 552a6ef+LOCAL` / `/root/m2_f2_weread_mini_writer` | 原 Writer 已 delivered loaded ACK `main@ba3a7af`；既有 Assignment 不改派、不重做，当前9文件 WIP已锁定分页、双身份、失败 / paused保留、未知进度、stale fencing、同requestId retry与no-call行为。一次 Kimi ACK-only调用结果未知且工作树零新增，已停止自动重试 / 降级 | 从已定位的失败保留回归恢复，补pagination、账户 / 连接 / book stale fencing、投影`bookId`与`progressPercent:null`，定向 / Mini全量 / typecheck / visual-contract / diff-check后冻结候选；全屏配置页仍需独立DESIGN-READY，f312 / Server / contracts不写 |
