...
- 当前执行波次：`CONTROL-TAKEOVER-2`；持续所有权合同已在 `main@528b86b` 范围单一提交，写入前已核对主线基线 `main@8b52f22`；新总控已回传 `CONTROL CONTRACT LOADED main@9764180`，其 `AGENTS.md` SHA-256 为 `703a6942…66b7`。自然语言续作快捷词已由治理提交 `main@4d32e24` 新增，当前 `AGENTS.md` SHA-256 为 `bbecabfc…1a76`；唯一受影响 live Agent 为本总控，已回传 `CONTROL SHORTCUT LOADED main@4d32e24`，现有开发 / 验证任务范围与停止条件不变。旧总控 `01a034b4-a73d-7ce2-8531-51585826e6d3` 已原样回复 `OLD CONTROLLER FROZEN`、最后一轮 `interrupted`、revision `32` / `notLoaded` 且已归档；其冻结前提交的 `b2f8276 / 2101695 / 5a18abc` 已逐项审查，过期 owner / 入口 / 候选状态已在 `1b72e59` 纠正，首波收据已在 `8b52f22` 登记。主线产品代码未因接管变化，仍只有三个既有未跟踪视觉候选目录。
- 当前活动项：协作树为总控 + `/root/auth_early_review` + `/root/dtext_main_final` + `/root/mini_early_review`，4 / 4 槽占用且三者状态均为 running。Mini 与认证 writer 均已回传 `CONTROL CONTRACT LOADED main@9764180` 和 `CONTINUOUS OWNERSHIP CONTRACT LOADED main@528b86b`；认证 writer 已在 `auth1@1bc8b3b` 提交 core / API checkpoint，留下 4 文件 UI WIP 与真实 EARLY 入口后停止并释放。新非作者认证 reviewer 创建调用已返回但一次有界等待无 ACK，只记 `DISPATCHED / ACK PENDING`；D-TEXT 同样仍待双 ACK，二者在 ACK 前不得升级 `ACTIVE` 或给 verdict。Mini 继续 `ACTIVE / EARLY`，f312 两项保护哈希与未提交现场不变。
- 下一 `READY` / 协调动作：当前 `0` 个空闲槽，不新增重复 writer；认证首个可运行 UI 已按优先级派给 `/root/auth_early_review`，ACK 后才升级 EARLY。首个槽位释放后按已到达事件重算：Mini EARLY `FAIL` 释放对应唯一 writer，D-TEXT FINAL `FAIL` 释放最小修复 writer，任一 `PASS` 则先回收候选并在本地 `main` 顺序集成、复验后再关闭该卡。Mini 通过 EARLY、冻结新候选后才进入 FINAL；`M2-F4-A` 等 F0-A 合入并复验且解除同写边界后才重算为 `READY`，本轮不提前派发或合并。
- main / 隔离入口收据：2026-08-25 21:01 +08:00 复核 main 4127 / 4198 仍未监听，旧 `main@5d89aa5` 可访问收据继续失效；D-TEXT reviewer 的隔离 Web `4311` 返回 200、Server `4310` 根路径返回 404，认证隔离 Web `5174` 返回 200、API `4181` 根路径返回 401。隔离入口存在只证明验收环境已启动，不代表 reviewer 已 ACK、真实 Case 已通过或 main 用户入口恢复；4110 / 4173 仍离线。
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / MAIN MERGED 9143638 / REVIEW DISPATCHED / ACK PENDING` / `/root/dtext_main_final` | `dentry@df96ee3` clean；焦点最小修复已合入。reviewer 的隔离 Web `4311` / Server `4310` 已启动，但 loaded ACK 与真实 Case 尚未回，故不升级 `ACTIVE`；4110 与 main入口仍离线 | ACK 后以当前 committed main内容与隔离 schema完成真实 Chrome；覆盖 1440 / 768、DOM 焦点、PPT 空态 / handoff、console / overflow，不得用进程、旧图或测试冒充 | loaded ACK 后升级 ACTIVE；PASS 只关闭文本入口 main FINAL，不扩大 D 父项；原生 200%、真实删除、真实 AI / PPTX 仍精确 VERIFY |
