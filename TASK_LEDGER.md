...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：缺真实可编辑的阶段返回、刷新丢未提交大纲、需求摘要状态错误、模板未按双列、生成中缺当前页 16:9 骨架、失败态缺真实重新生成 / 修改大纲，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，Reader 私有标注 / 笔记候选已派发且等待工作树收据，共享路由仍待 Web shell 释放；小程序候选链已冻结 `4fc422f → 630c051 → 5e270b1` 三个局部恢复点，但整体仍为六页与共享层 `FAIL / SYSTEM REWORK`，f312 继续独占 `apps/miniapp/**`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 在 `6afd@0e18846` 继续独占本包 Web 文件，并已获准在确需真实阶段回退保存时顺序修改 M0 runtime / route 与测试；原非作者 UI/UX 任务当前 idle，待新冻结 HEAD 后重验。`/root/reading_text_notes_web` 已从 main@`996d8b1` 派发隔离 D-TEXT Reader 私有候选，当前 `ACK PENDING`，在工作树收据前不登记 `ACTIVE`。小程序任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 保持 `ACTIVE`：conversation / drawer、Reader swipe、login / library / settings 三个检查点已冻结，当前连续推进 Reader 控制层 / 底部面板与 PPT 高风险布局；局部资产门和 f312 两个既有禁止差异保持。
- 协调下一动作：Web 作者关闭真实阶段返回 / 持久化、需求摘要、模板、生成骨架与失败动作后冻结新 HEAD，由原非作者按 02 / 04 / 05 / 06 / 33 逐阶段复验；通过后总控集成 / main 复验并顺序接入 D-TEXT 共享路由。D-TEXT 私有候选取得 clean 工作树收据后再升级 `ACTIVE`。小程序继续 Reader / PPT 与其余状态矩阵；冻结全端候选后再按四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 矩阵终验。
...
| `M1-F2-D-TEXT` | `DISPATCHED / ACK PENDING / WEB ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | contracts / migration / API / publisher 已在 main@`f85d86c` 复验；Reader 私有候选已从 main@`996d8b1` 派发，真实工作树收据前不称 `ACTIVE` | 私有 writer 只写 `text-reader-*` 与新增 annotation / book-detail 模块；`main.ts` / shared shell / styles 等 Web shell 合入后由总控顺序接入；旧 `18c7522` 只读 | 真实 EPUB / TXT 选区→划线 / 想法 / 老己笔记 CRUD，失败保留、刷新恢复、owner / fileVersion 隔离、返回原文与绑定参考真实 Chrome 通过 | 共享接缝合并 `f85d86c`；用户可见候选已派发但尚无 ACK / diff / 运行证据，不能称已实现 |
...
| `M1-F2-D-TEXT` | `DISPATCHED / ACK PENDING / WEB ENTRY QUEUED` / `/root/reading_text_notes_web` | 从 main@`996d8b1` 创建隔离 worktree；只允许 Reader 私有文件与新增 text-annotation / book-detail 模块，不碰 6afd 的 `main.ts` / shared shell / styles 或共享 contracts / Server；收到真实工作树收据后才升级 `ACTIVE` | 先完成真实选区三动作、划线 / 想法 / 无标题老己笔记 CRUD、保存失败保留与真实 API / Chrome 候选；全局路由在 Web shell 合入后由总控顺序接入，不得以私有模块或已合入 API冒充用户可见闭环 |
...
| `M1-F2-D-TEXT` 共享集成 | `263d3e2 → 18c7522` 私有种子；`94056be → ed95d95 → 5156b9c → main@f85d86c` | 共享 contracts / migration / API / publisher 已通过原非作者与 main 复验；私有种子不再单独合并。用户可见私有候选已派发但仍 `ACK PENDING`，shared entry 继续等待 Web shell 所有权释放。 |
