...
- 当前活动项：`M1-F2-D`
- 下一可见检查点：fresh store 的 `claim / active / draftFor / updateDraft / complete` 生命周期回归已 GREEN；hydrate 保留服务端 retry 草稿仍 RED，修复后冻结单一候选并重新交同状态 Chrome SELF。
...
| `M1-F2-D` | `ACTIVE / FIRST GREEN / SECOND RED` / `reader_chat_handoff_recovery` | fresh store 未发送草稿恢复及成功发送后清理已1项PASS；hydrate 仍把服务端失败草稿替换为Reader引用并清空 retryRequestId | 同一 `readchat` worktree 完成第二项最小修复并保持不自动发送 / 不重复预填；不得新建worktree，提交 / 废弃后由总控同事件移除 |
