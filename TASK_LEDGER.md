...
- 当前执行波次：`CONTROL-RESET-1`；Desktop `6afd@44e66af` 已获非作者 `PASS`，完整 `0969c33..44e66af` 已以 merge commit `main@deba325` 集成并完成在线 / 断网恢复真实 Chrome 复验。Mini 固定 `f312@2deb4f6 → 3fed4c7` 重开 conversation + drawer + Reader 系统视觉门；DESIGN-READY 以 `main@3199945` 的产品 / 视觉门、`main@4fe6989` 的可展开内容面板合同、`main@35798ab` 的新增视觉 / ImageGen 路由门、`main@54b4eeb` 的 UI/UX 候选裁决授权和 `main@0552ca6` 的矢量图标视觉忠实门为基线。D-TEXT Web 入口 follow-up 已冻结为 `dentry@6e62153` 并重新进入独立 FINAL。台账仍由项目总控独占。
- 当前活动项：用户最新 430×932 证据覆盖 Mini 旧局部视觉结论：conversation 的 PPT intent 常驻双按钮卡、开发边界、消息 / 任务层级、空白与输入趴宠构图，以及 drawer 的尺度 / 留白 / 山亭整体关系均为 `VISUAL FAIL / REWORK`；用户另否决当前 Reader 阅读背景面板和正常态内部技术文案。`IMG-04` 只保留资产 Alpha 与单独构图候选结论，不能代表实装整体 PASS；体积子包已冻结为 `f312@3fed4c7`，indexed PNG 165,998 B、Mini dist=1552 KiB，关闭 `<2 MiB` 机械门但不升级视觉。`IMG-05` 仅为阅读背景面板 `CANDIDATE / UIUX REVIEW REQUIRED`，不得照搬示例诗句、图标像素、生成背景或固定截图尺寸。失效 reviewer 已回收，替代 `/root/mini_design_ready_recovery` 只读 DESIGN-READY 为 `ACK PENDING`；conversation 与 Reader writer 均等 brief 后再按私有文件释放。D-TEXT follow-up `6e62153` 的替代非作者 FINAL 已启动隔离 API `4127` 与 Web `4387`；E-OWNER 唯一 writer 的 fail-closed account 绑定首个 RED 已由总控复跑确认。
- 协调下一动作：Mini UI/UX brief 返回后按互斥文件释放唯一 conversation writer与唯一 Reader writer；drawer token / 比例由单一 owner顺序承接。`IMG-05` 先由 UI/UX 给 adopt / exclude，若拒绝则只返回单一修图 prompt 或明确拒绝原因，不让开发者自行重设计。每个首个可运行候选先 EARLY，冻结后同一非作者 FINAL；完整 conversation + drawer + Reader 与包体门均通过后，总控才审查 `ff4767b..新 HEAD` 和最小 lock importer。D-TEXT `6e62153` 获非作者 PASS 后由总控顺序合入并复验 main；保护差异与 H3 阻塞不变。
...
| `M1-F2-E-OWNER` | `ACTIVE / ACK BY WORKTREE+RED` / `/root/owner_version_contract` | 为书籍、章节、位置和文本标注冻结会话中立的账户 owner、版本条件写入与迁移回退合同 | `/Users/echoman/.codex/worktrees/owner/SelfAlone`、`codex/m1-f2-e-owner@8ba2697`；首写仅 `apps/server/src/owner-contract.test.ts`。总控新鲜复跑 `pnpm exec vitest run apps/server/src/owner-contract.test.ts` 为 1 failed：缺失 account 时 `resolveAccountId` 未 fail-closed，命中预期根因 | 唯一 writer 只写 `packages/contracts/**`、owner / version 所需 `apps/server/src/**`、当前迁移目录与必要定向测试；不得碰 Web / Mini / 台账 / root lock | 两账户、旧 fileVersion、并发写入、未绑定账户隔离；记录 schema / 数据量 / owner / 约束 / 回退点；真实隔离 PostgreSQL、全仓 verify、scoped commit | 不等待真实邮箱、微信或小程序凭证；冻结后顺序释放 F1-A，避免 contracts / schema / app 同写 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / FROZEN 6e62153 / NONAUTHOR FINAL DISPATCHED / ACK PENDING / RUNTIME OBSERVED` / `/root/verify_dtext_entry_recovery` | `/Users/echoman/.codex/worktrees/dentry/SelfAlone`、`codex/m1-f2-d-text-entry@6e62153` clean；失效 reviewer 已回收，替代只读 reviewer已启动隔离 API `127.0.0.1:4127` 与 Web `127.0.0.1:4387`，但未返回原样 ACK / verdict，故不登记 ACTIVE | 作者 `pnpm verify` 32 files / 182 tests、typecheck / build / diff-check 与 1440 / 1200 / 1024 / 768×844 DPR2 真实 TXT / EPUB / 409 / owner / PPT handoff / overflow / console通过；替代 reviewer 按同一 Case FINAL。PASS 前不集成；原生200%、真实删除、真实AI/PPTX继续精确 VERIFY |
...
| 开发基线 | 当前控制与事实源恢复点 `main@0552ca6`，当前台账续接提交见 main HEAD；最近产品集成点 `main@deba325` | Desktop 完整统一壳已集成；`3199945 / 4fe6989 / 35798ab / 54b4eeb / 0552ca6` 只更新 Mini 产品 / 视觉门、IMG-05 候选、UI/UX 裁决授权与矢量图标忠实门，不代表 Mini 实现完成。D-TEXT 共享接缝已合并、Web 入口正在 FINAL；Mini `3fed4c7` 视觉整改链仍未集成。主线三个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
