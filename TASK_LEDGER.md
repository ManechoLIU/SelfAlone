...
| `M1-F2-WEREAD` | `RECOVERING / KIMI API PREFLIGHT / dfb0574+LOCAL dcae657` / 项目总控 | 原4测试文件74行 RED WIP保持；共享 `main@489d51d` 已等价同步。任务续作后原 native Writer不再 live；误建 native RESERVED recovery经新规则前零写入终止，未形成第二份WIP | 恢复动作：按 `main@ba3a7af` 为同一worktree建立唯一 Kimi K3 API Assignment，先no-call preflight与完整delivered ACK，再从既有7项RED继续；失败 / 未知不重试、不降级，checkpoint=Kimi ACK或精确外部门 |
...
| `M2-F2` | `RECOVERING / KIMI API PREFLIGHT / 73a0f11+LOCAL 552a6ef` / 项目总控 | 既有8文件WIP已关闭no-call、空成功快照、disconnect逻辑、同requestId retry与自由派生内嵌UI；共享 `main@489d51d` 已等价同步。任务续作后原 native Writer不再 live，WIP保持唯一恢复点 | 恢复动作：按 `main@ba3a7af` 为原worktree建立唯一 Kimi K3 API Assignment，先no-call preflight与完整delivered ACK，再补pagination、stale fencing、投影`bookId`与`progressPercent:null`；失败 / 未知不重试、不降级。全屏配置页仍需独立DESIGN-READY，f312未写入 |
