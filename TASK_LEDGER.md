...
- **活动实现线**：R2A `csrw@18b3923` 首候选被非作者审查判定两个 P1，原 writer 已按同 5 文件范围 `REOPEN 1`；R2B `csmsg@fc1245b` 已 nonauthor PASS、clean 冻结待组合；唯一 Mini writer 已提交设置状态候选 `f312@db14fed` 并继续长列表真实 EARLY。当前构成：controller / integrator 1、implementation writer 2、reviewer 0，3 / 4 槽占用；空槽不派无关 backlog，等待 R2A 修复候选后的短审查 / 组合门。
- **下一用户可见检查点**：R2A 关闭 reconcile 竞态与非 pending 二次发送后重新冻结，随已 PASS 的 R2B 顺序组合并打开真实 Chrome Selection EARLY；Mini 先在 390×844 复验设置状态，再完成 ref11 的 390 / 360 / 320、font16 / 23 长列表矩阵并给出 PASS / FAIL。
- **真实阻塞**：Mac 锁屏与子 Agent 控制面阻塞均已解除，当前里程碑无文件或环境阻塞。Goal 工具的 blocked / create 拒绝是状态面异常，不阻塞已恢复实现；Mini Library 仍独立受 M2-F1 session bridge / transport 阻塞，软件键盘与游客凭证仍留在 H3。
...
| `M1-F3B-C-R2A` | `ACTIVE` · `CANDIDATE 18b3923 / NONAUTHOR FAIL / REOPEN 1` / `/root/selection_recovery_web` | mutation级丢响应恢复、可执行重试与multi / free空值确认；不改Server、消息关联或shared seam | 首候选5文件、78 / 445与构建门均绿，但review确认reconcile等待期间旧draft / promise可覆盖新状态，且取回submitted / stale后仍二次发送；原writer原范围返工 | 仅Web `conversation-selection-controller{,.test}.ts`、`conversation-selection-view{,.test}.ts`、`conversation-selection.css` | 先以deferred `getQuestion`复现两个P1，再以attempt / generation guard或等价状态机最小修复；同根因第二次FAIL即重新拆小，旧候选`18b3923`只作恢复点 |
...
| `M2-F3-MINI-CONVERSATION-LONG-LIST` | `ACTIVE` · `CANDIDATE a9da90a / F312 db14fed / DEVTOOLS EARLY` / `/root/next_ready_audit` | developmentAdapter + `developmentLongList=1`确定性18项闭合抽屉长列表、搜索与末项露出；不冒充生产会话API，不处理软件键盘 / 凭证 / AI | 117项、typecheck / build代码门保持；Mac已解锁，仅允许唯一f312；真实FAIL只在既定5文件内最小返修 | `nav-drawer/index.test.ts` / `index.wxss`与`conversation/index.test.ts` / `index.ts` / `index.wxml`共5文件；f312保护差异不变 | ref11 SHA `9e764e…c36e`；正在验390 / 360 / 320、font16 / 23首 / 中 / 末段+固定底栏、搜索命中 / filtered-empty、fresh console / overflow；PASS后才可进main |
...
| `M2-F0-A` 当前候选 | `main@11f480a` 含Reader / Conversation / Drawer及send-retry限定闭包；唯一`f312@db14fed`另含未合入main的长列表与设置状态候选 | 设置工程文案已由`db14fed`替换为真实失败关闭状态；保护config / lock与QA图未带入候选。长列表代码门PASS并进入唯一DevTools EARLY；软件键盘、reduced-motion与PPT真实预览仍开放。 |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader与handoff scoped FINAL；Conversation已至`main@dea90d7` FINAL；Book Detail至`0cc5d46` scoped FINAL；Selection R2A `18b3923` FAIL后REOPEN 1，R2B `fc1245b` nonauthor PASS冻结 | R2A仅原5文件整改两个P1；R2B 12文件clean并等待组合。两候选都通过后才接shared seam / main并做四宽真实Chrome；当前main预览4173 / API4100。 |
