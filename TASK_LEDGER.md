...
- **活动实现线**：Auth 仍为 scoped Main Case PASS；Settings P0 候选 `c5d68b5` 经非作者 EARLY 后已顺序合入 `main@a9408ad`，同源 legacy v1、email-only failure、refresh 与 new-tab Main Case 均 `FINAL PASS`。Conversation 已在隔离 `cvmain` 以 `a9408ad` 为基线出现受控 CORE / API / DB / WEB diff，旧 P1 RED与 refs 02 / 36可追溯，状态升为 `ACTIVE`。Mini Reader `f312@db40e99` 已取得非作者 DevTools `FINAL PASS / READY FOR INTEGRATION`；Mini Conversation 非作者 EARLY在390首屏发现山亭不可辨识后立即 `FAIL`，reviewer已释放，唯一 scenery writer已按重诊断门重派。当前构成：controller / integrator 1、implementation writer 1（Conversation ACTIVE；Mini scenery exact preflight pending）、reviewer 0。
- **下一用户可见检查点**：Conversation owner 在隔离树保留 Auth / Settings 并关闭共享壳 P1；Mini scenery owner先区分 asset与 opacity / blend /遮罩 / z-index根因，再让390山亭低对比但肉眼可见且不侵入输入。两条各形成 scoped candidate后立即短 EARLY；Mini Conversation PASS后总控才按审计闭包建立main Mini VERIFY基线。
- **真实阻塞**：Mini Conversation `891d26e` 的390首屏山亭仍不可辨识，因同根因重复返工已禁止机械调 opacity，解除条件是唯一 writer根因证据、最小candidate与新非作者EARLY。main 又没有 tracked `apps/miniapp`，Reader顶层提交不能脱离初始壳、共享 drawer / route / asset链单摘；config / lock与f312两张QA图为受保护现场。软件键盘 / reduced-motion、Book Detail shared route seam及真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未闭合或未授权。
- **当前可见入口 / revision**：当前 main候选 Web `http://127.0.0.1:5188/` / API `http://127.0.0.1:4308` 绑定产品 revision `a9408ad`、schema `selfalone_main_auth_settings_20260826` 与 `/tmp/selfalone-main-auth-settings.XE5Dfd/`；Auth scoped Main Case与 Settings P0 Main FINAL PASS，但未替换稳定入口。稳定恢复点 `4127 / 4198` 仍绑定旧产品 revision `ab13310`。Mini唯一 DevTools为 `f312@db40e99`，保护 config / lock与两张既有 QA图未变。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE` · `CURRENT-MAIN MIGRATION / EARLY FAIL(P1) / REOPEN 1` / `/root/desktop_conversation_slice` | 输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话 UI；非目标：真实模型、图片消息、Settings / Auth UI。`cvmain` 已出现受控实现diff，正常共享外壳仍须移除实现态文案与不可用控件 | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、base `a9408ad`；必须保留Auth / Settings，owned外冲突即停 | 旧候选24个Conversation CORE / API / DB / WEB文件；当前实际diff仅落其范围，共享 `app.ts` / `index.ts` / `main.ts` 单writer | refs 02 `d84ffbae…` / 36 `1b6ec630…`，RED `conversation-early-b8f34b4/`；下一步关闭P1、定向 / 全量与真实Chrome，再交scoped candidate |
| `M1-F3B-A1-CONVERSATION-CORE` | `ACTIVE` · `CURRENT-MAIN MIGRATION / PRIVATE CASE RECEIPT RETAINED` / `/root/desktop_conversation_slice` | `97e1c53 → 56a850e → b8f34b4` 已闭合确定性回复、PostgreSQL会话 / 消息、API、Web controller / view与输入稳定候选；当前正迁移到Auth / Settings后的main基线 | `cvmain@a9408ad`直接状态证明Server / Web / Domain owned文件已写入；发生owned外冲突或Auth / Settings回退立即停写 | 旧候选24个Conversation文件，含三处共享接缝；禁止扩大到其它页面 | 私有persistence / failure Case收据保留；下一步完成current-main TDD与Chrome自验，未经过新基线真实端不得合入 |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE` · `cvmain@a9408ad / EARLY FAIL(P1) / REOPEN 1` / `/root/desktop_conversation_slice` | 输入、发送、确定性回复、刷新恢复、错误保留、统一对话 UI；非目标：真实模型 / 图片 / Settings。私有 Case PASS，current-main迁移已产生实际diff；共享壳仍须关闭实现态 / disabled占位 | `cvmain`为唯一writer；保留Auth / Settings，禁止owned外写入；旧P1为首个RED | 旧候选24文件与三处共享接缝；直接status显示当前diff均在owned范围 | 绑定02 / 36；证据 `conversation-early-b8f34b4/`。下一步同态1440作者自验与scoped candidate，再由非作者EARLY扩1200 / 1024 / 768 |
...
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `READER db40e99 FINAL PASS / READY FOR INTEGRATION; CONVERSATION EARLY FAIL` / 项目总控 | Reader在390 / 320 / 360 / 430真实DevTools关闭先前全屏底栏 / notes / 背景缺陷；Conversation `891d26e` 的390首屏山亭仍不可辨识。保护差异不变 | Conversation同根因重复返工，先重诊断再最小修复；其EARLY PASS前不进main。main Mini基线仍须code-only依赖闭包、project config / lock独立裁决、QA图排除 | Reader两文件、Conversation两文件及审计列出的最小运行时依赖；不得重复应用patch等价`2c99ab3` / `db40e99` | Reader `mini-reader-final-db40e99/`；Conversation FAIL `mini-conversation-early-db40e99/`。下一步 scenery candidate→新非作者EARLY→main VERIFY集成 |
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `VERIFY` · `CANDIDATE db40e99 / NONAUTHOR FINAL PASS / READY FOR INTEGRATION` / 项目总控 | refs 18～20老己笔记单列流、sheet手势、背景保留与共享底栏；390 / 320先前FAIL Case、360 / 430均真实通过；非目标：真实notes API / 跨设备 | main缺Mini初始壳与依赖链，故FINAL PASS只释放顺序集成；keyboard / reduced-motion未覆，不扩大为父项完成 | `pages/reader/index.wxss` / `page-state.test.ts`；保护 config / lock / 两QA图不带入 | 24项、diff-check与唯一DevTools非作者 FINAL；证据 `mini-reader-final-db40e99/`。下一步 code-only最小闭包入main VERIFY，再在同一DevTools路径复验 |
| `M2-F0-A-MINI-CONVERSATION-VISUAL` | `VERIFY` · `CANDIDATE 891d26e / EARLY FAIL / REOPEN 3 / WRITER REDISPATCHED` / `/root/mini_conversation_visual` | refs 13 / 30～32 的会话输入、选择、附件、错误恢复与同态矩阵；390首屏下部近纯白，山亭仅模糊残影且不可辨识；非目标：Reader、drawer、真实AI / PPTX | 同一scenery根因已重复，writer须先证明asset / opacity / blend /遮罩 / z-index哪层失真，禁止机械调opacity；keyboard0 / reduced-motion继续H3边界 | 唯一writer首选 `pages/conversation/index.{test.ts,wxss}`；若需asset或其它文件须停写扩边界；config / lock / QA图禁止变化 | FAIL证据 `mini-conversation-early-db40e99/390-normal-{initial,phone-crop}.jpeg`。下一步390 RED→最小candidate→作者320/390自验→另派非作者EARLY，PASS才释放main集成 |
...
| 当前桌面候选波次 | Auth scoped Main Case；Settings P0已在`main@a9408ad`同源Main FINAL关闭；Conversation shared seam释放并重新派发；Book Detail私有候选仍冻结 | main候选 `5188 / 4308@a9408ad`；证据 `main-settings-p0-final-a9408ad/`、`conversation-early-b8f34b4/`。当前稳定入口仍不替换。 |
| 当前 Mini 候选波次 | Reader `f312@db40e99` 非作者FINAL PASS / READY FOR INTEGRATION；Conversation `891d26e`真实EARLY FAIL且scenery writer重派，H3边界保留 | Reader `mini-reader-final-db40e99/`；Conversation `mini-conversation-early-db40e99/`。main只读审计已给最小运行时闭包，待Conversation新EARLY PASS后执行，project config / lock与QA保护现场不带入。 |
...
| 开发基线 | 产品顺序集成到 `main@a9408ad`；Auth scoped Main Case PASS，Settings P0 scoped Main FINAL PASS | main tracked clean，三个既有未跟踪视觉目录原样保留；当前main候选仍未替换稳定入口，Mini / Conversation候选与局部PASS不能自动 `DONE`。 |
