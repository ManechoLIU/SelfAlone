...
- 当前执行波次：`CONTROL-RESET-1`；Desktop `6afd@44e66af` 已获非作者 `PASS`，完整 `0969c33..44e66af` 已以 merge commit `main@deba325` 集成并完成在线 / 断网恢复真实 Chrome 复验。Mini drawer / Reader 从 `f312@a0e98f9` 继续唯一 writer 返工；D-TEXT `d65ab3b` 已获非作者 `PASS WITH RECEIPT CAVEATS`，独立入口 worktree 已出现精确私有模块写入，按实际 diff 升级 `ACTIVE / ACK BY EVIDENCE`。台账仍由项目总控独占。
- 当前活动项：Mini 唯一 `/root/mini_visual_rework_fix` 正在修改 Reader controls 私有样式 / 测试与 drawer 私有组件 / 测试，并确定性派生一个镜像 RGBA runtime；不得触碰两项保护差异与旧 EARLY 图。`IMG-04` 为 `ASSET VERIFY / RUNTIME REWORK`，没有新增图片生成调用；同一非作者 `/root/mini_visual_final_review` 已事件触发恢复为只读 `EARLY-VISUAL-VERIFY ACTIVE`，冻结 commit 后继续 FINAL，不创建重复 reviewer。D-TEXT 入口 `/Users/echoman/.codex/worktrees/dentry/SelfAlone@deba325` 已迁入 11 个私有 book-detail / annotation 新文件并修改 5 个 reader / library 私有文件；尚未写共享入口，owner 文本 ACK 仍待补，但不再是未启动状态。
- 协调下一动作：Mini 冻结新 HEAD 后交同一非作者 reviewer 做四宽、font23、深色 controls、抽屉滚动 / 命中 / safe-area FINAL；PASS 后总控审查完整 `ff4767b..新 HEAD` 并处理最小 lock importer。D-TEXT 收到 owner ACK 后迁入 `d65ab3b` 私有模块并接当前统一壳，完成真实 TXT / EPUB 与四视口、失败保留、非作者复验。保护差异与 H3 阻塞不变；不得只摘 Mini 末提交。
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `ACTIVE / ACK BY EVIDENCE / TEXT ACK PENDING` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dentry/SelfAlone`、`codex/m1-f2-d-text-entry@deba325`；实际 diff 为 11 个私有 book-detail / annotation 新文件与 5 个 reader / library 私有修改；唯一 writer 另拥有 `main.ts` 与必要入口测试，不改 server / contracts / schema / lock / Mini / 台账 | 实际文件写入已证明任务启动；仍须补 cwd / HEAD / status、RED / 复现与停止条件文本 ACK，并完成统一壳入口、真实 TXT / EPUB、持久化 / 409 保留、1440 / 1200 / 1024 / 768、reduced-motion、README revision、scoped commit 与非作者复验 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY / REWORK ACTIVE FROM a0e98f9 / H3 BLOCKED / NOT IN MAIN` / `/root/mini_visual_rework_fix` | Mini 非作者已实看四宽、font23、安全区、提示生命周期与 light/dark；唯一新增页面阻断是 320 深色 controls 长标题 `ellipsis`。唯一 writer 只改 Reader controls 直接文件 / 测试及后续获 brief 的 drawer runtime asset；保护 project config / lock 与两张旧 EARLY 图不得处理 | 标题必须自然可读、无 line-clamp / ellipsis / overflow 裁字，四宽 + font23 无横溢；软件键盘、真实多会话列表滚动、游客凭证 / console、真实 PPT preview 与 H3 仍未闭合。新候选须定向 / miniapp verify / DevTools / scoped commit 后交同一非作者 FINAL |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY / REWORK ACTIVE / EARLY-VISUAL-VERIFY ACTIVE / IMG-04 ASSET VERIFY` / `/root/mini_visual_rework_fix` + `/root/mini_visual_final_review` | `IMG-04` 已核验为真实 RGBA / fractional alpha / 无实底；唯一 brief 要求完整源水平镜像一次输出 `1024×737 RGBA` Mini 私有 runtime、140px slot / opacity .82 / 线性 mask / 不入滚动与点击层。writer 已有 Reader / drawer / asset WIP；reviewer 只读实看当前 WIP，不重调色、不再次生成 | EARLY 输出逐项 PASS / FAIL 与 covered / uncovered；新 HEAD 再做定向 / miniapp verify / DevTools 320 / 360 / 390 / 430 + font23、320 深色 controls、列表滚动 / 命中 / safe-area、非作者 FINAL。软件键盘、真实多会话列表与 PPT preview 继续精确保留 H3 / VERIFY 边界 |
...
| `M2-F0-A` 当前候选 | `ff4767b → … → a0e98f9`，状态 `VERIFY / REWORK ACTIVE / H3 BLOCKED / NOT IN MAIN` | Mini 专项非作者 verdict 为页面 `VERIFY`、`IMG-04 ASSET VERIFY`；320 深色 controls 长标题与山亭 runtime 仍需最小整改。完整链未集成且不含 lock importer；保护 project config / lock 不得带入，软件键盘、PPT 资产 / 真实预览仍缺。 |
...
| `IMG-04` | `ASSET VERIFY / RUNTIME REWORK ACTIVE` | OpenAI 内置 `image_gen`；具体模型 ID 未暴露 | 1 张，`1024×1536`；按参考生成小程序 drawer 底部低对比山亭 | 白底源 `redesign-v2/output/ui-design-preview/product-design/mobile-drawer/candidates/v1/mobile-drawer-landscape-white-bg-v1-candidate.png`，SHA-256 `37cbfb304a7b6a3d5f8c2013f84774c0e3339ab5474adb49e3de1993d7edb3ba`；透明裁切候选 `1024×737 RGBA`、SHA-256 `55a655177c84202c255786fefdb4752e2cb575ffcf6c11f6540ae51aa48e2a38`，已通过 Alpha 门但需确定性水平镜像后作为 Mini 私有 runtime 实装并做真实端 FINAL；无新增图片生成调用 | 已实际调用 `4 / 20`，剩余 `16` |
