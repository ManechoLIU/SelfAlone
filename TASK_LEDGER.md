...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复、北京时间双时段计费、真实 Store 并发硬上限与失败后重试本地门已分别在 `main@b637023+81f5f1f+df32890+ba70ced` 闭合。真实 Provider H4 继续等待三项外部条件；项目级旁路 `M1-F5-A1` 已冻结 clean 候选 `8725581`，下一事件先由非作者核验 11 文件实际 diff，并以 PostgreSQL `integer` 与 JS safe-integer 页数上限反例作为首个风险门；不得重做或并行写。
...
| `M1-F5-A` | `VERIFY / A1 BACKEND CANDIDATE` / 项目总控 | 对话 / 书籍双入口、单书来源、范围 / 需求与任务工作区 | `M1-F5-A1` 已冻结 clean 候选；先闭合非作者审查再接 Web 真实入口；已闭合的 Mini handoff 不重复开发、不触碰 f312 现场 |
| `M1-F5-A1` | `VERIFY / CANDIDATE 8725581 / NONAUTHOR REVIEW QUEUED` / 项目总控 | 真实账号 + 已发送用户消息门；当前会话内创建独立 PPT 草稿；来源关系按集合建模且 MVP 恰好一本书；固定字段仅用途、受众、页数范围、补充要求；换书保留通用要求 | clean 候选包含 `06153d7+35b1183+7ce1fa0+8725581` 共11文件；聚焦17、Server269、整仓122文件873项、typecheck/build/visual-contract/diff-check为交接收据，尚未取得非作者 PASS。下一事件先审实际 diff，并验证 PostgreSQL 32-bit `integer` 对超大页数的 fail-closed 行为；禁止模型、联网、模板、生成、下载、端侧视觉及并行重做 |
