...
- 当前 Goal：`M1-F2-B` EPUB / TXT 桌面阅读闭环收口；`B-UX1` 已给出并由总控批准唯一视觉 / 交互 brief，原 B 实施任务已提交首个候选并正按总控代码审查追加范围内 follow-up，之后进入非作者复验与 main 集成。M2-F0-A 保持未合并 idle，不与 B 争用文件。
- 当前阶段：桌面 Web `M1-F2-A` 已在 main `DONE`；`M1-F2-B` 已形成干净候选 `04f03d2` 并进入总控 `VERIFY`，但尚不允许集成。总控实审发现重新导入后的新 fileVersion 无法首次保存、EPUB 解压上限检查过晚、主题 / 专注重绘会删除正文选区，以及 11 张 `.png` 实为 JPEG；原生 200% 仍缺。B 保持 `ACTIVE` 做最小 follow-up，M2-F0-A H3 继续 `BLOCKED`。
...
- 当前执行波次：`M1-F2-B-IMPLEMENT`；A 已回收，M2-F0-A 未合并候选保持 idle，C 安全首包已完成集成并停止原任务。
- 当前活动项：实现任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89@04f03d2` 独占既有 B 路径，追加 Server 版本 CAS、Domain 解压上限、Web 选区保持 / 复制反馈、reader 私有山水 opacity 与证据扩展名的 TDD follow-up；不得改写原提交、共享入口 / contracts / schema / 台账。审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 已暂停对失效 revision 的复验，等待新 HEAD。M2-F0-A 在 `f312@91934f1` idle。
- 协调下一动作：B 提交最小 follow-up，证明 v1 旧位置存在时 v2 可首次保存且并发 CAS 正确、恶意 EPUB 不会先分配超限 Buffer、主题 / 专注不删除真实选区，并修正证据编码与 `.34 / .08` 私有左栏山水；随后原 UI/UX 任务复验新 revision，总控再审查、合入 main、补共享接缝和 main 回归。原生 200% 未取得前不得标视觉 `DONE`。
...
| `M1-F2-B` | `ACTIVE / VERIFY` / 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` | EPUB / TXT 正文、目录、复制、全书位置、专注模式和浅 / 深阅读背景的桌面阅读闭环 | `04f03d2` 已提交但总控发现四项 follow-up 门；修复前不集成。原生 200% 继续阻塞视觉 `DONE`，不阻塞修复后的候选代码复审 | `da89` / `codex/m1-f2-b` 只写既有 `text-reader*` Domain / Server / Web 与 B acceptance；共享入口 / 样式、contracts、schema、锁文件和台账不得写 | 既有 21 项定向、71 项全仓和真实 Chrome 收据需绑定新 HEAD；新增 v2 首次保存 / 并发 CAS、解压上限、选区保持、证据 MIME / 扩展一致，之后再由 `B-UX1` 复验 | 以 `04f03d2` 为恢复点追加提交，不改写历史；新 HEAD 干净、全验证和真实 Case 回传后再裁决集成 |
| `M1-F2-B-UX1` | `DONE / 等待复验` / 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` idle | 已完成浅 / 深阅读私有侧栏、普通态桌宠、首帧、焦点、滚动、TXT 与复制反馈的非作者 brief；不实现 | `04f03d2` 将由范围内 follow-up 替代，B 未交新 revision 前不重复轮询；PDF 页面一致性只锁视觉合同、不冒充当前实现 | 审查任务保持只读；事实源由总控以 `8f40658` 同步，实施只归 B 当前路径所有者 | 新候选须覆盖无缓存 / 有缓存 / 存储拒绝、账户与 fileVersion 隔离、浅 / 深、目录 / 专注焦点、TXT 去重、复制失败、100% / 200% 和规定视口 | 真实 Chrome初审为无 override `1440×900 @ DPR2`；精确 `1440×844` 与 `768` 仅 DPR1 override，原生 200% 和 PDF 页面仍缺，候选复验不得静默降门 |
...
当前唯一实现写入卡为 B follow-up：任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89@04f03d2` 只改既有 B Domain / Server / Web / acceptance 路径，不改写原提交；共享入口、contracts、schema、锁文件、事实源和台账仍由总控独占。原 UI/UX 任务只在新 HEAD 后复验；M2-F0-A `f312@91934f1` idle，C 原任务已停止。
...
| `M1-F2-B` 当前候选 | `44b81ea → 04f03d2`，状态 `ACTIVE / VERIFY / follow-up required` | 24 个新增文件且限定 B 范围，工作树 clean；定向 3 文件 / 21 项、全仓 16 文件 / 71 项、typecheck / build / diff check 与真实 EPUB / TXT / Chrome 收据已回传。总控实审确认四项门：旧 fileVersion position 会阻断新版本首次保存；inflate 上限在分配后；主题 / 专注全量重绘删除选区；11 个 `.png` 实际为 JPEG。另有原生 200% 未取证和 reader 私有山水 `.44 / .12` 未对齐 brief `.34 / .08`；新 HEAD 前不集成。 |
...
| 开发基线 | 本地 `main` @ `a306149` | A 已完整闭合；`44b81ea` 冻结 B / C 接缝，`589a22b` 纳入 C 安全首包，`8f40658` 固定 B 阅读私有外壳与交互规则，`a306149` 释放 B 实施。B 候选仍在 `da89`，M2 H3 保持阻塞；主线未跟踪视觉候选不得纳入台账提交。 |
...
| `M1-F2-B-UX1` 阅读视觉 / 交互 brief | `DONE / 等待复验` | 非作者初审判冻结 WIP `FAIL`；总控以 `8f40658` 批准私有左栏浅 / 深联动、无固定桌宠、首帧与焦点规则，B 已提交 `04f03d2` 并追加总控审查 follow-up，审查任务 idle 等待新 revision |
