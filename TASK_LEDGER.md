...
- 当前阶段：桌面 Web `M1-F2-A` 功能闭环保持通过；真实 Chrome 下的外壳、密度、桌宠与封面叠层问题已最小修正；三张本地书固定背景仍是未批准候选且未进入当前运行时，视觉结论保持 `VERIFY`。
...
- 当前活动项：`M1-F2-A`。
- 协调下一动作：保留既有功能，不合并、不推送；等待用户确认三张固定封面背景候选并由主任务纳入当前分支，再接入已预留的稳定 `cover-variant-1…5` 映射并在同一用户 Chrome 标签页复验。
...
| `M1-F2-A` | `VERIFY` / 主 Agent | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 功能无阻塞；外壳 / 密度 / 桌宠已复验；三张固定背景仍是未批准候选，纯色回退不得作为最终封面 | CORE、API、WORKER、DB、WEB、QA；变更限定于 A 的契约、领域解析、Server、Web、视觉事实源与验收证据 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名、作者缺失、刷新、账户隔离；Web 4 视口与 02/03 绑定参考 | `08-real-chrome-1440x844-dpr2.png`；稳定 `cover-variant-1…5` 边界已预留，等待候选批准后继续 |
...
- 本轮视觉证据：已集成主仓 `6ae3968`，`nav-surface` 恢复为 `#CFE1D7`，内容面使用 `#F8FAF6`；封面移除多层渐变、伪元素、半透明底层和实色偏移，保留安全回退表面与一层轻阴影。用户当前 Chrome 未设置模拟视口，实测外窗 `1440×900`、CSS `1440×844`、zoom `1`、DPR `2`；左栏 `184px`、工具区 `52px`、按钮 `152px`、五列封面 `160×224px`、两排来源完整，桌宠 `104px`、气泡 `44px`，控制台无 error/warning。浏览器控制截图 `08-real-chrome-1440x844-dpr2.png` SHA-256 `d0927caa481a86d18f52094faa633b07fb12effa366c94a8fe3c6b7a74d235ab`。
- 剩余视觉阻塞：主仓工作树存在 `local-cover-candidates/v1/` 三张 `1024×1434` 候选，但均未获批准且未进入当前分支；当前纯色封面只作为安全回退，不是完成候选。批准后运行时按书籍 ID 稳定哈希映射固定背景，叠加真实书名 / 作者，不做运行时 ImageGen、随机变化或 CSS 伪插画。
- 图标约束复核：筛选空与真实空已改用项目已有 `search` / `book` SVG；不再用 `.empty-glyph` 的 CSS 方块与文字冒充图标。真实 Chrome 在未模拟的 `1200×647 @ DPR 2` 筛选空态实测共享 SVG 为 1、旧 CSS 图标为 0、清除按钮高 `44px`，随后已恢复正常书架。
- 自动证据：候选分支 `pnpm verify` 通过 11 个测试文件 / 39 项测试和生产构建；定向视觉与状态回归 2 文件 / 8 项通过；证据索引为 `redesign-v2/output/acceptance/m1-f2-a/README.md`。这些结果不替代封面资产接入后的视觉复验。
...
| `M1-F2` 共享外壳 | `8688b6f` 仅复用已确认运行时资产 | 只复用 `desktop-left-rail-landscape-approved-v1.png` 及其事实源登记，不带入该提交的阅读、笔记或旧书架实现。 |
