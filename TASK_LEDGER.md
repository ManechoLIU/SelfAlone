...
- **活动实现线**：Auth / Settings与Mini Reader / Conversation既有main PASS保持。D-TEXT 768 `575a612` 已取得候选真实Chrome EARLY PASS并顺序合入 `main@cc0cec0`，正在同Case main FINAL。Mini Drawer短屏修复 `c409f97` 已顺序进入唯一 `f312@effece6`，正在390 / 360 / 320 re-EARLY。Desktop Conversation `cvmain@92e996b` 已关闭发送中编辑破坏retryId，但代码门又发现挂载旧hydrate可覆盖已开始的send；原writer正以单一epoch / dirty不变量返修。当前构成：controller / integrator 1、implementation writer 1（Conversation）、reviewer 2（Mini Drawer、D-TEXT main FINAL）。
- **下一用户可见检查点**：Conversation先冻结hydrate竞态候选并过非作者代码门，再进入隔离Chrome全Case；Mini Drawer在唯一f312对同三视口给re-EARLY verdict；D-TEXT在`main@cc0cec0`关闭同两视口FINAL后立即释放。槽位释放后按共享 `main.ts` 与Mini私有文件重算Book Detail seam或下一Mini包。
- **真实阻塞**：Conversation `92e996b` 当前P1为pending hydrate返回旧idle snapshot后无条件覆盖本地r1 send，使用户能发r2并触发409 / 状态错乱；解除条件是stale success / failure不覆盖dirty / sending、activeRun不伪装idle、确定性测试、scoped commit与非作者复审。Drawer `effece6` 仍等待真实短屏re-EARLY；长列表无真实数据。D-TEXT本卡只待main FINAL，不代表PPT handoff / banner或父项完成。Book Detail与D-TEXT PPT handoff继续受Conversation共享 `main.ts` 单writer约束；f312保护现场及外部授权门不变。
- **当前可见入口 / revision**：Desktop候选入口 `5188 / 4308` 仍绑定 `a9408ad`，尚不含Conversation或新D-TEXT main提交；稳定恢复点 `4127 / 4198` 绑定 `ab13310`。Mini唯一DevTools为 `f312@effece6`，main等价基线上追加Drawer两文件短高修复；config / lock与两张QA图未变。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE` · `CANDIDATE 92e996b / PRE-EARLY CODE FAIL(P1) / REOPEN ACTIVE` / `/root/desktop_conversation_slice` | 输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话UI；非目标不变。候选链已关闭route、导航handoff、直接retry / replay幂等与发送中输入锁；当前只返修hydrate竞态 | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、base `a9408ad`；必须保留Auth / Settings，owned外冲突即停 | 25个Conversation CORE / API / DB / WEB文件与三处共享接缝；`app.ts` / `index.ts` / `main.ts` 保持本包单writer | 52 files / 274 tests与PG幂等通过；代码门复现pending getSession→r1 send→stale idle snapshot→r2。下一步hydrate generation RED→最小修复→非作者代码门→Chrome EARLY |
| `M1-F3B-A1-CONVERSATION-CORE` | `ACTIVE` · `CURRENT-MAIN CANDIDATE 92e996b / PRIVATE CASE RETAINED` / `/root/desktop_conversation_slice` | 保留确定性回复、PostgreSQL会话 / 消息、API、Web controller / view、legacy workspace、导航handoff、Server幂等与发送锁；当前不扩业务功能 | async hydrate / local send协调是当前唯一P1；owned外冲突即停 | 同一25文件候选；禁止扩大到其它页面、DB/schema或Book Detail | 新门：stale hydrate success / failure不得覆盖dirty / sending，服务端activeRun不得被映射为idle；通过后仍须非作者代码门与Chrome |
...
| `M1-F2-D-TEXT-READER-768-FIX` | `VERIFY` · `MAIN MERGED cc0cec0 / CANDIDATE EARLY PASS / MAIN FINAL ACTIVE` / `/root/dtext_768_early_review` | 两文件修复已顺序合入main；候选真实768 / 1440证明长标题、toolbar、actions无内部裁切且横溢0 | main 22 / 22、typecheck / build / diff-check通过；reviewer使用隔离端口/schema同Case复验，不扩大到PPT / banner | main `apps/web/src/text-reader.css` / `text-reader.test.ts`；当前只读FINAL | 候选证据 `dtext-768-early-575a612/`；main证据待 `dtext-main-final-cc0cec0/`。200% / DPR2、其它两个FAIL与父项仍开放 |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE` · `cvmain@92e996b / PRE-EARLY CODE FAIL(P1) / REOPEN ACTIVE` / `/root/desktop_conversation_slice` | 输入、发送、确定性回复、刷新恢复、错误保留、统一对话UI；当前只返修pending hydrate覆盖本地send，非目标不变 | `cvmain`唯一writer；保留Auth / Settings；不得改DB/schema、Book Detail或Mini | 25个Conversation文件与三处共享接缝；当前只改controller/state协调及测试 | 绑定02 / 36；RED为pending hydrate旧snapshot不得解锁r1。下一步scoped commit→非作者代码门；PASS后才做作者1440与非作者四宽Chrome |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY` · `READER + CONVERSATION MAIN FINAL / DRAWER RE-EARLY ACTIVE / H3 BLOCKED` / 项目总控 | Reader与Conversation限定范围已复验；Drawer短高修复已进入唯一f312，等待同三视口判定 | 长列表无真实数据；软件键盘、游客凭证 / console、真实PPT preview继续H3 / VERIFY | Drawer `f312@effece6`只追加两文件；H3无writer | 首轮FAIL `mini-drawer-early-3f8375e/`；re-EARLY证据待 `mini-drawer-re-early-effece6/` |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `MAIN MERGED 31462d9 / READER + CONVERSATION SCOPED FINAL; DRAWER RE-EARLY ACTIVE` / 项目总控 | Reader与Conversation已关闭对应缺陷；Drawer首轮360 / 320FAIL后，两文件短高修复已进入f312同三视口复验 | 全Mini仍缺附件 / 真实发送重试、软件键盘、长列表与reduced-motion；Drawer不通过前不进main | main闭包不变；f312仅追加Drawer两文件，保护现场不变 | 首轮 `mini-drawer-early-3f8375e/`；re-EARLY待 `mini-drawer-re-early-effece6/`，PASS后才进入main |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `f312@effece6 / RE-EARLY ACTIVE / REOPEN 1` / `/root/mini_drawer_early_review` | ref11与正式山亭asset；390保持140px，360 / 320按88 / 56px收缩且不恢复layout padding；非目标不变 | author `c409f97` clean并已释放；长列表无真实数据继续uncovered | f312仅新增 `components/nav-drawer/index.test.ts`、`index.wxss`；保护现场不变 | 10 / 10、Mini 15 files / 99、typecheck / build / diff-check通过；待唯一DevTools三视口re-EARLY `mini-drawer-re-early-effece6/` |
...
| `M2-F0-A` 当前候选 | `main@31462d9` 含Mini code-only闭包；唯一 `f312@effece6` 在同源基线上追加Drawer两轮两文件修复 | Reader + Conversation限定范围FINAL；Drawer正在re-EARLY。保护project config / lock与QA图未带入main；附件 / 真实发送重试、软件键盘、长列表、reduced-motion与PPT真实预览仍未通过。 |
...
| `M1-F2-D-TEXT` 共享集成与私有候选 | 共享链至`main@f85d86c`；私有Web `d65ab3b`；768修复 `575a612 → main@cc0cec0` | 768候选非作者EARLY PASS并已合入；main 22 / 22、typecheck / build通过，正在同Case FINAL。PPT handoff / banner、200% / DPR2与父项仍开放。 |
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT 768合入`main@cc0cec0`并做FINAL；Conversation `cvmain@92e996b`因hydrate竞态原writer返修；Book Detail继续冻结 | Desktop入口仍为 `5188 / 4308@a9408ad`；Conversation未过代码门 / Chrome，D-TEXT未过main FINAL，不扩大完成。 |
| 当前 Mini 候选波次 | Reader + Conversation在`main@31462d9`限定范围FINAL；Drawer短高修复已在 `f312@effece6` re-EARLY | 首轮FAIL `mini-drawer-early-3f8375e/`；新证据待 `mini-drawer-re-early-effece6/`，保护现场不变。 |
...
| 开发基线 | 产品顺序集成到 `main@cc0cec0`；Desktop Auth / Settings、Mini Reader / Conversation限定范围PASS，D-TEXT 768待main FINAL | main仅有当前台账事件diff与三个受保护未跟踪视觉目录；Desktop入口仍绑定`a9408ad`，Conversation / Drawer与局部PASS不能自动 `DONE`。 |
