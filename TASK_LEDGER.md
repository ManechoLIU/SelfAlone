...
- **Goal**：工具真实状态仍为旧 Goal `blocked`；用户恢复后调用 `create_goal` 被工具以“unfinished goal”拒绝，未伪造切换。当前执行里程碑已按用户恢复指令激活：并行闭合 Desktop Selection R2A / R2B 私有候选与 Mini 长列表真实 DevTools EARLY，再顺序接入本地 main / 唯一 f312 复验；不纳入其他开放 backlog。
- **活动实现线**：R2A `csrw@1724f0e` 与 R2B `csmsg@fc1245b` 均 nonauthor PASS、clean 冻结；Selection shared-seam 只读审计已派发，随后由总控顺序组合。Mini 长列表在 `f312@db14fed` 六组矩阵真实 EARLY PASS，唯一 Mini writer 正补 Settings 390真实 EARLY。当前构成：controller / integrator 1、implementation writer 1、reviewer 1，3 / 4 槽占用。调度健康纠正：此前 R2A / R2B 的 ACK 已在子任务线程产生但未送达父级，active writer 被误中断 2 个，clean-but-live 误判 2 个；该批 dispatch→delivered ACK 延迟因通道漏送不可计量，后续逐包记录。
- **下一用户可见检查点**：按审计合同顺序组合两条 Selection 候选、接 shared seam 并打开真实 Chrome Selection EARLY；Mini 完成 Settings 390×844 ref22同态、fail-closed操作与fresh console / overflow PASS / FAIL。
- **真实阻塞**：旧 Desktop “执行控制面无响应”结论已纠正为 ACK 观测 / 传递误判，不是 writer 技术阻塞；历史 Mac 锁屏只曾独立阻塞 Mini 真实端且现已解除，从未构成 Desktop 或整个 Goal 阻塞。Goal 工具的 blocked / create 拒绝仍是状态面异常；Mini Library 另受 M2-F1 session bridge / transport 阻塞，软件键盘与游客凭证仍留在 H3。
- **当前可见入口 / revision**：main Web `127.0.0.1:4173/#/auth`（PID `29697`）与 API `127.0.0.1:4100`（PID `29534`）均从 `/Users/echoman/Documents/SelfAlone` 启动并绑定 `main@6e9e4c4`；API schema 为 `selfalone_main_preview_6e9e4c4_20260826`，`/health` 返回 `401 AUTH_REQUIRED`，Chrome 登录页真实可见但不冒充业务 PASS。唯一 DevTools 为 `f312@db14fed`；旧 5 errors / 17 warnings 清空后普通编译为 0 / 0，归因于游客 / DevTools 历史噪声，设置工程文案则是候选真实 FAIL 并已形成待 EARLY 修复提交。main 仅有三个受保护未跟踪视觉目录。
...
| `M1-F3B-C` | `VERIFY` · `EARLY PRECHECK FAIL / REOPEN 2 SPLIT` / 项目总控 | 简单选择、显式确认、只读历史和失效问题的私有纵向切片；不含真实模型、图片、书籍上下文或shared UI seam | `4b0aa19`的question级retry、空值确认和消息关联缺口已拆为R2A / R2B；两子包均通过actual diff前父包不接线 | 原`csel1`只作恢复点；R2A / R2B文件互斥，shared `app.ts` / `index.ts` / `main.ts` / `styles.css`继续冻结 | ref36 SHA `1b6ec630…a5796b40`与SPEC 155～158 / DESIGN 5.4；两候选顺序组合后再走1440 / 1024 / 1200 / 768真实EARLY |
| `M1-F3B-C-R2A` | `VERIFY` · `CANDIDATE 1724f0e / NONAUTHOR PASS / REOPEN 1 CLOSED` / 项目总控 | mutation级丢响应恢复、可执行重试与multi / free空值确认；不改Server、消息关联或shared seam | generation fence阻止过期reconcile发送 / 缓存 / 回写；submitted / stale接受服务端真相并清recovery / draft，禁止二次answer；nonauthor确认两条P1均关闭 | 组合后仍仅Web `conversation-selection-controller{,.test}.ts`、`conversation-selection-view{,.test}.ts`、`conversation-selection.css` | 定向18项、全量78 / 447、typecheck / build / diff-check与只读复审PASS；候选clean冻结，待与`fc1245b`顺序组合和真实Chrome，不提前DONE |
| `M1-F3B-C-R2B` | `VERIFY` · `CANDIDATE fc1245b / NONAUTHOR PASS` / 项目总控 | 选择问题稳定绑定原assistant message并贯通API / 持久化 / Web状态；不渲染、不改shared conversation文件 | `csmsg@fc1245b` clean，12文件范围正确；缺messages表、legacy无关联行均事务回滚，messages复合主键支持FK，role在创建 / legacy校验且当前不可变 | Server 8个`conversation-selection*`源 / 测试与Web `conversation-selection-client/state{,.test}.ts`共12文件 | 定向6文件 / 26项、全量78 / 448、typecheck / build / diff-check PASS；nonauthor无P0～P2；候选冻结，待R2A通过后顺序组合与main真实Case |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `F312 db14fed / SETTINGS CANDIDATE + LONG-LIST EARLY ACTIVE / H3 OPEN` / 项目总控 | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪能力以真实未连接 / 未配置 / 暂不可用状态失败关闭，不暴露工程台账文案 | f312是唯一DevTools且保护现场不变；设置候选与长列表均未合入main，真实EARLY由唯一Mini writer顺序执行 | Reader / Conversation / Drawer既有证据 + `mini-conversation-send-early-854c577/`；下一步设置390复验与长列表三宽 / 大字体；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F0-A-MINI-SETTINGS-STATE` | `VERIFY` · `CANDIDATE db14fed / REAL DEVTOOLS EARLY ACTIVE` / `/root/next_ready_audit` | 移除“开发适配边界 / 等待F*”工程文案，未实现能力显示真实“暂不可用 / 未配置 / 未连接”并失败关闭；不接真实账户、模型或微信读书 | 唯一f312；ref22 `ce28f938…f657288`；需要凭证 / 外部服务或偏离四文件即暂停 | `pages/settings/index.ts` / `index.wxml` / `index.wxss` / `index.test.ts`；dist仅build生成 | 首个2项RED已关闭；16文件 / 119项、typecheck / build / diff-check PASS；下一步390×844真实DevTools同态、console / overflow与状态操作，不通过不得进main |
| `M2-F3-MINI-CONVERSATION-LONG-LIST` | `VERIFY` · `CANDIDATE a9da90a / F312 db14fed / REAL DEVTOOLS EARLY PASS` / 项目总控 | developmentAdapter + `developmentLongList=1`确定性18项闭合抽屉长列表、搜索与末项露出；不冒充生产会话API，不处理软件键盘 / 凭证 / AI | 唯一f312完成390×844 / 360×640 / 320×568 × font16 / 23；游客 / 灰度 / preload噪声与源码错误分离；保护差异未动 | `nav-drawer/index.test.ts` / `index.wxss`与`conversation/index.test.ts` / `index.ts` / `index.wxml`共5文件；本轮无新源码提交 | 首 / 中 / 末、固定底栏、搜索会话09、filtered-empty与横溢均PASS；证据`apps/miniapp/visual-qa/early-db14fed-long-list-2026-08-26/`；待Settings EARLY后顺序进main / FINAL |
...
| `M2-F0-A` 当前候选 | `main@11f480a` 含Reader / Conversation / Drawer及send-retry限定闭包；唯一`f312@db14fed`另含未合入main的长列表与设置状态候选 | 长列表六矩阵真实EARLY PASS；设置工程文案已替换为真实失败关闭状态且正补390真实EARLY。保护config / lock与QA图未带入候选；软件键盘、reduced-motion与PPT真实预览仍开放。 |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader与handoff scoped FINAL；Conversation已至`main@dea90d7` FINAL；Book Detail至`0cc5d46` scoped FINAL；Selection R2A `1724f0e`与R2B `fc1245b`均nonauthor PASS冻结 | R2A组合范围5文件、R2B 12文件且均clean；shared-seam只读合同审计已派发，随后总控顺序组合 / 接线并做四宽真实Chrome；当前main预览4173 / API4100。 |
| 当前 Mini 候选波次 | Reader / Conversation / Drawer既有范围保持；send/retry已至`main@11f480a` scoped FINAL；Library合同`20d61d6`因auth BLOCKED；设置`db14fed`候选、长列表`a9da90a → f312@db14fed`正在EARLY | 唯一f312保护config / lock与两张QA图不变；设置390先验，长列表390 / 360 / 320、font16 / 23、首 / 中 / 末与搜索两态随后验，PASS后才可进main。 |
| 当前 Desktop main 预览 | `RUNNING / MAIN 6e9e4c4 / NOT FUNCTION PASS` | Web `127.0.0.1:4173/#/auth` PID `29697`、API `127.0.0.1:4100` PID `29534`；schema `selfalone_main_preview_6e9e4c4_20260826`与隔离artifact / books目录。Chrome登录页可见；`/health`为`401 AUTH_REQUIRED`，只证明入口绑定当前main。 |
| 开发基线 | 本台账事件提交的父为`main@6e9e4c4`，产品代码revision仍为`1e7763a`；Desktop Auth / Settings、D-TEXT reader / handoff、Conversation与Book Detail限定范围PASS，Mini Reader / Drawer与send-retry限定范围PASS | main仅允许本次台账事件diff与三个受保护未跟踪视觉目录；Selection、Mini设置 / 长列表与Library合同仍未完成，不能自动 `DONE`。 |
...
| `M2-F0-A` 原生运行壳 | `VERIFY / MAIN MERGED 11f480a / SEND-RETRY SCOPED FINAL / H3 OPEN` | Reader / Conversation / Drawer与send-retry限定范围完成main复验；软件键盘、凭证化clean console、长列表与PPT真实预览仍开放，Mini父项不关闭 |
