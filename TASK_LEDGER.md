...
- 当前活动项：协作树为总控 + `/root/auth_resume` + `/root/dtext_main_final` + `/root/mini_early_review`，4 / 4 槽占用且三者状态均为 running。`main@528b86b` 的 exact commit / delta / scope / stop conditions 已通知三者；Mini 已回传 `CONTINUOUS OWNERSHIP CONTRACT LOADED main@528b86b`，因只交 ACK 未给 EARLY verdict 而由同一任务恢复，继续 `ACTIVE / EARLY`。认证与 D-TEXT 在一次有界等待后仍为 `DISPATCHED / ACK PENDING`，不再轮询、不新增重复执行者，二者回传 loaded ACK 前暂停下一次受影响写入、冻结或验收裁决。Mini 两个等价 checkpoint 与四项保护现场不变；认证 14 文件 dirty WIP 不变。
- 下一 `READY` / 协调动作：当前 `0` 个空闲槽，不新增重复 writer；首个槽位释放后按已到达事件重算，认证出现首个可运行 UI 时优先释放非作者 EARLY reviewer，Mini EARLY `FAIL` 时释放对应唯一 writer，D-TEXT FINAL `FAIL` 时释放最小修复 writer，任一 `PASS` 则先回收候选并在本地 `main` 顺序集成、复验后再关闭该卡。等待认证或 D-TEXT exact-contract ACK 只做当前已发生的一次有界等待，无 ACK 时不再轮询。Mini 通过 EARLY、冻结新候选后才进入 FINAL；`M2-F4-A` 等 F0-A 合入并复验且解除同写边界后才重算为 `READY`，本轮不提前派发或合并。
- main 可访问入口收据：2026-08-25 20:38 +08:00 对 4127 / 4198 / 4110 / 4173 的监听与 HTTP 复核均失败，旧 `main@5d89aa5` 可访问收据已因进程退出失效。当前准确状态为 Desktop main 用户入口与 D-TEXT review 入口均离线；后续 reviewer须使用隔离 schema / 端口恢复自己的验收入口，main 长驻入口另行恢复，不得继续引用旧 200 收据。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `DISPATCHED / ACK PENDING` / `/root/auth_resume` | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | `auth1@9764180`、branch `codex/m1-f1-a-auth` 保留 14 文件 dirty WIP；新 writer创建已返回且当前 running，但 loaded ACK 未返回，故尚未取得写入活动确认。`E-OWNER` 已以 `main@f1334ad` 冻结共享 owner 接缝 | CORE、API、DB、WEB、QA；不得与 Mini、D-TEXT 或后续共享 schema / app 集成并写 | 密码摘要、Cookie、安全错误、刷新会话、跨账户隔离；绑定参考 01 的 4 视口、键盘和非作者 EARLY / FINAL | ACK 后承接现有 RED / diff；Argon2 本地依赖已有历史用户授权。首次可运行 auth HTML / CSS 立即停在非作者 EARLY；真实邮件 / 微信仍停在授权门 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / MAIN MERGED 9143638 / REVIEW DISPATCHED / ACK PENDING` / `/root/dtext_main_final` | `dentry@df96ee3` clean；焦点最小修复已合入；新只读 reviewer创建已返回且 running，但 loaded ACK 未返回，4110 与 main入口仍离线 | ACK 后以当前 committed main内容与隔离 schema / 端口恢复真实 Chrome；覆盖 1440 / 768、DOM 焦点、PPT 空态 / handoff、console / overflow，不得用旧图或测试冒充 | loaded ACK 后升级 ACTIVE；PASS 只关闭文本入口 main FINAL，不扩大 D 父项；原生 200%、真实删除、真实 AI / PPTX 仍精确 VERIFY |
