...
- 当前活动项：`M1-F2-D`
- 下一可见检查点：两项数据保留回归及相关5文件55项均已 GREEN；writer 现跑Web回归 / typecheck / diff-check并完成同状态 Chrome SELF，之后冻结单一候选交root实际diff审查。
...
| `M1-F2-D` | `ACTIVE / TWO GREEN / SELF` / `reader_chat_handoff_recovery` | fresh store 未发送草稿恢复 / 成功发送后清理已PASS；hydrate 保留服务端draft、retryRequestId / retryText；定向5文件55项PASS | 同一 `readchat` worktree 完成Web回归与真实Chrome SELF后冻结候选；不得新建worktree，提交 / 废弃后由总控同事件移除 |
