...
- 当前阶段：桌面 Web `M1-F2-A` 功能闭环保持通过；真实 Chrome 下的外壳、密度、桌宠与封面叠层问题已最小修正，本地书固定生成背景资产尚未提供，视觉结论保持 `VERIFY`。
...
- 当前活动项：`M1-F2-A`。
- 协调下一动作：保留既有功能，不合并、不推送；等待主任务提供经确认的固定封面背景资产，再接入已预留的稳定 `cover-variant-1…5` 映射并在同一用户 Chrome 标签页复验。
...
| `M1-F2-A` | `VERIFY` / 主 Agent | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 功能无阻塞；外壳 / 密度 / 桌宠已复验，缺少经确认的本地书固定生成背景资产，纯色回退不得作为最终封面 | CORE、API、WORKER、DB、WEB、QA；变更限定于 A 的契约、领域解析、Server、Web、视觉事实源与验收证据 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名、作者缺失、刷新、账户隔离；Web 4 视口与 02/03 绑定参考 | `08-real-chrome-1440x844-dpr2.png`；稳定 `cover-variant-1…5` 边界已预留，等待批准资产后继续 |
...
- 剩余视觉阻塞：当前纯色封面只作为安全回退，不是完成候选；需主任务提供一组经确认的固定生成背景资产，运行时按书籍 ID 稳定哈希映射，叠加真实书名 / 作者，不做运行时 ImageGen、随机变化或 CSS 伪插画。
- 自动证据：候选分支 `pnpm verify` 通过 11 个测试文件 / 38 项测试和生产构建；定向视觉与状态回归 2 文件 / 7 项通过；证据索引为 `redesign-v2/output/acceptance/m1-f2-a/README.md`。这些结果不替代封面资产接入后的视觉复验。
