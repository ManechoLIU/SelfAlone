...
- 下一可见检查点：`M1-F4-C-WEB-HANDOFF` 已以 `ec9ca36 → 3c3e903` 顺序进入 current main，非作者 PASS，main Web `296/296`、typecheck、build、视觉契约、diff-check PASS。真实 Chrome current-main `#/conversation` 被正常导向 `#/auth` 且 console 0；下一检查点为已有本地验收账户完成登录后，在不调用付费模型的可控平台状态下重放三错误态与设置返回。
- 当前阻塞：`M1-F4-C-WEB-HANDOFF` 仅真实端 H3 受本地 Chrome 无已认证会话、仓库无可控三类 `PLATFORM_*` 生产 fixture 约束；代码/main 门已通过。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
- 规则版本：总控已从真实 `main@207631b` 加载当前 AGENTS（SHA-256 `5cf99259…56582`），其 host-aware fallback 使 Grok 安全失败后可先路由到当前宿主内部 GPT，不重置 execution lineage / recovery budget；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。Adaptive Agent Runtime 安装副本已加载并由同一总控机器 ACK 精确 revision `1ad20f6fadee2a297ce2d6e1bd72f096eaf0a0f7`（manifest SHA-256 `b10dfa43…a771`），当前规则与 runtime 以 Git common-dir 握手、五状态投影、Assignment 启动前 ACK/lease、证据进展指纹与 recovery budget 为准。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控。
...
| `M1-F2-WEREAD-H4` | `ACTIVE / GROK BACKEND GATE / ASSIGNMENT M1-F2-WEREAD-H4-GROK-01` / 项目总控 | 腾讯官方 WeRead Gateway 真实 Adapter：Bearer API Key、skill_version、允许列表 api_name、errcode / upgrade_info fail-closed；Grok 4.6 正在隔离 worktree 做两文件 TDD，不持有真实 Key | 用户已在本地产品入口填写真实 `wrk-...` Key；Key 不进入聊天、源码、命令、日志、截图或台账。Adapter PASS 后再释放 Server HTTP/composition 与 Desktop H4 |
...
| `M1-F4-C-WEB-HANDOFF` | `VERIFY / MAIN 3c3e903 / NONAUTHOR + MAIN CODE PASS / REALEND BLOCKED` / 项目总控 | `a810c325 → f5e1b71` 六文件链经原作者 TDD 恢复与 `/root/web_handoff_code_review` PASS 后成为 main `ec9ca36 → 3c3e903`；current-main Web `296/296`、typecheck、build、视觉契约、diff-check PASS。EXHAUSTION/未配置引导配置，UNAVAILABLE 稍后重试；完整 conversation/PPT hash 经设置 loading/error/success/back 保留且不自动发送 | 真实 Chrome current-main 打开会话被导向登录壳，console 0；现无已认证会话或可控三错误态 production fixture。已有本地验收账户登录并具备可控平台状态后重放三错误态、返回/刷新/草稿/焦点；此前不把代码门或登录壳冒充真实端 PASS |
...
- 当前 worktree 共3个：main、唯一 `f312@f6a08f9` 与 `quota-cost@f47a990`。f312 继续作为唯一 Mini DevTools / visual-qa 现场，代码已由 `main@43f1fc3` 等价吸收；`quota-cost@f47a990` 的完整成本链已等价顺序进入 `main@1bb0b76`，两者均已由候选生命周期记为 `absorbed`，禁止重复集成。旧 `m1-f4c-server@08b86254` 的两文件局部 WIP 已被 `main@99ef0ac` 完整实现取代，现以稳定 stash 对象 `3840b17546b1e592e7aa78389a562c88cd532a10`（标题含原路径与 HEAD）归档后移除 worktree；依赖安装产物未归档。不得为 reviewer / 占位 / 纯消息任务新建长期 worktree。历史归档 stash 共13条。
