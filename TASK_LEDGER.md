...
- 当前执行波次：`CONTROL-RESET-1`；Web `221d5bb` 的阶段、草稿、滚动与焦点证据仍有效，但 UI/UX 已因书架 `.library-rail` 与对话 `.desktop-rail` 两套组件树撤销 FINAL 前提，改判 `STRUCTURAL FAIL / REWORK` 并先补共享外壳 brief。D-TEXT `d65ab3b` 已获非作者 `PASS WITH RECEIPT CAVEATS`，可进入共享入口的顺序集成门，但不代表父项 DONE 或完整视觉 PASS。小程序 `f81b463` 全矩阵审查已因 drawer 与 Reader 视觉偏差判 `VISUAL FAIL / REWORK`；旧全端合同只保留历史。台账仍由项目总控独占。
- 当前活动项：Web 唯一 writer `/root/desktop_shell_unify` 尚未返回文字 ACK，但 `6afd@221d5bb` 已出现边界内 5 文件 WIP（`main.ts`、`styles.css`、`ui/desktop-shell.ts` 及两项必要测试，`+107/-98`），已按文件证据登记 `IMPLEMENTATION ACTIVE / TEXT ACK MISSING`；UI/UX 已交当前 DESIGN-READY / REWORK brief 并回到 idle，等待冻结候选后再做 FINAL。D-TEXT 非作者 reviewer 已完成并释放容量；入口集成等待 Desktop writer 释放 `main.ts` 所有权后由总控顺序安排。Mini `/root/mini_visual_rework` 同样尚无文字 ACK，但 f312 已出现限定 drawer / Reader WIP 与 4 张新鲜候选截图，按文件证据登记 ACTIVE；`IMG-04` 仍只是候选，正式运行时资产未通过。
- 协调下一动作：UI/UX 交共享壳 brief，Desktop writer 以单一 shell / nav / brand / rail 组件闭合 fresh conversation、library 往返与状态恢复，补 RED/GREEN、隔离真实 Chrome 与 scoped commit 后重新 FINAL。D-TEXT 不抢共享入口；Mini 必须重跑当前最终 WIP 并提交测试 / DevTools 证据，完成 EARLY→FINAL。文件活动只证明已开工，不等于根因关闭或视觉通过。
...
| `M1-WEB-SHELL-01` | `STRUCTURAL FAIL / REWORK / DESIGN BRIEF READY / IMPLEMENTATION ACTIVE BY FILE EVIDENCE / TEXT ACK MISSING` / `/root/desktop_shell_unify` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | `6afd@221d5bb` 是返工基线；当前边界内 5 文件 WIP（`main.ts`、`styles.css`、`ui/desktop-shell.ts` 与两项测试，`+107/-98`）已开始让 library 复用 shared desktop rail。唯一 writer 不得碰 Reader / D-TEXT 私有文件；尚无新测试、提交或视觉通过收据 | 保留 `221d5bb` 已通过的阶段、草稿、滚动、焦点与四宽子证据；当前 DESIGN-READY / REWORK brief 已交付，审查者 idle 等待冻结。按 02 / 03 抽成单一 shell / nav / brand / rail；fresh `#/conversation`、conversation→library→return、刷新 / 前后退 / `?stage=` / 断网恢复必须落同一组件树与当前会话 / 阶段 / 草稿；补 RED/GREEN、首个真实 Chrome 同状态比较→scoped commit→非作者 FINAL，通过前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / VISUAL REWORK / IMPLEMENTATION ACTIVE BY FILE EVIDENCE / EARLY-VERIFY PENDING / H3 BLOCKED` / UI/UX + `/root/mini_visual_rework` | `f81b463` 后仅 drawer / Reader 视觉文件允许唯一 writer 顺序写入；现场已有 5 个限定源 / 测试 WIP 与 4 张新鲜候选截图，文字 ACK 仍缺。原 f312 writer 已停止这两处，台账与保护文件禁止触碰 | UI/UX 已基于真实 360×640/font23 判 drawer / Reader FAIL；当前最终 WIP 尚未重跑测试或完成 EARLY。RED/GREEN与DevTools→EARLY-VERIFY→scoped commit→FINAL-VERIFY 后才可重新冻结；正式山亭运行时资产仍保持阻塞 |
| `M2-F0-A-VISUAL-REWORK` | `ACTIVE BY FILE EVIDENCE / TEXT ACK MISSING / ASSET CANDIDATE / RUNTIME BLOCKED` / `/root/mini_visual_rework` | 当前仅 `nav-drawer/index.wxml|wxss|test.ts`、`reader/index.wxss|page-state.test.ts` 与新鲜 360 / 390 / 430 候选收据；保护文件哈希仍为 project config `bb02f467…`、lock `9a0d8a10…`，不得改其他六页或保护文件 | 重跑当前最终 WIP；320 / 360 / 390 / 430 与 font23 实测 drawer 列表滚动 / surface、Reader 标题与提示生命周期。`IMG-04` 只可作为候选交 UI/UX、实装与真实 DevTools 对照，未通过前不得升级运行时资产。首屏可运行即交 UI/UX EARLY，冻结后再 FINAL |
