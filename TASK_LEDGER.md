...
- 下一可见检查点：`M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` 已由 Kimi K3 在隔离 worktree `codex/m2-f6-ppt-handoff-main-adapt@e088bbd` delivered ACK 后 ACTIVE；当前只等待它完成真实 RED→6文件 current-main adaptation→GREEN→候选提交。总控不写业务代码；候选返回后立即派独立非作者复审。并行 `M1-F4-B` 继续 BLOCKED；每次状态变化继续项目级重算 READY。
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY / F312 cf6a8c5 / SELF PASS / NONAUTHOR PASS / MAIN ADAPT ACTIVE` | 累计 `0fa1447 + cf6a8c5` 语义由独立 Kimi reviewer 判定符合 SPEC 143–145：当前会话复用、pre-send 无 task/sheet/auto-send、可编辑预填、失败/刷新恢复与 stale fencing 均通过只读审查；current-main 现有3文件基线测试59/59 PASS | current-main adaptation 由 ACTIVE `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` 驱动；不得整支 f312 合入或由总控手改实现 |
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` | `ACTIVE / Kimi K3 WRITER / DELIVERED ACK e088bbd` / `kimi-k3-m2f6-ppt-main-adapt` | worktree=`/Users/echoman/.codex/worktrees/m2-f6-ppt-handoff-main-adapt/SelfAlone`，branch=`codex/m2-f6-ppt-handoff-main-adapt`，owned=6文件且 ACK 前 observed modified files=0；以 `0fa1447 + cf6a8c5` / f312 `f6a08f9` 仅作只读参考，禁止整支 merge/cherry-pick | 当前动作=先真实 RED 锁住旧 v1 提前 taskId/awaiting-confirmation/sheet，再 TDD 重织 `onShow/beginSend/persistLocalState`，保留 current-main hydration/persistence/API；停止条件=第7文件、install、lock/config/visual、production/network 或无法保留现有语义。GREEN 后 scope-only commit 并交候选，不集成 |
