...
- 协调下一动作：Web 候选逐阶段终验通过后由总控集成 / main 复验并立即释放 D-TEXT Web 唯一入口写入者；若失败则只返修精确 P1。小程序收 UI/UX 阈值 / 滞回 brief 后，由 f312 顺序修共享几何、键盘高风险面、reader 提示和六页 / 抽屉 / 面板；冻结后按四宽四高、状态 / 安全区 / 键盘矩阵复验。正式山亭与 PPT 预览资产未登记前保持局部阻塞。
