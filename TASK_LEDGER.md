...
- 当前活动项：`M1-F2-D`
- 下一可见检查点：writer 已取得写入权，先交失败测试 RED（精确引用 / 书籍身份、0 auto-send、current→recent→create、失败保留与一次消费），再冻结单一候选并做非作者验证。
...
| `M1-F2-D` | `ACTIVE / DELIVERED ACK` / `/root/reader_chat_handoff_recovery` | 文本标注、想法、手工笔记、详情入口已有 scoped PASS；当前包只连接 TXT / EPUB 精确引用→现有会话草稿 | `readchat@b376a5b` clean起步；exact规则 / 12文件 / RED / stop已ACK，现从失败测试推进 |
