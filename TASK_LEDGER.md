...
- **活动实现线**：Desktop Settings `set1@039cb596` 与 Desktop Conversation `f3bc@56a850e` 均已出现各自获批文件内的返工 WIP；Mini Conversation 的 scenery 范围在 `f312@891d26e` 完成作者真实 DevTools 局部复验，释放槽位后已轮转为 Reader / 老己笔记短非作者 EARLY。Book Detail `75e764c + 6c50a76` 私有 Web / Server 候选继续冻结，等待 shared route seam。
- **下一用户可见检查点**：先取得同一 `f312@891d26e` Reader 的初始 → 全屏 → 回落 → 重开背景保留与老己笔记真实 DevTools PASS / FAIL；并行取得 Settings 失败保留 / 焦点修复候选与 Conversation 逐键输入 / 三栏正常态修复候选；任一候选通过 EARLY 后按 Auth → Settings → Conversation → Book route 的共享接缝顺序合入本地 main 并复验。
- **真实阻塞**：无全局串行阻塞。Settings 与 Conversation 的两个 P1 正在各自私有边界返工；Mini Conversation scenery 与四宽 / 状态矩阵局部通过，但 DevTools 无软件键盘和 reduced-motion 控制，故不能升级完整 EARLY；Book Detail 真实 HTTP / 浏览器仍等待 `app.ts` / `index.ts` 接缝。共享文件只允许顺序集成；真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未授权。
- **当前可见入口 / revision**：Settings 候选入口 `http://127.0.0.1:4299/` → `http://127.0.0.1:5178/#/settings`，revision `039cb596`，API `4298` / Web `5178` / bootstrap `4299` 绑定隔离 schema `settings1_early_20260825`，HTTP 可达但真实 EARLY 为 P1 FAIL；稳定 main Web `http://127.0.0.1:4127/` / API `http://127.0.0.1:4198/api/v1/workspace` 仍绑定旧产品 revision `ab13310`，只作恢复点；Mini 唯一 DevTools 项目为 `f312@891d26e`，当前页面已轮转至 Reader 复验。
...
| `M2-F4` 小程序书架与阅读 | `VERIFY` · `NON-AUTHOR EARLY DISPATCHED / PREFLIGHT PENDING` / `/root/mini_conversation_visual` | 目标：统一书架、连续正文、内容面板、老己笔记与背景恢复；非目标：真实 notes API / 跨设备。Reader 私有候选 `866f237` 已有作者 390 收据 | 当前唯一候选为 `f312@891d26e`；只读复验，不得改 Reader、config、lock、QA 图；首条视觉 / 状态 / console / overflow FAIL 即停 | refs 18～20；执行者回传 exact preflight 后才升级 ACTIVE；先验 390 初始 / 展开 / 回落 / 重开背景、Notes / 编辑 / 失败保留，PASS 才扩 320 / 360 / 430 |
...
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `VERIFY` · `NON-AUTHOR EARLY DISPATCHED / PREFLIGHT PENDING` / `/root/mini_conversation_visual` | 目标：参考 19 的老己笔记单列全宽流、sheet 手势与浅色背景重进保留；非目标：真实 notes API / 跨设备。Reader candidate `866f237` 已进入 f312 | 当前唯一候选为 `f312@891d26e`；只读复验，首条 FAIL 即停；不得改保护 config / lock / QA 图 | Reader 四个私有源 / tests；绑定 refs 18～20，SHA-256 `2ddebcb6…152f` / `23d078e3…6077` / `e76a2817…19cb` | 作者旧 390 局部收据在 `mini-reader-early-866f237/`；执行者回 exact preflight 后做当前 revision 390 首门，PASS 才扩 320 / 360 / 430 |
...
| 当前 Mini 候选波次 | Conversation `891d26e` scenery / 四宽 / 状态矩阵局部通过但 H3 未覆；Reader `866f237` 已在同一 f312 派发短非作者 EARLY | Conversation 稳定证据 `mini-conversation-early-891d26/`；Reader 绑定 refs 18～20，先做 390 首门。唯一 DevTools 路径和 project config / lock 保护现场未变。 |
...
| 开发基线 | 台账事件写入前 `main@025206c`；所有当前产品候选未合入 main | main tracked clean，三个既有未跟踪视觉目录原样保留；候选提交、测试或作者自验不能自动 `DONE`。 |
