...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复及北京时间双时段计费门已分别在 `main@b637023+81f5f1f+df32890` 闭合：正式 Server 从独立部署侧平台凭证与六项高峰 / 空闲价格创建 DeepSeek 平台模型，单次请求只读取一次时钟并按工作日 9:00–12:00、14:00–18:00 选档；旧单档、缺项、控制字符、空白包裹、超安全整数及非法时钟全部 fail closed。下一本地动作是对账并发硬上限、失败释放与 ACK 丢失恢复的现有测试宇宙，只补真实能力层仍缺的因果 Case；外部 H4 继续等待独立平台配置与付费调用授权。
...
| `M1-F4-V1` | `VERIFY / MAIN b637023+81f5f1f+df32890 / LOCAL CONCURRENCY AUDIT NEXT` / 项目总控 | 领取、AI / PPT消耗、失败恢复和并发硬上限 Case；正式 Server 已接入独立平台环境配置，fake HTTP 证明用户模型未配置且领取后按 reserve→provider usage→settle 顺序成功；真实隔离 PostgreSQL RED→GREEN 证明 settle 已提交但 ACK 丢失时只回读一次相同金额并返回原回答，不重复 provider，未提交的结算失败仍释放并映射 exhaustion；双时段动态计费及非法配置拒绝已进入 main | 先对账现有成本 Store 并发硬上限、能力层失败释放与结算 ACK 丢失恢复的因果覆盖，只补仍缺的真实能力层 Case；真实 Provider H4、真实成本流水和浏览器成功路径仍须独立平台 Key、六项当次价格快照及明确付费调用授权 |
