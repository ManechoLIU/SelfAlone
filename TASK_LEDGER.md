...
- 当前执行波次：`CONTROL-RESET-1`；Web 已冻结 clean tracked 候选 `47db65b`，16 文件 / `+929 -89`，单一右侧阶段工作区、hash / history 单一 `renderRoute`、阶段草稿 / scroll / focus 与失败恢复已有测试和作者侧真实 Chrome 收据；总控新鲜定向 4 文件 / 30 项及全仓 24 文件 / 131 项、typecheck / build 通过，现由非作者执行真实端终验，禁止提前合并。D-TEXT 私有 Web `e2a6ac4` 的非作者 verdict 为 `FAIL / REWORK`：保存后 Selection 残留造成重复记录 / 错误引用、详情 tabs / AX 与底层焦点错误、失败恢复首屏不可达及并发冲突重试不可自愈；原作者已在五个私有测试文件写入回归红灯。小程序已追加 `6586bca → b28aa9c` 两个局部点；Reader failure 已改为稳定 shell 原位失败 / 重试恢复，但父项仍为 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 已恢复为 D-TEXT 私有唯一写入者；总控复现定向 5 files / 41 tests 为 6 failed / 35 passed，状态 `ACTIVE / TDD RED CONFIRMED`，共享入口继续排队。同一非作者 UI/UX 已清理 D-TEXT 端口 / schema 并转入 `6afd@47db65b` 桌面对话 FINAL-VISUAL-VERIFY；4173 旧 main 已排除。小程序同一 writer 已冻结 `b28aa9c` Reader failure checkpoint并继续 PPT failure 子项；总控现场已见 `page-state.test.ts / page-state.ts` 私有 diff，定向 1 file / 6 tests 由红转绿，状态 `ACTIVE / GREEN IN PROGRESS`。两个受保护差异保持未提交，其最终复验仍排在 Web 之后。
- 协调下一动作：D-TEXT 原作者在限定私有文件完成 GREEN、真实 Chrome 重验与 scoped commit；UI/UX 完成 `47db65b` 桌面对话后再复验小程序冻结候选，每项独立清理端口 / schema / 进程。小程序 PPT 子项继续完成稳定 shell / context、retry、真实 DevTools 与 scoped commit；全端候选冻结后再做四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验。
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / TDD RED CONFIRMED / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | clean 基线 `dtxtui@e2a6ac4` / `codex/m1-f2-d-text-web` 已完成独立真实 Chrome 复验；0 P0 / 4 P1 / 1 P2。原作者现仅修改 5 个私有 Web 测试文件；总控复现 41 tests 为 6 failed / 35 passed，覆盖 Selection 清理 / 单请求去重、真实 tab / hidden panel、409 刷新版本后保留输入并自重试、44px 恢复与图标派生 | 只准改原 text-reader / text-annotation / book-detail 私有文件，不碰 `main.ts` / shared shell / styles 或共享 contracts / schema；聊天 handoff 在共享路由接入前保持 disabled fail-closed | 真实 EPUB / TXT 选区单次保存后退出、独立笔记无残留引用、真实 tab / AX / 焦点圈定、失败与并发冲突原位保留并可恢复、浅深与两视口非作者通过 | 真实 quote intent、共享 main 入口、VoiceOver、1200 / 1024、长书滚动和完整 loading / 载入失败仍未覆盖；不能称用户可见闭环完成 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c` 已形成十一个局部恢复点；Reader failure checkpoint 已冻结，后续 PPT failure 子项正以同一 workspace/context 保留合同返工 | 总控核对 Reader 11 文件范围、保护哈希和定向 2 files / 13 tests；PPT 子项现场 `page-state.test.ts / page-state.ts` 私有 diff 为 1 file / 6 tests GREEN，WXML / CSS / retry / 真实 DevTools / scoped commit 尚未完成。drawer 正式山亭仍 ASSET BLOCKED，软件键盘仍为 0；父项与 H3 均不升级 |
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / TDD RED CONFIRMED / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@e2a6ac4`、`codex/m1-f2-d-text-web`；非作者已完成独立真实 TXT / EPUB、Selection、持久化、失败 / 并发、主题 / 专注、AX 与两视口复验并清理环境 | 原作者已写入 5 个私有测试文件；总控定向复现 6 failed / 35 passed。停止条件为 GREEN + 全仓 verify / diff-check + 同一真实 Chrome Case 重验 + scoped commit；全局路由仍在 Web shell 与私有候选分别通过后由总控顺序接入 |
...
| `M1-WEB-SHELL-01` | `VERIFY / CANDIDATE FROZEN / NONAUTHOR ACTIVE` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main 旧 M0；`6afd@47db65b` 为 16 文件 / `+929 -89` 冻结提交，新增单一阶段工作区、`conversationHref` / `lastConversationStage`、workspace / cache、requirements / outline draft、双区 scroll 与单一 `renderRoute`；acceptance README 已纳入提交，未跟踪 `data/` 不属于候选 | 总控定向 4 文件 / 30 项、全仓 24 文件 / 131 项、typecheck / build / diff-check 通过。作者收据声明 768×844 全阶段 composer 可见、fresh #/conversation→library→返回、reader→conversation、刷新 / 前后退 / `?stage=`、断网草稿 / scroll / focus 与真实 retry 恢复；非作者已在 D-TEXT 清理后转入真实 Chrome，重验消息密度、02 Token、桌宠锚、四宽与路由同源，结论前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / ELEVEN CHECKPOINTS COMMITTED / DESIGN-READY / STATE-MATRIX REWORK / PPT FAILURE GREEN IN PROGRESS / REVERIFY QUEUED` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `b28aa9c` 的 Reader failure checkpoint 已复核；PPT 后续子项当前只改 `page-state.test.ts / page-state.ts`，总控现场复跑 1 file / 6 tests GREEN，后续只准继续 PPT 私有 page-state / index / WXML / WXSS。非作者当前审 Web，Mini 仍 QUEUED；软件键盘仍未实显，正式山亭缺失。全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 十一个局部点已冻结；`b28aa9c` 只关闭 Reader failure 稳定 shell / 上下文与重试恢复局部实现。软件键盘、正式山亭 / PPT 资产、其余状态与最终全路由视觉均未完成；既有受保护差异保持未提交。 |
...
| 开发基线 | 产品集成恢复点 `main@f85d86c`；当前控制面续接于 `main@5519eb4` | A 功能闭合但视觉重开；B / C 共享接缝和 C 安全首包已进入 main，D-TEXT 共享接缝已合并但 Web 闭环返工中；Web shell / miniapp 候选未集成。主线两个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c` 是十一个局部恢复点；Reader failure checkpoint 不替代全小程序六页 / 共享层四宽四高和状态矩阵，真实软件键盘、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
