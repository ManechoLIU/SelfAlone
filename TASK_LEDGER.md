...
- 当前活动项：Web 唯一 writer `/root/desktop_shell_unify` 尚未返回文字 ACK，但 `6afd@221d5bb` 已出现边界内 5 文件 WIP（`main.ts`、`styles.css`、`ui/desktop-shell.ts` 及两项必要测试，`+107/-98`），已按文件证据登记 `IMPLEMENTATION ACTIVE / TEXT ACK MISSING`；UI/UX 继续只读形成 02 / 03 共享左栏 brief，不再继续旧候选 FINAL。D-TEXT 非作者 reviewer 已完成并释放容量；入口集成等待 Desktop writer 释放 `main.ts` 所有权后由总控顺序安排。Mini `/root/mini_visual_rework` 同样尚无文字 ACK，但 f312 已出现限定 drawer / Reader WIP 与 3 张新鲜候选截图，按文件证据登记 ACTIVE；`IMG-04` 仍只是候选，正式运行时资产未通过。
...
| `M1-WEB-SHELL-01` | `STRUCTURAL FAIL / REWORK / DESIGN BRIEF ACTIVE / IMPLEMENTATION ACTIVE BY FILE EVIDENCE / TEXT ACK MISSING` / `/root/desktop_shell_unify` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | `6afd@221d5bb` 是返工基线；当前边界内 5 文件 WIP（`main.ts`、`styles.css`、`ui/desktop-shell.ts` 与两项测试，`+107/-98`）已开始让 library 复用 shared desktop rail。唯一 writer 不得碰 Reader / D-TEXT 私有文件；尚无新测试、提交或视觉通过收据 | 保留 `221d5bb` 已通过的阶段、草稿、滚动、焦点与四宽子证据，但先按 02 / 03 brief 抽成单一 shell / nav / brand / rail 组件。fresh `#/conversation`、conversation→library→return、刷新 / 前后退 / `?stage=` / 断网恢复必须落同一组件树与当前会话 / 阶段 / 草稿；补 RED/GREEN、首个真实 Chrome 同状态比较→scoped commit→非作者 FINAL，通过前禁止合并 |
