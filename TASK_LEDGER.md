...
- 当前执行波次：`CONTROL-RESET-1`；Web `47db65b` 的非作者终验已判 `FAIL / REWORK`，原作者已在 4 个限定 Web 文件中开始修正 1024px 覆盖、生成中锁定文案与纸白 / 青瓷密度，总控全仓 24 files / 133 tests、typecheck / build / diff-check 全绿，但真实 Chrome 与冻结提交尚缺。D-TEXT 私有 Web返工已扩为 13 个私有文件；总控定向 41 项与全仓 30 files / 145 tests、typecheck / build / diff-check 全绿，仍需同一真实 Chrome Case 与提交。小程序已推进到 `e4d94b4`，其中 `79c9d8b` 关闭 PPT failure→retry 局部恢复，`e4d94b4` 只补四尺寸机械矩阵；父项保持 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 为 D-TEXT 私有唯一写入者，状态 `ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING`，共享入口继续排队。原 Web 作者 `/root/desktop_shell_m0_fix` 已以 4 个文件 / `+67 -19` 的实际 WIP 回执进入 `ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING`，非作者已清理上一轮 Web 端口 / schema。小程序同一 writer 已提交 `79c9d8b → e4d94b4`；前者经总控复核 8 文件、保护哈希、11 files / 61 tests 与两张 DevTools 收据通过，后者仅含 4 张 320 / 360 / 390 / 430 PPT JPEG；当前又以新增 `pages/library/index.test.ts` 进入 Library direct-failure TDD RED，1 file / 1 test 命中首次失败无 recovery snapshot。UI/UX 保持 `QUEUED / CANDIDATE MOVING`，两个受保护差异保持未提交。
- 协调下一动作：Web 作者完成 1024px、生成中与密度的真实 Chrome 四宽短高复验和 scoped commit；D-TEXT 完成同一真实 Chrome Case 与 scoped commit。小程序先最小闭合 Library 首次失败保留 3 本已知书与内联 notice，再继续剩余系统矩阵；只有同一 writer 明确冻结完整候选并回传唯一 clean scoped HEAD 后，总控才向 UI/UX 释放全端四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验；禁止混用 `598e34f` 与后续链证据。
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | 基线 `dtxtui@e2a6ac4` / `codex/m1-f2-d-text-web` 已完成独立真实 Chrome 首轮复验；原作者当前只改 13 个私有 Web state / view / runtime / CSS / test 文件。总控复跑原 5 files / 41 tests 与全仓 30 files / 145 tests、typecheck / build / diff-check 全绿，覆盖 Selection 清理 / 单请求去重、真实 tab / hidden panel、409 刷新版本后保留输入并自重试、44px 恢复与图标入口 | 只准改原 text-reader / text-annotation / book-detail 私有文件，不碰 `main.ts` / shared shell / styles 或共享 contracts / schema；聊天 handoff 在共享路由接入前保持 disabled fail-closed | 真实 EPUB / TXT 选区单次保存后退出、独立笔记无残留引用、真实 tab / AX / 焦点圈定、失败与并发冲突原位保留并可恢复、浅深与两视口非作者通过 | 同一真实 Chrome Case、scoped commit 和再次非作者复验尚未完成；quote intent、共享 main 入口、VoiceOver、1200 / 1024、长书滚动和完整 loading / 载入失败仍未覆盖 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4` 已形成十三个局部恢复点；`79c9d8b` 仅关闭 PPT later-load failure 保留 workspace / draft / stage / previews 与同工作区 retry，`e4d94b4` 仅补该状态的四尺寸证据 | 总控核对 `79c9d8b` 的 8 文件范围、保护哈希、commit diff-check、11 files / 61 tests 与两张 360×640 DevTools 收据；`e4d94b4` 仅含 4 张 320 / 360(font23) / 390 / 430 JPEG，四态均 keyboard=0。PPT 预览仍为开发适配器且不冒充真实产物；drawer 正式山亭仍 ASSET BLOCKED，软件键盘仍为 0；父项与 H3 均不升级 |
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@e2a6ac4`、`codex/m1-f2-d-text-web`；非作者已完成独立真实 TXT / EPUB、Selection、持久化、失败 / 并发、主题 / 专注、AX 与两视口复验并清理环境 | 当前 13 个私有 Web 文件 / `+386 -57`；总控定向 5 files / 41 tests、全仓 30 files / 145 tests、typecheck / build / diff-check 通过。停止条件仍为同一真实 Chrome Case 重验 + scoped commit；全局路由在 Web shell 与私有候选分别通过后由总控顺序接入 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main 旧 M0；`6afd@47db65b` 非作者已覆盖 requirements / outline / template / 真实 generating / failed / completed、四宽、路由 / 草稿 / scroll / focus / retry / AX / console 并清理环境；原作者当前只改 conversation view / test、desktop-shell test 与共享 styles 4 文件 | WIP `+67 -19` 已处理 1024px compact、生成中精确锁定文案与纸白 / 青瓷密度，总控全仓 24 files / 133 tests、typecheck / build / diff-check 通过；仍须隔离端口真实 Chrome 1440 / 1200 / 1024 / 768×844、各阶段输入可见和首次进入 / 往返同源 Case、scoped commit，再由非作者按 02 终验。完成态无独立定稿只可验结构 / 共享语言，禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / THIRTEEN CHECKPOINTS COMMITTED / DESIGN-READY / STATE-MATRIX REWORK / LIBRARY FAILURE TDD RED / REVERIFY QUEUED / CANDIDATE MOVING` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `79c9d8b → e4d94b4` 只关闭 PPT failure→retry 的局部逻辑与 320 / 360 / 390 / 430 机械矩阵；当前 Library direct-failure 回归 1 file / 1 test 处于 RED，要求保留已知 3 本书并原位显示可恢复失败。旧 `598e34f` 已过期，UI/UX 保持 `QUEUED / CANDIDATE MOVING`；软件键盘仍未实显，正式山亭与真实 PPT 预览缺失，全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 十三个局部点已冻结；`79c9d8b` 关闭 PPT later-load failure 稳定 workspace / context 与 retry，`e4d94b4` 只补四尺寸 mechanical matrix。软件键盘、正式山亭 / PPT 资产、其余状态与最终全路由视觉均未完成；既有受保护差异保持未提交。 |
...
| `M1-F2-D-TEXT` 共享集成与私有候选 | `263d3e2 → 18c7522` 私有种子；`94056be → ed95d95 → 5156b9c → main@f85d86c`；私有 Web `e2a6ac4` | 共享 contracts / migration / API / publisher 已通过原非作者与 main 复验；私有种子不再单独合并。`dtxtui@e2a6ac4` 非作者 verdict 为 `FAIL / REWORK`，已退回原作者且 shared entry 继续等待 Web shell 与私有候选分别通过。 |
| 开发基线 | 产品集成恢复点 `main@f85d86c`；当前控制面续接于 `main@f216fba` | A 功能闭合但视觉重开；B / C 共享接缝和 C 安全首包已进入 main，D-TEXT 共享接缝已合并但 Web 闭环返工中；Web shell / miniapp 候选未集成。主线两个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4` 是十三个局部恢复点；Reader / PPT failure checkpoints 与机械矩阵不替代全小程序六页 / 共享层四宽四高和状态矩阵，真实软件键盘、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
