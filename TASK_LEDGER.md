...
- 当前执行波次：`CONTROL-RESET-1`；项目总控已从唯一主线完整加载 `main@9764180` 的 `AGENTS.md` 并回传 `CONTROL CONTRACT LOADED main@9764180`。Desktop `6afd@44e66af` 已获非作者 `PASS` 并合入 `main@deba325`；D-TEXT `dentry@df96ee3` 已合入 `main@9143638`，但独立 main Chrome FINAL 仍为 `VERIFY / BLOCKED`；E-OWNER `owner@9512c40` 已以 `main@f1334ad` 集成并通过 main 定向 4 / 4、全仓 33 files / 187 tests、typecheck / build。主线除本台账未暂存控制面 diff 外，仍只有三个既有未跟踪视觉候选目录。
- 当前活动项：用户明确否决 `f312@cb4752f` conversation 视觉，旧直接 FINAL 已撤销；未返回 `main@9764180` 加载 ACK 的旧 reviewer 已中断，因此当前没有 conversation 视觉验收执行者，结论保持 `FAIL / REWORK / EARLY PENDING`。Reader 已冻结 clean checkpoint `rdrs@387c152`（仅 5 个 Reader 私有文件），drawer 已冻结 clean checkpoint `drwr@23e95f0`（仅 4 个 drawer 私有文件）；两 writer 均已加载 `main@9764180`、停止写入与 DevTools 操作。旧 drawer writer 已回收，由 replacement 完成 checkpoint；不存在未 ACK 仍继续受影响写入的 live Agent。
- 协调下一动作：总控审查并把 `387c152`、`23e95f0` 顺序集成到唯一候选 `f312@cb4752f`，保护既有 `project.config.json` / `pnpm-lock.yaml` 差异与旧 EARLY 图；随后只从 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp` 派非作者对 conversation / Reader / drawer 做真实 DevTools EARLY，FAIL 交原范围最小返工，PASS 后才冻结并进入 FINAL。`/root/auth_email_session` 已回传 `auth1@9764180` clean、精确 CORE / API / WEB 所有权、首个认证 RED 与停止门 ACK，现已触发继续执行；优先非视觉 core / API，首次可运行 auth HTML / CSS 必须立即进入非作者 EARLY。
- main 可访问入口收据：4198 / 4127 仍运行 `main@5d89aa5` 产品树，隔离 schema 为 `control_main_5d89aa5_1205`；API ready、Web `/` / `#/conversation` 与代理 health 均为 200。治理主线已前进至 `main@9764180`，故此入口只作为稳定可访问恢复点，不冒充最新治理 HEAD 的重启证据，也不替代 D-TEXT Chrome FINAL。原 4173 仍返回 404，排除。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `ACTIVE / ACK RECEIVED` / `/root/auth_email_session` | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | 独立 clean `auth1@9764180`、branch `codex/m1-f1-a-auth`；`E-OWNER` 已以 `main@f1334ad` 冻结共享 owner 接缝，候选 `658ba50` 仅作种子。owner 已回传 domain / contracts / server / web 精确边界、认证 RED 与停止门；账户迁移或其他共享 writer 冲突时暂停 | CORE、API、DB、WEB、QA；不得与 Mini、D-TEXT 或后续共享 schema / app 集成并写 | 密码摘要、Cookie、安全错误、刷新会话、跨账户隔离；绑定参考 01 的 4 视口、键盘和非作者 EARLY / FINAL | 先写 Argon2id、HttpOnly session refresh / revoke、重复邮箱与错误不泄露、隔离和旧 header 不越权 RED；本地假邮件无需外部凭证，真实邮件 / 微信停在授权门 |
...
| `M2-F3` 小程序会话 | `VISUAL FAIL / FROZEN cb4752f / EARLY PENDING / OWNER NONE` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | `f312@cb4752f` 只含 conversation 私有 6 文件；行为定向 9 / 9 已绿，但用户新证据确认当前按钮 / 构图仍不符合绑定参考，故撤销直接 FINAL。旧 reviewer 未回 `main@9764180` 加载 ACK，已中断，不允许旧结论覆盖 | Reader / drawer checkpoint 集成同一 f312 后，派新非作者只用唯一常驻 DevTools 对参考 13 + 30～32做 EARLY；覆盖 320 / 360 / 390 / 430、font23、safe-area、选择层生命周期、摘要 / 单入口、1 / 3 / 5 行与附件、趴宠和正常文本扫描 | EARLY FAIL 后释放唯一 conversation writer最小返工；通过 EARLY、冻结新候选后才做 FINAL |
| `M2-F4` 小程序书架与阅读 | `READER VISUAL FAIL / DESIGN-READY / CHECKPOINT FROZEN 387c152 / EARLY PENDING` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | `rdrs@387c152` clean，仅 5 个 Reader 私有文件，定向 20 / 20、miniapp 76 / 76、typecheck / build / diff-check 已由 writer 报绿；总控已核对提交范围与可见内部文本扫描。隔离 writer已停止，不再操作DevTools | 总控顺序集成 f312 后，只在唯一常驻 DevTools 做 320 / 360 / 390 / 430、font23、safe-area、reduced-motion EARLY；EARLY FAIL 最小返工，PASS 后再 FINAL。rdrs 最近入口只待安全移除，不删除源码 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / MAIN MERGED 9143638 / MAIN FINAL REVIEW DISPATCHED / ACK PENDING` / `/root/verify_dtext_main_final` | `dentry@df96ee3` clean；焦点最小修复已合入，main 既有 `pnpm verify` 为 32 files / 183 tests、typecheck / build PASS。新 reviewer 已固定预期 `main@2101695` 并获只读任务边界，创建调用真实返回；一次有界等待尚无 ACK，故不写 ACTIVE | 旧 Chrome 曾被 `ERR_BLOCKED_BY_CLIENT` 阻塞；本轮要求隔离 schema / 端口与真实 Chrome 重试 1440 / 768、DOM 焦点、PPT 空态 / handoff、console / overflow。ACK 前不得启动验收，不得改用 in-app Browser、旧图或测试冒充 | 收到 cwd / HEAD / status / 环境计划 ACK 后才升级 ACTIVE；PASS 只关闭本次文本入口 main FINAL，不扩大 D 父项 | 原生 200%、真实删除、真实 AI / PPTX 仍精确 VERIFY |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / SYSTEM VISUAL REWORK / SIZE PASS 3fed4c7 / READER+DRAWER CHECKPOINTS FROZEN / H3 BLOCKED / NOT IN MAIN` | 用户最新 430×932 证据否决 conversation + drawer 整体视觉；旧 Reviewer 的 Reader / Alpha / 四宽机械 PASS 只保留为局部证据。`3fed4c7` 仅关闭机械包体门；Reader `387c152` 与 drawer `23e95f0` 已分别 clean 冻结，尚未集成 f312 或取得统一 EARLY | conversation reviewer因规则传播 ACK 缺失已中断；总控下一事件顺序集成两个 checkpoint，再从 f312 统一 EARLY。软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION EARLY PENDING / READER 387c152 + DRAWER 23e95f0 FROZEN / INTEGRATION PENDING / IMG-04 ALPHA VERIFY / IMG-05 STRUCTURE-ONLY ADOPT` | 唯一常驻 DevTools 项目为 `f312@cb4752f`；Reader / drawer 隔离 writer均已加载 `main@9764180`、完成 clean scoped checkpoint并停止。旧 conversation reviewer未 ACK后已中断；当前 Mini无写入者或 reviewer | 总控审查后顺序集成两个互斥 checkpoint至 f312，再派新非作者从唯一 f312 做 conversation / Reader / drawer EARLY；FAIL 按文件边界派唯一 writer，PASS 后才冻结并独立 FINAL。430、真实键盘、长列表、reduced-motion 与未覆盖五态不得静默升级 |
