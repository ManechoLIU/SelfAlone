...
- **活动实现线**：Auth / Settings与Mini Reader / Conversation既有main PASS保持。D-TEXT 768已合入`main@cc0cec0`，隔离入口恢复后补main FINAL。Desktop Conversation `cvmain@87a598c`的epoch / generation修复已过非作者代码门，正在隔离Chrome EARLY。Mini Drawer第二次FAIL后，原writer冻结`cb2b1e4`并释放；两文件已顺序进入唯一`f312@9e88e25`，正在三视口re-EARLY。当前为短验收窗口：controller / integrator 1、implementation writer 0、reviewer 3；首个verdict释放即恢复下一实际开发包，不让reviewer长期占满。
- **下一用户可见检查点**：Conversation `87a598c`给隔离Chrome全Case EARLY verdict；Mini Drawer `f312@9e88e25`给390 / 360 / 320 verdict；D-TEXT在`5198 / 4318@cc0cec0`补齐1440与交互后给scoped main FINAL。Conversation PASS后由总控顺序集成main并复验，再释放Book Detail seam或D-TEXT PPT handoff唯一writer。
- **真实阻塞**：Conversation当前无已知未修P1，但未过Chrome不得称EARLY。Drawer第二次同根因FAIL已由`cb2b1e4`重构短高节奏并进入f312，是否解除只看真实320落款与390 / 360不回退；长列表仍因无真实数据未覆盖。D-TEXT先前仅因`4318 / 5198`退出而中断，现已恢复隔离服务并返回ready_text超长标题书籍；1440、背景 / 专注 / 笔记、console与环境清理仍未闭合。PPT handoff / banner和父项继续开放；Book Detail与D-TEXT PPT handoff仍受Conversation共享 `main.ts` 单writer约束。
- **当前可见入口 / revision**：D-TEXT main候选入口 `5198 / 4318` 绑定产品`cc0cec0`、schema `selfalone_dtext_main_final_cc0cec0_20260826`；Desktop候选入口 `5188 / 4308` 的既有验收绑定仍记`a9408ad`，Conversation另用审查隔离入口。稳定恢复点 `4127 / 4198` 绑定`ab13310`。Mini唯一DevTools为`f312@9e88e25`；config / lock与两张QA图未变。
...
| `M1-F3B` 桌面对话与 AI | `ACTIVE` | 会话、图文消息、选择与书籍上下文 | A 已过候选代码门、正在Chrome EARLY；B～D仍按各自依赖，图片能力按需使用 F3A-B |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `VERIFY` · `CANDIDATE 87a598c / CODE PASS / CHROME EARLY ACTIVE / REOPEN 4` / `/root/conversation_final_review` | 输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话UI；非目标不变。原writer用local epoch + hydrate generation关闭旧snapshot覆盖本地send，并把服务端activeRun恢复为发送锁后释放 | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、base `a9408ad`；隔离端口 / schema / 产物，owned外冲突即停 | 25个Conversation CORE / API / DB / WEB文件与三处共享接缝；本轮仅controller / state及测试3文件 | 非作者定向22 / Web155、typecheck / build / diff-check PASS；现做发送 / 恢复 / retry / 导航 / legacy / Auth Settings / 四宽真实Chrome，首P0/P1立即FAIL |
| `M1-F3B-A1-CONVERSATION-CORE` | `VERIFY` · `CURRENT-MAIN CANDIDATE 87a598c / CODE PASS / CHROME ACTIVE` / `/root/conversation_final_review` | 保留确定性回复、PostgreSQL会话 / 消息、API、Web controller / view、legacy workspace、导航handoff、Server幂等与发送锁；hydrate竞态代码门已关闭，不扩业务功能 | 候选仍须真实Chrome；owned外冲突即停 | 同一25文件候选；禁止扩大到其它页面、DB/schema或Book Detail | stale success / failure、并发generation与activeRun代码门PASS；下一步真实EARLY verdict，PASS后才允许顺序进main |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `MAIN MERGED 31462d9 / READER + CONVERSATION SCOPED FINAL / DRAWER RE-EARLY ACTIVE / H3 BLOCKED` / 项目总控 | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务继续显式开发适配器，不冒充真实跨端 | main闭包不变；f312是唯一DevTools且保护现场不变。Drawer短高两文件已顺序进入 `f312@effece6` | Reader + Conversation证据 `mini-main-final-31462d9/`；Drawer re-EARLY待 `mini-drawer-re-early-effece6/`。软件键盘、tourist凭证、长列表、真实PPT preview继续H3 |
...
| `M2-F3` 小程序会话 | `VERIFY` · `MAIN MERGED 31462d9 / NORMAL + SELECTION SCOPED FINAL / DRAWER RE-EARLY ACTIVE` / 项目总控 | 会话抽屉、文字 / 选择 / 附件状态、输入增长、失败保留；非目标：真实AI / PPTX。normal与selection限定范围已进main，Drawer短高独立复验 | 附件真实选择 / 移除、发送重试、软件键盘、长会话列表与reduced-motion仍未完成，不扩大PASS | `mini-conversation-early-332a737/`、`mini-main-final-31462d9/`；Drawer待 `mini-drawer-re-early-effece6/` |
...
| `M1-F2-D-TEXT-READER-768-FIX` | `VERIFY` · `MAIN MERGED cc0cec0 / MAIN FINAL PARTIAL / REVIEW ACTIVE` / `/root/dtext_768_early_review` | 两文件修复已顺序合入main；候选EARLY PASS。main真实768首帧长标题、toolbar / actions、目录展开与横溢0已保留；中断的隔离服务已恢复 | main 22 / 22、typecheck / build / diff-check通过；`5198 / 4318`、独立schema / 产物只补1440、背景 / 专注 / 笔记、console与清理 | main `apps/web/src/text-reader.css` / `text-reader.test.ts`；当前只读FINAL | 候选 `dtext-768-early-575a612/`；main局部证据 `dtext-main-final-cc0cec0/`。补项后严格verdict；200% / DPR2、其它两个FAIL与父项仍开放 |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `VERIFY` · `cvmain@87a598c / CODE PASS / CHROME EARLY ACTIVE / REOPEN 4` / `/root/conversation_final_review` | 输入、发送、确定性回复、刷新恢复、错误保留、统一对话UI；hydrate竞态三文件候选已冻结并过代码门，原writer释放，非目标不变 | 保留Auth / Settings；隔离DB/schema/端口；不得改代码、Book Detail或Mini；Chrome FAIL回原writer | 25个Conversation文件与三处共享接缝；本轮只改`conversation-chat-controller{,.test}.ts`、`conversation-chat-state.ts` | 绑定02 / 36；非作者22 / 155与构建通过。当前做发送、刷新、retry、导航、legacy、账户与四宽真实Case；PASS后顺序进main |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY` · `READER + CONVERSATION MAIN FINAL / DRAWER f312@9e88e25 RE-EARLY ACTIVE / REOPEN 2 / H3 BLOCKED` / 项目总控 | Reader与Conversation限定范围已复验；Drawer `cb2b1e4`把短高节奏与nav offset最小修复顺序放入唯一f312，正在三视口复验 | 长列表无真实数据；软件键盘、游客凭证 / console、真实PPT preview继续H3 / VERIFY | f312只新增Drawer WXSS / test提交`9e88e25`；H3无writer，保护差异不变 | writer 11 / 11、Mini 15 files / 100、typecheck / build / diff-check通过；EARLY证据待 `mini-drawer-re-early-9e88e25/` |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `MAIN MERGED 31462d9 / READER + CONVERSATION SCOPED FINAL; DRAWER f312 RE-EARLY ACTIVE` / 项目总控 | Reader与Conversation已关闭对应缺陷；Drawer第二次FAIL后`cb2b1e4 → f312@9e88e25`，不再机械压缩56px | 全Mini仍缺附件 / 真实发送重试、软件键盘、长列表与reduced-motion；Drawer不通过前不进main | main闭包不变；f312只追加Drawer两文件，保护现场不变 | 首轮与第二轮FAIL均已落盘；当前同三视口EARLY，PASS后才进入main |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `f312@9e88e25 / RE-EARLY ACTIVE / REOPEN 2` / `/root/mini_drawer_early_review` | ref11与正式山亭asset；390 / 360既有局部PASS，`cb2b1e4`为320压缩固定节奏、72px落款带与nav offset后已顺序进入唯一f312；非目标不变 | writer clean释放；长列表无真实数据继续uncovered，唯一DevTools外即停 | f312只追加 `components/nav-drawer/index.test.ts`、`index.wxss`；保护现场不变 | ref11 SHA `9e764e…c36e`；11 / 11、Mini100与构建通过；当前三视口真实EARLY `mini-drawer-re-early-9e88e25/` |
...
| `M2-F0-A` 当前候选 | `main@31462d9` 含Mini code-only闭包；唯一 `f312@9e88e25` 在同源基线上追加Drawer三轮两文件修复 | Reader + Conversation限定范围FINAL；Drawer `cb2b1e4`已入唯一f312并做三视口re-EARLY。保护project config / lock与QA图未带入main；附件 / 真实发送重试、软件键盘、长列表、reduced-motion与PPT真实预览仍未通过。 |
...
| `M1-F2-D-TEXT` 共享集成与私有候选 | 共享链至`main@f85d86c`；私有Web `d65ab3b`；768修复 `575a612 → main@cc0cec0` | 768候选非作者EARLY PASS并已合入；main自动验证通过，768局部FINAL证据已保存，正在补1440 / 交互 / console与清理。PPT handoff / banner、200% / DPR2与父项仍开放。 |
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT `5198 / 4318@cc0cec0`补FINAL；Conversation `cvmain@87a598c`代码门PASS、Chrome EARLY中；Book Detail继续冻结 | Conversation尚未Chrome verdict，D-TEXT尚未完整main FINAL，不扩大完成。D-TEXT隔离schema / runtime与端口已登记，完成后由总控清理。 |
| 当前 Mini 候选波次 | Reader + Conversation在`main@31462d9`限定范围FINAL；Drawer `cb2b1e4 → f312@9e88e25`正在三视口re-EARLY | 前两轮FAIL证据已保留；新证据写 `mini-drawer-re-early-9e88e25/`，保护现场不变。 |
...
| 开发基线 | 产品顺序集成到 `main@cc0cec0`；Desktop Auth / Settings、Mini Reader / Conversation限定范围PASS，D-TEXT 768待main FINAL | main仅有当前台账事件diff与三个受保护未跟踪视觉目录；Conversation代码门PASS但未Chrome，Drawer仅在f312，均不能自动 `DONE`。 |
...
| `M2-F0-A` 原生运行壳 | `VERIFY / MAIN MERGED 31462d9 / DRAWER RE-EARLY ACTIVE / H3 BLOCKED` | Reader + Conversation限定范围已完成main复验；Drawer `f312@effece6`正在唯一DevTools三视口re-EARLY。真实软件键盘、凭证化clean console、长列表与PPT真实预览仍为未覆盖边界 |
