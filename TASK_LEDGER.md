...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段终验已证实结构主干通过但候选仍 `FAIL / REWORK`：缺阶段返回、刷新丢未提交大纲、模板未按双列、生成中缺当前页 16:9 骨架，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，用户可见 Web 入口待 Web shell 返修终验 / 集成后释放；小程序首个系统适配检查点已提交 `4fc422f`，但整体仍为六页与共享层 `FAIL / SYSTEM REWORK`，f312 继续独占 `apps/miniapp/**`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 已在 `6afd@0e18846` 恢复为唯一写入者，按非作者终验四项阻断做最小 TDD 返修；`01a03652-a5d4-76c1-b0fa-ff9303243664` 继续只读补齐断网 / 恢复 / 控制台证据，待新 HEAD 后重验。D-TEXT Web 入口等待 Web shell 所有权释放。小程序任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 保持 `ACTIVE`：`4fc422f` 已冻结 conversation / drawer 四宽视口基线，当前进入 reader 回顶 / 重进提示恢复 TDD；局部资产门和 f312 两个既有禁止差异保持。
- 协调下一动作：Web 作者先修阶段返回、刷新草稿、模板双列与生成中骨架，冻结新 HEAD 后由原非作者复验；通过后总控集成 / main 复验并立即释放 D-TEXT Web 唯一入口写入者。小程序不等待完整 UI/UX 终验：f312 完成 conversation / drawer 四宽高后按已确认规则做 reader swipe reset TDD，并连续推进其余页面响应式基线；只有依赖精确视觉阈值或资产的局部文件等待 brief。冻结后按四宽四高、状态 / 安全区 / 键盘矩阵复验。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f` 首包仅含 14 个 `apps/miniapp/**` 文件：共享 viewport / keyboard tracker、conversation、nav-drawer、类型 / 测试与 5 张 DevTools 证据；typecheck、8 文件 / 43 项、build `1352 KiB`、diff-check 通过。320×568、360×640、390×844、430×932 无可见横溢，320 抽屉完整可滚；这是视口基线，不是最终视觉通过 | 当前进入 reader 回顶 / 重进提示恢复 TDD，再扩至其余页面 / 状态矩阵。游客 console、真实软件键盘、系统字体、透明山亭及真实 PPT 预览继续阻塞对应门，锁文件与 project config 既有差异禁止触碰 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / ACTIVE` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 固定失败候选 `6afd@0e18846`；作者继续独占 app-state / conversation-view / main / styles / desktop-shell 与本包测试，审查者只读 | 非作者已证明单一右侧工作区、中栏无完整阶段页、唯一 current step 与无横溢通过；阻断为缺前后阶段返回、刷新丢未提交大纲、模板未按可用空间双列、生成中缺同尺寸 16:9 当前页骨架。作者已恢复返修，新 HEAD 冻结前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / CHECKPOINT COMMITTED / DESIGN-BRIEF PARTIAL` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | 首包 `4fc422f` 已提交：viewport / keyboard、conversation、nav-drawer 在真实 DevTools 的 320 / 360 / 390 / 430 四尺寸基线无可见横溢，320 抽屉完整可滚；typecheck、8 文件 / 43 项、build 与 diff-check 通过。当前转入 reader swipe reset TDD；全路由 / 状态矩阵前不合并 |
| `M2-F0-A-H3-REWORK` | `ACTIVE / KEYBOARD H3 UNCOVERED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 与系统适配共用同一 f312 writer，先做不涉及视觉裁决的 H3；既有 `project.config.json` / 锁文件禁止触碰 | `4fc422f` 已证明四宽视口、安全区和键盘覆盖不重复扣减的状态合同；各截图 keyboard 仍为 0，模拟器未弹软件键盘，因此软件键盘 / 字体、游客 console 与凭证化环境仍为真实外部门，不因首包提交降低 H3 总门 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | `4fc422f` 冻结 conversation / drawer 四宽视口基线；reader swipe reset、其余四高 / 状态 / 安全区 / 键盘矩阵与最终视觉未完成。既有锁文件 / project config SHA 保持，未进入提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f` 是首个已提交的 conversation / drawer 四宽视口恢复点；全小程序六页、抽屉、输入 / 键盘与面板的真实四宽四高和状态矩阵、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
