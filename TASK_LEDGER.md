...
| `M1-F2-WEREAD-RUNTIME` | `READY / STORES+ISOLATION MAIN d16159a+c252cc7+5d6227c+9bcaa15 / COMPOSITION NEXT` / 项目总控 | 账户隔离 AES-256-GCM 连接存储，以及账户内 requestId 幂等的书架 / 批注 run、单领取恢复、终态重放与 last-success 快照均已进入 main；连接替换后旧账号书籍 / 批注不会再被新连接身份返回，失败指纹不受对象键序影响 | 下一包把 validation、Gateway Adapter、stores、worker 与8个HTTP端点组成正式 runtime，并用 fake gateway 完成 API→DB→恢复 Case。真实 Key H4仍是独立授权门 |
