...
- 下一可见检查点：`quota-cost@f53c930` 当前 clean；既有 migration/store 定向14/14 PASS 但不覆盖 reviewer 攻击面。总控已用临时测试在隔离 schema 真重现两个 RED 并清理现场：迁移后可直接 INSERT `settled` reservation；兼容旧 schema 中 `reserve.created_at > settle.created_at` 的反序 audit 历史仍被 migration 接受。唯一 READY=`M1-F4-B-REWORK-03`，复用原 worktree、仅拥有 `cost-ledger-migration.ts/.test.ts`；先取得 Grok 4.6 delivered ACK，再 TDD 修复并形成新候选，禁止扩 `store` / config / lock / refs。
...
| `M1-F4-B-REWORK-03` | `READY / Grok 4.6 WRITER / REUSE quota-cost@f53c930` / 待 delivered ACK | owned files=`apps/server/src/cost-ledger-migration.ts` + `apps/server/src/cost-ledger-migration.test.ts`；首个 RED 必须把“direct terminal INSERT”与“reverse audit chronology”两条攻击测试永久写入并观察失败，再做最小实现；不得修改 store / config / lock / refs，不安装依赖 | dispatch 路由=`grok-build / grok-4.6 / oauth`；ACK 后转 ACTIVE。GREEN 至少含 migration 定向 + cost-ledger store 定向 + Server typecheck + diff-check，scope-only commit 后交独立 reviewer；若需第三文件立即停并回总控 |
