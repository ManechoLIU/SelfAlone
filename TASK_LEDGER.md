...
- 当前 Goal：`M1-F2-A` 候选验收、范围提交、本地 `main` 集成与复验里程碑已经完成；下一小里程碑是并行产出 B 的文本阅读候选与 C 的 PDF 安全首包，并由项目总控集成已冻结的共享接缝。实现任务内旧 `blocked` Goal 不作为项目状态来源，执行仍以本台账为准。
- 当前阶段：桌面 Web `M1-F2-A` 已在候选 `ed2cec6` 通过门禁，并以 `38f9ec3` 合入、复验本地 `main`；文本 / PDF 阅读共享接缝已在 `44b81ea` 冻结。B 与 C 已从该基线进入互斥工作树并行，小程序客户端基础包继续独立推进，不整体等待 `M1-V1`。
...
- 当前执行波次：`M1-F2-B-TEXT-READER`、`M1-F2-C-PDF-FIRST-PACK` 与 `M2-F0-MINIAPP-SHELL` 三波并行；执行任务文件范围互斥，共享 contracts / schema / 入口由项目总控单写入。
- 当前活动项：B 由任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` / `codex/m1-f2-b` 从 `44b81ea` 开始，独占 `text-reader*` 与 B 证据；C 由任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` 在 `7ef0` / `codex/m1-f2-c` 从同一基线开始，独占 `pdf-reader*`、PDF Worker / 缓存模块与 C 证据；`M2-F0-A` 由任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` / `codex/m2-miniapp-shell` 从 `ff4767b` 开始，唯一写入 `apps/miniapp/**`。三项均不得写 `TASK_LEDGER.md`。
- 协调下一动作：B 完成真实 EPUB / TXT 的章节、定位、恢复、状态与桌面验收；C 完成真实样本、渲染适配器、安全上限、缓存键、租约 / 重启 / 旧版本防回写和失败关闭证据，并回传生产依赖推荐，未获依赖授权前不做伪页面。项目总控并行准备 contracts、迁移、Server / Web 组合入口与 Worker 共享接入；M2-F0-A 先闭合运行壳与 H3，随后在不与当前文件重叠的下一包接 `38f9ec3` 的真实书架 / 导入契约。
...
| `M1-F2-A` | `DONE` / 项目总控验收 | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 无未闭合阻塞；真实 `1440×1024` Chrome 受物理屏幕限制，24 号只作为明确标注的构图检查而非真实窗口证据 | CORE、API、DB、WEB、QA、DOC；后续任务不得回写 A 候选或旧台账 diff | 真实三类文件；五类状态；搜索、作者缺失、刷新、账户隔离；Chrome 100% / 200%、键盘、视觉与无溢出 | 候选 `ed2cec6`，合并 `38f9ec3`；main 定向 16 / 16、`pnpm verify` 13 文件 / 50 项通过。隔离 schema 的真实 Chrome 上传、刷新、筛选空态恢复、100% `1440×844 @ DPR2` 和 200% 三列 / 6px 安全区通过 |
...
当前有三个互不重叠的执行卡。B 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` / `codex/m1-f2-b` 只写 `text-reader*` 与 B 证据，目标是真实 EPUB / TXT 阅读闭环；C 任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` 在 `7ef0` / `codex/m1-f2-c` 只写 `pdf-reader*`、PDF Worker / 缓存与 C 证据，先闭合真实样本、安全、缓存、恢复和 fail-closed，再由总控裁决生产依赖；M2-F0-A 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` / `codex/m2-miniapp-shell` 只写 `apps/miniapp/**`，先交付客户端运行壳和 H3。总控独占 contracts、schema、共享入口、Worker 注册、台账和最终集成，不让执行任务互相写同一文件。
...
| `M1-F2-A` 已集成候选 | `ed2cec6 → 38f9ec3`，状态 `DONE` | 22 号真实 `1440×844`、23 号真实 200%、24 号明确标注的非真实窗口构图检查、非作者视觉审查与 main 隔离 schema / Chrome 复验均已通过；`47b4` 的旧台账 diff、失败图片候选和无效 IAB 24 图未纳入主线。 |
| 开发基线 | 本地 `main` @ `44b81ea` | `38f9ec3` 已集成 A，`44b81ea` 已冻结 B / C 共享接缝；主线两组未跟踪视觉候选继续保留且不得纳入台账提交。B / C / M2 各自从台账记录的基线、工作树和文件边界推进。 |
...
| `M1-F2-A` 书架闭环 | `DONE` | `ed2cec6 → 38f9ec3`；定向 16 / 16、`pnpm verify` 13 文件 / 50 项与 main 隔离 schema 的真实 Chrome 上传、刷新、筛选空态、100% / 200% 复验通过 |
