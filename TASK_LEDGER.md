...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：除阶段数据 / 失败动作外，当前可见 main 仍是旧 M0，6afd WIP 的 844 短高输入区与青瓷 / 纸白组件语言也未通过，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，Reader 私有标注 / 笔记候选已在隔离工作树开始 TDD，共享路由仍待 Web shell 释放；小程序候选链已冻结 `4fc422f → 630c051 → 5e270b1 → 5e0188d` 四个局部恢复点，并新增“书籍制作 PPT 必须先回当前会话”的产品流程 FAIL，整体仍为六页与共享层 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 已在不改现场的前提下重新触发，但连续有界等待仍无文本 ACK，当前只记 `DISPATCHED / ACK PENDING`，不得以进程 running 代替有效执行；原非作者 UI/UX 已在真实候选端口 4174 / 隔离 API 4101 固定 WIP 快照并开始短高输入与共享视觉语言审查，4173 旧 main 已排除。`/root/reading_text_notes_web` 继续私有 Web TDD。小程序任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 已 ACK 并进入 Reader→当前会话→PPT intent TDD；现有 PPT viewport WIP 暂停并排除本次流程提交。
- 协调下一动作：UI/UX 先回 6afd 844 短高输入 / 消息密度 / 青瓷纸白 brief，Web 作者据此连同阶段持久化与失败动作冻结新 HEAD，再按 02 / 04 / 05 / 06 / 33 逐阶段复验。D-TEXT 继续私有范围候选。小程序同一 writer 先以红灯关闭书籍制作 PPT 绕过会话，再继续 PPT viewport 与其余状态矩阵；冻结全端候选后做四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d` 已形成四个局部恢复点；第四包只含 4 个 Reader 源 / 测试和 4 张 DevTools JPEG，typecheck、8 文件 / 45 项、build `1360 KiB`、diff-check 通过。320×568 / 430×932 的 live viewport / safe-area、自然滚动区、控制层和面板局部几何成立；均不是最终视觉通过 | 产品流程 `FAIL` 已 ACK / TDD：新增私有 `ppt-intent`，DevelopmentClient 仅以 wxStorage 保存 current conversation + pending book/task intent，trial / release fail closed；必须证明换书不换会话、重建可恢复、仅确认后进入 PPT。现有 PPT viewport WIP 和两个禁止差异排除本提交 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / IMPLEMENTATION ACK PENDING / UIUX ACTIVE` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main@`4a94b4f` 旧 M0，只证明用户仍看不到候选；`6afd@0e18846` 加 WIP 在 4174 / 隔离 API 4101 运行。原 writer 已重新触发但未在限定窗口回 ACK，现场和唯一所有权保留，不另建重复任务 | 6afd 除阶段返回 / 持久化、模板、生成骨架和失败动作外，现有 844 短高证据还显示输入区出屏、生成中输入不稳定、消息卡 / 密度 / 纯白表面与 02 及读书青瓷纸白体系不一致。UI/UX 正在当前 WIP 实看并出短 brief；作者 ACK 与新 HEAD 前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / FOUR CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f` 冻结 conversation / drawer，`630c051` 冻结 Reader swipe，`5e270b1` 冻结 login / library / settings，`5e0188d` 冻结 Reader viewport layers；当前优先关闭 Reader→conversation→PPT 产品流程，再推进 PPT。全路由 / 状态矩阵前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 四个检查点分别冻结 conversation / drawer、Reader swipe、login / library / settings 与 Reader viewport layers 的局部基线。Reader→conversation→PPT 产品流程、PPT、其余状态 / 键盘 / 字体矩阵与最终视觉仍未完成；既有禁止差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d` 是四个局部恢复点；Reader→conversation→PPT 产品流程、全小程序六页 / 共享层四宽四高和状态矩阵、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
