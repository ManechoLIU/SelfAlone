...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime 代码门已在 `main@b637023+81f5f1f` 闭合：正式 Server 从独立部署侧平台凭证与计费快照创建 DeepSeek 平台模型，保留领取、成本预占 / 按供应商 usage 结算 / 失败释放与用户模型优先级；结算已提交但确认响应丢失时会回读同一 reservation，只有状态和实际金额一致才返回原回答，避免重复供应商调用。下一步只在具备独立平台凭证与已核验人民币微元价格快照后执行真实 Provider H4、成本流水与浏览器成功路径；缺少这些输入时继续 fail-closed 为 `PLATFORM_UNAVAILABLE`，不把 fake HTTP / PostgreSQL 测试冒充真实能力。
- 当前阻塞：`M1-F4-C-WEB-HANDOFF` 的真实端 H3 已解除，`M1-F4-V1` 的代码缺口也已关闭；当前唯一直接缺口转为真实平台部署输入与调用验收：本机尚未验证独立平台 Key 和当次计费换算快照，既有用户模型 Key 授权不静默扩展为平台凭证。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
...
| `M1-F2-WEREAD-HTTP` | `VERIFY / MAIN 6c9c2e3 / ROUTE COMPOSITION CODE GATE` / 项目总控 | 共享 WeRead JSON 合同对应8个账户隔离 Server HTTP 端点与 `createApp` 可选组合接缝已落地；PUT / DELETE revision、opaque cursor、两类202 sync、严格 schema 与 safe error 均由真实 Fastify inject 覆盖；本包未建凭证持久化、未调用腾讯、未改 Web / Mini | current-main 路由7项、Server全量40文件222项、typecheck、build、visual-contract与diff-check通过；路由只有注入 runtime 才开放，不冒充正式4322已可连接。释放 `M1-F2-WEREAD-RUNTIME` |
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-CODEX-02 / ROUND-6 NONAUTHOR FAIL / REMEDIATION` | `49712c5→e5d481f→85bf8b0→c5f339c→a9f058d→af78414` 均未进入 main：前五轮发现的旧快照、抢占、恢复、双 runtime 与不健康 drain 问题已逐项补同案修复；第六轮全量集成 247/248 复现同一 `created_at` 下按随机 `run_id` 先 claim R2，实际 `[R1,R2,R1]` 而非 `[R1,R1,R2]`，R1 闭合前 exact ref 仍可能被后续失败覆盖。原 Writer 已在 clean worktree 对精确候选 delivered ACK，控制收据 `M1-F2-WEREAD-RUNTIME-R6-FAIL-REWORK` 已通过，owned files 仍限 runtime 与 sync-store 各自实现 / 测试及既有 index 接线 | 新增只认领 queued 状态的原子 claim-by-id，恢复路径先安全精确认领 tracked R1 并处理 peer 抢占，再释放普通队列处理 R2；补同 timestamp 确定性 ID 的 RED→GREEN 与 peer-claim 回归，保持前六轮全部反例后形成新增量候选并再次非作者复核。真实 Key H4 仍是独立授权门，禁止读取凭证或调用真实 Provider |
...
| `M1-F4-V1` | `VERIFY / MAIN b637023+81f5f1f / PLATFORM RUNTIME + SETTLEMENT RECOVERY CODE GATE` / 项目总控 | 领取、AI / PPT消耗、失败恢复和并发硬上限 Case；正式 Server 已接入独立平台环境配置，fake HTTP 证明用户模型未配置且领取后按 reserve→provider usage→settle 顺序成功；真实隔离 PostgreSQL RED→GREEN 证明 settle 已提交但 ACK 丢失时只回读一次相同金额并返回原回答，不重复 provider，未提交的结算失败仍释放并映射 exhaustion | Server 全量39文件215项、成本并发 / 平台恢复定向10项、typecheck、build、visual-contract与diff-check通过；当前4174返回200、4322未登录入口返回401。真实 Provider H4、真实成本流水、浏览器成功路径与并发硬上限仍须独立平台 Key、当次价格快照及调用授权，不得由本代码门代替 |
