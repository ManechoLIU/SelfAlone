...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已转 `BLOCKED`：Checkpoint A 仍保持 controller-verified `20/20 PASS`，不重做。Checkpoint B 的 Assignment `M1-F4-C-SERVER-GATE-B-01` 已完成 ACK + lease PASS，但真实 launch 被 `run_external_agent.mjs` 在 spawn 前以 `rule-handshake ledger_stale` 拒绝；runner 当前固定对 Writer worktree 执行 launch-guard，而该旧 worktree 的台账基线不含已加载 revision `65e8c52f…`。尝试仅把 canonical 台账临时镜像到该 worktree 作为 launch-guard 输入时又被平台安全层拒绝，不能合法绕过。解除条件：runner 支持 canonical ledger / linked-worktree-safe launch guard，或平台允许受控镜像后重新 lease+launch。WEB-HANDOFF 继续等待 server contract 冻结。
...
- 规则版本：总控已从真实 `main@46ab1fd` 加载当前 AGENTS（SHA-256 `0cf10094…d0d`）；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 安装副本已加载并机器 ACK 精确 revision `65e8c52fe05e639a6398578f232a384966bd9d7a`（manifest SHA-256 `2393cef2…f1b84`）：总控完整重载 SKILL / agent-model-routing / external-agent-auth / agent-delivery contract / long-task governance / control-event / lifecycle / rule-handshake，规则与 runtime 以 Git common-dir 握手、五状态投影、Assignment 启动前 ACK/lease、证据进展指纹与 recovery budget 为准；外部 Kimi/Grok 可见状态卡仍只能由 `run_external_agent.mjs --render-status-card` 生成。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控；已有工作包不因规则升级重做或改派。
...
| `M1-F4-C-SERVER-GATE` | `BLOCKED / CHECKPOINT A ACCEPTED / CHECKPOINT B LAUNCH-GUARD FAILURE / HEAD 08b8625` | Checkpoint A 冻结 RED→GREEN 已由总控独立复验：conversation-store `20/20 PASS`；Checkpoint B Assignment=`M1-F4-C-SERVER-GATE-B-01`，ACK 文件 `/tmp/m1f4c-b-ack.json`，lease guard=`allowed`。真实 runner launch 在 Agent spawn 前失败：`rule-handshake ledger_stale for installed revision 65e8c52f…`；原因是 runner 固定以 Writer worktree 为 `--repo`，其旧 `TASK_LEDGER.md` 不含 canonical main 已同步的规则 revision。受控临时台账镜像又被平台安全层拒绝，不能绕过规则门 | 解除条件：获得 linked-worktree-safe/canonical-ledger launch-guard 路径，或平台允许受控镜像；解除后从已验收 Checkpoint A 直接派 Checkpoint B，不重做 A。当前无 live Grok/Kimi，不伪称 ACTIVE |
