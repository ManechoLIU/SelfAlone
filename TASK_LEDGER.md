...
- 下一可见检查点：`M1-F4-B-REWORK-03` 保持同一 Assignment / 同一 `quota-cost@f53c930` 恢复，不改派。Grok 4.6 session `01a04c6c-74d8-7b40-9b67-8dfc6a181937` 已真实执行但首个 assistant 行为先调用 repo tool、未先交 delivered ACK，随后因 action-stationarity 自停；真实 worktree 仍 clean 且 HEAD 未变。当前恢复动作是在 retained-candidate 治理收据闭合后恢复同一 Assignment/session，先重新取得合规 delivered ACK，再按已锁两条 RED 做两文件 TDD；禁止扩 `store` / config / lock / refs，也不创建或改派新 Writer。
...
| `M1-F4-B-REWORK-03` | `RECOVERING / SAME Grok 4.6 WRITER ASSIGNMENT / session 01a04c6c-74d8-7b40-9b67-8dfc6a181937` | 已执行 Grok 4.6 恢复但首个 delivered-ACK 门 FAIL：assistant 在任何 ACK 文本前先调用 repo tool；session 后因 action-stationarity 自停，真实 `quota-cost` 仍 clean `f53c930`、无新 HEAD/commit。owned files 仍仅 `cost-ledger-migration.ts` + `.test.ts`；两条已实证 RED 与 TDD 边界不变 | 继续恢复同一 Assignment/session，不补派、不改派：恢复后的第一输出必须是完整 delivered ACK，随后才可触碰 repo；ACK 后再转 ACTIVE 并做两文件 TDD。若 HEAD 产生新 scoped commit，则 retained `f53c930` 自动失效，新 HEAD 回 live candidate 正常非作者审查 |
