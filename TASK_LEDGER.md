...
- **活动实现线**：Auth scoped Main Case 与 Settings P0 Main FINAL仍保持 PASS。Mini Reader + Conversation 的 code-only依赖闭包已顺序合入 `main@31462d9`，唯一 f312同源 / dist复验取得限定范围 `FINAL PASS`；Drawer最小布局候选 `d379c76` 已顺序进入 `f312@3f8375e`，正在非作者 DevTools EARLY。Desktop Conversation候选 `cvmain@ac93da0` 已关闭 back / forward叠加 in-flight load的导航竞态，但完整代码门又以隔离PostgreSQL证明失败后重试会重复写入同一用户输入；原 writer正按同范围修复idempotency / retry。长期冻结的D-TEXT 768两文件候选 `575a612` 已利用独立槽进入短非作者EARLY。当前构成：controller / integrator 1、implementation writer 1（Desktop Conversation）、reviewer 2（Mini Drawer、D-TEXT 768）。
- **下一用户可见检查点**：Desktop Conversation先冻结幂等修复候选，经非作者代码门后在隔离Chrome完成输入、发送、确定性回复、刷新恢复、失败保留、legacy route与back / forward；Mini Drawer在唯一f312完成390、360 font23与短屏同态EARLY；D-TEXT只复验768与受影响1440后立即释放。任一槽释放即按真实文件冲突重算Book Detail route seam或下一Mini私有包。
- **真实阻塞**：Conversation `ac93da0` 当前阻塞为可复现的失败→重试重复user context：解除条件是未改draft复用同一requestId、Server幂等复用 / 冲突关闭、真实PostgreSQL回归、scoped commit与非作者复审；未通过前不进Chrome。Mini整体仍未覆盖真实附件 / 发送重试、软件键盘、长列表与 reduced-motion；Drawer须先过本轮EARLY。Book Detail与 D-TEXT PPT handoff仍受 Conversation共享 `main.ts` 单writer接缝约束。f312 config / lock与两张QA图继续作为受保护现场；真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未闭合或未授权。
- **当前可见入口 / revision**：Desktop候选 Web `http://127.0.0.1:5188/` / API `http://127.0.0.1:4308` 仍绑定产品 revision `a9408ad`、schema `selfalone_main_auth_settings_20260826` 与 `/tmp/selfalone-main-auth-settings.XE5Dfd/`；尚不含未通过的 Conversation。稳定恢复点 `4127 / 4198` 仍绑定旧产品 revision `ab13310`。Mini唯一 DevTools为 `f312@3f8375e`；其 `332a737`源 / dist与 `main@31462d9` 等价，新增 Drawer两文件候选正在EARLY，config / lock与两张既有QA图未变。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE` · `CANDIDATE ac93da0 / PRE-EARLY CODE FAIL(P1) / REOPEN ACTIVE` / `/root/desktop_conversation_slice` | 输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话 UI；非目标：真实模型、图片消息、Settings / Auth UI。`f82b334 → 5667273 → f4a9ea0 → ac93da0` 已关闭普通 / legacy route及导航in-flight竞态；当前只返修失败重试幂等 | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、base `a9408ad`；必须保留Auth / Settings，owned外冲突即停 | 25个Conversation CORE / API / DB / WEB文件与三处共享接缝；`app.ts` / `index.ts` / `main.ts` 保持本包单writer | refs 02 `d84ffbae…` / 36 `1b6ec630…`；新账户NULL外键Case合法；真实P1为第一次response失败后第二次新requestId重复写user context。下一步controller + Store真实PG RED→幂等修复→非作者代码门→Chrome EARLY |
| `M1-F3B-A1-CONVERSATION-CORE` | `ACTIVE` · `CURRENT-MAIN CANDIDATE ac93da0 / PRIVATE CASE RETAINED` / `/root/desktop_conversation_slice` | `97e1c53 → 56a850e → b8f34b4 → f4a9ea0 → ac93da0` 保留确定性回复、PostgreSQL会话 / 消息、API、Web controller / view、legacy workspace、输入稳定与导航handoff；当前不扩业务功能 | 仅阻塞于同包retry idempotency P1；发生owned外冲突或Auth / Settings回退立即停写 | 同一25文件候选；禁止扩大到其它页面、DB/schema或Book Detail | 31 / 31定向、52 files / 267 tests、typecheck / build / diff-check通过；新幂等回归与修复后须重跑并经非作者复审，未经过真实Chrome不得合入 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `MAIN MERGED 31462d9 / READER + CONVERSATION SCOPED FINAL / DRAWER EARLY ACTIVE / H3 BLOCKED` / 项目总控 | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务继续显式开发适配器，不冒充真实跨端 | main已含code-only运行闭包；f312是唯一DevTools且config / lock / QA图受保护。Drawer仅两文件EARLY，不与其它页writer重叠 | Reader + Conversation限定范围证据 `mini-main-final-31462d9/`；Drawer待 `mini-drawer-early-3f8375e/`。软件键盘、tourist凭证、长列表、真实PPT preview继续阻塞H3，不阻塞私有页READY |
| `M2-F4-A` 真实书架 / 导入接入 | `READY` · `F0-A MAIN DEPENDENCY RELEASED / OWNER PENDING` | 将运行壳接入真实书籍摘要、搜索、导入与解析状态；不把开发适配器或静态五态冒充真实持久化 | A已以`38f9ec3`合入且F0-A现为`main@31462d9`；下一包须先拆Mini私有client / library页与共享contracts / lock总控接缝，真实账户仍停在M2-F1边界 | 真实EPUB / TXT / PDF、五类状态、刷新恢复、DevTools与账户隔离；当前Goal释放槽位后按互斥文件派发 |
...
| `M2-F3` 小程序会话 | `VERIFY` · `MAIN MERGED 31462d9 / NORMAL + SELECTION SCOPED FINAL / DRAWER EARLY ACTIVE` / 项目总控 | 会话抽屉、文字 / 选择 / 附件状态、输入增长、失败保留；非目标：真实AI / PPTX。normal与selection限定范围已进main，Drawer独立返工 | 附件真实选择 / 移除、发送重试、软件键盘、长会话列表与reduced-motion仍未完成，不扩大PASS；Drawer只写自身组件 | `mini-conversation-early-332a737/`、`mini-main-final-31462d9/`；下一步Drawer EARLY，随后按未覆盖真实Case拆分，不重开已通过状态 |
| `M2-F4` 小程序书架与阅读 | `VERIFY` · `MAIN MERGED 31462d9 / READER SCOPED FINAL / LIBRARY NEXT READY` / 项目总控 | 统一书架、连续正文、内容面板、老己笔记与背景恢复；非目标：真实notes API / 跨设备。Reader视觉 / 手势范围已关闭，真实Library / API接入仍开放 | Reader已释放；`M2-F4-A`因F0-A main集成完成转READY。真实账户、跨端持久化仍按M2-F1 / owner门 | refs 18～20；`mini-reader-final-db40e99/`与`mini-main-final-31462d9/`。当前Goal槽释放后派Library私有首包，不返工已通过Reader |
...
| `M1-F2-D-TEXT-READER-768-FIX` | `VERIFY` · `CANDIDATE 575a612 / NONAUTHOR EARLY ACTIVE` / `/root/dtext_768_early_review` | `dtr8@575a612` clean；提交仅含 `apps/web/src/text-reader.css` / `text-reader.test.ts`，main 越界污染已精确隔离恢复 | 22 / 22定向与作者768 / 1440 Chrome收据已保留；reviewer使用隔离端口/schema，只复验同一两视口，不扩大到PPT / banner | 候选冻结、author已释放；当前reviewer只读，首个PASS/FAIL即释放 | 证据待 `dtext-768-early-575a612/`；未覆盖原生200% / DPR2与D-TEXT另两个独立FAIL，本卡不是父项DONE |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE` · `cvmain@ac93da0 / PRE-EARLY CODE FAIL(P1) / REOPEN ACTIVE` / `/root/desktop_conversation_slice` | 输入、发送、确定性回复、刷新恢复、错误保留、统一对话 UI；非目标：真实模型 / 图片 / Settings。普通 / legacy与导航竞态已拆清；当前返修失败重试重复user context | `cvmain`唯一writer；保留Auth / Settings；不得改DB/schema、Book Detail或Mini | 25个Conversation文件与三处共享接缝；当前只改幂等状态 / Store路径及对应测试 | 绑定02 / 36；隔离PG复现 `r1:user, r2:user, r2:assistant`。下一步同requestId retry回归与修复→非作者代码门；PASS后才做作者1440与非作者四宽Chrome |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY` · `READER + CONVERSATION MAIN FINAL / DRAWER EARLY ACTIVE / H3 BLOCKED` / 项目总控 | 320 / 360 / 390 / 430的Reader与Conversation限定范围已复验；Drawer布局候选进入唯一f312，仍不代表全系统适配通过 | Drawer真实短屏 / font23 / 列表滚动未判；软件键盘、游客凭证 / console、真实PPT preview继续H3 / VERIFY | Drawer当前只改 `components/nav-drawer/index.{test.ts,wxss}`；H3无writer | Drawer证据待 `mini-drawer-early-3f8375e/`；其PASS后再决定main集成，未覆盖项保留 |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `MAIN MERGED 31462d9 / READER + CONVERSATION SCOPED FINAL; DRAWER EARLY ACTIVE` / 项目总控 | Reader在四宽关闭全屏底栏 / notes / 背景缺陷；Conversation在390 normal与selection关闭山亭不可辨识与状态层问题；Drawer最小布局候选正验 | 全Mini仍缺附件 / 真实发送重试、软件键盘、长列表与reduced-motion；Drawer EARLY未决 | main已含102文件Mini code-only闭包；f312保护config / lock / QA图未带入；Drawer两文件后续按结果顺序集成 | `mini-reader-final-db40e99/`、`mini-conversation-early-332a737/`、`mini-main-final-31462d9/`；下一步Drawer EARLY→审查→main复验 |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `f312@3f8375e / READY FOR EARLY / REVIEWER ACTIVE` / `/root/mini_drawer_early_review` | ref 11与正式山亭asset；移除列表伪 `padding-bottom`，让山亭绝对层不占会话列表且不遮底部导航；非目标：重画asset、Conversation / Reader、真实多账户 | author候选 `d379c76` 已释放并顺序进入唯一f312；真实长列表若无数据须明确保留uncovered | `components/nav-drawer/index.test.ts`、`index.wxss`；config / lock / QA图禁止变化 | 9 / 9、Mini 15 files / 98 tests、typecheck / build / diff-check通过；下一步唯一DevTools 390、360 font23、320短屏 EARLY，证据 `mini-drawer-early-3f8375e/` |
...
| `M2-F0-A` 当前候选 | `main@31462d9` 已含经真实端复验的Mini code-only闭包；唯一 `f312@3f8375e` 在同源 / dist等价基线上追加Drawer两文件候选 | Reader + Conversation限定范围 `FINAL PASS`；Drawer正在EARLY。保护project config / lock与QA图未带入main；附件 / 真实发送重试、软件键盘、长列表、reduced-motion与PPT真实预览仍未通过。 |
...
| 当前桌面候选波次 | Auth scoped Main Case；Settings P0在`main@a9408ad`同源Main FINAL关闭；Conversation `cvmain@ac93da0`为PRE-EARLY候选，导航P1已修但retry idempotency P1由原writer返修；Book Detail私有候选继续冻结 | Desktop候选入口 `5188 / 4308@a9408ad`；Conversation新账户PostgreSQL Case通过、失败重试重复user context Case失败，未经过新代码门 / Chrome，禁止合入。 |
| 当前 Mini 候选波次 | Reader + Conversation已在`main@31462d9`取得限定范围Main FINAL；Drawer `f312@3f8375e`正在短EARLY，H3边界保留 | 证据 `mini-reader-final-db40e99/`、`mini-conversation-early-332a737/`、`mini-main-final-31462d9/`；Drawer待 `mini-drawer-early-3f8375e/`。 |
...
| 开发基线 | 产品顺序集成到 `main@31462d9`；Desktop Auth / Settings与Mini Reader / Conversation限定范围PASS | main tracked clean，三个既有未跟踪视觉目录原样保留；Desktop入口仍绑定`a9408ad`，Conversation / Drawer与局部PASS不能自动 `DONE`。 |
...
| `M2-F0-A` 原生运行壳 | `VERIFY / MAIN MERGED 31462d9 / DRAWER EARLY ACTIVE / H3 BLOCKED` | Reader + Conversation限定范围已完成main复验；Drawer `f312@3f8375e`正在唯一DevTools EARLY。真实软件键盘、凭证化clean console、长列表与PPT真实预览仍保留为未覆盖边界，不回退已通过范围 |
