...
- 当前活动项：Mini conversation `/root/mini_conversation_rework` 在 f312 产生 4 / 4 RED，但未原样 ACK；Reader `/root/mini_reader_sheet_rework` 使用独立 `rdrs@3fed4c7`，baseline Reader 13 / 13；drawer `/root/mini_drawer_visual_rework` 使用独立 `drwr@3fed4c7`，baseline drawer 5 / 5。三者文件互斥，均不得碰保护 config / lock、台账或对方页面。D-TEXT blocker 只影响 main 页面终验，不回退已合并实现；E-OWNER 已关闭本地 owner / 版本合同子卡，真实认证与跨端仍在后续边界。
- 协调下一动作：三个 Mini writer 先回原样 ACK / RED，再分别做 EARLY、冻结提交与非作者 FINAL；任一完成即回收并释放下一审查槽。D-TEXT 须先释放或更换可控制的真实 Chrome 标签再重派 FINAL，不用 in-app Browser / 静态证据替代。`M1-F1-A` 已由 E-OWNER 集成释放为 READY，但当前 4 槽被总控与三个 Mini writer 占满，下一槽优先派发且不得与共享 app / contracts 并写。保护差异与真实软件键盘等 H3 阻塞不变。
...
| `M2-F3` 小程序会话 | `FAIL / DESIGN-READY / WRITER DISPATCHED / ACK PENDING / RED 4 OF 4` / `/root/mini_conversation_rework` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | `f312@3fed4c7` 的常驻 PPT intent 双按钮卡、工程诊断文案与会话层级违反 `main@3199945`；非作者 brief 已返回。唯一 writer 仅拥有 conversation 私有 page 与测试，尚未原样 ACK，故不写 ACTIVE。总控新鲜复跑私有 `index.test.ts` 为 4 / 4 RED，分别命中正常态内部文案、旧常驻任务卡 / 双动作、缺半屏选择层与缺单一 PPT 阶段入口 | 收到原样 ACK 后才升级 ACTIVE；实现后覆盖正常文本扫描、半屏选择层打开 / 关闭 / 保留 / 确认摘要、单一全屏阶段入口，以及 320 / 360 / 390 / 430、font23、safe-area 真实 DevTools EARLY / FINAL；软件键盘外部门单列 |
...
| `M2-F0-A-VISUAL-REWORK` | `FAIL / DESIGN-READY / THREE WRITERS DISPATCHED / ACK PENDING / IMG-04 ALPHA VERIFY / IMG-05 STRUCTURE-ONLY ADOPT` | conversation 固定 f312 私有页；Reader 独立 `rdrs@3fed4c7`；drawer 独立 `drwr@3fed4c7`。三者文件互斥，均以当前 DESIGN-READY 与绑定参考实现 | conversation 已有 4 / 4 RED 但无原样 ACK；Reader baseline 13 / 13、drawer baseline 5 / 5，均已创建 writer 但未原样 ACK，故不写 ACTIVE。IMG-04 runtime Alpha / size 可复用但整体 drawer 仍失败；IMG-05 仅采纳紧凑表面、浅暗纸面预览与选中反馈，不直接作为运行时 | 每个首版做 EARLY，冻结后做独立 FINAL；430、真实键盘、长列表、reduced-motion 与未覆盖五态不得静默升级 |
