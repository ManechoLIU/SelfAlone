...
- 当前执行波次：`CONTROL-RESET-1`；Web 候选 `1727f95` 因阶段结构互斥合同实现错误被用户判为 `FAIL / IA REOPEN`，当前由非作者先补 02 / 04 / 05 / 06 / 33 四阶段 structure matrix，brief 前不恢复 6afd 写入；D-TEXT 共享候选 `ed95d95` 正在独立代码 / PostgreSQL 复审；小程序 `d4f587c` 已冻结 reader 短屏和非视觉定位修正，等待同一 UI/UX 任务按已完成的全端 `DESIGN-READY` 合同复验。`f312` 仍独占 `apps/miniapp/**`，后续全端视觉整改由同一写入者顺序承接；台账仍由项目总控独占。
- 当前活动项：`/root/reading_notes_integration_review` 只读复审 `dint@ed95d95`；UI/UX 任务 `01a03652-a5d4-76c1-b0fa-ff9303243664` 正在为 `6afd@1727f95` 输出阶段互斥整改 brief，完成后再交原实现者修正，随后才复验 Web 与 `f312@d4f587c`。`M2-UX-CONTRACT-ALL` 的逐页合同与 covered / uncovered 矩阵已完成，抽屉、登录 / 设置山亭、会话 / 阅读趴宠锚点、reader 操作层 / 内容面板和 PPT 编辑器为明确整改项；小程序透明山亭与真实 PPT 模板 / 页面预览资产仍是局部资产阻塞。既有未暂存 `pnpm-lock.yaml` 禁止触碰。
- 协调下一动作：Web 先完成 structure matrix，再由 6afd 唯一写入者把四阶段改为右侧单一当前任务工作区的互斥视图；中栏只保留会话、轻量阶段摘要 / 系统消息和输入，完整 PPT 瀑布不得在中栏重复。D-TEXT 与小程序候选分别取得非作者裁决后，总控审查实际 diff、合并并复验 main。小程序后续按 `b45a7a8` 与 4.1 合同顺序处理不依赖新资产的整改；山亭与真实 PPT 预览只在正式运行时资产登记后接入。每个局部包关闭后仍有安全 `READY` 项就立即切换，不把局部提交当停止条件。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY / DESIGN-READY / FINAL-VISUAL PENDING / H3 BLOCKED` / `/root/miniapp_h3_repair` | 原生小程序运行壳、登录 / 书架 / 连续阅读 / PPT 状态边界与五类状态；未就绪服务走显式开发适配器 | `d4f587c` 已冻结真实 block 几何、空章节 fail closed、reader 三档密度、自然增高和首滑提示生命周期；全端合同已就绪，但当前 WIP 不是视觉 PASS，trial / release 继续 fail-closed、不合并 | 先由原 UI/UX 任务复验 reader；其余页面由同一 f312 写入者顺序整改。游客 console、软件键盘、系统字体、小程序透明山亭及真实 PPT 模板 / 预览资产仍阻塞相应 H3 / 视觉门，既有未暂存锁文件禁止触碰 |
...
| `login` | `21-mobile-auth-login.png`（`3cda4b1cd6b5f1479b8a29c26dc66e403840d773122ce43d1690ed4db8bc6381`） | loading / 授权失败 / 正常、微信 / 邮箱主次、协议、键盘开关、安全区、长文案 | `PARTIAL / FAIL`：390 未登录主态与邮箱长值已覆盖，背景缺山亭；恢复 loading、认证失败、键盘实显、320 / 430 冻结态仍 `UNCOVERED`。游客身份只阻塞真实授权结果 |
...
| `library` | `14-mobile-reading-library.png`（`3e80d4a9d64567c439ffab09b8e90d0444f0025a794a44e8d4100d69d7465212`） | loading / 真实空 / 筛选空 / 失败 / 正常、搜索 / 导入、2 / 3 列、长书名、四宽与安全区 / 溢出 | `NORMAL COVERED / NEAR PASS`：390 / 320 页头、搜索、连续网格、进度和来源成立；430 三列冻结证据及其余四态仍 `UNCOVERED` |
| `reader` | `15～17`（`82c09e3849e000dbe3b4a4cf796b1498e9de94efc323a933d1395903652537e3` / `dcaacd30403fabfd8d01d12704ea8b5ab40572ac299f1dd2685756242393df36` / `a847e81aad0d1935cb91586ea761fb8f1abfd6ad905f4f1c2b457056a12d9608`）、`18～20`（`2ddebcb614497d88efc79e0bf77197fc26192c5641ea2833d2a7a820cef8152f` / `23d078e390f6fc2b4422587b81b990f50154ca6e98535a94ccf65c418ec76077` / `e76a281751844095d080d8564980f13cc2d0ed7b9112dc3b842f6262b98b19cb`） | 介绍首屏 / 连续正文 / 操作层、浅深、短中长高、长标题 / 正文、内容三面板五态、趴宠与 44px 对话入口锚点、安全区 / 字体 / reduced-motion | `PARTIAL / FAIL`：320 旧态、430 当前 WIP、连续滚动与三面板正常态已覆盖；短屏首屏、操作层透明盒锚点、面板遮工具栏和缺“新建笔记”失败。`d4f587c` 已修短屏 / 提示，待独立复验；浅 / 深两态本身不再是缺陷，其余状态 / 键盘 / 选区仍 `UNCOVERED` |
...
| `settings` | `22`（`ce28f93874444d6877713576716b1395dff6752da36650d6a5792e293f657288`）、`39`（`e74f589f85b9c8f9bf6654ef68bca8d5e6f5f87399a34a69744722f718b5ed17`） | 总览五态、服务长状态、模型配置、凭证显隐 / 键盘、保存失败、安全区、长文案 / 溢出 | `NORMAL LIST COVERED / PAGE FAIL`：390 开放列表、分隔、状态与退出成立；山亭缺失。loading / 失败、39 详情、键盘、320 / 430 冻结态仍 `UNCOVERED` |
...
| `M1-F2-D-TEXT-INTEGRATION` | `VERIFY / INDEPENDENT REVIEW` / `/root/reading_notes_integration_review` | 固定候选 `/Users/echoman/.codex/worktrees/dint/SelfAlone@ed95d95`，只读复审 contracts / migration / publisher / domain export / app / Server；不写 Web / miniapp / 台账 | 作者候选已提交且 clean；独立复审真实 `40P01`、同版本引用完整性、fail-closed migration / readiness、共享 DTO / routes 与真实并发 Case，通过前不合并 |
...
| `M1-WEB-SHELL-01` | `FAIL / IA REOPEN / DESIGN-BRIEF ACTIVE` / `01a03652-a5d4-76c1-b0fa-ff9303243664` | 固定失败候选 `6afd@1727f95` 只读；brief 返回前原实现者不写。原范围仍限共享 shell / conversation / main / styles / app-state、测试与证据，不改 reader / Server / 台账 / 事实源 / 锁文件 | 用户确认四阶段必须是同一右侧工作区的互斥视图；当前 `renderTaskPanel()` 常驻“范围与需求”且生成态又在中栏重复完整结果，禁止合并。非作者先按 02 / 04 / 05 / 06 / 33 输出阶段结构、标题、1440 / 1200 / 1024 / 768、前进 / 返回 / 刷新恢复 Case，再交原实现者整改 |
...
| `M2-F0-A-READER-SHORT` | `VERIFY / FINAL-VISUAL PENDING` / `/root/miniapp_h3_repair` | 固定 `f312@d4f587c`；仅 reader 页面 / 测试 / 本包证据，同一自然连续 scroll-view；未触碰既有锁文件 | 三档密度、内容安全降级、首滑提示状态 / reduced-motion 与 320×568 / 375×667 / 430×932 已由作者提交；等待原 UI/UX 任务按 17 和新合同独立复验，不以 current-* 图自证通过 |
| `M2-F0-A-H3-REWORK` | `VERIFY / DESIGN-READY / H3 BLOCKED` / `/root/miniapp_h3_repair` | `f312@d4f587c`；仅 `apps/miniapp/**`，既有 `pnpm-lock.yaml` diff 禁止触碰 | 非视觉定位 / 空章节 / reader 短屏候选已冻结；其余全端视觉可按合同由同一写入者顺序恢复。游客 console、键盘 / 字体、小程序山亭和真实 PPT 预览资产仍不得降门 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c`，状态 `VERIFY / DESIGN-READY / FINAL-VISUAL PENDING / H3 BLOCKED / 不合并` | `d4f587c` 在真实 block / 空章节基础上加入 reader 三档高度、自然增高、首滑提示生命周期与 reduced-motion；定向 7 文件 / 34 项、全仓 14 文件 / 57 项、typecheck / build / diff-check 通过，作者已有 320×568 / 375×667 / 430×932 与长内容证据。独立视觉复验、360、键盘 / 字体、游客 console、小程序山亭与真实 PPT 预览资产仍未闭合；既有锁文件 SHA 保持不变。 |
...
| `M2-F0-A` 原生运行壳 | `VERIFY / DESIGN-READY / FINAL-VISUAL PENDING / H3 BLOCKED` | `d4f587c` 已冻结不等高正文定位、空章节与 reader 短屏；全小程序合同已就绪，但独立视觉复验、安全区 / 键盘 / 字体矩阵、凭证化 clean console、小程序山亭和真实 PPT 预览资产门保持，仍不合并 |
