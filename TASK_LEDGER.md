...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。D-TEXT handoff已以`main@1e7763a`完成scoped FINAL。F3B-C `csel1@4b0aa19`实际diff FAIL后，原大包连续无ACK / diff已回收；REOPEN 2按互斥边界拆为Web恢复`csrw`与消息关联`csmsg`两棵隔离树，均从`4b0aa19`建立且基线78文件 / 440项PASS，当前两个writer均`DISPATCHED / ACK PENDING`，不得写ACTIVE或接shared seam。Mini长列表`mlong@a9da90a → f312@5878400`代码门PASS，真实DevTools EARLY因Mac锁屏暂停。当前构成：controller / integrator 1、implementation writer 0、dispatched writer 2、reviewer 0；一个子槽空闲但无另一独立READY，不创建重复writer。
- **下一用户可见检查点**：Selection 2A先回multi / free丢响应恢复与空值确认RED，2B先回原assistant message关联RED；各自候选经actual diff PASS后顺序组合并接shared seam，再开真实Chrome EARLY。Mac解锁后Mini长列表立即在唯一f312完成三宽 / 大字体真实EARLY。
- **真实阻塞**：Mini Library真实runtime仍受M2-F1 session bridge与DevTools transport阻塞，`20d61d6`只保存未接线合同。Mini长列表代码门已闭合，但本轮Computer Use再次返回“Mac is locked”，解除条件是本机解锁后续跑唯一f312；不反向阻塞桌面。Selection shared seam因`4b0aa19`恢复与消息上下文合同FAIL冻结，解除条件是REOPEN 2候选通过actual diff。Mini父项另缺软件键盘、游客凭证化clean console与真实PPT preview。
- **当前可见入口 / revision**：旧稳定Desktop仍是`4127 / 4198@ab13310`，不得误认当前进度；当前main前端预览为`5340@1e7763a`（PID `13653`），API复用`5331@0cc5d46`与schema `selfalone_book_detail_early_c91b691_20260826`，D-TEXT当前书fail-closed与匹配书正常态真实Chrome scoped FINAL PASS；Book Detail `5330 / 5331@0cc5d46`仍可见。本台账事件提交基于父`main@1e7763a`；唯一DevTools候选为`f312@5878400`，保护config / lock与两张QA图未变，长列表尚未合入main。
...
| `M1-F3B-C-R2A` | `READY` · `DISPATCHED / ACK PENDING` / `/root/selection_recovery_web` | mutation级丢响应恢复、可执行重试与multi / free空值确认；不改Server、消息关联或shared seam | `csrw@4b0aa19`基线78文件 / 440项PASS；未回cwd / branch / status / RED前不视为ACTIVE | 仅Web `conversation-selection-controller{,.test}.ts`、`conversation-selection-view{,.test}.ts`、`conversation-selection.css` | multi lost-response→精确重放→新version确认；free输入不丢 / 不提交旧稿；空值禁用与可访问恢复动作；下一步preflight ACK / RED |
| `M1-F3B-C-R2B` | `READY` · `DISPATCHED / ACK PENDING` / `/root/selection_message_assoc` | 选择问题稳定绑定原assistant message并贯通API / 持久化 / Web状态；不渲染、不改shared conversation文件 | `csmsg@4b0aa19`基线78文件 / 440项PASS；若正确FK / backfill必须越界，停在精确seam诊断；未回ACK前不视为ACTIVE | Server全部`conversation-selection*`及Web `conversation-selection-client/state{,.test}.ts` | 同owner / conversation且assistant角色才可创建；跨账户 / 会话 / user / missing fail-closed；历史可按messageId原位合并；下一步preflight ACK / RED或精确技术阻塞 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `F312 5878400 / LONG-LIST READY FOR EARLY / H3 OPEN` / 项目总控 | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务继续显式开发适配器，不冒充真实跨端 | f312是唯一DevTools且保护现场不变；长列表candidate已顺序放入f312但尚未合入main；真实EARLY因Mac锁屏暂停 | Reader / Conversation / Drawer既有证据 + `mini-conversation-send-early-854c577/`；下一步唯一f312长列表三宽 / 大字体；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F3-MINI-CONVERSATION-LONG-LIST` | `VERIFY` · `CANDIDATE a9da90a / F312 5878400 / READY FOR EARLY` / 项目总控 | developmentAdapter + `developmentLongList=1`确定性18项闭合抽屉长列表、搜索与末项露出；不冒充生产会话API，不处理软件键盘 / 凭证 / AI | writer已释放；117项、typecheck / build PASS。唯一f312真实EARLY因Mac锁屏暂停，解锁后从同项目续验，不建第二入口 | `nav-drawer/index.test.ts` / `index.wxss`与`conversation/index.test.ts` / `index.ts` / `index.wxml`共5文件；f312保护差异不变 | ref11 SHA `9e764e…c36e`；待390 / 360 / 320、font16 / 23首段 / 中段 / 末段+固定底栏、搜索命中 / filtered-empty真实PASS后才可进main |
...
| `M2-F0-A` 当前候选 | `main@11f480a` 含Reader / Conversation / Drawer及send-retry限定闭包；唯一`f312@5878400`另含未合入main的长列表候选 | text / attachment-only、失败恢复 / retry与三宽保持；保护config / lock与QA图未带入main。长列表代码门PASS但DevTools EARLY受Mac锁屏阻塞；软件键盘、reduced-motion与PPT真实预览仍开放。 |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader与handoff scoped FINAL；Conversation已至`main@dea90d7` FINAL；Book Detail至`0cc5d46` scoped FINAL；Selection `4b0aa19`实际diff FAIL后已拆`csrw` / `csmsg`两包，均DISPATCHED / ACK PENDING | 两包文件互斥且基线各78 / 440 PASS；候选须分别actual diff PASS后顺序组合，shared seam仍冻结；旧`bdport`WIP只读保留；当前main前端预览5340 / API5331。 |
| 当前 Mini 候选波次 | Reader / Conversation / Drawer既有范围保持；send/retry已至`main@11f480a` scoped FINAL；Library合同`20d61d6`因auth BLOCKED；长列表`a9da90a → f312@5878400` READY FOR EARLY | 唯一f312保护config / lock与两张QA图不变；Mac解锁后续跑390 / 360 / 320、font16 / 23、首 / 中 / 末段与搜索命中 / 空态，PASS后才可进main。 |
...
| 开发基线 | 本台账事件提交的父为`main@1e7763a`；Desktop Auth / Settings、D-TEXT reader / handoff、Conversation与Book Detail限定范围PASS，Mini Reader / Drawer与send-retry限定范围PASS | main仅允许本次台账事件diff与三个受保护未跟踪视觉目录；Selection、Mini长列表与Library合同仍未完成，不能自动 `DONE`。 |
