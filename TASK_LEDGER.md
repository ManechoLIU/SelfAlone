...
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@2437e00a10b1a1cfdc71c119750af12f9a2f265a` 开始；主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。本提交只写本台账，不改产品 / 视觉事实源。
- **活动实现 / 复验线**：Desktop `/root/desktop_quota_close` 已在 `/Users/echoman/.codex/worktrees/quota2/SelfAlone`、`codex/m1-f4-a-quota-close@2437e00` 以约 `60s` delivered ACK，clean 起步并独占 quota Server / Web 文件，现 `ACTIVE`；先核历史 `3974a76 + 8c5fb5f` 实际差异与 current-main 等价性，再走隔离 schema / 端口真实领取 Case。Mini 原 writer `/root/mini_drawer_rework_writer`（原任务 `01a03c93-92c8-7062-b980-15fe390d7912`）以约 `30s` delivered ACK 后只读复核：当前运行实例为 `f312@63782ba`，但外部 Project List 清理尚未闭合，故按停止条件暂停，未 fresh compile / 写源码。当前 controller + Desktop 占用 `2 / 4`，可用 `2`；误中断 `0`，Desktop 首次 clean-but-live 经 running / follow-up / worktree 四类证据确认后未回收。
- **READY 重算**：Desktop `M1-F4-A` 因 `M1-F3B-A` 已 DONE 而直接 READY，已派发并 ACTIVE；`M1-F2-A` 书架视觉、`M1-F2-B` 阅读稳定壳 / 200% 与 `D-TEXT` 剩余视觉边界同样可执行，但会与本轮 Desktop 的 `main.ts / styles.css` 共享接缝，待 quota checkpoint 后顺序派发。`M1-F2-C` 完整页面受生产 PDF 依赖 / 许可证授权门，`M1-F1-V1` 受真实邮件边界，F3A/F3B-B/D及F4-B后续仍有上游依赖。Mini Drawer 已派发且 ACK；当前只被已授权但尚未完成的 Project List 清理阻塞。Settings / 长列表复验与 Drawer 共用唯一 DevTools，不并行；`M2-F4-A` 等 session bridge，`M2-F1` 等真实 AppID / 身份授权，其余 Mini无无冲突 READY。
- **下一用户可见检查点**：Desktop 新本地账户在真实 Chrome 完成“未领取 → 领取成功短反馈 → 重复 / 刷新保持 → 并发幂等 → 两账户隔离 → 断服 / 恢复”，由 writer 交 current-main 等价收据或范围单一候选。Mini 在 Project List 只剩 `f312` 后立即重启原 writer，先证明 `developmentLongList=1` 送达并出现 18 项，再完成 `320 / 360 / 390 / 430` 同状态自验与非作者 EARLY。
- **真实阻塞**：Mini writer 本轮只读实测 Project List 仍为 `f312 / cvre / rdrs / 6f5c / miniprogram-1` 五项；当前运行实例确为 f312，四项历史记录的授权清理由外部主 Agent 正在执行，本总控与 writer 不重复删除、不改凭证、不新增入口、不触碰源码 / worktree。此前 token 过期与 route timeout 证据仍保留，只有清理闭合后才重新判断 query。远端、发布、真实邮件 / 微信 / 模型 / Presenton 与付费调用继续保持原门。
- **当前可见入口 / revision**：真实 main 为 `2437e00`；当前主线 Web `127.0.0.1:4276` PID `4051` 通过代理连接 API `127.0.0.1:4212` PID `4652`，隔离 schema `selection_main_c858a06_20260826`，两端 health ready。Mini 当前唯一活动 DevTools 运行实例路径为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、源码 HEAD `63782ba`；Project List 的唯一记录约束尚待外部清理完成。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `ACTIVE` · `current-main audit / delivered ACK` / `/root/desktop_quota_close` | 新账户一次性领取、账户grant与桌面对话轻量领取条；并发幂等、accounts外键、成功约1.2秒brief后消失；不含F4-B/C、模型或付费调用 | F3B-A已main FINAL，故本项READY；历史`3974a76 + 8c5fb5f`不是main祖先，只作实际diff证据，禁止按提交名直带 | `quota2@2437e00`独占`trial-quota-*`、conversation quota/directory、必要`main.ts / styles.css`与直接测试；不写conversation migration/store/chat-view | 先核current-main等价性与首个RED；隔离PG / API / Web、真实Chrome覆盖未领取 / 领取 / 重复刷新 / 并发 / 双账户 / 断服恢复、1440 / 1024、console / overflow / focus。等价则clean交VERIFY收据；缺口则最小候选并EARLY，不提前DONE |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `f312 63782ba / DRAWER DISPATCHED + ACK / PROJECT-LIST CLEANUP WAIT` / 项目总控 + `/root/mini_drawer_rework_writer` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；本轮只重开 Drawer 同态与保留长列表 / Settings scoped NO-GO，不冒充完整Mini闭环 | Drawer v2四文件已在唯一f312；writer delivered ACK并确认运行实例为f312、保护config / lock / QA原样。外部最近项目清理尚未完成，writer未重复操作 / fresh compile / 写源码 | 清理闭合后先验query与18项，再做非ImageGen Alpha、多背景、320 / 360 / 390 / 430 writer自验、EARLY与FINAL；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F3-MINI-CONVERSATION-LONG-LIST` | `VERIFY` · `MAIN MERGED 35bad75 / SCOPED FINAL FAIL-NO-GO / CURRENT DEVTOOLS QUERY BLOCKED` / 项目总控 | developmentAdapter + `developmentLongList=1`确定性18项闭合抽屉长列表、搜索与末项露出；不冒充生产会话API，不处理软件键盘 / 凭证 / AI | main实现与既有视觉 / 交互子证据保留；本轮唯一`f312@63782ba`两种显式重开 / 编译均未把query送入页面，日志另有token过期与route timeout | main长列表五源文件；当前仅同一f312运行环境可复验 | 旧首 / 中 / 末、固定底栏、搜索09、filtered-empty与无横溢子证据不能证明当前Drawer v2；先恢复有效DevTools登录 / query，再补本轮18项同状态矩阵；`mini-drawer-f312-63782ba-devtools-recovery/` |
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `f312 63782ba / V2 CANDIDATE / ACK CLOSED / PROJECT-LIST CLEANUP WAIT` / `/root/mini_drawer_rework_writer` | ref11与既有山亭asset的同态返工；用户截图确认横向矩形接缝、山亭位置 / 尺度 / 上缘渐隐、列表 / 底栏节奏偏离，旧 `hasAlpha` 与旧整页 PASS 不再有效 | 四文件候选`ae92912`已顺序放入唯一f312；v2 asset SHA `87cf9828…8876`。writer绑定ref11 / 用户FAIL / DESIGN 45-48与四宽计划，但本轮实测五条最近记录仍在，按停止条件暂停；当前运行实例为f312，禁止新增导入 / 重复清理 / 盲试 | 清理完成即复测query / 18项，随后normal / open / current、single fallback、filtered-empty、底栏 / 44px / 安全区 / console / overflow与接缝、山亭、节奏；通过只交READY FOR NONAUTHOR EARLY，偏差回原mdrv2四文件最小修复 |
...
| `M2-F0-A` 当前候选 | main已含`35bad75`长列表与`14b69dc` Settings；Drawer v2四文件`ae92912`已顺序放入唯一`f312@63782ba`，asset SHA `87cf9828…8876` | Settings / 长列表旧 scoped FINAL仍`FAIL / NO-GO`。Drawer 320局部证据不构成EARLY；同一f312的query / token / route环境阻塞已由writer与非作者两层复现，保护config / lock与既有QA不动，未新增第六DevTools入口。 |
...
| 当前 Mini 候选波次 | Reader / Conversation既有限定结论保持；长列表`35bad75`与Settings`14b69dc`已在main但scoped FINAL为NO-GO；Drawer v2`ae92912`在唯一`f312@63782ba`，旧整页PASS仍被用户截图否定 | v2源码 / 资产checkpoint已落f312，保护现场不变；本轮writer ACK后确认运行实例为f312，但Project List仍有5项，故等待外部授权清理闭合，未fresh compile / 写源码 / 新增入口；token / route旧证据待清理后重新判定。 |
| 当前 Desktop main 预览 | `RUNNING / CURRENT MAIN 2437e00 / HEALTH READY` | Web `127.0.0.1:4276` PID `4051`、API `127.0.0.1:4212` PID `4652`；schema `selection_main_c858a06_20260826`。旧`4173 / 4100`无监听；Selection收据继续有效，quota使用独立环境，不以共享服务冒充完成。 |
| 开发基线 | 本台账事件父为`main@2437e00a10b1a1cfdc71c119750af12f9a2f265a`；本轮仅再写`TASK_LEDGER.md` | 6个受保护DOCX / Word锁文件与3组受保护视觉候选目录仍与冻结a233副本同字节，禁止暂存 / 修改 / 删除；Mini唯一f312的2个保护修改与5组既有QA未改变。 |
