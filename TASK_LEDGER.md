...
- 当前执行波次：`CONTROL-TAKEOVER-2`；持续所有权合同已在 `main@528b86b` 范围单一提交，写入前已核对主线基线 `main@8b52f22`；新总控已回传 `CONTROL CONTRACT LOADED main@9764180`。项目内 `继续总控` 快捷词曾由 `4d32e24` 新增，用户纠正语义后已由原写入者在 `main@9858491` 范围单一删除并移入个人 Skill；唯一受影响 live Agent 为本总控，已回传 `SHORTCUT MOVE LOADED main@9858491`。当前 `AGENTS.md` SHA-256 为 `703a6942…66b7`，个人 Skill 不复制进本台账，现有开发 / 验证任务范围与停止条件不变。旧总控 `01a034b4-a73d-7ce2-8531-51585826e6d3` 已原样回复 `OLD CONTROLLER FROZEN`、最后一轮 `interrupted`、revision `32` / `notLoaded` 且已归档；其冻结前提交的 `b2f8276 / 2101695 / 5a18abc` 已逐项审查，过期 owner / 入口 / 候选状态已在 `1b72e59` 纠正，首波收据已在 `8b52f22` 登记。主线产品代码未因接管变化，仍只有三个既有未跟踪视觉候选目录。
- 当前活动项：D-TEXT reviewer 已回传 `CONTROL CONTRACT LOADED main@9764180` 与 `CONTINUOUS OWNERSHIP CONTRACT LOADED main@528b86b`，14 文件收据已与 `/tmp` 源逐字节一致复制到 `/Users/echoman/.codex/visualizations/2026/08/25/01a038e8-5ee8-79b1-8bdd-daafc2d58e0e/dtext-main-final-9858491/` 并停止释放。当前协作树为总控 + `/root/auth_early_review` + `/root/mini_early_review`，3 / 4 槽占用；认证 reviewer 已从入口失效的 `UNCOVERED` 恢复同一任务补 ACK、重启隔离入口并重验，Mini 继续 `ACTIVE / EARLY`，f312 两项保护哈希与未提交现场不变。
- 下一 `READY` / 协调动作：当前 `1` 个空闲槽，立即用于 `D-TEXT-READER-768-FIX`，不新增共享接缝 writer。`D-TEXT-PPT-HANDOFF-FIX` 需要 `main.ts`，与认证 4 文件 UI WIP 冲突，保持 `QUEUED` 到认证候选冻结。Mini EARLY `FAIL` 仍按对应私有文件释放唯一 writer，`PASS` 才冻结 FINAL；`M2-F4-A` 等 F0-A 合入复验且解除同写边界后才重算为 `READY`。
- main / 隔离入口收据：21:01 曾确认 D-TEXT 隔离 Web `4311` 为 200、Server `4310` 根路径 404，认证隔离 Web `5174` 为 200、API `4181` 根路径 401；这些进程均为临时验收入口。2026-08-25 21:08 +08:00 再验时 4127 / 4198 / 4310 / 4311 / 4181 / 5174 / 4110 / 4173 已全部退出，故当前没有可访问的 main、D-TEXT 或认证 Web 入口；旧 200 只保留启动收据，不代表当前 Case、视觉或 main 可用。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `ACTIVE / CORE CHECKPOINT 1bc8b3b / UI EARLY DISPATCHED / ACK PENDING` / 项目总控 + `/root/auth_early_review` | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | `auth1@1bc8b3b` 已提交 15 文件 core / API checkpoint；UI 仅保留 `main.ts`、`styles.css`、`auth-page.ts`、`auth-page.test.ts` 未提交 WIP，原 writer 已停止。非作者 reviewer 创建已返回但 loaded ACK 未回，故 EARLY 尚未升级 `ACTIVE`。`E-OWNER` 已以 `main@f1334ad` 冻结共享 owner 接缝 | CORE、API、DB、WEB、QA；不得与 Mini、D-TEXT 或后续共享 schema / app 集成并写 | 密码摘要、Cookie、安全错误、刷新会话、跨账户隔离；绑定参考 01 的 4 视口、键盘和非作者 EARLY / FINAL | writer 双 ACK；定向 15 tests、Web/API/state 10 tests 与 typecheck 已报告通过，待总控复核。真实入口 `5174/#/auth` + API `4181`、隔离 schema `auth1_early_20260825`；EARLY verdict 前不扩写，真实邮件 / 微信仍停在授权门 |
...
| `M2-F3` 小程序会话 | `VISUAL FAIL / CANDIDATE 5f2193c / EARLY ACTIVE / ACK RECEIVED` / `/root/mini_early_review` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | reviewer 已加载 `main@9764180`，固定 `f312@5f2193c` 与两项保护哈希，只操作唯一常驻 DevTools | 覆盖 320 / 360 / 390 / 430、font23、safe-area、选择层生命周期、摘要 / 单入口、1 / 3 / 5 行与附件、趴宠和正常文本扫描 | EARLY FAIL 后释放唯一 conversation writer最小返工；通过 EARLY、冻结新候选后才做 FINAL |
| `M2-F4` 小程序书架与阅读 | `READER VISUAL FAIL / DESIGN-READY / CANDIDATE a2d613f / EARLY ACTIVE / ACK RECEIVED` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | `a2d613f` 与 `rdrs@387c152` patch-id 相同，已进入 `f312@5f2193c`；与会话共用同一只读非作者，保护哈希已登记 | 只在唯一常驻 DevTools 做 320 / 360 / 390 / 430、font23、safe-area、reduced-motion EARLY；EARLY FAIL 最小返工，PASS 后再 FINAL |
...
| `M1-F2-D-TEXT-READER-768-FIX` | `READY / SLOT AVAILABLE` / 待立即派发 | 仅修 768 阅读标题和 toolbar 内部裁切；不改 PPT、认证或共享壳 | `apps/web/src/text-reader.css`、`apps/web/src/text-reader.test.ts`，不得写 `main.ts` / `styles.css` 或认证 WIP | 先 RED 复现内部 `scrollWidth > clientWidth` / title right > viewport，再最小修复；真实 768×844 + 1440 回归、console / overflow，首个可运行版本即停在非作者 VERIFY |
...
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CANDIDATE f312@5f2193c / EARLY ACTIVE / ACK RECEIVED / IMG-04 ALPHA VERIFY / IMG-05 STRUCTURE-ONLY ADOPT` | conversation `cb4752f`、Reader `a2d613f`、drawer `5f2193c` 已进入同一候选；patch-id 对账通过，保护差异与旧 QA 图仍未提交。`/root/mini_early_review` 已加载合同与保护哈希并继续真实 DevTools EARLY | 逐页 / 逐状态 EARLY：FAIL 按文件边界派唯一 writer，PASS 后才冻结并独立 FINAL。430、真实键盘、长列表、reduced-motion 与未覆盖五态不得静默升级 |
