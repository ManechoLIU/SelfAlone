...
- 当前活动项：Mini 短屏视觉修正已冻结为 `f312@2deb4f6`，EARLY 的 320 slot 阻断已按实装图关闭；但新增 1024×737 RGBA runtime 使 Mini dist 达 2236 KiB，超过既有 `<2 MiB` 门，故候选仍 `FAIL / SIZE REWORK`，非作者正同时裁决视觉与确定性体积优化合同。两项保护差异、旧 EARLY 图与 diagnostic 未纳入。D-TEXT `48e5f66` 为 26 files clean candidate，真实四宽 / TXT / EPUB / CRUD / 409 收据已齐；200% 保持 VERIFY，chat / PPT 入口 fail-closed，非作者结论前不集成。
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY / FROZEN 2deb4f6 / SIZE FAIL / H3 BLOCKED / NOT IN MAIN` / `/root/mini_visual_rework_fix` | 11-file scoped commit 已关闭 Reader controls 省略与 320 scenery 几何，提交四宽 / font23 真实 DevTools 图；保护 project config / lock 与两张旧 EARLY 图未纳入 | 定向 13 files / 68 tests、typecheck / diff-check 通过；build dist 2236 KiB，未过 `<2 MiB`，不得集成。体积最小修复后重新 build、真实 DevTools 与非作者 FINAL；软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY / FROZEN 2deb4f6 / IMG-04 VISUAL FINAL ACTIVE / SIZE REWORK BRIEF` / `/root/mini_visual_rework_fix` + `/root/mini_visual_final_review` | runtime `1024×737 RGBA`、SHA `875ebae56229a657a505d1071e76000078fd1250b4c0894b3f71ca7bb43d2e50`、无实底；320 / 360 / 390 / 430 实装图已齐，短屏 slot=100px。非作者除视觉 FINAL 外须给兼容 Mini 的确定性体积优化合同，不再次生成 | 优化后仍须真实 Alpha、参考11构图 / 色彩 / 渐隐等价、四宽 / font23 / safe-area / 命中通过并使 build `<2 MiB`。真实多会话长列表、软件键盘与 PPT preview 继续精确保留 UNCOVERED / H3，不因静态或开发态数据升级 |
