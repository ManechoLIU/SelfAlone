...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已从已验证 RED 恢复到 ACTIVE：fresh Grok 4.6 session `01a04e65-3381-7082-8fca-32892a62e02b` 在 current HEAD `08b86254a3cf0cb214509685c522d8d057220870` 上 ACK-only `tool_call=0`，delivered ACK 绑定 controller RED=`1 FAIL / 19 PASS` 与 tracked dirty `conversation-store.test.ts`，新版 Assignment lease PASS。现在只允许在既定8 owned files 实现：own verified model 优先且零平台计费；fallback 仅 claim+capacity；reserve→settle/release；可区分 exhaustion/config-required/unavailable；同 request 不重复计费；严禁安装依赖/live paid provider。WEB-HANDOFF 继续等待 server error contract 冻结。
...
- 规则版本：总控已从真实 `main@46ab1fd` 加载当前 AGENTS（SHA-256 `0cf10094…d0d`）；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 已从真实源仓 `/Users/echoman/Documents/Codex/adaptive-delivery` 的 clean HEAD `58ee5836bc92cadfc04f59fad1f9ce4aec9d3f7d` 同步到真实安装副本并完成 loaded ACK：53/53 tracked 文件逐字节一致、missing=0、mismatched=0，并重新加载 SKILL / agent-delivery contract / long-task governance / control-event / lifecycle 关键规则。新规则继续保留 `absorbed / parked` retained semantics；同时把 Bug 修复及 migration / money-cost / authorization / state-machine / external-call / release-critical required review 升级为风险裁剪 TDD 因果证据审查：`tdd_required=true` 时必须绑定 `red_evidence + candidate_revision + green_evidence + red_green_same_case=true + reviewer_counterexample + verdict=PASS|FAIL`，低风险机械/文案修改不强造 RED/GREEN。后续每次 adaptive-delivery 更新固定执行“精确 revision 通知 → loaded ACK → 台账同步”。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控；已有工作包不因规则升级重做或改派。
...
| `M1-F4-C-SERVER-GATE` | `ACTIVE / Grok 4.6 WRITER / session 01a04e65-3381-7082-8fca-32892a62e02b / DELIVERED ACK PASS / RED CONTROLLER PASS` | controller 已验证同案 RED=`1 FAIL / 19 PASS`：expected `PLATFORM_EXHAUSTION`、received `CONVERSATION_REPLY_FAILED`，draft/context 保留；fresh ACK-only 回合 `tool_call=0` 绑定 current HEAD `08b8625` 与唯一 tracked dirty `conversation-store.test.ts`，新版 Assignment lease PASS。owned files 仍仅既定8个 | 当前实现门已开放：Grok 4.6 只在 owned files 实现 own-model priority/no platform charge、claim+capacity fallback、reserve→settle/release、distinguishable failure+retry/idempotency；禁止安装依赖/第三文件/live paid provider/伪造实际 cost。候选须 TDD causal nonauthor review，PASS 后才集成 |
