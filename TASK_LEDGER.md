...
- 当前活动项：`01a03652-a5d4-76c1-b0fa-ff9303243664` 正在 `6afd@0e18846` 做 Web FINAL-VISUAL-VERIFY，随后回传小程序 swipe-hint brief；D-TEXT Web 入口等待 Web shell 所有权释放。既有小程序实现任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 已收到 f312 全端续作指令并被唤醒，当前登记 `DISPATCHED / ACK PENDING`，收到 cwd / HEAD / 首批文件与测试回执前不称 `ACTIVE`。透明山亭与真实 PPT 模板 / 预览仍是局部资产阻塞；f312 既有未暂存 `project.config.json` 与 `pnpm-lock.yaml` 禁止触碰。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / DISPATCHED / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `d4f587c` 只证明 reader-short 局部修正；430×932 早期出屏证据已撤销，但回顶 / 重进恢复规则和其他页面多设备适配未闭合。续作消息已送达且任务工具状态已唤醒，首批文件 / 测试 ACK 尚未返回，当前不称执行中 | 实现者可先做不依赖视觉阈值的共享几何、键盘、状态与机械门；swipe 数值 / 资产等待 UI/UX。游客 console、软件键盘、系统字体、透明山亭及真实 PPT 预览继续阻塞对应门，锁文件禁止触碰 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `DISPATCHED / ACK PENDING / DESIGN-BRIEF PARTIAL` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | 续作指令已送达；实现者先回 cwd / HEAD / status、首组文件 / 红灯 / 停止条件，再升级 `ACTIVE`。不依赖视觉数值的共享几何 / 键盘 / 五态可并行；swipe 阈值与资产等待 UI/UX |
| `M2-F0-A-H3-REWORK` | `DISPATCHED / ACK PENDING / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 与系统适配共用同一 f312 writer，先做不涉及视觉裁决的 H3；既有 `project.config.json` / 锁文件禁止触碰 | 续作已发送，真实开工回执前不称 `ACTIVE`；游客 console、软件键盘 / 字体、凭证化环境保持真实外部门，不因构建通过降低 H3 总门 |
