...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / CONNECTION STORE MAIN d16159a / SYNC STORE RED NEXT` / 项目总控 | 账户隔离 AES-256-GCM 连接存储已进入 main：同请求同内容安全重放、同请求异内容冲突、不同首次并发只一项成功，断开清空密文 / nonce / 认证标签；继续建立 sync run 与 last-success 快照，再把 Gateway Adapter 与 HTTP 接缝注入正式 Server | Connection Store 定向2项、Server全量41文件224项、typecheck / build / visual-contract / diff-check通过，未调用真实腾讯。下一步先做 sync run / snapshot RED；完成本地 fake-gateway Case 后仍需独立真实 Key H4，禁止由存储测试代替 |
