...
- 当前 Goal：工具状态 `active`；objective 为“完成当前用户可见候选波次——并行推进桌面 Settings、正常 Conversation 与 Mini Reader/Conversation，候选经真实端 EARLY/FINAL、顺序合入本地 main 并复验；任一子任务或 turn 完成不得关闭整个 Goal。”当前仅完成候选或控制事件，均不得关闭该 Goal。
- 当前阶段：桌面 Auth `b7a6894` 与 Mini Reader `f312@866f237` 均为 `READY FOR EARLY` 候选；Settings 在 `set1`、正常 Conversation 在 `f3bc` 形成私有纵向切片，尚未完成 UI / shared seam EARLY；D-TEXT 768 修复 `575a612` 已完成作者真实 Chrome 自验并释放。所有候选均未合入本地 `main`，不得标 `DONE`。
...
- 当前执行波次：`VISIBLE-CANDIDATE-WAVE-1`。本轮台账写入前唯一主线为 `main@c75f745`；质量门 `main@e3a771b` 已作为各 writer preflight 生效，不再把治理 ACK 本身作为其他互斥 `READY` 的交付阻塞。本轮已真实维持 controller `1` / implementation writers `3` / reviewers `0`（Desktop `2`、Mini `1`）；Settings 与 Mini Conversation 随后分别产出 FAIL / candidate 事件并释放，空槽在本台账提交后立即按事件续派。
- 当前活动项：Desktop Book Detail writer 在 `/Users/echoman/.codex/worktrees/bdv1/SelfAlone`、`codex/m1-f2-book-detail-visual@c75f745` `ACTIVE`，仅写 book-detail 私有 state / view / CSS / tests。Settings `set1@b7a6894` 已从三个缺模块 RED 形成私有 CORE / API / DB / WEB 候选，定向 9 / 9、全仓 227 / 227、typecheck / build / diff-check 通过；真实 Chrome 仅到失败壳，因 `index.ts` 未注入 `accountSettings`、`desktop-shell.ts` 设置入口仍 disabled 判 `FAIL / BLOCKED`，writer已释放、无提交。Mini Conversation 已在 `mcv1@f2b625b` clean 冻结三文件候选并释放，精确 5 行计算与测试、11 / 11、Mini TS、diff-check、lock SHA 均通过，但未进入唯一 DevTools，仍非 EARLY。正常 Conversation 包保留在 `f3bc@97e1c53` 为 `ACTIVE / PRIVATE WIP / WRITER RELEASED`：CORE / API / DB / controller / view 已绿，但 UI / shared seam 尚未 EARLY。
- 下一 `READY` / 协调动作：总控先审查并顺序把 Mini Conversation `f2b625b` 放入唯一 `f312`，再恢复原 writer做真实 DevTools EARLY；Settings 原 writer可在确认 `index.ts` / `desktop-shell.ts` 无并发 owner 后以最小接缝恢复，候选冻结后再释放正常 Conversation 的 `app.ts` / `main.ts` 接缝。Auth `b7a6894` 与 Mini Reader `866f237` 均等待短非作者 EARLY；D-TEXT `575a612` 已释放。任一 writer释放即按真实文件冲突重算，桌面与小程序无依赖时至少各保留一条实现线。
- main / 隔离入口收据：本轮台账写入前 main HEAD 为 `c75f745`，产品服务仍绑定 `ab13310`；稳定 Desktop Web `http://127.0.0.1:4127/`（PID `72285`）与 API `http://127.0.0.1:4198/api/v1/workspace`（PID `71125`）使用隔离 schema `selfalone_main_control_ab13310` 和运行目录 `/tmp/selfalone-main-control-ab13310.vimPyg/`。该入口只作恢复点，`/health` 的既有 `404` 不得作为功能 `PASS`。main tracked 在本轮台账写入前 clean；三个既有未跟踪视觉目录原样保留。
- 当前授权边界：已获本地连续开发、验证合格后合并回本地 `main`，已在范围内且通过真实依赖门的 `READY` 项不再逐项等待用户授权；新功能或候选只有经真实业务 Case、实际 diff 审查、本地提交、顺序合入并在 `main` 复验后才能关闭。最多 20 次外部图片生成调用额度保持不变，额度只按实际发生的调用计数，预登记不扣减。非重大视觉范围内 UI/UX 可直接比较候选并给 adopt / exclude；同步事实源后才释放开发。已有绑定参考或明确派生不得为复刻 / 响应式修复重新生图；栅格山亭、纹理、插画可按缺口定向生成。功能图标继续用成熟 SVG / 矢量实现，但轮廓、比例、线宽、圆角、留白、颜色及选中 / 按压状态必须忠实跟随 UI/UX 选定候选与绑定参考，不能直接使用 ImageGen 像素图标。每次调用必须记录模型 / 提供方、次数或张数、尺寸、用途、输出目录与累计余额，生成物先为 `CANDIDATE`，只有实装和真实端对照通过后才能升级。只有已精确登记解除条件的真实阻塞、无法消解且会实质改变结果的事实冲突、重大功能 / 信息架构 / 核心流程变化、大范围返工选择，或超出 20 次额度、其他付费或外部服务与凭证、push、PR、发布、部署、删除及其他难恢复动作才暂停并请求用户；事件触发式 idle 仅表示当前无可执行 `READY`，不表示停止履职。远端 push 不是完成条件，也未获授权。

### 1.1 项目总控、台账所有权与证据入口

- 唯一持续项目总控：任务 `01a038e8-5ee8-79b1-8bdd-daafc2d58e0e` 直接运行在 `/Users/echoman/Documents/SelfAlone` 的本地 `main`，以 `adaptive-delivery` 为持续运行合同，在独占台账写入之外负责总目标、需求影响扫描、阶段与真实依赖、动态拆分、执行 / QA / 非作者审查调度、设计前置门、共享契约与写入边界、候选集成、冲突处理、本地 `main` 复验和最终验收；完成任一 Goal 后先核对真实控制面、重算并继续下一可共同验收的小里程碑，只有没有可执行项时才准确登记阻塞与解除条件并回到事件触发式 idle。实现继续交给边界互斥的执行者，不为每份文档或每个小问题创建独立管理 Agent。旧总控已冻结并归档，不再拥有写入权。
...
- 统一通道：向 `codex://threads/01a038e8-5ee8-79b1-8bdd-daafc2d58e0e` 提交工作包 ID、仓库 / 工作树 / 分支 / 基线与 HEAD、精确 status / diff、目标与非目标、测试命令与结果、真实业务 Case、浏览器或 Computer Use 证据、非作者审查、阻塞、下一步、提交 / 集成状态、授权需求；有图片生成时另附提供方 / 模型、实际调用次数与张数、尺寸、用途、输出目录、哈希 / 透明度、候选结论和累计余额。
- 紧急恢复例外：只有不立即写入就会丢失恢复点、误触授权边界或造成并行覆盖时，开发任务才可临时写台账；必须同时说明原因、精确 diff、恢复点和未走专职通道的理由，并交本任务复核。未经复核不得据此升级状态或称 `DONE`。
- 当前交接：旧总控归档后 revision 32 连续两次无变化；`TASK_LEDGER.md` inode / size / mtime 自 20:30 起稳定且接管前 diff 为 0。交接摘要中的 18 行草案实际已被旧总控拆成三个 main 提交，本轮保留仍符合现场的 checkpoint / 授权事实，并按真实 task、候选、服务与 owner 状态纠正其余内容；所有既有未跟踪资产和其他 worktree现场保持原样。
...
- 当前系统影响裁剪：小程序抽屉、会话输入 / 键盘与跨页面共享层仍由 `M2-UX-CONTRACT-ALL` 做系统影响扫描；`书籍内容 → 老己笔记` 因已有独立绑定参考 19、事实源与单一 Reader 文件边界，按用户最新反馈拆为 `M2-F0-A-MINI-NOTES-VISUAL-REWORK`，不重新设计或生成整页。它与 Reader 背景保留修复共享同一 writer 边界，须顺序执行，不得新增第二 Reader writer；Conversation 与 Drawer 保持两个独立队列包。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `CANDIDATE b7a6894 / READY FOR EARLY / REOPEN 1` / 项目总控 | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | `auth1@b7a6894` clean；范围只含 `main.ts`、`styles.css`、`auth-page.ts`、`auth-page.test.ts`，原 EARLY 的 50/50 硬切与弹窗焦点缺陷已由原 writer返工 | CORE checkpoint + 上述四个 UI 文件；不得写 D-TEXT、Mini 或 Settings 私有实现 | writer 真实 Chrome 覆盖 1440 / 1200 / 1024 / 768、DPR2、1440×1024，横溢 / console clean，弹窗初始焦点、Tab trap、Escape、焦点恢复与失败保留通过 | 候选尚未非作者 EARLY、未合入 main；真实成功登录、200%、reduced-motion 与真实邮件 / 微信仍未覆盖 |
| `M1-F1-B` | `FAIL / BLOCKED ON TWO UNOWNED SEAMS / PRIVATE GREEN / AUTHOR RELEASED` / 项目总控 | 邮箱验证、找回、修改邮箱 / 密码、身份重验和桌面设置 UI 的完整闭环 | `set1@b7a6894` 从 account settings migration / service / web state 三个缺模块 RED 起步；当前 `apps/server/src/index.ts` 未注入 `accountSettings`，`apps/web/src/ui/desktop-shell.ts` 仍禁用设置入口 | 已写私有 account-settings migration / routes / service、settings state / page / CSS / tests 与 `app.ts` / `main.ts`；两个缺失接缝需总控确认无 owner 后恢复原 writer | 定向 9 / 9、全仓 227 / 227、typecheck、build、diff-check通过；Chrome 四宽失败壳 console 0 / overflow 0 / 键盘 / reduced-motion通过，但 normal overview 未可达 | 绑定参考 09 `e5a22a0…cc4d` 与共享 02 `d84ffbae…94f6`；无 commit、未 EARLY、未合入 main；解除条件为补齐两处最小接缝后重跑真实 normal / failure Case |
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE / PRIVATE VERTICAL SLICE WIP / NOT EARLY` / 项目总控 | 会话新建 / 切换 / 删除、文字发送、确定性本地回复、草稿、刷新恢复、错误保留和桌面对话 UI 的本地闭环；真实模型仍不在本候选完成范围 | `f3bc@97e1c53` 已扩展私有 CORE / API / DB / WEB；当前作者释放，shared `app.ts` / `main.ts` 与 Settings writer 冲突，等待其冻结后顺序接缝 | conversation domain / runtime、migration / store / routes、chat client / state / controller / view / CSS / tests；暂不写 shared seam | 隔离 schema 已证明发送→确定性回复→新 Store 刷新恢复、失败保留草稿与账户隔离；全仓 42 files / 216 tests、typecheck / build / diff-check 通过 | UI / shared seam 与真实 Chrome 尚未 EARLY，未提交、未合入 main；不能以私有测试称用户可见闭环完成 |
| `M1-F3B-A1-CONVERSATION-CORE` | `ACTIVE CHECKPOINT / f3bc@97e1c53 / AUTHOR RELEASED` / 项目总控 | 会话草稿 / 上下文 / 任务隔离、当前会话锁、停止 / 失败 / 完成解锁和 stale revision 合同，作为 A 的私有种子 | 原四文件 checkpoint 已加载质量门；后续新增 migration / routes / store 与 Web controller / view 均保留在同一隔离树，未写 shared seam | 仅 `f3bc` 当前未提交私有差异；不得与 Settings 并发写 `app.ts` / `main.ts` | CORE / API / DB / controller / view 测试与刷新恢复 Case 已绿；候选 commit、真实 Chrome、EARLY / FINAL 均未发生 | Settings seam 冻结后恢复原范围 writer，先接缝与真实 Chrome，不领取新包 |
...
| `M2-F4-A` 真实书架 / 导入接入 | `QUEUED / BLOCKED ON F0-A MAIN INTEGRATION` / F0-A 后同端下一包 | 将运行壳接入真实书籍摘要、搜索、导入与解析状态 | A 已以 `38f9ec3` 合入；F0-A 虽已提交候选但仍在集成修正，为避免同写 `apps/miniapp/**`，等待其合入 main 并复验，共享 contracts / lockfile 由总控集成 | 真实 EPUB / TXT / PDF、五类状态、开发者工具与账户隔离 |
...
| `M2-F3` 小程序会话 | `CANDIDATE f2b625b / AUTHOR RELEASED / NOT READY FOR EARLY` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | conversation 私有候选已移除正常态常驻 PPT 阶段标签并修正输入 / 锚点；精确 5 行按 `5×24 + 2×9 = 138px` 计算并由测试解析 CSS 验证 | clean commit只含 `pages/conversation/index.{test.ts,wxml,wxss}`；不得触碰 Reader、config、lock 或 QA 图 | direct Vitest 11 / 11、Mini TS、diff-check通过且 lock SHA不变；尚未顺序进入 f312 或唯一 DevTools，不能 `READY FOR EARLY` |
| `M2-F4` 小程序书架与阅读 | `CANDIDATE f312@866f237 / READY FOR EARLY / REOPEN 1` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | Reader 绑定参考 18～20 的老己笔记、sheet 手势和浅色背景重进保留已由原 writer整改并顺序进入唯一 f312 | 当前候选 commit 只含 Reader 四个私有源 / tests；f312 既有 `project.config.json`、lock 与 QA 图保护差异未纳入 | writer 真实 WeChat DevTools 390×844 / font23 / safe-area 覆盖初始→全屏→回落→关闭重进、notes、草稿 / 失败重试 / 背景保留，console 0、无底部溢出 | 作者自验收据 `mini-reader-early-866f237/`；尚未非作者 EARLY，真实 API 持久化、跨设备、其他视口与暗色 editor 未覆盖 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / MAIN MERGED 9143638 / FINAL FAIL / ACK RECEIVED / DURABLE RECEIPT` / 项目总控 | main 产品路径与运行快照等价；真实 Chrome 已覆盖 1440 / 768、详情 / 阅读 / 标注笔记 / PPT / 恢复 / focus / console / overflow。主控独立审读矩阵与关键图，确认 768 标题 / toolbar 内部裁切、PPT 交接仍显示固定旧书且恢复滚动时 banner 被 sticky header遮挡；不关闭父项 D | reviewer 双 ACK 已闭合；14 文件稳定收据位于 `/Users/echoman/.codex/visualizations/2026/08/25/01a038e8-5ee8-79b1-8bdd-daafc2d58e0e/dtext-main-final-9858491/`，`FINAL-RECEIPT.md` SHA-256 `add56a5d…8a12f1a` | console error / warn 为空；文件上传、原生 200% / DPR2、真实删除、AI / PPTX 仍 `VERIFY`。修复后必须重跑失败 Case 与受影响 1440 / 768 回归 |
| `M1-F2-D-TEXT-READER-768-FIX` | `CANDIDATE 575a612 / READY FOR EARLY / AUTHOR RELEASED` / 项目总控 | `dtr8@575a612` clean；提交仅含 `apps/web/src/text-reader.css` / `text-reader.test.ts`，main 越界污染已精确隔离恢复 | 22 / 22 定向测试；writer 真实 Chrome 768×844 与 1440×844 DPR1 覆盖内容 / document overflow、actions、恢复和 console，均 clean | 候选冻结后 writer已释放；下一步短非作者 EARLY，未通过不得合入 main | 未覆盖原生 200% / DPR2 与 D-TEXT 另两个独立 FAIL；本卡不是 D-TEXT 父项 DONE |
| `M1-F1-A-AUTH-EARLY` | `CANDIDATE b7a6894 / READY FOR EARLY / AUTHOR RELEASED` / 项目总控 | `/Users/echoman/.codex/worktrees/auth1/SelfAlone@b7a6894` clean；仅四个认证 UI 文件 | writer Chrome 四宽 / DPR2、焦点闭环、失败保留、console / overflow 自验已完成 | 分派短非作者 EARLY；PASS 后冻结 FINAL，FAIL 原 writer原范围返工 |
| `M1-F1-B-SETTINGS` | `FAIL / BLOCKED / AUTHOR RELEASED` / 项目总控 | `set1@b7a6894`；私有 account-settings / settings-page / state / CSS / tests 与 `app.ts` / `main.ts` 已绿；缺 `index.ts` 注入和 `desktop-shell.ts` active rail | 两处文件确认无并发 owner后恢复原 writer；补齐 normal / failure Chrome Case、范围单一 commit后才可 `READY FOR EARLY`；不合并 Auth / Conversation候选 |
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE / PRIVATE WIP / WRITER RELEASED / SHARED SEAM QUEUED` / 项目总控 | `f3bc@97e1c53`；conversation CORE / API / DB / Web client / state / controller / view / CSS / tests，当前不写 `app.ts` / `main.ts` | Settings seam释放即恢复原 writer做接缝与真实 Chrome；候选未提交、未 EARLY，不得称正式用户可见交付 |
| `M1-F2-BOOK-DETAIL-VISUAL` | `FIRST EARLY FAIL / REOPEN 1 / WRITER ACTIVE` / `/root/book_detail_visual` | `bdv1@c75f745`；仅 `apps/web/src/book-detail-{state,view}{,.test}.ts` 与 `book-detail.css` | 非作者按 refs 08 / 10 实图判定：PPT tab 错显“新建笔记”、笔记主操作与行尾动作层级偏离，且单条笔记 / PPT 空态不足以证明内容密度及 generating / completed `16:9` 两列。原 writer原范围返工并用隔离真实候选状态补同态矩阵；不能构造则回精确 blocker，不扩 shared / API。修复后仍只到 `READY FOR EARLY`，不触碰 Settings / Conversation shared seam |
| `M1-F2-D-TEXT-PPT-HANDOFF-FIX` | `QUEUED / AUTH SHARED-SEAM CONFLICT` / 待认证候选冻结 | 当前书 PPT intent 必须驱动或明确 fail-closed，不能与固定旧书 workspace 同屏；恢复滚动不得把 intent banner 藏在 sticky header 下 | 预计 `apps/web/src/main.ts` 与对应私有测试；`main.ts` 当前属于认证 UI WIP，解除前不得并发 writer，能否免写 `styles.css` 由 RED 后裁决 | 当前书 / 固定 seed 不混淆，1440 / 768 刷新恢复、banner 可见、PPT空态 / handoff、console / overflow；不把真实 AI / PPTX 冒充完成 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / SYSTEM VISUAL REWORK / SIZE PASS 3fed4c7 / CANDIDATE 5f2193c / H3 BLOCKED / NOT IN MAIN` | 用户最新 430×932 证据否决 conversation + drawer 整体视觉；旧 Reader / Alpha / 四宽机械 PASS 只保留局部证据。Reader / drawer 等价 patch 已顺序进入 f312，未取得统一 EARLY | 首波只读非作者从唯一 f312做统一 EARLY。软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `READER f312@866f237 READY FOR EARLY / CONVERSATION f2b625b FROZEN NOT EARLY / REOPEN 1` | Reader `866f237` 已进入唯一 DevTools 候选并完成作者 390×844 自验；conversation 三文件 clean commit `f2b625b` 仍在 mcv1，尚未集成。f312 的 config / lock / QA 图保护差异原样保留 | 总控审查并顺序放入 f312 后恢复原 writer，从同一 DevTools 入口 EARLY；drawer 继续独立排队 |
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `CANDIDATE 866f237 / READY FOR EARLY / AUTHOR SELFTEST PASS` / 项目总控 | 绑定 `19-mobile-book-content-notes.png` SHA `23d078e3…6077` 与 18～20；Reader 四个私有源 / tests 已从 mn19 clean checkpoint 顺序进入唯一 f312 | 390×844 / font23 / safe 47/34 已覆盖初始 / fullscreen / returned / close-reopen、notes 单列全宽、轻量新建、独立近全屏 editor、草稿与失败重试保留、浅色背景重进；console 0、无底部溢出 | 下一步短非作者 EARLY / 扩展视口；真实 notes API / 跨设备、暗色 editor、320 / 360 / 430 尚未覆盖，不得 `FINAL` / `DONE` |
| `M2-F0-A-MINI-CONVERSATION-VISUAL` | `CANDIDATE f2b625b / AUTHOR RELEASED / NOT READY FOR EARLY` / 项目总控 | `/Users/echoman/.codex/worktrees/mcv1/SelfAlone@f2b625b` clean；仅 conversation `index.test.ts`、`index.wxml`、`index.wxss`，禁止 Reader / config / lock / QA 图 | refs 13 / 30 / 31 / 32；精确 5 行根因已修，11 / 11、TS、diff-check、lock SHA通过。总控审查并顺序进入 f312 后恢复原 writer做 DevTools；真实端前不得升级 |
| `M2-F0-A-H3-REWORK` | `VERIFY / OWNER NONE / KEYBOARD H3 UNCOVERED` | 旧任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 为 `notLoaded`；f312 live viewport合同与未提交保护差异保留，当前无 H3 writer | 所有焦点图仍 keyboard=0；软件键盘 / 字体、游客 console 与凭证化环境为真实外部门。先完成本轮 EARLY，不把静态适配或旧任务状态冒充 H3 ACTIVE / PASS |
...
| 当前桌面候选波次 | Auth `b7a6894` READY FOR EARLY；Settings `set1@b7a6894` FAIL / BLOCKED；Conversation `f3bc@97e1c53` ACTIVE WIP；Book Detail `bdv1@c75f745` ACTIVE | Auth clean但未非作者 EARLY；Settings 私有 / shared 已绿但缺 `index.ts` 注入和 active rail、无 commit；Conversation 已有 CORE / API / DB / controller / view 与隔离恢复 Case，但未 commit / UI seam / Chrome；Book Detail 仍为私有 WIP。任一差异或 worker 状态变化即局部回写。 |
| 当前 Mini 候选波次 | Reader `f312@866f237` READY FOR EARLY；Conversation `mcv1@f2b625b` FROZEN NOT EARLY | Reader 作者真实 DevTools 390×844 PASS 只支持送 EARLY；Conversation 三文件 clean commit已修精确 5 行但未进入唯一 f312 / DevTools。二者均不在 main，不得 `DONE`。 |
...
| 开发基线 | 本轮台账写入前 `main@c75f745`；当前台账续接提交见 main HEAD | 当前 Auth / Settings / Conversation / D-TEXT 768 与 Mini `866f237` 均在隔离候选，尚未顺序合入 main；候选提交、测试或作者自验不能自动 `DONE`。主线三个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
