...
- 当前 Goal：`M2-F6 / MINI PPT DRAFT HANDOFF FINAL`；上一 `M2-F2` develop-only scoped Goal 已在 main 闭合。本 Goal 只关闭小程序“书籍→当前会话 draft/context handoff”的最终门：累计候选 `0fa1447 + cf6a8c5` 尚未以等价 patch 进入 main，目标行为以 SPEC 4.1 / 5 与 TECHNICAL 3 的 handoff 契约为准——携带 bookId / 书名、预填可编辑“帮我制作这本书PPT”、发送前不 auto-send、不创建 PPT draft/task、不打开范围 sheet，跳转 / 恢复失败保留草稿与书籍上下文。完整 AI / 大纲 / 模板 / PPTX 继续属于父 `M2-F6 / M1-F5`，不得静默扩张。
- 下一可见检查点：`M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT-REVIEW` 已显式任务化为 READY；立即派新的 Kimi K3 非作者 reviewer 在 clean 候选 `9132809` worktree 只读审查。PASS 后总控仅顺序集成 `9132809` 到 current main 并做 main 回归/真实端验收；FAIL 则原包 RECOVERING。并行 `M1-F4-B` 继续 BLOCKED；每次状态变化继续项目级重算 READY。
...
- 规则版本：总控已从真实 `main@46ab1fd` 加载当前 AGENTS（SHA-256 `0cf10094…d0d`）；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 已从真实安装副本加载精确 `e078dc87fe0f5fc2062b4358c24b2b069a6ec54c`：源仓本地 HEAD、远端 `origin/main` 与本轮7个变更文件的安装副本逐文件 SHA-256 均一致；从本事件起任务式下一步必须先有显式任务行，RECOVERING 必须绑定 delivered ACK / 可验证恢复动作，pending 控制事件只允许一次无进展续作，且 `control_event_guard.py --repo` 内联台账一致性。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控；已有 ACTIVE / CANDIDATE 不重做或改派。
...
| `M1-F4-B` | `BLOCKED / CANDIDATE f53c930 NONAUTHOR FAIL / NO VALID GROK CANDIDATE` / 项目总控 | 两个 cost-ledger migration 文件的并发 / 回滚局部证据保留；FAIL仍为运行期可直接插入终态 reservation 且 audit 反序历史未被拒绝，`f53c930` 不得合 main。最近 Grok 恢复虽口头声称已修复，但真实 worktree clean、HEAD/reflog 无新增，日志同时证明 npm cache EPERM、错误命令与被管道掩盖的失败，且没有合规首个 delivered ACK | 外部执行通道阻塞：本轮无候选可审且按失败门停止自动重试；只有形成新的可验证 Grok 执行路由 / 授权事件后才恢复该两文件包，不影响无文件冲突的 Mini 主线 |
...
| `M2-F6` | `VERIFY / CURRENT GOAL / DRAFT HANDOFF FINAL / FULL FLOW OPEN` / 项目总控 | 书籍→当前会话 draft/context 已在唯一 f312 形成 SELF PASS 候选；本 Goal 只关闭该 handoff 非作者与 main 门 | handoff 最终门由 `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` 驱动；完整AI / 大纲 / 模板 / PPTX仍依赖M2-F3、F4、M1-F5与下载域名 |
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY / MAIN-ADAPT CANDIDATE 9132809 / REVIEW READY` | 历史累计 `0fa1447 + cf6a8c5` 已非作者 PASS；current-main adaptation `9132809` 由独立 Kimi Writer 以真实 RED 9 fail / 56 pass 后完成，6文件 scope-only，Mini 243/243、typecheck、build/package-size、diff-check PASS | 由 `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT-REVIEW` 做 current-main 候选非作者门；PASS 后总控顺序集成并验收，FAIL 原包恢复；不得整支 f312 合入 |
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT` | `CANDIDATE / Kimi K3 WRITER / 9132809 / CLEAN` / `kimi-k3-m2f6-ppt-main-adapt` | candidate=`9132809cc07ea929bfd6fb2e64dcdb8906e7d8b3`，worktree clean，恰好6 owned files；RED=9 fail/56 pass，GREEN=targeted65/65、Mini243/243、typecheck、build 1,573,572/2,097,152、diff-check PASS；Writer final=`CANDIDATE_READY_FOR_CONTROLLER` | 文件租约冻结给候选；下一门=`M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT-REVIEW` 只读非作者审查；Writer 不再写、不集成 |
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT-REVIEW` | `READY / NONAUTHOR READ-ONLY / Kimi K3` / 待派 | 只读审 candidate `9132809` 相对 parent `e088bbd` 的6文件实际 diff；对照 SPEC 143–145 / TECH handoff，重点检查 pre-send 零 task/sheet/auto-send、send-only activate、同会话、完整 metadata、失败/刷新恢复、stale fencing，以及 current-main hydration/persistence/API 未被回退 | 禁止写文件/安装/改 refs/config/lock/创建 worktree；在候选 worktree 运行 targeted65、Mini全量、typecheck、diff-check，独立核对 Writer build 收据；输出 PASS/FAIL 和最小阻断边界，reviewer 不集成 |
