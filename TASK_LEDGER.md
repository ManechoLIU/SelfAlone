...
- 当前 Goal：关闭 `CONTROL-TAKEOVER-2` 接管波次：新总控在真实本地 `main` 上收回唯一控制面，复核 Mini 候选、认证 WIP、Desktop / D-TEXT 门与服务入口，向首波互斥执行者传播 `main@9764180` 控制合同并取得 loaded ACK。该 Goal 不代表项目或任一模块完成。
...
- 当前执行波次：`CONTROL-TAKEOVER-2`；真实主线为 `main@5a18abc`，`AGENTS.md` 与 `main@9764180` 的 SHA-256 均为 `703a6942…66b7`，新总控已回传 `CONTROL CONTRACT LOADED main@9764180`。旧总控 `01a034b4-a73d-7ce2-8531-51585826e6d3` 已原样回复 `OLD CONTROLLER FROZEN`、最后一轮 `interrupted`、revision `32` / `notLoaded` 且已归档；其冻结前提交的 `b2f8276 / 2101695 / 5a18abc` 已逐项审查，过期 owner / 入口 / 候选状态由本提交纠正。主线产品代码未因接管变化，仍只有三个既有未跟踪视觉候选目录。
- 当前活动项：接管时协作树仅新总控 1 个 live Agent，3 个执行槽可用。Mini 唯一候选已由旧总控在冻结前顺序吸收 Reader 等价提交 `a2d613f` 与 drawer 等价提交 `5f2193c`，patch-id 分别匹配 `387c152 / 23e95f0`；`f312` 仍保留未提交 `project.config.json` 换行差异、`pnpm-lock.yaml` importer 与两张旧 QA 图，禁止覆盖或带入冻结提交。认证 `auth1@9764180` 保留 7 个已跟踪修改与 7 个未跟踪文件，最后 mtime 20:34:53，当前 owner 为空；D-TEXT 与 Mini 均无 live reviewer。
- 协调下一动作：本台账提交后，在 3 个互斥槽分别恢复 `M1-F1-A` 认证 WIP writer、重派 D-TEXT main FINAL 只读 reviewer、重派唯一 `f312@5f2193c` 的 Mini conversation / Reader / drawer 非作者 EARLY；三者必须先回传 cwd / worktree / HEAD / status、精确文件或只读边界、隔离环境、首个 RED / Case 与 `CONTROL CONTRACT LOADED main@9764180`，ACK 前只记 `DISPATCHED`，不得记 `ACTIVE`。Mini EARLY FAIL 才释放对应唯一 writer，PASS 后才冻结 FINAL；总控不在此时把 Mini 候选合入 main。
...
- 唯一持续项目总控：任务 `01a038e8-5ee8-79b1-8bdd-daafc2d58e0e` 直接运行在 `/Users/echoman/Documents/SelfAlone` 的本地 `main`，以 `adaptive-delivery` 为持续运行合同，在独占台账写入之外负责总目标、需求影响扫描、阶段与真实依赖、动态拆分、执行 / QA / 非作者审查调度、设计前置门、共享契约与写入边界、候选集成、冲突处理、本地 `main` 复验和最终验收；实现继续交给边界互斥的执行者，不为每份文档或每个小问题创建独立管理 Agent。旧总控已冻结并归档，不再拥有写入权。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `READY / WIP PRESERVED / OWNER NONE` | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | `auth1@9764180`、branch `codex/m1-f1-a-auth` 保留 14 文件 dirty WIP；旧执行者随总控冻结后不再 live，禁止新执行者重做或覆盖。`E-OWNER` 已以 `main@f1334ad` 冻结共享 owner 接缝，账户迁移或其他共享 writer 冲突时暂停 | CORE、API、DB、WEB、QA；不得与 Mini、D-TEXT 或后续共享 schema / app 集成并写 | 密码摘要、Cookie、安全错误、刷新会话、跨账户隔离；绑定参考 01 的 4 视口、键盘和非作者 EARLY / FINAL | 首波恢复原 worktree并先核对现有 RED / diff；Argon2 本地依赖已有历史用户授权。首次可运行 auth HTML / CSS 立即停在非作者 EARLY；真实邮件 / 微信仍停在授权门 |
...
| `M2-F3` 小程序会话 | `VISUAL FAIL / CANDIDATE 5f2193c / EARLY READY / OWNER NONE` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | `f312@5f2193c` 已在 `cb4752f` conversation 基础上顺序吸收 Reader 与 drawer 等价 patch；行为绿灯不改变用户视觉 FAIL，旧 reviewer结论失效 | 新非作者只用唯一常驻 DevTools 对参考 13 + 30～32做 EARLY；覆盖 320 / 360 / 390 / 430、font23、safe-area、选择层生命周期、摘要 / 单入口、1 / 3 / 5 行与附件、趴宠和正常文本扫描 | EARLY FAIL 后释放唯一 conversation writer最小返工；通过 EARLY、冻结新候选后才做 FINAL |
| `M2-F4` 小程序书架与阅读 | `READER VISUAL FAIL / DESIGN-READY / CANDIDATE a2d613f / EARLY READY` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | `a2d613f` 与 `rdrs@387c152` patch-id 相同，已进入 `f312@5f2193c`；旧 writer测试收据只支持行为 checkpoint，不代表视觉通过 | 只在唯一常驻 DevTools 做 320 / 360 / 390 / 430、font23、safe-area、reduced-motion EARLY；EARLY FAIL 最小返工，PASS 后再 FINAL。隔离 worktree保持只读恢复点 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / MAIN MERGED 9143638 / REVIEW READY / OWNER NONE` | `dentry@df96ee3` clean；焦点最小修复已合入，main 既有 `pnpm verify` 为 32 files / 183 tests、typecheck / build PASS。旧 dispatch 未形成 live reviewer，4110 与 main入口均离线 | 首波重派只读 reviewer，以当前 main 内容与隔离 schema / 端口恢复真实 Chrome；覆盖 1440 / 768、DOM 焦点、PPT 空态 / handoff、console / overflow，不得用旧图或测试冒充 | loaded ACK 后升级 ACTIVE；PASS 只关闭文本入口 main FINAL，不扩大 D 父项；原生 200%、真实删除、真实 AI / PPTX 仍精确 VERIFY |
