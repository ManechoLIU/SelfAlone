...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。Desktop Conversation已在`main@dea90d7`完成真实Case FINAL PASS。Book Detail私有候选`e08e7ef`经总控实际diff审查判FAIL并REOPEN 1，原9文件writer正做fail-closed最小返工；F3B-C Selection在`csel1@7317274`的preflight / RED / ref36已加载，唯一私有writer恢复。D-TEXT handoff的`dpth@7317274`已建且clean，但连续未交完整ACK后中断，保留READY恢复点。Mini长列表`mlong@a9da90a → f312@5878400`代码门PASS，真实DevTools EARLY因Mac锁屏暂停。当前构成：controller / integrator 1、implementation writer 2 ACTIVE、code reviewer 1 ACTIVE；D-TEXT writer 0 PAUSED。
- **下一用户可见检查点**：Book Detail先关闭跨书响应与多draft歧义后交新私有候选；Selection交精确RED / 文件边界后释放私有纵向切片；D-TEXT在下一槽用现有clean dpth恢复短preflight；Mac解锁后Mini长列表立即在唯一f312完成三宽 / 大字体真实EARLY。任一FAIL回原范围最小整改。
- **真实阻塞**：Mini Library真实runtime仍受M2-F1 session bridge与DevTools transport阻塞，`20d61d6`只保存未接线合同。Mini长列表代码门已闭合，但微信DevTools当前因Mac锁屏无法读取，解除条件是本机解锁后续跑唯一f312；不反向阻塞桌面writer。Book Detail shared route仍未接，不扩大私有checkpoint为endpoint PASS。Mini父项另缺软件键盘、游客凭证化clean console与真实PPT preview；D-TEXT PPT handoff / banner已解除Auth / Conversation共享接缝等待并派发。
- **当前可见入口 / revision**：稳定Desktop仍是`4127 / 4198@ab13310`，不得误认当前进度；Conversation已在隔离`main`产品revision`dea90d7`完成FINAL后关闭临时入口。D-TEXT `5198 / 4318`已关闭，schema `selfalone_dtext_main_final_cc0cec0_20260826`与runtime `/tmp/selfalone-dtext-main-final-cc0cec0.0LEzLV`保留，未授权删除。仓库`main@7317274`（产品源码`dea90d7`）；唯一DevTools候选为`f312@5878400`，保护config / lock与两张QA图未变，长列表尚未合入main。
...
| `M1-F3B-C` | `ACTIVE` · `PREFLIGHT / RED / REF36 LOADED / UNIQUE WRITER` / `/root/mini_conversation_devtools_fix` | 简单选择、显式确认、只读历史和失效问题的私有纵向切片；不含真实模型、图片、书籍上下文或shared UI seam | A已FINAL释放；`csel1@7317274` before clean；RED为当前仅文本消息且无selection API / DB / controller / view；任何shared seam需求即停 | 仅新建`conversation-selection*`：Server service / migration / store / routes及tests，Web client / state / controller / view / css及tests共17文件；禁止现有Conversation、`main.ts` / `styles.css` / `app.ts` / `index.ts`、contracts / domain | ref36 SHA `1b6ec630…a5796b40`与SPEC / DESIGN-WEB已ACK；下一步私有定向回归→scoped candidate，shared接线后再走1440 / 1024 / 1200 / 768真实EARLY |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `MAIN MERGED 11f480a / SEND-RETRY SCOPED FINAL / H3 OPEN` / 项目总控 | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务继续显式开发适配器，不冒充真实跨端 | f312是唯一DevTools且保护现场不变；`f312@854c577`的Mini source / dist与main等价；长列表已拆包派发 | Reader / Conversation / Drawer既有证据 + `mini-conversation-send-early-854c577/`；软件键盘、tourist凭证、长列表与真实PPT preview继续开放 |
...
| `M1-F2-BOOK-DETAIL-VISUAL` | `ACTIVE` · `CANDIDATE e08e7ef / CONTROLLER DIFF FAIL / REOPEN 1` / `/root/desktop_book_detail_main_port` | 9文件私有候选已有book-scoped service / runtime、Notes / Highlights与16:9作品面；尚未放行shared endpoint或Chrome EARLY | 总控发现跨书/坏响应被伪装`empty`并清空旧作品，以及多个non-stale draft按不可比较version / 随机id任意选current；原writer仅修同根fail-closed，旧`bdport` / shared仍冻结 | 原9文件不扩界；本轮只允许`book-detail-runtime{,.test}.ts`与`book-presentation{,.test}.ts`最小修复 | refs07 / 08 / 10；首候选私有28、相关42、全量349、typecheck / build PASS但不替代审查；下一步fix commit→重审→shared接缝 / Chrome EARLY |
| `M1-F2-D-TEXT-PPT-HANDOFF-FIX` | `READY` · `DPTH 7317274 CLEAN / PREFLIGHT ACK TIMEOUT` / `/root/desktop_conversation_focus_fix` | 当前书 PPT intent必须驱动对应workspace或明确fail-closed；恢复滚动时banner不得被sticky header遮挡 | Auth / Settings / Conversation seam已释放；dpth已创建且零diff，但两次有界等待仍未交精确RED / owned-files ACK，writer已中断释放槽；不丢工作树 | 预计`apps/web/src/main.ts`与必要私有`text-reader-ppt-handoff{,.test}.ts`；禁止`styles.css`、Book Detail、Server / contracts / domain | 当前书 / 固定seed不混淆，1440 / 768刷新恢复、banner可见、PPT空态 / handoff、console / overflow；解除条件为下一槽从clean dpth回短ACK后立刻恢复 |
...
| `M2-F0-A` 当前候选 | `main@11f480a` 含Reader / Conversation / Drawer及send-retry限定闭包；唯一`f312@854c577`与main Mini source / dist等价 | text / attachment-only、失败恢复 / retry与三宽通过；保护config / lock与QA图未带入main。软件键盘、长列表、reduced-motion与PPT真实预览仍开放。 |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader scoped FINAL；Conversation已至`main@dea90d7`且EARLY PASS，main真实Case FINAL中；Book Detail v2 current-main私有port已DISPATCHED | Conversation只剩main Case verdict；旧`bdport`WIP只读保留，新`bdport2`先闭合不触碰shared seam / `styles.css`的私有checkpoint。D-TEXT端口关闭，schema / runtime保留。 |
| 当前 Mini 候选波次 | Reader / Conversation / Drawer既有范围保持；send/retry已至`main@11f480a` scoped FINAL；Library合同`20d61d6`因auth BLOCKED；长列表已DISPATCHED | 唯一`f312@854c577`与main source / dist等价，三宽failed / retry / attachment / restored通过；保护config / lock与两张QA图不变。下一步长列表checkpoint→f312 EARLY。 |
...
| 开发基线 | 产品顺序集成到 `main@dea90d7`；Desktop Auth / Settings、D-TEXT reader与Conversation代码 / EARLY、Mini Reader / Drawer与send-retry限定范围PASS | main仅允许本次台账事件diff与三个受保护未跟踪视觉目录；Desktop Conversation main FINAL、Mini长列表、Library合同和Book Detail未完成，不能自动 `DONE`。 |
