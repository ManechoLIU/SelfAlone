...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-CODEX-02 / ROUND-4 NONAUTHOR FAIL / REMEDIATION` | `49712c5→e5d481f→85bf8b0→c5f339c` 均未进入 main：前三轮 7 个 Important 已补同案修复；第四轮双 runtime barrier 又复现 A 将故障 run 恢复为 queued 后被 B 抢先 claim，A 随即永久 unhealthy 且无 timer 1 个 Important。原 Writer 已从 clean worktree delivered ACK，owned files 仍限 runtime 与 sync-store 各自实现 / 测试及既有 index 接线 | 精确 run 读到 queued 即清本次故障健康状态再正常 drain，终态 / ACK 丢失同样清，仍 running 才退避，NOT_FOUND 按工作已不存在清引用；保持前四轮全部反例后形成新增量候选并再次非作者复核。真实 Key H4仍是独立授权门，禁止读取凭证或调用真实 Provider |
