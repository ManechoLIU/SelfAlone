...
- 当前执行波次：`CONTROL-RESET-1`；Desktop `6afd@44e66af` 已获非作者 `PASS`，完整 `0969c33..44e66af` 已以 merge commit `main@deba325` 集成并完成在线 / 断网恢复真实 Chrome 复验。Mini 固定 `f312@2deb4f6 → 3fed4c7` 重开 conversation + drawer + Reader 系统视觉门；DESIGN-READY 以 `main@3199945` 的产品 / 视觉门、`main@4fe6989` 的可展开内容面板合同、`main@35798ab` 的新增视觉 / ImageGen 路由门、`main@54b4eeb` 的 UI/UX 候选裁决授权和 `main@0552ca6` 的矢量图标视觉忠实门为基线。D-TEXT Web 入口 `dentry@6e62153` 在总控真实 Chrome FINAL 中发现 modal 焦点 P1，已重开最小返工；E-OWNER 已冻结 `owner@4ef4772` 并进入非作者复核。台账仍由项目总控独占。
- 当前活动项：用户最新 430×932 证据覆盖 Mini 旧局部视觉结论：conversation 的 PPT intent 常驻双按钮卡、开发边界、消息 / 任务层级、空白与输入趴宠构图，以及 drawer 的尺度 / 留白 / 山亭整体关系均为 `VISUAL FAIL / REWORK`；用户另否决当前 Reader 阅读背景面板和正常态内部技术文案。`IMG-04` 只保留资产 Alpha 与单独构图候选结论，不能代表实装整体 PASS；体积子包已冻结为 `f312@3fed4c7`，indexed PNG 165,998 B、Mini dist=1552 KiB，关闭 `<2 MiB` 机械门但不升级视觉。`IMG-05` 仅为阅读背景面板 `CANDIDATE / UIUX REVIEW REQUIRED`，不得照搬示例诗句、图标像素、生成背景或固定截图尺寸。失效 Mini reviewer 已回收；替代只读 `/root/mini_visual_final` 已创建但尚未原样 ACK，只登记 `DISPATCHED / ACK PENDING`。D-TEXT follow-up `6e62153` 的失效 reviewer 已回收；总控复用其隔离 API `4127` / Web `4387` 做新鲜 Chrome 终验，确认详情 modal 从“划线与想法”前向 Tab 后焦点落到 `BODY`，因此当前为 `P1 FAIL / REWORK`，最小修复 writer `/root/dtext_focus_fix` 已创建但尚未原样 ACK。E-OWNER 已冻结 `owner@4ef4772`，只读 reviewer `/root/owner_contract_review` 已创建但尚未原样 ACK。
- 协调下一动作：Mini UI/UX brief 返回后按互斥文件释放唯一 conversation writer与唯一 Reader writer；drawer token / 比例由单一 owner顺序承接。`IMG-05` 先由 UI/UX 给 adopt / exclude，若拒绝则只返回单一修图 prompt 或明确拒绝原因，不让开发者自行重设计。每个首个可运行候选先 EARLY，冻结后同一非作者 FINAL；完整 conversation + drawer + Reader 与包体门均通过后，总控才审查 `ff4767b..新 HEAD` 和最小 lock importer。D-TEXT 先修 modal 前后向焦点循环并重新完成非作者真实 Chrome FINAL，PASS 后才顺序合入 main；E-OWNER 非作者通过后再由总控审查共享 contracts / schema / app 接缝并释放 `M1-F1-A`。保护差异与 H3 阻塞不变。
...
| `M1-F2-E-OWNER` | `VERIFY / FROZEN 4ef4772 / NONAUTHOR DISPATCHED / ACK PENDING` / `/root/owner_contract_review` | 为书籍、章节、位置和文本标注冻结会话中立的账户 owner、版本条件写入与迁移回退合同 | `/Users/echoman/.codex/worktrees/owner/SelfAlone`、`codex/m1-f2-e-owner@4ef4772` clean；writer 报告 owner Case 4 / 4、全测 159 / 159、typecheck / build / diff-check 通过。只读 reviewer 已创建但尚无原样 ACK，故不登记 ACTIVE 或 PASS | 候选仅含 `packages/contracts/**`、owner / version 所需 `apps/server/src/**`、当前迁移与定向测试；不得碰 Web / Mini / 台账 / root lock | 两账户、旧 fileVersion、并发写入、未绑定账户隔离；记录 schema / 数据量 / owner / 约束 / 回退点；真实隔离 PostgreSQL、全仓 verify、scoped commit | 非作者 PASS 后由总控审查共享接缝并顺序释放 F1-A；不等待真实邮箱、微信或小程序凭证 |
...
| `M2-F3` 小程序会话 | `FAIL / DESIGN-READY DISPATCHED / ACK PENDING / WRITE HELD` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | 当前 `2deb4f6` 的常驻 PPT intent 双按钮卡、工程诊断文案与会话层级违反 `main@3199945`；替代 reviewer 尚无原样 ACK，brief 返回前不释放 conversation 私有写入 | 正常模式内部文案扫描；选择层打开 / 关闭 / 保留 / 确认摘要；单一阶段入口；320 / 360 / 390 / 430、font23、safe-area 真实 DevTools EARLY / FINAL；软件键盘外部门单列 |
| `M2-F4` 小程序书架与阅读 | `ACTIVE / READER VISUAL FAIL / DESIGN-READY REQUIRED / WRITE HELD` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | `main@4fe6989` 新增内容面板初始→拖拽→全屏→回落→关闭合同；用户否决当前阅读背景面板与正常态技术文案。UI/UX brief 返回前不释放 Reader 写入；书架与其他无冲突能力不因此停滞 | 内容面板五态 / 手势接管 / 标签滚动草稿保留；背景面板浅深持久化与状态；正常模式文本扫描；320 / 360 / 390 / 430、font23、safe-area、reduced-motion 真实 DevTools EARLY / FINAL；软件键盘外部门单列 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `FAIL / FROZEN 6e62153 / P1 FOCUS TRAP / REWORK DISPATCHED / ACK PENDING` / `/root/dtext_focus_fix` | `/Users/echoman/.codex/worktrees/dentry/SelfAlone`、`codex/m1-f2-d-text-entry@6e62153` clean；失效 reviewer 已回收。总控复用隔离 API `127.0.0.1:4127` 与 Web `127.0.0.1:4387` 做新鲜 Chrome 终验：详情 modal 的 roving tabs 含 `tabindex=-1` 项，当前“划线与想法”后前向 Tab 实际落到 `BODY`，违反焦点圈定；Escape 可关闭但不能抵消 P1。最小 writer 已创建但未原样 ACK | 作者 `pnpm verify` 32 files / 182 tests、typecheck / build / diff-check 与既有业务 Case仍可复用；修复只允许 `text-annotation.ts` 与私有测试 / 收据，须补前后向焦点边界真实 Chrome。重新非作者 PASS 前不集成；原生200%、真实删除、真实AI/PPTX继续精确 VERIFY |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / SYSTEM VISUAL REWORK / SIZE PASS 3fed4c7 / H3 BLOCKED / NOT IN MAIN` | 用户最新 430×932 证据否决 conversation + drawer 整体视觉；旧 Reviewer 的 Reader / Alpha / 四宽机械 PASS 只保留为局部证据。`3fed4c7` 仅把 runtime 从 865,820 B 降至 165,998 B，并以 13 files / 69 tests、typecheck / build、dist=1552 KiB 关闭机械包体门；保护 project config / lock 与旧 EARLY 图未纳入 | 失效 reviewer 已回收；替代只读 `/root/mini_visual_final` 已创建，当前 `DISPATCHED / ACK PENDING`。conversation / Reader writer 等 brief 后释放。软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION+DRAWER+READER DESIGN-READY DISPATCHED / ACK PENDING / IMG-04 ALPHA VERIFY / IMG-05 UIUX REVIEW` | 以 `main@3199945` 与 11 / 13 / 30–32 修正 conversation / drawer；以 `main@4fe6989`、`main@35798ab`、`main@54b4eeb`、`main@0552ca6`、15 / 16 / 18–20 与 `IMG-05` 定义内容 sheet 五态、阅读背景 sheet 的密度 / 组件 / 状态及矢量图标忠实门 | `IMG-04` runtime 的真实 Alpha与 size `3fed4c7` 可复用，但整体实装视觉已被用户推翻；`IMG-05` 由 UI/UX 直接比较并给 adopt / exclude，仍不能自动升级参考或运行时。失效 reviewer 已回收，替代只读 `/root/mini_visual_final` 已创建但尚无原样 ACK；conversation 与 Reader writer 均不提前派发。后续按 conversation 私有页、Reader 私有页、drawer 文件三个互斥 owner顺序 / 并行裁剪；320 / 360 / 390 / 430、font23、safe-area、reduced-motion、EARLY / FINAL 必须由非作者真实 DevTools 复验 |
...
| 开发基线 | 当前控制与事实源恢复点 `main@c50e7f3`，当前台账续接提交见 main HEAD；最近产品集成点 `main@deba325` | Desktop 完整统一壳已集成；`3199945 / 4fe6989 / 35798ab / 54b4eeb / 0552ca6 / ad9e3c2` 只更新 Mini 产品 / 视觉门、IMG-05 候选、UI/UX 裁决授权、矢量图标忠实门与调度闭环，不代表 Mini 实现完成。D-TEXT 共享接缝已合并、Web 入口因真实 Chrome 焦点 P1 重开；Mini `3fed4c7` 视觉整改链仍未集成。主线三个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
