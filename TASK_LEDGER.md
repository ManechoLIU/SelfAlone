...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复及北京时间双时段计费门已分别在 `main@b637023+81f5f1f+df32890` 闭合：正式 Server 从独立部署侧平台凭证与六项高峰 / 空闲价格创建 DeepSeek 平台模型，单次请求只读取一次时钟并按工作日 9:00–12:00、14:00–18:00 选档；旧单档、缺项、控制字符、空白包裹、超安全整数及非法时钟全部 fail closed。现于隔离工作树补真实能力层 + CostLedgerStore 的并发硬上限与失败后重试 Case；外部 H4 继续等待独立平台配置与付费调用授权。
- 当前阻塞：`M1-F4-C-WEB-HANDOFF` 的真实端 H3 已解除；`M1-F4-V1` 的动态计费本地门已闭合，剩余工作分为可本地核实的并发 / 恢复闭环与外部真实 Provider H4。无泄密检查已确认当前 shell、监听 4322 的 Server 与仓库可见普通环境文件（仅 `.env.example`）均未提供平台专用 Key 或价格变量，既有用户模型 Key 授权不静默扩展为平台凭证；唤醒真实 H4 需要安全注入独立平台 Key、与官方当前两时段一致的六项人民币微元价格快照，并保留明确的真实付费调用授权。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
...
| `M1-F4-V1` | `VERIFY / MAIN b637023+81f5f1f+df32890 / LOCAL CONCURRENCY CASE ACTIVE` / 项目总控 | 领取、AI / PPT消耗、失败恢复和并发硬上限 Case；正式 Server 已接入独立平台环境配置，fake HTTP 证明用户模型未配置且领取后按 reserve→provider usage→settle 顺序成功；真实隔离 PostgreSQL RED→GREEN 证明 settle 已提交但 ACK 丢失时只回读一次相同金额并返回原回答，不重复 provider，未提交的结算失败仍释放并映射 exhaustion；双时段动态计费及非法配置拒绝已进入 main | 现补真实能力层 + Store 的确定性并发拒绝与失败释放后重试，只在该因果测试暴露真实缺陷时改 owned source；真实 Provider H4、真实成本流水和浏览器成功路径仍须独立平台 Key、六项当次价格快照及明确付费调用授权 |
...
| `M1-F4-V1-CONCURRENCY-RECOVERY` | `ACTIVE / WT SelfAlone-wt-m1-f4-concurrency@a90ed80 / CONTROLLER SAFE FALLBACK` / 项目总控 | 现有 Store 已有真实 PostgreSQL 并发 hard-cap，能力单测已有超限零 Provider 与失败释放，ACK lost 也有真实 Store；缺口是用真实 capability + Store 串起“首请求占用时第二请求被硬拒绝且零 Provider”，以及真实 Store 上失败释放后同 request 新 attempt 重试。Grok OAuth `M1-F4-V1-CONCURRENCY-RECOVERY-GROK-01` 预检通过但 canonical ACK 写入被沙箱拒绝，调用已中断，owned 源码零写入、临时 probe 已清理、`result_known / no-repeat` | 当前宿主总控沿同一范围恢复：只写 settlement recovery 测试；新 Case 若初次即 GREEN 则如实记验证缺口闭合，不伪造 RED；若暴露缺陷仅可最小修改 platform capability / CostLedgerStore，越界即停。禁止真实 Provider、凭证、迁移、依赖、Web / Mini、push |
