...
- 下一可见检查点：`M1-F5-A-WEB-BOOK-SEAM` 累计候选 `cfc8588c7` 已由新一轮非作者复审通过（Critical / Important / Minor=`0/0/0`），并按等价补丁顺序进入 `main@8b0b86f+a689a9b`；current-main 聚焦36项、Web全量306项、typecheck、build、visual-contract 与 diff-check 均通过。当前开放 `M1-F5-A-WEB-DESIGN-READY-06`：先绑定桌面共享外壳 ref02、精确阶段 / 状态 / 视口 / 排除项，再释放首个真实 UI EARLY 包；不得把接缝代码门冒充桌面用户入口完成。
...
| `M1-F5-A-WEB-DESIGN-READY-06` | `READY / 待分配 / READ-ONLY UIUX` | 只读形成桌面需求工作区 DESIGN-READY 合同：绑定 ref02 `d84ffbae…94f6`、共享外壳、四阶段原子一致性、send-gated 可见性、保存 / 恢复、`768/1024/1200/1440` 与键盘 / overflow 验收 | 依赖 `main@a689a9b` 接缝与 SPEC / DESIGN / DESIGN-WEB；输出精确 owned files 建议、状态矩阵、实现 / 排除边界和 EARLY 截止点。不得改源码 / 文档、不得重新设计核心流程、不得使用 Mini 参考覆盖桌面外壳 |
