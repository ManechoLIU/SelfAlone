...
- 规则版本：总控已加载当前 AGENTS；Adaptive Agent Runtime 安装副本由 logical controller `01a03c61-5dd2-7553-968b-a3bc2f5777c9` 机器 ACK 精确 revision `78285b066e37decd0836799f1ce875a7d24eeb2b`（manifest SHA-256 `03405911…9de`），并已完成 canonical current target 的 CONFIRMED wake、post-wake `CLOSED / validation_errors=[]` control cycle 与 `accept-live-e2e`；当前 handshake=`current / blocking=false`。继续复用同一唯一 Controller，不创建第二 logical Controller。
