...
- 下一可见检查点：`M2-F2-REWORK-01` 在 `weread-mini@e4ed48e` 形成Settings配置入口scope-only candidate；root随后审实际diff与测试，合格才进入唯一f312。`M1-F4-B@f53c930` 保持NONAUTHOR FAIL / RECOVERING，唯一f312候选 `4cdce97` 仅作为共享DevTools入口，不整体合main。
...
| `M2-F2` | `RECOVERING / e4ed48e NO-GO / KIMI ACTIVE` / 外部Kimi `kimi-k3 / api / high / os-keychain` | root审累计候选发现Settings WXML无editor / Key输入 / save / retry / disconnect，用户无法连接；现有测试还反向断言不渲染editor，定向绿灯无效。Kimi已对绝对cwd、clean HEAD、Settings四文件、首个RED与停止条件回完整ACK | recovery动作：只改 `settings/index.ts/test.ts/wxml/wxss`，依据SPEC 112–128、DESIGN-MINIAPP 81 / 100–103与ref39派生全屏配置页；禁止真实Key / 调用、共享合同 / Server / f312 / main / lock / 资产写入。检查点=scope-only candidate + exact tests，随后root审diff并顺序进唯一f312 |
