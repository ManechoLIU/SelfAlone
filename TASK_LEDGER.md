...
- 下一可见检查点：`M1-F5-A-WEB-DESIGN-READY-06` 已基于真实 ref02 `1440×1024 / d84ffbae…94f6`、当前 `main@2248a3b` 与新 Conversation Chat 实现形成完整只读合同，冻结 send-gated 原子可见性、状态矩阵、四区 / 768 响应式、键盘 / overflow 与 EARLY 截止点。当前开放 `M1-F5-A-WEB-REQUIREMENTS-EARLY-07`：在隔离工作树完成首次可运行 HTML/CSS、Writer 自己先做真实 Chrome 主同态与四宽检查，随后立即停交非作者 EARLY；不得扩到大纲、模板、生成或 Server / Mini。
- 当前阻塞：当前 Goal 无外部阻塞。`M1-F4-V1` 的本地实现与价格事实已清空，但真实成功路径仍缺独立平台 Key 与本次真实平台调用次数 / 金额上限；既有用户模型 Key 授权不静默扩展为平台凭证。其他既有外部门保持原边界：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal 未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
...
| `M1-F5-A` | `ACTIVE / SEAM MAIN a689a9b / DESKTOP UI NEXT` / 项目总控 | 书籍 CTA→当前会话可编辑 `帮我制作这本书PPT`→仅成功持久化的正向意图创建 / 复用单书 workspace 的十文件接缝已闭合 current-main 代码门；发送失败、否定 / 非意图、无 book context 均保持零 workspace 调用 | 当前只代表 state / controller / client 接缝，不代表可见右栏或真实用户入口。下一步 `M1-F5-A-WEB-DESIGN-READY-06` 先冻结 ref02 下的桌面需求工作区合同，再派发首个 UI EARLY writer；不扩 Server、Mini、大纲、模板或生成 |
...
| `M1-F5-A-WEB-REQUIREMENTS-EARLY-07` | `ACTIVE / DELIVERED ACK / NATIVE TERRA FALLBACK` / `/root/m1_f5_a_web_requirements_early_writer` | 隔离工作树 `m1-f5-a-web-requirements-early@49fe0ce`；首个真实 UI EARLY 只消费已闭合 `bookPptEntryFromHash`、Conversation Controller callback 与 workspace client，在消息成功持久化且合法单书 snapshot 到达后原子显示 ref02 右侧“范围与需求”；发送前 / 失败 / workspace pending 不显示空右栏 | owned=`main.ts`、conversation-chat controller / view 及 tests、新 requirements workspace state / view / CSS 共10文件；Kimi 前序 `transport_failure_before_write` 已 terminal 且 `result_unknown=false`，无调用 resolver 已判同宿主 `gpt-5.6-terra / medium` safe fallback。Writer 首次可运行 HTML/CSS 后必须自验并停止交 EARLY，不扩 GET / 保存、大纲 / 模板 / 生成、Server / Mini |
