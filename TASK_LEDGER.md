...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复、北京时间双时段计费、真实 Store 并发硬上限与失败后重试本地门已分别在 `main@b637023+81f5f1f+df32890+ba70ced` 闭合。真实 Provider H4 继续等待三项外部条件；项目级旁路已重算并启动 `M1-F5-A1`，先完成无需模型的真实账号、已发送消息、单书来源和四项固定需求后端持久化闭环。
...
| `M1-F5` | `ACTIVE / A1 BACKEND BYPASS` / 项目总控 | 业务父闭环：书籍 / 会话带上下文→范围 / 需求→大纲 / 模板→生成→可编辑 PPTX 下载 | F4-V1 的外部 H4 不阻塞无模型的 A1；后续大纲与生成仍分别受文本模型、联网、模板和 Presenton 边界约束 |
| `M1-F5-A` | `ACTIVE / A1 BACKEND FOUNDATION` / 项目总控 | 对话 / 书籍双入口、单书来源、范围 / 需求与任务工作区 | 先落 A1 后端持久化，再接 Web 真实入口；已闭合的 Mini handoff 不重复开发、不触碰 f312 现场 |
| `M1-F5-A1` | `ACTIVE / PLAN READY / CONTROLLER-OWNED` / 项目总控 | 真实账号 + 已发送用户消息门；当前会话内创建独立 PPT 草稿；来源关系按集合建模且 MVP 恰好一本书；固定字段仅用途、受众、页数范围、补充要求；换书保留通用要求 | 实施计划：`docs/superpowers/plans/2026-09-02-m1-f5-a1-ppt-workspace-backend.md`。本包禁止模型、联网、模板、生成、下载和端侧视觉；legacy M0 只保留兼容面，不得冒充 M1 |
