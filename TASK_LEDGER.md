...
- 当前 Goal：`STAGE-READ-NOTES-V1-J / M1-F2-WEREAD ACTIVE`；Desktop 候选 `dfb0574+9065921` 已顺序进入 `main@42188f6+273c691`，实际10个 Web 文件 diff、定向27项、Web全量290项、typecheck / build / visual-contract / diff-check通过。current-main Chrome 对同一隔离账户的未连接空态→no-call连接→0本书架→刷新恢复已通过，但固定 no-call client 无 fail-once入口，不能真实触发同requestId重试，父包保持 `MAIN MERGED / REWORK`；真实 WeRead Key / H4 仍为独立授权门。Mini `M2-F2@e4ed48e` 是独立待审候选，不改变本 Goal 完成条件。
- 下一可见检查点：`M1-F2-WEREAD-WEB-REWORK-02` 原 Writer在既有 `weread-web` 工作树只补 development/no-call 可控 fail-once接缝，真实 Chrome 证明失败保留、同requestId重试与刷新恢复后冻结 follow-up candidate；随后 root 审 diff并增量集成。旁路 `M2-F2@e4ed48e` 下一独立候选事件做实际 diff 审查与唯一 f312 顺序集成；f312 仍为 `4cdce97` 与既有保护现场。
...
| `M1-F2-WEREAD` | `ACTIVE / MAIN 42188f6+273c691 / CURRENT-MAIN PARTIAL PASS` / `/root/m1_f2_weread_web_writer` | 实际10个 Web 文件候选已审并顺序合 main；current-main隔离账户真实 Chrome 已通过未连接空态、no-call连接、掩码、0本书架与刷新恢复。FAIL：固定 `createNoCallWeReadClient` 没有浏览器可控 fail-once接缝，现有 unit不能代替失败 / retry真实 Case | 原 Writer已对 clean `weread-web@9065921` 回 delivered recovery ACK，assignment lease PASS；只写 `main.ts`、`weread-client.ts/test.ts` 补 development/no-call 可控失败接缝。checkpoint=首次可运行 SELF，必须证明失败保留、pending requestId不变、点击重试成功与刷新恢复；不得读取真实Key、扩Server / contracts / Mini或冒充真实H4 |
...
| `M2-F2` | `VERIFY / CANDIDATE e4ed48e / ROOT REVIEW QUEUE` / 项目总控 | clean Mini 9文件候选已冻结；Writer收据为定向40、Mini全量208、typecheck / build / visual-contract / diff-check通过，未做真实WeRead / HTTP / DevTools | 下一独立候选事件审实际diff与合同边界，合格后顺序放入唯一f312并做production接缝 / 失败恢复 Case；AppID / apiBaseUrl / 域名只阻塞依赖的真实H3，不阻塞代码审查 |
...
- 当前 worktree 共5个：main、唯一f312、待恢复候选 `quota-cost`、待独立审查 / 集成的 `weread-mini@e4ed48e` 与当前恢复中的 `weread-web@9065921`；不得为 reviewer / 占位 / 纯消息任务新建 worktree。候选合入 / 废弃后同事件移除clean worktree；脏现场先形成scoped commit或带原路径+HEAD的恢复点。历史归档stash共12条，f312仅在唯一DevTools入口仍需时保留。
