...
- **活动实现线**：Desktop Settings `set1@039cb596` 在真实 EARLY P1 后由唯一 writer 原范围返工；Desktop Conversation `f3bc@56a850e` 已冻结功能候选，总控 diff 审查发现输入失焦与虚假右栏 P1，原 writer 正在四个私有 chat 文件内返工；Mini Conversation 的可见山亭修复 `891d26e` 已顺序进入唯一 `f312`，原 writer 正在真实 DevTools 自验。Book Detail 已冻结 `75e764c + 6c50a76` 私有 Web / Server 候选，等待 shared route seam。
- **下一用户可见检查点**：先取得 `f312@891d26e` 的 Conversation 真实 DevTools PASS / FAIL；并行取得 Settings 失败保留 / 焦点修复候选与 Conversation 逐键输入 / 三栏正常态修复候选；任一候选通过 EARLY 后按 Auth → Settings → Conversation → Book route 的共享接缝顺序合入本地 main 并复验。
- **真实阻塞**：无全局串行阻塞。Settings 真实失败态清空新邮箱 / 当前密码且焦点落 `BODY`；Conversation 候选逐字符重建 textarea 并在无任务时保留诊断右栏；Book Detail 已有私有查询但真实 HTTP / 浏览器仍等待 `app.ts` / `index.ts` 接缝。共享文件只允许顺序集成；真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未授权。
- **当前可见入口 / revision**：Settings 候选入口 `http://127.0.0.1:4299/` → `http://127.0.0.1:5178/#/settings`，revision `039cb596`，API `4298` / Web `5178` / bootstrap `4299` 已恢复并绑定隔离 schema `settings1_early_20260825`，HTTP 可达但真实 EARLY 为 P1 FAIL；稳定 main Web `http://127.0.0.1:4127/` / API `http://127.0.0.1:4198/api/v1/workspace` 仍绑定旧产品 revision `ab13310`，只作恢复点；Mini 唯一 DevTools 项目为 `f312@891d26e`，当前自验进行中。
...
| `M1-F1-B` | `VERIFY` · `EARLY FAIL P1 / REWORK DISPATCHED` / `/root/book_detail_visual` | 目标：邮箱验证 / 找回 / 修改与桌面设置闭环；非目标：真实邮件送达、微信绑定、Conversation 合并。`set1@039cb596` 为 16 文件候选 | fresh session / 1440×1024 真实失败态清空新邮箱与当前密码、焦点落 `BODY`；未修复前禁止合入。与 Conversation 重叠的 shared seam 仍由总控顺序集成 | 原 16 文件边界；本次 writer 仅 `apps/web/src/main.ts`、`settings-page.test.ts`，根因必要时再纳入 `settings-state.ts/.test.ts` | 旧测试与作者 Chrome 只作局部收据；候选入口 `4299 → 5178/#/settings`、schema `settings1_early_20260825`。下一步 TDD 最小修复并重跑同一 failure / focus Case，再补受影响 normal / logout / keyboard |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `VERIFY` · `CANDIDATE 56a850e / P1 REWORK DISPATCHED` / `/root/desktop_conversation_slice` | 目标：输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话 UI；非目标：真实模型、图片消息、Settings / Auth UI | `56a850e` 已冻结 22 文件候选；总控审查确认逐字符全量重绘导致 textarea 失焦，且无任务正常对话错误预留“会话上下文 / 版本”右栏。shared seam 继续冻结 | 已冻结 CORE / API / DB / Web 候选；本次只写 `conversation-chat-controller.ts/.test.ts` 与 `conversation-chat-view.ts/.test.ts` | 旧全仓 218 项与作者 Chrome 不覆盖两项 P1，不能升级 EARLY。下一步 TDD 修复，真实逐键 / IME / Enter / Shift+Enter、发送 / 刷新 / 失败保留、三栏 02 / 36 对照后再交 EARLY |
| `M1-F3B-A1-CONVERSATION-CORE` | `VERIFY` · `CANDIDATE 56a850e / SHARED SEAM FROZEN` / 项目总控 | `97e1c53 → 56a850e` 已闭合确定性回复、PostgreSQL 会话 / 消息、API 与 Web controller / view 候选；非目标仍为真实 AI 与其他页面 | 与 Settings 重叠的 `app.ts` / `index.ts` / `main.ts` 不再扩写；私有 P1 未修前不集成 | 22 个已提交 conversation 文件；当前 writer 只改四个私有 chat 文件 | 作者全仓 verify 与 Chrome 仅为候选收据；总控已审查实际 diff。下一步完成 P1 修复 commit，再顺序适配 Auth / Settings 主线并做真实 persistence Case |
...
| `M2-F3` 小程序会话 | `ACTIVE` / `/root/mini_conversation_visual`（EARLY FAIL reopen 2） | 目标：会话抽屉、文字 / 选择 / 附件状态、输入增长、失败保留；非目标：真实 AI / PPTX。scenery 同根因二次 FAIL 后已重新诊断为透明资产叠加 `.16` 双重衰减；修复 `891d26e` 已进入唯一 f312 | `891d26e` 仅改 Conversation WXSS / test，将同语义 opacity 校准到 `.62`；不得碰 Reader / drawer / config / lock / QA 图。真实端未 verdict 前保持 ACTIVE | 总控 diff 审查、定向 test、Mini build、diff-check与保护哈希通过；当前从 `f312@891d26e` 先重跑 390 山亭首门，PASS 才扩四宽 / 状态 / keyboard / console / overflow |
| `M2-F4` 小程序书架与阅读 | `VERIFY` / 项目总控 | 目标：统一书架、连续正文、内容面板、老己笔记与背景恢复；非目标：真实 notes API / 跨设备。Reader 私有候选 `866f237` 已有作者真实 DevTools 390×844 收据 | 当前唯一候选已前进到 `f312@891d26e`；Conversation 视觉复验未闭合前不把整树称 EARLY PASS，Reader 代码与受保护 config / lock / QA 图不得改写 | 参考 18～20；证据 `mini-reader-early-866f237/`。下一步在同一 f312 DevTools 保留 Reader 局部收据并补当前 revision 的非作者 EARLY / 扩展视口；真实持久化、暗色 editor 仍未覆盖 |
...
| `M1-F1-B-SETTINGS` | `VERIFY` · `EARLY FAIL P1 / REWORK DISPATCHED` / `/root/book_detail_visual` | 目标：桌面设置 normal / failure / logout / account 服务闭环；非目标：真实邮件、Conversation / Auth 扩围。候选 `039cb596` clean | fresh session / 1440×1024 failure 清空新邮箱 / 当前密码并丢失焦点；未修前不合入，shared seam 继续 Settings→Conversation 顺序 | 原 16 文件；本次 writer 只写 `main.ts`、`settings-page.test.ts`，必要时经 ACK 纳入 settings state / test | 预览 4299→5178、API4298、schema `settings1_early_20260825` 已恢复；下一步同一 failure / focus RED→修复→真实 Chrome，再交 EARLY |
| `M1-F3B-A-CONVERSATION-SLICE` | `VERIFY` · `CANDIDATE 56a850e / P1 REWORK DISPATCHED` / `/root/desktop_conversation_slice` | 目标：输入、发送、确定性回复、刷新恢复、错误保留、统一对话 UI；非目标：真实模型 / 图片 / Settings。22 文件候选已冻结 | shared seam 继续冻结；总控实际 diff 已定位 textarea 重建失焦与无任务诊断右栏，未修前不交 EARLY | 本轮仅 `conversation-chat-controller.ts/.test.ts`、`conversation-chat-view.ts/.test.ts` | 绑定 02 / 36；下一步逐键 / IME 与三栏 RED→修复→真实 Chrome，随后才适配 Settings 主线 |
...
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `VERIFY` / 项目总控 | 目标：参考 19 的老己笔记单列全宽流、sheet 手势与浅色背景重进保留；非目标：真实 notes API / 跨设备。Reader candidate `866f237` 已进入 f312 | 当前唯一候选为 `f312@891d26e`；Conversation 重验未闭合前不扩大旧 Reader 局部收据；不得改保护 config / lock / QA 图 | Reader 四个私有源 / tests；绑定 refs 18～20，SHA `23d078e3…6077` | 作者真实 DevTools 390×844 / font23 / safe 47/34 局部收据仍有效；证据 `mini-reader-early-866f237/`。下一步同一 f312 非作者 EARLY / 扩展视口 |
| `M2-F0-A-MINI-CONVERSATION-VISUAL` | `ACTIVE` / `/root/mini_conversation_visual`（EARLY FAIL reopen 2） | 目标：refs 13 / 30～32 的会话输入、选择、附件、错误恢复与同态矩阵；非目标：Reader、drawer、真实 AI / PPTX。同根因二次 FAIL 已重新诊断为透明资产 + `.16` 双重衰减 | 修复 `891d26e` 只改 index WXSS / test 并已进入 f312；当前真实端首门，首条新错误即停止 | `pages/conversation/index.{test.ts,wxss}`；不得修改 nav-drawer / Reader / project config / lock / QA 图 | 总控审查、定向 test、TS / build / diff-check及保护哈希通过；下一步 `f312@891d26e` 390 山亭 PASS / FAIL，PASS 才扩四宽 / 状态矩阵 |
...
| 当前桌面候选波次 | Settings `039cb596` EARLY FAIL / P1 writer dispatched；Conversation `56a850e` CANDIDATE / 两项 P1 writer dispatched；Book Detail `75e764c + 6c50a76` 私有候选冻结 | Settings 预览 4299→5178、API4298、schema `settings1_early_20260825`；Conversation shared seam 冻结；Book route seam 等 Settings / Conversation 顺序释放。三者均未合入 main、不得写 DONE。 |
| 当前 Mini 候选波次 | Reader `866f237` 保留作者局部收据；Conversation `891d26e` 已顺序进入唯一 f312，原 writer真实 DevTools自验 | 同根因二次 FAIL 已经 alpha 数据重新诊断；修复仅 2 行，定向 test / build / diff-check与 project config / lock 保护哈希通过。当前 revision 仍须真实 DevTools verdict。 |
...
| 开发基线 | 台账事件写入前 `main@b5fdd41`；所有当前产品候选未合入 main | main tracked clean，三个既有未跟踪视觉目录原样保留；候选提交、测试或作者自验不能自动 `DONE`。 |
