...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` Checkpoint A 已完成并由总控独立复验 conversation-store `20/20 PASS`：冻结 RED test 未改弱，`conversation-store.ts` 已补齐 PLATFORM_EXHAUSTION mapping，HEAD 仍 `08b8625` 且尚未形成 candidate。现在进入 Checkpoint B：在既定8 owned files 内完成 platform gate/composition；完成后 Checkpoint C 全量验证+scope-only commit。WEB-HANDOFF 继续等待 server contract 冻结。
...
- 规则版本：总控已从真实 `main@46ab1fd` 加载当前 AGENTS（SHA-256 `0cf10094…d0d`）；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 已从真实源仓 `/Users/echoman/Documents/Codex/adaptive-delivery` 的 clean HEAD `d803562550042b2d2e90d1e1f235e107532bc3f1` 同步到真实安装副本并完成 loaded ACK：56/56 tracked 文件逐字节一致、missing=0、mismatched=0，并重新加载 SKILL / agent-model-routing / external-agent-auth / agent-delivery contract / long-task governance / control-event / lifecycle 关键规则；外部 Kimi/Grok 可见状态卡必须由 `run_external_agent.mjs --render-status-card` 生成并逐字复制，禁止手写状态或配色。新规则继续保留 `absorbed / parked` retained semantics；同时把 Bug 修复及 migration / money-cost / authorization / state-machine / external-call / release-critical required review 升级为风险裁剪 TDD 因果证据审查：`tdd_required=true` 时必须绑定 `red_evidence + candidate_revision + green_evidence + red_green_same_case=true + reviewer_counterexample + verdict=PASS|FAIL`，低风险机械/文案修改不强造 RED/GREEN。后续每次 adaptive-delivery 更新固定执行“精确 revision 通知 → loaded ACK → 台账同步”。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控；已有工作包不因规则升级重做或改派。
...
| `M1-F4-C-SERVER-GATE` | `READY / CHECKPOINT A ACCEPTED / CHECKPOINT B DISPATCH PENDING / HEAD 08b8625` | Checkpoint A 冻结 RED→GREEN 已由总控独立复验：conversation-store `20/20 PASS`；`conversation-store.ts` 补稳定 PLATFORM_EXHAUSTION mapping，保留 draft/request retry 语义，尚未 commit。当前 tracked dirty=RED test + conversation-store.ts | Checkpoint B 已成为下一 dispatch：仅在既定8 owned files 完成 platform gate/composition；当前无 live Agent，不伪称 ACTIVE。禁止安装依赖/live paid provider/第9文件。完成后 Checkpoint C targeted/full Server+typecheck+diff-check并 scope-only commit；只有新 Git HEAD+clean scope 才标 CANDIDATE |
