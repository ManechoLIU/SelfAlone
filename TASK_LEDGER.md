...
- 当前活动项：无
- 下一可见检查点：recovery writer 回传完整 ACK 与首个 RED（当前 Reader 入口只跳普通 `#/conversation`、quote / book identity 未进入 composer）；随后冻结单一候选并做非作者验证。
...
| `M1-F2-D` | `READY / RECOVERING ACK PENDING` / `/root/reader_chat_handoff_recovery` | 文本标注、想法、手工笔记、详情入口已有 scoped PASS；当前包只连接 TXT / EPUB 精确引用→现有会话草稿 | 原握手两次无ACK且0文件变化，回收 `previous_status=running`；同一 clean `readchat@b376a5b` 补派一次，完整ACK前停写 |
