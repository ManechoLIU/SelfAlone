...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：当前可见 main 仍是旧 M0；6afd 原 writer 已由 11 个文件的持续 WIP 证明恢复执行，但 844 短高输入区、青瓷 / 纸白组件语言、首次 / 往返对话恢复与 `location.reload()` 分叉仍未关闭，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，Reader 私有标注 / 笔记候选正在隔离 TDD；小程序已冻结 `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28` 七个局部恢复点，分别保留系统视口、Reader、账户 / 书架、会话意图、PPT 视口与账户页字体证据，但整体仍为 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
