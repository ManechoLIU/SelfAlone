...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / FIFTEEN CHECKPOINTS COMMITTED / DESIGN-READY / FROZEN f81b463 / FINAL-VISUAL-VERIFY ACTIVE / PREFLIGHT ACK PENDING` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | f312 writer 已冻结；原 UI/UX 只读审查六页、drawer、输入 / 键盘与底部面板，台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | 唯一审查对象为 `f81b463`，不得混用 `598e34f` 或其他旧快照；逐页 / 状态 covered-uncovered 与真实 DevTools 矩阵返回前不合并。软件键盘仍未实显，正式山亭与真实 PPT 预览缺失；这些外部门不自动掩盖实现 FAIL，也不因其他矩阵通过而解除 |
