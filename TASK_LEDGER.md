...
- 下一可见检查点：`M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` 立即派 Kimi K3 做非作者只读累计 diff 审查；PASS 后由项目总控核对 current-main 无手改集成链并跑定向 / Mini 全量 / typecheck / build，再回写本 Goal。并行 `M1-F4-B` 继续保持 BLOCKED，除非出现新的可验证 Grok 执行路由 / 授权事件。每次本 Goal 状态变化后继续项目级重算 `READY`，不得因本 Goal 的 reviewer / candidate 存在而让其他无冲突可执行项 idle。
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY / F312 cf6a8c5 / SELF PASS / NONAUTHOR RECOVERING` | 累计实现为 `0fa1447 + cf6a8c5`：携带bookId / 书名，可编辑预填；发送前不auto-send / sheet / task；失败保留草稿与上下文 | 非作者 reviewer 已有 delivered ACK，当前由 `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` 从原 Kimi session 恢复；PASS 后只进入总控 current-main 集成 / 回归，不重写实现；证据 `mini-ppt-f312-self-cf6a8c5/` |
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` | `RECOVERING / NONAUTHOR READ-ONLY / Kimi K3 / DELIVERED ACK f01b184 / session_d6023358-b0ee-4470-96f7-02591579a3c0` | 根因假设已验证：外层执行通道在 Kimi `turnStep=0.13` 已发请求但未收 response 时终止子进程；此前 delivered ACK 合规、12 个只读步骤和 observed modified files=0 均可恢复 | 恢复负责人=项目总控调度原 Kimi reviewer；下一动作=用 Kimi `--session session_d6023358-b0ee-4470-96f7-02591579a3c0` 原 session 续跑到 PASS/FAIL，不重发新 reviewer；检查点=原 session 产生 terminal verdict 或再次出现可验证执行通道失败；禁止写文件、安装依赖、改 refs/config/lockfiles |
