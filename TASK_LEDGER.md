...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime 基础代码门已在 `main@b637023+81f5f1f` 闭合：正式 Server 从独立部署侧平台凭证与计费快照创建 DeepSeek 平台模型，保留领取、成本预占 / 按供应商 usage 结算 / 失败释放与用户模型优先级；结算已提交但确认响应丢失时会回读同一 reservation，只有状态和实际金额一致才返回原回答，避免重复供应商调用。2026-09-01 对账 DeepSeek 官方中文价格页后发现当前 `deepseek-v4-flash` 已按北京时间工作日 9:00–12:00、14:00–18:00 分高峰 / 空闲两档；动态选档与 fail-closed 候选已冻结为 `127f63b`，先完成独立 TDD 因果复核并顺序进入主线，再执行真实 Provider H4、成本流水与浏览器成功路径。
- 当前阻塞：`M1-F4-C-WEB-HANDOFF` 的真实端 H3 已解除；`M1-F4-V1` 当前只剩一个本地候选复核 / 集成门和一个外部部署门。无泄密检查已确认当前 shell、监听 4322 的 Server 与仓库可见普通环境文件（仅 `.env.example`）均未提供平台专用 Key 或价格变量，既有用户模型 Key 授权不静默扩展为平台凭证；唤醒真实 H4 需要安全注入独立平台 Key、与官方当前两时段一致的人民币微元价格快照，并保留明确的真实付费调用授权。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
...
| `M1-F4-V1` | `VERIFY / MAIN b637023+81f5f1f / PRICING SCHEDULE REOPENED` / 项目总控 | 领取、AI / PPT消耗、失败恢复和并发硬上限 Case；正式 Server 已接入独立平台环境配置，fake HTTP 证明用户模型未配置且领取后按 reserve→provider usage→settle 顺序成功；真实隔离 PostgreSQL RED→GREEN 证明 settle 已提交但 ACK 丢失时只回读一次相同金额并返回原回答，不重复 provider，未提交的结算失败仍释放并映射 exhaustion | 2026-09-01 官方价格已分北京时间高峰 / 空闲两档，现有单档环境价格会在跨时段时错误结算，先由子包闭合动态选档与 fail-closed 配置门；之后真实 Provider H4、真实成本流水、浏览器成功路径与并发硬上限仍须独立平台 Key、当次价格快照及调用授权 |
| `M1-F4-V1-PRICING-SCHEDULE` | `VERIFY / CANDIDATE 127f63b / REVIEW ACTIVE` / 项目总控 | 原 Grok 4.6 OAuth Writer 在十分钟进度窗内留下同案 RED（组合测试 3 failed / 3 passed）与部分 diff，但未交付候选；总控终止该 attempt、落 `cancelled / result_known / no-repeat` 收据后只恢复同一四文件范围，冻结 clean 候选 `127f63b`。候选按北京时间在每次外部调用前只读一次时钟并选工作日 9:00–12:00、14:00–18:00高峰，其余为空闲；平台 Key 与六项价格全缺时保持 unavailable，旧单档、部分、非法配置或非法时钟均 fail closed 且不调用 Provider | 新 Grok 4.6 OAuth 只读 reviewer `M1-F4-V1-PRICING-SCHEDULE-REVIEW-GROK-01` 已在任何仓库工具前 delivered ACK（session `01a05d72-aefe-7ec3-a88a-a5648bf7155c`），绑定 `base=2f2f2a4 / candidate=127f63b`；现核 RED→GREEN 因果、时区边界、单次时钟与解析反例。候选同案定向10/10、Server集成253/253、typecheck、build、visual-contract、diff-check均 PASS；复核 PASS 后顺序进入 main。禁止读取 Key、调用真实 Provider或修改 Web / Mini / 成本表 |
