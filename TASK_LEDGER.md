...
- 当前 Goal：关闭 `CONTROL-TAKEOVER-2` 接管波次：新总控在真实本地 `main` 上收回唯一控制面，复核 Mini 候选、认证 WIP、Desktop / D-TEXT 门与服务入口，向首波互斥执行者传播 `main@9764180` 控制合同并取得 loaded ACK。该 Goal 只覆盖本轮可共同验收的小里程碑，不代表项目或任一模块完成；关闭后项目总控必须立即从真实 `main`、本台账、活动任务、候选与授权门重算 `READY` 并切换下一小里程碑 Goal，不因一个 Goal、波次或 turn 结束而释放持续所有权。
...
- 当前执行波次：`CONTROL-TAKEOVER-2`；持续所有权合同已在 `main@528b86b` 范围单一提交，新总控已回传 `CONTROL CONTRACT LOADED main@9764180`。快捷入口迁移已加载 `main@9858491`。首轮候选质量门已由范围单一提交 `main@e3a771b` 固化，delta 仅为根 `AGENTS.md` 4 insertions / 1 deletion，当前 SHA-256 `87ef545f…5157`；scope 只改变 writer preflight、自验、候选状态与越界停止门，不改变产品 / 视觉事实、文件 owner 或授权边界。总控已加载；D-TEXT、F3B、Mini Reader 与 Auth writer 在逐项 ACK 前不得升级 `READY FOR EARLY / FINAL`。旧总控已冻结归档，三个既有未跟踪视觉目录继续原样保留。
- 当前活动项：Mini 与认证的短 EARLY reviewer 均已给出真实端 `FAIL` 后停止释放；Mini 48 张 DevTools 原始截图已稳定到 `/Users/echoman/.codex/visualizations/2026/08/25/01a038e8-5ee8-79b1-8bdd-daafc2d58e0e/mini-early-f312-5f2193c/`，认证收据位于同级 `auth1-early-review/`。`M1-F3B-A1-CONVERSATION-CORE` 已在 `f3bc@97e1c53` 形成四文件 clean checkpoint 并双合同 ACK 后释放。当前为合同提交前恢复点；最终并发数量只在 Mini / Auth 创建调用返回后更新，不能把 reviewer 满槽或 `ACK PENDING` 算作实现推进。
- 下一 `READY` / 协调动作：第三实现槽优先给 `M2-F0-A-MINI-NOTES-VISUAL-REWORK`；认证 EARLY FAIL 与 Mini Reader 文件 / worktree 完全互斥，两个真实槽均可用时随后并行恢复认证唯一 UI writer。D-TEXT 越界来源已闭合并恢复，候选留在 dtr8 等质量门 ACK 与 writer 自验；`D-TEXT-PPT-HANDOFF-FIX` 继续只因 `main.ts` 与认证 WIP 的文件级冲突排队。
- main / 隔离入口收据：当前 main HEAD 为 `e3a771b`，产品服务仍绑定 `ab13310`；稳定 Desktop Web `http://127.0.0.1:4127/`（PID `72285`）与 API `http://127.0.0.1:4198/api/v1/workspace`（PID `71125`）使用隔离 schema `selfalone_main_control_ab13310` 和运行目录 `/tmp/selfalone-main-control-ab13310.vimPyg/`。Web 与 workspace API 当前可达，`/health` 为 `404`；只登记入口恢复，不能作为功能或视觉 `PASS`。误写 test 已从 main 精确移除，tracked 仅本台账待提交；三个既有未跟踪视觉目录原样保留。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `CANDIDATE 1bc8b3b + UI WIP / FIRST EARLY FAIL / REOPEN 1 / WRITER RETURN READY` / 项目总控 | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | 非作者在隔离 schema `auth1_early_20260825` 判 FIRST EARLY FAIL，四文件保护哈希未漂移。真实端逃逸缺陷为 50/50 硬切视觉、未登记背景资产，以及微信弹窗无初始焦点 / Tab trap / Escape / 焦点恢复；只允许 `main.ts`、`styles.css`、`auth-page.ts`、`auth-page.test.ts` 最小返工 | CORE checkpoint + 上述四个 UI 文件；不得写 D-TEXT、Mini 或共享 schema 接缝 | 1440 / 1200 / 1024 / 768 无横溢及验证失败保留已覆盖；按参考 01 修复并由 writer 真实 Chrome 自验后才可 `READY FOR EARLY` | 稳定证据 `auth1-early-review/`；1440×1024、200%、reduced-motion 与真实成功登录仍未覆盖；真实邮件 / 微信保持授权门 |
...
| `M1-F3B-A1-CONVERSATION-CORE` | `CANDIDATE 97e1c53 / QUALITY ACK PENDING` / 项目总控 | 只冻结会话草稿 / 上下文 / 任务隔离、当前会话锁、停止 / 失败 / 完成解锁、删除保留已启动任务 / 作品和 stale revision 拒绝；不是 F3B 用户可见完成 | `f3bc@97e1c53` clean；只新增四个私有文件，未写 index、contracts、DB、route、UI 或外部模型。writer 已加载旧控制合同，仍须加载 `e3a771b` 后总控审查候选 | `packages/domain/src/conversation-session{,.test}.ts`、`apps/server/src/conversation-runtime{,.test}.ts` | 9 files / 41 tests、domain / server typecheck、diff-check 通过；不因测试或 checkpoint 升级完成 | 与认证 UI、D-TEXT CSS 和 Mini Reader 文件互斥；只作后续 F3B 接缝种子，不提前释放父项 |
...
| `M2-F3` 小程序会话 | `CANDIDATE 5f2193c / FIRST EARLY FAIL / REOPEN 1 / QUEUED` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | 真实端逃逸缺陷：conversation 缺受控 5 行 / 真实键盘 / 完整状态矩阵且正常态泄露诊断文案；drawer 在 320 有底部导航风险、360 被过大 scenery 侵入，长列表未覆盖 | conversation 私有文件与 drawer `index.wxss` / 必要 `index.wxml` 为两个独立顺序 writer 包；不触碰 Reader | 首次 EARLY 证据已稳定，reviewer 释放；Reader 优先，后续槽释放即分别派发，仍从唯一 DevTools 顺序集成验收 |
| `M2-F4` 小程序书架与阅读 | `CANDIDATE a2d613f / FIRST EARLY FAIL / REOPEN 1 / NOTES DESIGN-READY` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | 真实端逃逸缺陷：浅色选择经内容 sheet 回落 / 关闭后恢复为深色；老己笔记内联输入 / 细下划线 tabs / 稀疏列表偏离参考 19 | Reader 四个私有源文件由同一唯一 writer顺序处理；隔离 checkpoint 后由总控顺序放入唯一 f312 DevTools | 老己笔记与背景保留按同一整改包修复；writer 自验明显偏差后才可 `READY FOR EARLY`，不以局部手势通过替代页面 PASS |
...
| `M1-F2-D-TEXT-READER-768-FIX` | `CANDIDATE / QUALITY ACK PENDING / MAIN CONTAMINATION ISOLATED` / 项目总控 | `dtr8@ab13310` 仅保留 `text-reader.css` / `text-reader.test.ts`；定向测试从 21 PASS / 1 FAIL 到 22 / 22，尚无真实 Chrome 自验、未提交 | 来源已证明为 writer 首次相对 `apply_patch` 在默认 main cwd 的误写；main / dtr8 test 文件 SHA `5f6387f3…3af9`、patch SHA `51d87fd5…e39c` 一致且 inode 独立。确认 dtr8 原样保存后已只从 main 精确移除 20 行 | 仅绝对路径写 dtr8；加载 `e3a771b` 后由原 writer完成 768 / 1440、console / overflow 自验，明显偏差不得交 reviewer | 未满足 `READY FOR EARLY`，不因测试绿色升级 |
...
| `M2-F0-A-VISUAL-REWORK` | `CANDIDATE f312@5f2193c / FIRST EARLY FAIL / REOPEN 1 / REVIEWER RELEASED` | conversation `cb4752f`、Reader `a2d613f`、drawer `5f2193c` 已进入同一候选；保护差异与旧 QA 图仍未提交。首次真实 DevTools EARLY 已覆盖四宽并给分范围 FAIL | Reader / conversation / drawer 按互斥文件独立返工；只有 writer 自验、再次 EARLY、冻结候选、FINAL 后才可由总控考虑集成。430、真实键盘、长列表、reduced-motion 与未覆盖五态不得静默升级 |
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `DESIGN-READY / WRITER DISPATCH NEXT` / 项目总控 | 绑定 `19-mobile-book-content-notes.png` SHA `23d078e3…6077`；18～20 共同采用共享大圆角面板、三段切换、内容密度、动态高度、安全区、初始工具栏、上滑全屏 / 下滑回落。老己笔记采用轻量“新建笔记”、单列全宽正文 / 可选只读引用 / 元信息 / 行尾更多与细分隔线；首行不标题化 | 当前 `a2d613f` 是底部细下划线 tabs、常驻带边框 textarea 和稀疏 body/meta，缺参考的分段表面、轻量新建入口、引用 / 行尾动作和内容密度。禁止重新 ImageGen、卡片化笔记、独立标题字段、截图像素 / 模糊生成图标；成熟 SVG 需服从参考比例 / 线宽 / 状态 | 唯一 writer 只写 `apps/miniapp/src/pages/reader/{index.ts,index.wxml,index.wxss,page-state.ts}` 与现有 Reader 测试；从 `f312@5f2193c` 隔离 checkpoint，不导入 DevTools、不写 config / lock / QA 图 | 首个可运行版本立即停在同状态真实 DevTools EARLY；矩阵为 notes 正常 / loading / empty / filtered-empty / failed，初始 / dragging / fullscreen / returned / closed，320 / 360 / 390 / 430、safe-area、font23、reduced-motion、scroll / tab / draft / theme 保留、console / overflow。参考已存在，ImageGen 保持 `5 / 20` |
