...
- **活动实现线**：Auth / Settings与Mini Reader / Conversation既有main PASS保持。D-TEXT 768 `main@cc0cec0`经同Case reader scoped FINAL PASS，子包关闭，PPT / banner父缺口仍开放。Desktop Conversation `87a598c`代码门PASS但真实1440 EARLY因缺新建 / 搜索与新账户领取条FAIL；旧writer两次无ACK后已中断，新唯一writer `/root/desktop_conversation_reopen5`已派发但待preflight，未冒充ACTIVE。Mini Drawer第三次FAIL已定位双mask，`f35acc3`顺序进入唯一`f312@b6e7b66`，正在同矩阵EARLY。当前构成：controller / integrator 1、implementation writer 0 ACTIVE + 1 DISPATCHED / ACK PENDING、reviewer 1（Mini Drawer），另1槽因Desktop shared seam暂不重复派发。
- **下一用户可见检查点**：Conversation新writer先交ACK + quota / new / search RED，再闭合真实行为并作者Chrome自验；Mini Drawer `f312@b6e7b66`给390 / 360 / 320 verdict。Conversation PASS后由总控顺序集成main并复验，再释放Book Detail seam或D-TEXT PPT handoff唯一writer；Mini verdict后立即集成或升级资产级阻塞。
- **真实阻塞**：Conversation真实EARLY缺ref02的新建 / 搜索与ref36的新账户一次性领取条；本轮裁决`M1-F4-A`不再错误等待`M1-F3B-V1`，只释放真实claim最小闭环，F4-B / C不扩。Drawer布局侵入已关闭，当前只验证移除短屏slot第二层mask后亭子辨识度；若仍FAIL即停止CSS微调并登记资产构图门。D-TEXT reader子包无阻塞；PPT handoff / banner和父项继续开放。Book Detail与D-TEXT PPT handoff仍受Conversation共享`main.ts`单writer约束。
- **当前可见入口 / revision**：Desktop候选入口`5188 / 4308`的既有验收绑定仍记`a9408ad`；Conversation EARLY隔离入口已清理。D-TEXT `5198 / 4318`已关闭，schema `selfalone_dtext_main_final_cc0cec0_20260826`与runtime `/tmp/selfalone-dtext-main-final-cc0cec0.0LEzLV`保留，未授权删除。稳定恢复点`4127 / 4198@ab13310`。Mini唯一DevTools为`f312@b6e7b66`；config / lock与两张QA图未变。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE` · `CANDIDATE 87a598c / CODE PASS / CHROME EARLY FAIL / REOPEN 5 DISPATCHED` / `/root/desktop_conversation_reopen5` | 输入、发送、确定性回复、刷新恢复、错误保留与统一对话UI；代码竞态闭合。真实1440新账户状态缺新建 / 搜索和一次性领取条，需补真实行为而非静态视觉 | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、base `a9408ad`；新writer ACK前不得写ACTIVE，shared seam仍只此包 | 原25个Conversation文件 + 最小F4-A私有模块 / tests与shared app / index / main；禁止Book Detail / Settings私有 / Mini | 代码门22 / 155 PASS；EARLY FAIL `conversation-early-87a598c/`。下一步preflight ACK + RED→claim / new / search闭环→作者Chrome→候选复审 |
| `M1-F3B-A1-CONVERSATION-CORE` | `VERIFY` · `CURRENT-MAIN CANDIDATE 87a598c / CODE PASS / VISUAL REOPEN` / `/root/desktop_conversation_reopen5` | 确定性回复、PG会话 / 消息、API、Web、legacy、导航、幂等、发送锁与hydrate竞态代码门保留；本轮不重做已过范围 | 当前视觉 / 缺行为REOPEN不推翻代码局部PASS；owned外即停 | 同一25文件候选及F4-A最小接缝；禁止其它页面 | 首个真实FAIL止于1440 idle，后续发送 / 恢复Case未运行；补齐入口后重跑完整EARLY |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `READY` · `DISPATCHED / ACK PENDING` / `/root/desktop_conversation_reopen5` | 新账户一次性领取、按账户grant状态和桌面对话首页轻量领取条闭环；不建设券、余额、充值、成本账本或常驻记录 | Auth + F3B-A共享外壳已具备；不再等待F3B-V1，shared seam与A同一writer；外部 / 付费调用即停 | 最小CORE / API / DB / WEB / QA新增文件及A已登记shared seam | claim幂等 / 并发 / 隔离；成功短反馈后移除，失败保留聊天输入；ref36 / 38且无金额 / 余额 / 进度。下一步ACK + RED |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE` · `cvmain@87a598c / CHROME EARLY FAIL / REOPEN 5 DISPATCHED` / `/root/desktop_conversation_reopen5` | 既有功能 / 并发修复保留；本轮只闭合真实new / search与unclaimed claim条后重跑EARLY | 新writerpreflight未闭合前仅DISPATCHED；shared seam与F4-A同writer，Book Detail / D-TEXT handoff继续等 | 原25文件 + F4-A最小新增模块；禁止其它私有页面 / Mini / ledger | ref02 / 36；FAIL图与DOM在`conversation-early-87a598c/`。下一步ACK + RED→实现→作者Chrome→非作者EARLY |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY` · `READER + CONVERSATION MAIN FINAL / DRAWER f312@b6e7b66 RE-EARLY ACTIVE / REOPEN 3 / H3 BLOCKED` / 项目总控 | Reader与Conversation限定范围已复验；Drawer布局层已通过，双mask根因候选`f35acc3`已顺序进入唯一f312 | 长列表无真实数据；软件键盘、游客凭证 / console、真实PPT preview继续H3 / VERIFY | f312只新增Drawer WXSS / test提交`b6e7b66`；H3无writer，保护差异不变 | writer12 / 12、Mini101、typecheck / build / diff-check通过；EARLY证据待 `mini-drawer-re-early-b6e7b66/` |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `MAIN MERGED 31462d9 / READER + CONVERSATION SCOPED FINAL; DRAWER f312 RE-EARLY ACTIVE` / 项目总控 | Reader与Conversation已关闭对应缺陷；Drawer双mask最小修复`f35acc3 → f312@b6e7b66`，不再改高度 / offset / asset | 全Mini仍缺附件 / 真实发送重试、软件键盘、长列表与reduced-motion；本轮若辨识度仍FAIL则升级资产构图门 | main闭包不变；f312只追加Drawer两文件，保护现场不变 | 三轮FAIL均已保留；当前同三视口EARLY，PASS后才进入main |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `f312@b6e7b66 / RE-EARLY ACTIVE / REOPEN 3` / `/root/mini_drawer_early_review` | ref11与正式山亭asset；390布局PASS，360 / 320侵入已关；短屏slot第二层mask已移除，仅保留image渐隐 | writer clean释放；长列表无真实数据继续uncovered，唯一DevTools外即停 | f312只追加 `components/nav-drawer/index.test.ts`、`index.wxss`；保护现场不变 | ref11 SHA `9e764e…c36e`；12 / 12、Mini101与构建通过；当前三视口真实EARLY `mini-drawer-re-early-b6e7b66/` |
...
| `M2-F0-A` 当前候选 | `main@31462d9` 含Mini code-only闭包；唯一 `f312@b6e7b66` 在同源基线上追加Drawer四轮两文件修复 | Reader + Conversation限定范围FINAL；Drawer双mask候选已入唯一f312并做三视口re-EARLY。保护project config / lock与QA图未带入main；附件 / 真实发送重试、软件键盘、长列表、reduced-motion与PPT真实预览仍未通过。 |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader scoped FINAL；Conversation `cvmain@87a598c`代码门PASS、Chrome EARLY FAIL后REOPEN 5已派发；Book Detail继续冻结 | Conversation缺真实new / search / claim；新writer ACK前不写ACTIVE。D-TEXT端口关闭，schema / runtime保留。 |
| 当前 Mini 候选波次 | Reader + Conversation在`main@31462d9`限定范围FINAL；Drawer `f35acc3 → f312@b6e7b66`正在三视口re-EARLY | 三轮FAIL证据已保留；新证据写 `mini-drawer-re-early-b6e7b66/`，保护现场不变。 |
...
| 开发基线 | 产品顺序集成到 `main@cc0cec0`；Desktop Auth / Settings、Mini Reader / Conversation限定范围PASS，D-TEXT reader scoped FINAL | main仅有当前台账事件diff与三个受保护未跟踪视觉目录；Conversation EARLY FAIL、Drawer仅在f312，均不能自动 `DONE`。 |
