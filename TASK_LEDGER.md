...
- **Goal**：本有终点里程碑为“Desktop Selection focus 候选非作者 EARLY、顺序集成 main 与真实 Chrome main 验收，并行把 Mini Drawer v2 放入唯一 `f312` 后推进至 EARLY 或闭合真实环境阻塞”；唯一项目总控任务仍为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`。Desktop 已闭合；Mini 在两种安全同入口恢复方式均失败后，以精确 DevTools 环境阻塞收束本 Goal，不把候选或局部截图冒充 EARLY。
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@caa67e76bbe9ca387c4052b798dbe5eb4ac8e3b4` 开始；主线 `AGENTS.md` 来自 `19d44285aed7aa70711514a3a81fa25da4070fda`（SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`），`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`，全局 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。本提交只写本台账，不改产品 / 视觉事实源。
- **活动实现 / 复验线**：Desktop 原 writer 先交 `f6a6694`，首次非作者 EARLY 命中“确认后焦点落 `BODY`”；原 writer 在同两文件范围追加 `bba257e`，第二次非作者 EARLY `PASS` 后，总控顺序集成为 main `858bf19 → c858a06`。Mini writer 已交四文件候选 `ae92912`，总控顺序放入唯一 `f312@63782ba`；writer 320 局部自验后因 query 未送达停在 `SELF BLOCKED`，非作者环境恢复再以两种安全方法复现同阻塞，writer / reviewer 均已释放。当前仅 controller 占用 `1 / 4`，可用 `3`，误中断 `0`。
- **READY 重算**：Desktop focus 门已完成，无返工 READY。Mini Drawer 源码候选保持在唯一 `f312`，下一动作必须先解除 DevTools `INVALID_LOGIN / access_token expired` 与 `routeTo appLaunch timeout`，使 `developmentLongList=1` 真实送入 `pages/conversation/index`；未解除前不得继续截图矩阵或派 reviewer。其余本 Goal 无文件 / 依赖互斥的 READY 实现包；Library session bridge、软件键盘与游客凭证 console 仍各有既有门。
- **下一用户可见检查点**：Desktop 当前 main 可从 `http://127.0.0.1:4276/#/conversation` 直接看到真实对话与已确认 Selection；API `4212` 断开时真实错误态、恢复后“重新连接”、整页刷新及全新 Chrome 标签直接进入均已通过。Mini 下一检查点是恢复同一 `f312` 的有效 DevTools 登录 / 路由 query 后，由原 writer补齐 `320 / 360 / 390 / 430` 同状态自验，再交非作者 EARLY。
- **真实阻塞**：Mini `f312` 两次安全重开 / 显式编译均只进入普通会话，`developmentLongList=1` 未送达；日志出现 `INVALID_LOGIN, access_token expired` 与 `routeTo appLaunch timeout`，故 Drawer v2 尚无完整 writer 自验或 EARLY verdict。Project List 的 `cvre / rdrs / 6f5c / miniprogram-1` 四项是历史 writer / reviewer 遗留，当前 `mdrv2` 未新增第六入口；未经授权不移除最近项目记录，也不删除源码或 worktree。真实 AI 问题生成、软件键盘、游客凭证、远端、发布与真实外部调用继续保持原门。
- **当前可见入口 / revision**：Selection focus 代码基线为 `main@c858a06`；当前主线 Web `127.0.0.1:4276` PID `77554` 通过代理连接 API `127.0.0.1:4212` PID `78244`，隔离 schema `selection_main_c858a06_20260826`。旧 `4173 / 4100` 不作为本轮验收。Mini 唯一活动 DevTools 入口为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、源码 HEAD `63782ba`；不新增导入。
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `f312 63782ba / V2 CANDIDATE / WRITER PARTIAL / DEVTOOLS ENV BLOCKED BEFORE EARLY` / 项目总控 | ref11与既有山亭asset的同态返工；用户截图确认横向矩形接缝、山亭位置 / 尺度 / 上缘渐隐、列表 / 底栏节奏偏离，旧 `hasAlpha` 与旧整页 PASS 不再有效 | 四文件候选`ae92912`已顺序放入唯一f312；v2 asset SHA `87cf9828…8876`。writer完成320 normal / open / current、scroll、filtered-empty局部证据，但360切换后query丢失；两种安全同入口恢复均失败，禁止新增导入或继续盲试 | 截图SHA `1af1637…35f4`、ref11 `9e764e…c36e`、brief `8f352a2…6ae8`；writer / recovery均未改保护config / lock / QA。当前无EARLY verdict；解除DevTools token / route阻塞后原writer补齐四宽与全状态，再交非作者EARLY；`mini-drawer-f312-63782ba-writer-self/`、`mini-drawer-f312-63782ba-devtools-recovery/` |
...
| 当前 Mini 候选波次 | Reader / Conversation既有限定结论保持；长列表`35bad75`与Settings`14b69dc`已在main但scoped FINAL为NO-GO；Drawer v2`ae92912`在唯一`f312@63782ba`，旧整页PASS仍被用户截图否定 | v2源码 / 资产checkpoint已落f312，保护现场不变；当前精确阻塞为DevTools token过期 / route timeout导致query不送达，故停在writer partial、无EARLY。5项Project List中除f312外均为历史遗留，未清理、未新增。 |
| 当前 Desktop main 预览 | `RUNNING / CURRENT MAIN CODE c858a06 / REAL CASE + RECONNECT PASS` | Web `127.0.0.1:4276/#/conversation` PID `77554`、API `127.0.0.1:4212` PID `78244`；schema `selection_main_c858a06_20260826`。旧`4173 / 4100`排除；收据`main-selection-focus-c858a06/FINAL-RECEIPT.json`。 |
| 开发基线 | 本台账事件父为`main@c858a06c0e6d3ad2b4e562be34549bcfe8ce504b`；本轮仅再写`TASK_LEDGER.md` | 6个受保护DOCX / Word锁文件与3组受保护视觉候选目录仍与冻结a233副本同字节，禁止暂存 / 修改 / 删除；Mini唯一f312的2个保护修改与5组既有QA未改变。 |
