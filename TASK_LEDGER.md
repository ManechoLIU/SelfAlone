...
- 当前 Goal：`STAGE-READ-NOTES-V1-K / M2-F2 RECOVERING`；`7667456` 非作者代码审查FAIL：断开后清空只读快照，合法A→B替换被误判stale且B失败快照可能复用A缓存。Reviewer测试/构建全绿但未覆盖上述产品合同；同一候选已由Kimi新Assignment限定Settings / Library四文件从两条RED恢复。PASS后才顺序放入唯一 `f312@4cdce97` 做no-call连接、hydrate、失败保留、同requestId重试与刷新恢复真实DevTools Case。真实WeRead Key / AppID / apiBaseUrl / 域名H3不在本Goal。
- 下一可见检查点：`M2-F2-REWORK-02` 在 `weread-mini@7667456` 冻结断开保留与A→B隔离的四文件scope-only candidate；root随后派非作者复审，PASS才进唯一f312并复用 `/root/m2_f2_uiux_acceptance` 跑ref22/ref39 EARLY / 真实端Case。并行 `M2-F4-B@b7c3dcb` 已冻结两文件scope-only candidate，`/root/m2_f4b_code_review` 以新Assignment `M2-F4-B-CODE-REVIEW-02` 完整ACK并在审空白 / non-BMP UTF-16双向换算；`M1-F4-B@f53c930` 保持NONAUTHOR FAIL / RECOVERING。
...
| `M2-F2` | `RECOVERING / CANDIDATE 7667456 NONAUTHOR FAIL / KIMI ACTIVE` / 外部Kimi `M2-F2-REWORK-02 / kimi-k3 / api / high` | Reviewer实际审累计11文件并运行定向25文件/213、Mini全量213、typecheck、build、visual-contract、diff-check均PASS，但证明两项阻断：断开→null会清空已同步书/划线；合法A→B替换被stale guard丢弃且B的failed/paused快照可混入A缓存，违反SPEC 125–128。新Assignment已对clean HEAD、Settings / Library四文件、两条RED与停止条件完整ACK，lease guard PASS | recovery动作：只在 `settings/index.ts/test.ts` 与 `library/index.ts/test.ts` 锁断开后只读保留、合法换号与跨账号隔离；禁止WXML/WXSS / 共享合同 / Server / 真实Key调用。检查点=scope-only candidate + exact tests，随后非作者复审；ref22/ref39 EARLY合同保持待唯一f312验证 |
...
| `M2-F4-B` | `VERIFY / CANDIDATE b7c3dcb / NONAUTHOR REVIEW ACTIVE` / `/root/m2_f4b_code_review` Assignment `M2-F4-B-CODE-REVIEW-02` | Kimi writer在原两文件边界以RED锁定leading / trailing whitespace与non-BMP UTF-16 hydrate→restore→save漂移，冻结clean `b7c3dcb`（+135/-3）；定向24、Mini全量185、typecheck、build、visual-contract、diff-check均PASS。Reviewer已对clean cwd / branch / HEAD / 两文件 / 首检 / 停止条件完整ACK，assignment lease PASS | 检查点=非作者实际diff与换算合同verdict；PASS后由root顺序合main并回归，FAIL则仅返原两文件恢复，不受AppID / GUI / 域名阻塞 |
