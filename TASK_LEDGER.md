...
- 当前执行波次：`CONTROL-RESET-1`；Desktop `6afd@44e66af` 已获非作者 `PASS`，完整 `0969c33..44e66af` 已以 merge commit `main@deba325` 集成并完成在线 / 断网恢复真实 Chrome 复验。Mini 固定 `f312@2deb4f6 → 3fed4c7` 重开 conversation + drawer + Reader 系统视觉门；DESIGN-READY 以 `main@3199945` 的产品 / 视觉门、`main@4fe6989` 的可展开内容面板合同和 `main@35798ab` 的新增视觉 / ImageGen 路由门为基线。D-TEXT Web 入口 follow-up 已冻结为 `dentry@6e62153` 并重新进入独立 FINAL。台账仍由项目总控独占。
...
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION+DRAWER+READER DESIGN-READY DISPATCHED / ACK PENDING / IMG-04 ALPHA VERIFY / IMG-05 UIUX REVIEW` | 以 `main@3199945` 与 11 / 13 / 30–32 修正 conversation / drawer；以 `main@4fe6989`、`main@35798ab`、15 / 16 / 18–20 与 `IMG-05` 定义内容 sheet 五态和阅读背景 sheet 的密度、组件与状态 | `IMG-04` runtime 的真实 Alpha与 size `3fed4c7` 可复用，但整体实装视觉已被用户推翻；`IMG-05` 只可由 UI/UX 拆解 adopt / exclude，不能升级参考或运行时。替代 `/root/mini_visual_design_ready_v2` 已恢复 running 但尚无原样 ACK；conversation 与 Reader writer 均不提前派发。后续按 conversation 私有页、Reader 私有页、drawer 文件三个互斥 owner 顺序 / 并行裁剪；320 / 360 / 390 / 430、font23、safe-area、reduced-motion、EARLY / FINAL 必须由非作者真实 DevTools 复验 |
...
| 开发基线 | 当前控制与事实源恢复点 `main@35798ab`；最近产品集成点 `main@deba325` | Desktop 完整统一壳已集成；`3199945 / 4fe6989 / 35798ab` 只更新 Mini 产品 / 视觉门与 IMG-05 候选，不代表 Mini 实现完成。D-TEXT 共享接缝已合并、Web 入口正在 FINAL；Mini `3fed4c7` 视觉整改链仍未集成。主线三个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
