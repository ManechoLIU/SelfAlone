...
- 当前活动项：Web 与 D-TEXT 原作者任务均已完成并释放写入，候选分别为 `221d5bb`、`d65ab3b`；UI/UX 任务当前 `active` 复验 Web，D-TEXT 排队。小程序原 f312 writer 已停止 drawer / Reader 写入；当前 f312 仅保留 4 个视觉 WIP 源 / 测试文件、1 个新 test 与 2 张 EARLY JPEG，保护哈希不变。新唯一 writer `/root/mini_visual_rework` 已由临时接管者派发但总控尚未收到首个 ACK，状态 `DISPATCHED / ACK PENDING`；正式 drawer 山亭资产保持 `ASSET BLOCKED`。
...
| `M2-F0-A-VISUAL-REWORK` | `DISPATCHED / ACK PENDING / ASSET BLOCKED` / `/root/mini_visual_rework` | 当前仅 `nav-drawer/index.wxml|wxss|test.ts`、`reader/index.wxss|page-state.test.ts` 与对应新收据；不得改其他六页或保护文件 | 重跑当前最终 WIP；320 / 360 / 390 / 430 与 font23 实测 drawer 列表滚动 / surface、Reader 标题与提示生命周期；无正式山亭资产时只保留无图 slot，不得伪造。首屏可运行即交 UI/UX EARLY，冻结后再 FINAL |
