...
- 当前执行波次：`CONTROL-RESET-1`；Web 已冻结 clean tracked 候选 `47db65b`，16 文件 / `+929 -89`，单一右侧阶段工作区、hash / history 单一 `renderRoute`、阶段草稿 / scroll / focus 与失败恢复已有测试和作者侧真实 Chrome 收据；总控新鲜定向 4 文件 / 30 项及全仓 24 文件 / 131 项、typecheck / build 通过，现由非作者执行真实端终验，禁止提前合并。D-TEXT 私有 Web `e2a6ac4` 的非作者 verdict 为 `FAIL / REWORK`；原作者已在 9 个私有 Web 文件完成首轮实现，原 41 项回归全部转绿，仍需全仓与同一真实 Chrome Case 复验。小程序已追加 `6586bca → b28aa9c` 两个局部点；Reader failure 已改为稳定 shell 原位失败 / 重试恢复，但父项仍为 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 为 D-TEXT 私有唯一写入者；总控复跑定向 5 files / 41 tests 全绿且 diff-check 通过，状态 `ACTIVE / TARGETED GREEN / REAL CHROME PENDING`，共享入口继续排队。同一非作者 UI/UX 已清理 D-TEXT 端口 / schema 并转入 `6afd@47db65b` 桌面对话 FINAL-VISUAL-VERIFY；4173 旧 main 已排除。小程序同一 writer 已冻结 `b28aa9c` Reader failure checkpoint并继续 PPT failure 子项；总控现场定向 1 file / 6 tests 为 `ACTIVE / GREEN IN PROGRESS`。两个受保护差异保持未提交，其最终复验仍排在 Web 之后。
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / TARGETED GREEN / REAL CHROME PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | 基线 `dtxtui@e2a6ac4` / `codex/m1-f2-d-text-web` 已完成独立真实 Chrome 首轮复验；原作者当前只改 9 个私有 Web state / view / runtime / test 文件。总控复跑原 5 files / 41 tests 全绿并通过 diff-check，覆盖 Selection 清理 / 单请求去重、真实 tab / hidden panel、409 刷新版本后保留输入并自重试、44px 恢复与图标入口 | 只准改原 text-reader / text-annotation / book-detail 私有文件，不碰 `main.ts` / shared shell / styles 或共享 contracts / schema；聊天 handoff 在共享路由接入前保持 disabled fail-closed | 真实 EPUB / TXT 选区单次保存后退出、独立笔记无残留引用、真实 tab / AX / 焦点圈定、失败与并发冲突原位保留并可恢复、浅深与两视口非作者通过 | 全仓 verify、同一真实 Chrome Case、scoped commit 和再次非作者复验尚未完成；quote intent、共享 main 入口、VoiceOver、1200 / 1024、长书滚动和完整 loading / 载入失败仍未覆盖 |
...
| `M1-F2-D-TEXT` | `FAIL / REWORK / ACTIVE / TARGETED GREEN / REAL CHROME PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@e2a6ac4`、`codex/m1-f2-d-text-web`；非作者已完成独立真实 TXT / EPUB、Selection、持久化、失败 / 并发、主题 / 专注、AX 与两视口复验并清理环境 | 当前 9 个私有 Web 文件 / `+283 -15`；总控定向 5 files / 41 tests 与 diff-check 通过。停止条件仍为全仓 verify + 同一真实 Chrome Case 重验 + scoped commit；全局路由在 Web shell 与私有候选分别通过后由总控顺序接入 |
