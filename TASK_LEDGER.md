...
- 当前活动项：Mini 非作者 EARLY 已确认 Reader controls / hint、drawer 360 / 390 / 430 与 IMG-04 Alpha 通过；唯一视觉阻断为 320×568 山亭上缘侵入当前会话行，长列表滚动仍未覆盖。唯一 `/root/mini_visual_rework_fix` 只做短屏 slot 几何最小修正、补证并冻结；两项保护差异、旧 EARLY 图与 diagnostic 不得纳入。D-TEXT 入口 `/Users/echoman/.codex/worktrees/dentry/SelfAlone@deba325` 已迁入 11 个私有 book-detail / annotation 新文件并修改 reader / library / `main.ts`，并开始四宽 Chrome 取证；owner 文本 ACK 仍待补。
