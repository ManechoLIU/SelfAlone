...
- **Goal**：当前有终点里程碑为“Desktop Selection shared-seam 主线集成与非作者 EARLY 启动”；唯一项目总控任务仍为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`。本 Goal 只闭合候选审查、顺序集成、main 复验、台账同步与下一 READY ACK；在本提交和调度收据后结束，不等待独立 Mini verdict，也不吞并其余 backlog。
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@97ede67ec34120aee4eb634a1376d4426cc897a6` 开始，顺序集成后为 `main@940d3906bddf6c58235b56c99040d01c26911590`；主线 `AGENTS.md` 仍来自 `19d44285aed7aa70711514a3a81fa25da4070fda`（SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`），`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`，context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。
- **活动实现 / 复验线**：Desktop writer `01a03c67-e0b2-7de2-a458-c43192cc38e0` 已交付并释放；`6feede9 + 00e3ca7` 按序成为 main `9ff9877 + 940d390`。非作者 EARLY reviewer `01a03c7c-88d7-7402-bdc7-25c7176f759f` 已在 main@`940d390` 闭合 cwd / branch / HEAD / status、ref36、隔离端口 / schema、首个 RED 与停止条件 ACK，dispatch→ACK `86.1s`，现为 `ACTIVE`。Mini 非作者 FINAL `01a03c68-f9c0-7423-9717-7883e84622ac` 继续独立 `ACTIVE`；当前 controller 1 + reviewer 2，占用 `3 / 4`，可用 `1`，本波误中断 / clean-but-live 误判均为 `0`。
- **READY 重算**：Desktop main 非作者 EARLY 是本事件唯一新增 READY，已通过协作树内 `luna_worker` 派发并取得 ACK；Selection后续 FINAL / 修复依赖其 verdict，Mini后续修复依赖现有 FINAL verdict，当前无第三个无依赖 READY。剩余1槽保持可用，不因等待任一视觉结果阻塞另一端。
- **下一用户可见检查点**：Desktop reviewer 给出 main@`940d390` 同消息挂载、刷新、single / multi / free / stale、四宽、focus、console / overflow 的 EARLY PASS / FAIL；Mini reviewer独立给出 Settings / 长列表唯一 DevTools scoped FINAL。
- **真实阻塞**：Selection shared seam 已进入 main，代码 / writer H3 门已闭合；父项仍缺非作者 EARLY / FINAL，且真实AI生成问题不在本包，故保持 `VERIFY`。Mini旧截图红色DevTools计数、Library session bridge / transport、软件键盘与游客凭证各自保持原门；远端、发布与真实外部调用继续受授权限制。
- **当前可见入口 / revision**：main Selection代码现为 `940d390`；8个定向文件 / 42项、全仓typecheck、全仓build、最终diff-check通过，7个集成文件与候选tip逐字节一致，故候选隔离H3 Chrome收据对该代码快照仍有效。既有 Web `4173` / API `4100` 进程来源仍是旧 `6e9e4c4`，不作为当前 main 验收。
...
| `M1-F3B-C` | `VERIFY` · `MAIN MERGED 940d390 / SELF H3 + MAIN CODE PASS / NONAUTHOR EARLY ACTIVE` / 项目总控 + `01a03c7c-88d7-7402-bdc7-25c7176f759f` | 简单选择、显式确认、只读历史和失效问题的纵向切片；不含真实模型、图片或书籍上下文 | 私有链 `a08fbbc → 30e0423 → 6e21cad` 与 shared seam `9ff9877 → 940d390` 已在 main；当前依赖非作者 EARLY verdict，真实AI问题生成不在本包 | main Server `app.ts` / `index.ts` / seam test 与 Web chat / selection view / `main.ts` / seam test 共7个shared-seam文件 | writer隔离Chrome/API覆盖消息内挂载、刷新、single / multi / free / stale及1440 / 1200 / 1024 / 768、console / overflow；main 8文件 / 42项、typecheck / build / diff-check通过且7文件与候选tip同内容。ref36仅绑定通用会话外壳；非作者EARLY未返回前不标DONE |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `MAIN MERGED 35bad75 + 14b69dc / NONAUTHOR FINAL ACTIVE / H3 OPEN` / 项目总控 + `01a03c68-f9c0-7423-9717-7883e84622ac` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；本轮仅复验长列表与未就绪能力真实失败关闭，不冒充完整Mini闭环 | main与唯一`f312@db14fed`对应9源文件SHA SAME；只读 reviewer 使用既有f312入口，保护config / lock / QA不动，新证据只写visualizations | Reader / Conversation / Drawer既有范围保留；长列表 / Settings先给fresh compile、三宽 / 大字体、状态操作、console / overflow scoped FINAL；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F3` 小程序会话 | `VERIFY` · `MAIN MERGED 11f480a / SEND-RETRY SCOPED FINAL` / 项目总控 | 会话抽屉、文字 / 选择 / attachment-only、输入增长、失败保留 / retry与刷新恢复；非目标：真实AI / PPTX | 软件键盘、长会话列表、reduced-motion与真实API仍开放，不扩大父包PASS | `mini-conversation-send-early-854c577/`、`mini-main-final-31462d9/`、`mini-drawer-main-final-b98c118/` |
...
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `MAIN MERGED 11f480a / SEND-RETRY SCOPED FINAL` / 项目总控 | Reader、Conversation、Drawer及send / retry / attachment对应限定缺陷已关闭；不扩大为真实AI / API | 全Mini仍缺软件键盘、长列表与reduced-motion；游客clean-console另有凭证门 | f312保护现场不变；Mini source / dist与main等价 | 既有视觉证据 + `mini-conversation-send-early-854c577/`；下一步长列表独立EARLY |
...
| `M2-F0-A-MINI-SETTINGS-STATE` | `VERIFY` · `MAIN MERGED 14b69dc / NONAUTHOR FINAL ACTIVE` / `01a03c68-f9c0-7423-9717-7883e84622ac` | 移除工程台账文案，未实现能力显示真实“暂不可用 / 未配置 / 未连接”并失败关闭；不接真实账户、模型或微信读书 | main4源文件与`f312@db14fed`逐一SHA SAME；ref22 `ce28f938…f657288`；旧截图红色DevTools计数未闭合 | `pages/settings/index.ts` / `index.wxml` / `index.wxss` / `index.test.ts`；reviewer只读 | `git show --check 14b69dc`通过，main fresh Mini合计3文件 / 39项通过；唯一DevTools须复验390×844同态、fail-closed操作、fresh console / overflow，结果只可scoped PASS / FAIL |
| `M2-F3-MINI-CONVERSATION-LONG-LIST` | `VERIFY` · `MAIN MERGED 35bad75 / NONAUTHOR FINAL ACTIVE` / `01a03c68-f9c0-7423-9717-7883e84622ac` | developmentAdapter + `developmentLongList=1`确定性18项闭合抽屉长列表、搜索与末项露出；不冒充生产会话API，不处理软件键盘 / 凭证 / AI | main5源文件与`f312@db14fed`逐一SHA SAME；ref11 `9e764e…c36e`；旧矩阵可见但`5 errors / 8或11 warnings`未解释闭合 | `nav-drawer/index.test.ts` / `index.wxss`与`conversation/index.test.ts` / `index.ts` / `index.wxml`共5文件；reviewer只读 | `git show --check 35bad75`通过，main fresh Mini合计3文件 / 39项通过；唯一DevTools须复验390 / 360 / 320 × font16 / 23、首 / 中 / 末、固定底栏、搜索09、filtered-empty、fresh console / overflow |
...
| `M2-F0-A-H3-REWORK` | `BLOCKED` · `KEYBOARD / CREDENTIAL H3 UNCOVERED` / 项目总控（阻塞裁决） | H3已拆出不受外部门影响的长列表独立包并派发；本行只保留软件键盘与游客凭证化console，不冒充ACTIVE | 真实键盘 / 凭证环境未就绪；长列表由`M2-F3-MINI-CONVERSATION-LONG-LIST`执行，不再被本行阻塞 | f312唯一DevTools与外部环境证据 | 所有既有焦点图keyboard=0；凭证clean-console未产生。解除条件为真实键盘 / 可授权凭证环境 |
...
| `M2-F0-A` 当前候选 | `main@18f709d` 已含 `35bad75` 长列表与 `14b69dc` Settings；对应9源文件与唯一`f312@db14fed`逐一SHA SAME | 旧六矩阵 / Settings图证明状态可见但带`5 errors / 8或11 warnings`，不作为clean-console FINAL；非作者只读fresh compile / DevTools复验 ACTIVE。保护config / lock与既有QA不动；软件键盘、reduced-motion与PPT真实预览仍开放。 |
...
| 当前桌面候选波次 | Auth / Settings、D-TEXT、Conversation与Book Detail既有限定结论保持；Selection已完整集成至`main@a08fbbc → 30e0423 → 6e21cad → 9ff9877 → 940d390`，非作者EARLY ACTIVE | shared seam 7文件actual diff已审；main 8文件 / 42项、typecheck / build / diff-check通过且与候选tip同内容。writer H3收据保留，非作者task `01a03c7c-88d7-7402-bdc7-25c7176f759f`复验四宽 / 状态 / console；父项仍VERIFY。 |
| 当前 Mini 候选波次 | Reader / Conversation / Drawer既有范围保持；长列表`35bad75`与Settings`14b69dc`已在main；Library合同`20d61d6`因auth BLOCKED | main / f312对应9源文件SHA SAME；非作者任务从唯一f312做fresh FINAL，证据只写visualizations；旧红色console计数使当前状态保持VERIFY，不因提交名 / 截图自动DONE。 |
| 当前 Desktop main 预览 | `RUNNING / PROCESS PROVENANCE 6e9e4c4 / CURRENT MAIN 940d390 CODE VERIFIED` | Web `127.0.0.1:4173/#/auth` PID `29697`、API `127.0.0.1:4100` PID `29534`；schema `selfalone_main_preview_6e9e4c4_20260826`。旧进程不证明当前Selection；非作者使用独立schema / 端口复验main@940d390。 |
| 开发基线 | 本台账事件父为`main@97ede67ec34120aee4eb634a1376d4426cc897a6`，顺序集成后为`main@940d3906bddf6c58235b56c99040d01c26911590`；本轮仅再写`TASK_LEDGER.md` | 6个受保护DOCX / Word锁文件与3组受保护视觉候选目录仍与冻结a233副本同字节，禁止暂存 / 修改 / 删除；Mini reviewer现场不动。 |
...
| `M2-F0-A` 原生运行壳 | `VERIFY / MAIN MERGED 35bad75 + 14b69dc / NONAUTHOR FINAL ACTIVE / H3 OPEN` | Reader / Conversation / Drawer与send-retry既有限定结论保留；长列表与Settings已进main但旧console红计数未闭合，非作者fresh FINAL进行中；软件键盘、凭证化clean console与PPT真实预览仍开放，Mini父项不关闭 |
