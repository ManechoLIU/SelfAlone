...
- 当前 Goal：无；首个待授权 Goal 为 `M1-F2-A`，不得在用户明确要求开始开发前创建或激活。
- 当前阶段：桌面 Web 建设准备完成，等待开发授权；建设项默认使用功能级纵向闭环，验收时再按真实 Case 拆细。
- 主线：本地 `main` @ `2b639e0`；安全候选链为 `codex/m1-f2-preflight`，基于 `main` 并包含截至 `386185d` 的七个治理提交。脏 `main` 和其他工作树保持只读，合入 `main` 仍需单独授权。
- 当前执行波次：`M1-F2-WAIT-AUTH`。
- 当前活动项：无。
- 协调下一动作：等待用户明确要求开始开发；收到授权后，先重验 `codex/m1-f2-preflight` 的 HEAD / diff 和共享 PostgreSQL，再创建 `M1-F2-A` Goal、补齐负责人及文件所有权并改为 `ACTIVE`，不一次性启动整个模块。
...
| `M1-F2` 桌面书库、阅读与笔记 | `READY` | 本地书导入、阅读、记录、详情与账户隔离 | 等待开发授权后只启动 `M1-F2-A` |
...
| `M1-F2-A` | `READY` / 待分配 | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | R1、R2、M0 已满足；等待用户明确开发授权，文件格式或视觉规则冲突时暂停 | CORE、API、WORKER、DB、WEB、QA | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名异源、作者缺失、刷新、账户隔离；Web 4 视口对照书架参考 | 未产生；获授权后创建 Goal 并改为 `ACTIVE`，通过后释放 B、C |
...
当前没有 `ACTIVE` 工作包。下一张执行卡是 `M1-F2-A`，但只在用户明确要求开始开发后建立：先固定 `codex/m1-f2-preflight` 的 HEAD / diff，登记隔离 PostgreSQL schema、端口与产物目录，再补齐负责人、文件所有权、回退点和真实文件验收矩阵。未获授权前保持 `READY`，不实施功能。
...
| 开发候选基线 | `codex/m1-f2-preflight` | 基于 `main@2b639e0` 并包含截至 `386185d` 的治理链；脏 `main` 与其他工作树不作为开发目录。开始 `M1-F2-A` 前重验 HEAD / diff / 共享 PostgreSQL；合入 `main` 仍需授权。 |
...
| `M1-F2` 开发前收口 | `VERIFY` | PDF 唯一技术边界、`0204` 只读排除和具名安全候选链已形成；当前提交通过文档与 Git 审查后冻结，合入主线前保持 `VERIFY` |
