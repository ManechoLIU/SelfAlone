...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY / F312 cf6a8c5 / SELF PASS / NONAUTHOR QUEUED` | 累计实现为 `0fa1447 + cf6a8c5`：携带bookId / 书名，可编辑预填；发送前不auto-send / sheet / task；失败保留草稿与上下文 | 由 `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` 做非作者最终门；PASS 后只进入总控 current-main 集成 / 回归，不重写实现；证据 `mini-ppt-f312-self-cf6a8c5/` |
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` | `READY / NONAUTHOR READ-ONLY` / 待派 Kimi | 只读审累计 `0fa1447 + cf6a8c5` 的6文件 handoff 边界，对照 SPEC 143–145 与 TECHNICAL handoff 契约；检查 pre-send 无任务副作用、失败/刷新恢复、旧 context/stale fencing 与 current-main 漂移 | 禁止写文件、安装依赖或创建 reviewer worktree；输出 PASS/FAIL、精确阻断、定向测试与 current-main 集成风险。PASS 后由总控处理候选，不由 reviewer 集成 |
