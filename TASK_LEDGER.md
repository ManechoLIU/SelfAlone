...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `DISPATCHED / ACK PENDING` / `/root/auth_email_session` | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | 独立 `auth1@9764180`、branch `codex/m1-f1-a-auth`；`E-OWNER` 已以 `main@f1334ad` 冻结共享 owner 接缝，候选 `658ba50` 仅作种子。创建调用已返回，但未收到 cwd / status / 文件所有权 / RED / 停止门 ACK 前不升级 ACTIVE；账户迁移或其他共享 writer 冲突时暂停 | CORE、API、DB、WEB、QA；不得与 Mini、D-TEXT 或后续共享 schema / app 集成并写 | 密码摘要、Cookie、安全错误、刷新会话、跨账户隔离；绑定参考 01 的 4 视口、键盘和非作者 EARLY / FINAL | 本地假邮件与确定性会话无需外部凭证；真实邮件 / 微信停在授权门，通过后释放 B |
