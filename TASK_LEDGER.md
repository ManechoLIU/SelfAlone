...
- 当前 Goal：`CONTROL-RESET-1` 取得三个可共同检查的执行恢复点：`M1-WEB-SHELL-01` 修正候选、已完成的 `M1-F2-D-TEXT` 共享接缝 main 复验与下一 Web 入口释放判断、`M2-F0-A-SCREEN-ADAPT-ALL` 全端修正候选或真实阻塞裁决。该 Goal 不代表项目或任一模块完成。
- 当前阶段：`M1-F2-A` 只保留本地导入 / 书架功能 `DONE`，本轮独立审查已将视觉门重开为 `FAIL / REWORK`；`M1-F2-B` 仍为 `VERIFY / VISUAL REOPEN`，`M1-F2-D` 存在明确功能缺口。桌面对话 `M1-F3B-A～D / V1` 均未开发，当前 M0 / 演示页、fixture 或假适配器不得登记为 F3B 成果。M2-F0-A 已取得全端 `DESIGN-READY` 合同并进入六页 / 共享层系统返工；conversation / drawer 四尺寸视口与 Reader swipe lifecycle 各有独立冻结点，但完整视觉、状态矩阵与 H3 外部门均未通过。
...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：缺真实可编辑的阶段返回、刷新丢未提交大纲、需求摘要状态错误、模板未按双列、生成中缺当前页 16:9 骨架、失败态缺真实重新生成 / 修改大纲，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，Reader 私有标注 / 笔记候选已在隔离工作树开始 TDD，共享路由仍待 Web shell 释放；小程序候选链已冻结 `4fc422f → 630c051 → 5e270b1` 三个局部恢复点，但整体仍为六页与共享层 `FAIL / SYSTEM REWORK`，f312 继续独占 `apps/miniapp/**`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 在 `6afd@0e18846` 继续独占本包 Web 文件，并已获准在确需真实阶段回退保存时顺序修改 M0 runtime / route 与测试；原非作者 UI/UX 任务当前 idle，待新冻结 HEAD 后重验。`/root/reading_text_notes_web` 已在 `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@996d8b1` / `codex/m1-f2-d-text-web` 进入 `ACTIVE`，首批写入仅为 `text-reader.test.ts` 与新增 book-detail / text-annotation 私有测试 / 状态文件，尚无通过结论。小程序任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 保持 `ACTIVE`：conversation / drawer、Reader swipe、login / library / settings 三个检查点已冻结，当前连续推进 Reader 控制层 / 底部面板与 PPT 高风险布局；局部资产门和 f312 两个既有禁止差异保持。
- 协调下一动作：Web 作者关闭真实阶段返回 / 持久化、需求摘要、模板、生成骨架与失败动作后冻结新 HEAD，由原非作者按 02 / 04 / 05 / 06 / 33 逐阶段复验；通过后总控集成 / main 复验并顺序接入 D-TEXT 共享路由。D-TEXT 继续范围内 TDD、真实 API / Chrome 候选，未冻结前不触碰共享入口。小程序继续 Reader / PPT 与其余状态矩阵；冻结全端候选后再按四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 矩阵终验。
...
| `M1-F2-D-TEXT` | `ACTIVE / PRIVATE WEB TDD / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | `dtxtui@996d8b1` / `codex/m1-f2-d-text-web` 已真实存在且从 clean 基线开始；首批 diff 仅 `text-reader.test.ts` 与新增 book-detail / text-annotation 私有测试 / 状态文件，尚无 green 或运行结论 | 私有 writer 只写 `text-reader-*` 与新增 annotation / book-detail 模块；`main.ts` / shared shell / styles 等 Web shell 合入后由总控顺序接入；旧 `18c7522` 只读 | 真实 EPUB / TXT 选区→划线 / 想法 / 老己笔记 CRUD，失败保留、刷新恢复、owner / fileVersion 隔离、返回原文与绑定参考真实 Chrome 通过 | 共享接缝合并 `f85d86c`；当前只证明 TDD 已开始，不能称用户可见功能已实现 |
...
| `M2-UX-CONTRACT-ALL` | `DONE / DESIGN-READY BRIEF / IDLE-FINAL` / `01a03652-a5d4-76c1-b0fa-ff9303243664` | 对 `f312@d90d0fb` 的实际六路由、`nav-drawer`、底部面板和键盘层完成一次完整只读合同审查；不写实现 | Stable `2.02.2608040`、基础库 `3.17.1`、touristappid；接受 390×844 / 320×568 / 430×932 新鲜证据并明确 360、键盘实显、完整五态与冻结候选矩阵仍待终验 | 逐页采用点 / 排除项、派生、Token / 几何 / 资产、状态 / 视口 / 安全区 / 键盘、owner、Case 与 covered / uncovered 已回传；过期“五背景”结论已撤销，主线 `b45a7a8` 保持浅 / 深两态并同步新增合同。待冻结候选后复用原任务做最终复验 |
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1` 已形成三个范围单一恢复点：conversation / drawer 四尺寸视口、Reader swipe lifecycle，以及 login / library / settings 的 live viewport / safe-area 基线。第三包只含 20 个 `apps/miniapp/**` 文件与 7 张 320 / 430 DevTools 图，typecheck、8 文件 / 45 项、build `<2 MiB`、diff-check 通过；320 书架两列 / 设置可滚到底 / 登录长邮箱上下文和 430 书架三列 / 设置全项 / 登录协议安全区成立。均不是最终视觉通过 | 当前连续推进 Reader 控制层 / 底部面板，再进入 PPT 高风险布局。游客 console、真实软件键盘、系统字体、透明山亭及真实 PPT 预览继续阻塞对应门，锁文件与 project config 既有差异禁止触碰 |
...
| `M1-F2-D-TEXT` | `ACTIVE / PRIVATE WEB TDD / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@996d8b1`、`codex/m1-f2-d-text-web`；只写 Reader 私有文件与新增 text-annotation / book-detail 模块，不碰 6afd 的 `main.ts` / shared shell / styles 或共享 contracts / Server | 首批测试 / 状态文件已出现但尚无 green；继续真实选区三动作、划线 / 想法 / 无标题老己笔记 CRUD、失败保留与真实 API / Chrome 候选。全局路由在 Web shell 合入后由总控顺序接入 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / ACTIVE` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 固定失败候选 `6afd@0e18846`；作者独占本包 Web 文件，并在确需真实回退保存时顺序拥有 M0 runtime / route 与测试；审查者只读 | 已通过单一右侧工作区、中栏无完整阶段页、唯一 current step 与无横溢；仍须关闭真实可编辑返回 / 刷新草稿、需求摘要状态、模板双列、生成中 16:9 骨架和失败态重新生成 / 修改大纲。新 HEAD 冻结前禁止合并 |
| `M2-UX-CONTRACT-ALL` | `DONE / DESIGN-READY BRIEF` / `01a03652-a5d4-76c1-b0fa-ff9303243664` | 已只读覆盖 `f312@d90d0fb` 六页、抽屉和面板 / 键盘态；不与 `f312` 抢写 | 逐包卡与完整 covered / uncovered 已接受，过期“五背景”已撤销；主线事实源提交 `b45a7a8`。后续仅在冻结候选返回时复用该任务做 FINAL-VISUAL-VERIFY |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / THREE CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f` 冻结 conversation / drawer 四尺寸，`630c051` 冻结 Reader swipe lifecycle，`5e270b1` 冻结 login / library / settings 320 / 430 live viewport / safe-area 基线；第三包 8 文件 / 45 项、typecheck / build / diff-check 与 7 张 DevTools 图通过。当前推进 Reader 控制层 / 面板与 PPT；全路由 / 状态矩阵前不合并 |
| `M2-F0-A-H3-REWORK` | `ACTIVE / KEYBOARD H3 UNCOVERED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 与系统适配共用同一 f312 writer，先做不涉及视觉裁决的 H3；既有 `project.config.json` / 锁文件禁止触碰 | `4fc422f` 证明四宽视口、安全区和键盘覆盖不重复扣减；`630c051` 增加 Reader 回顶 / 重进语义，`5e270b1` 把同一 live viewport 合同接入 login / library / settings。所有焦点图仍 keyboard=0，软件键盘 / 字体、游客 console 与凭证化环境仍为真实外部门，H3 总门不降级 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 三个新检查点分别冻结 conversation / drawer 四尺寸、Reader swipe lifecycle、login / library / settings 的 320 / 430 布局基线。Reader 控制层 / 面板、PPT、其余状态 / 键盘 / 字体矩阵与最终视觉仍未完成；既有锁文件 / project config SHA 保持且未进入提交。 |
...
| `M1-F2-D-TEXT` 共享集成 | `263d3e2 → 18c7522` 私有种子；`94056be → ed95d95 → 5156b9c → main@f85d86c` | 共享 contracts / migration / API / publisher 已通过原非作者与 main 复验；私有种子不再单独合并。`dtxtui@996d8b1` 私有 Web TDD 已启动，shared entry 继续等待 Web shell 所有权释放。 |
| 开发基线 | 产品集成恢复点 `main@f85d86c`；当前控制面续接于最新 main HEAD | A 功能闭合但视觉重开；B / C 共享接缝和 C 安全首包已进入 main，D-TEXT 共享接缝已合并但 Web 闭环未实现；Web shell / miniapp 候选未集成。主线两个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1` 分别是 conversation / drawer 四尺寸、Reader swipe lifecycle、login / library / settings 320 / 430 布局基线的已提交恢复点；全小程序六页、共享层的真实四宽四高和状态矩阵、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
