...
- 下一 `READY` / 协调动作：当前 `0` 个空闲槽，不新增重复 writer；首个槽位释放后按已到达事件重算，认证出现首个可运行 UI 时优先释放非作者 EARLY reviewer，Mini EARLY `FAIL` 时释放对应唯一 writer，D-TEXT FINAL `FAIL` 时释放最小修复 writer，任一 `PASS` 则先回收候选并在本地 `main` 顺序集成、复验后再关闭该卡。等待认证或 D-TEXT exact-contract ACK 只做当前已发生的一次有界等待，无 ACK 时不再轮询。Mini 通过 EARLY、冻结新候选后才进入 FINAL；本轮不提前把 Mini 候选合入 main。
