...
- 下一可见检查点：`M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT-REVIEW` 已由新的 Kimi K3 非作者 reviewer 在 clean candidate `9132809` 上 delivered ACK 后 ACTIVE；当前只等待其只读 verdict。PASS 后总控仅顺序集成 `9132809` 到 current main 并做 main 回归/真实端验收；FAIL 则原包 RECOVERING。并行 `M1-F4-B` 继续 BLOCKED；每次状态变化继续项目级重算 READY。
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-MAIN-ADAPT-REVIEW` | `ACTIVE / NONAUTHOR READ-ONLY / Kimi K3 / DELIVERED ACK 9132809` / `kimi-k3-m2f6-ppt-main-review` | 只读审 candidate `9132809` 相对 parent `e088bbd` 的6文件实际 diff；ACK 在任何 repo tool 前送达，候选 worktree clean、observed modified files=0；对照 SPEC 143–145 / TECH handoff 检查 current-main adaptation | 当前动作=运行 targeted65、Mini全量、typecheck、diff-check并做语义审查；禁止写文件/安装/改 refs/config/lock/创建 worktree。输出 PASS/FAIL 后由总控接管候选，reviewer 不集成 |
