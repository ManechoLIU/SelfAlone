...
- 当前执行波次：`CONTROL-RESET-1`；Desktop `6afd@44e66af` 已获非作者 `PASS`，完整 `0969c33..44e66af` 已以 merge commit `main@deba325` 集成并完成在线 / 断网恢复真实 Chrome 复验。Mini drawer / Reader 从 `f312@a0e98f9` 继续唯一 writer 返工；D-TEXT Web 入口已冻结为 `dentry@48e5f66` 并取得 owner 完整 ACK，独立非作者 `/root/verify_dtext_entry` 正在 FINAL。台账仍由项目总控独占。
- 当前活动项：Mini 短屏视觉修正已冻结为 `f312@2deb4f6`；非作者已判 Reader、四宽抽屉、font23 / safe-area 与 `IMG-04` 构图 / Alpha `PASS`，但新增 1024×737 truecolor RGBA runtime 使 Mini dist 达 2236 KiB，超过既有 `<2 MiB` 门，故完整候选仍 `FAIL / SIZE REWORK`。同一 Mini writer 正按已批准的 indexed PNG + `PLTE` / `tRNS` 确定性方案返工；两项保护差异、旧 EARLY 图与 diagnostic 未纳入。D-TEXT `48e5f66` 为 26 files clean candidate，真实四宽 / TXT / EPUB / CRUD / 409 收据已齐；200% 保持 VERIFY，chat / PPT 入口 fail-closed，独立非作者 FINAL 正在运行。
- 协调下一动作：Mini writer 冻结体积修复新 HEAD 后，唤醒同一只读 reviewer 重做四宽 / font23 / Alpha / 包体 FINAL；PASS 后总控审查完整 `ff4767b..新 HEAD` 并处理最小 lock importer。D-TEXT `48e5f66` 收到独立 verdict 后由总控决定最小返工或顺序集成当前统一壳。保护差异与 H3 阻塞不变；不得只摘 Mini 末提交。
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / FROZEN 48e5f66 / NONAUTHOR FINAL ACTIVE` / `/root/reading_text_notes_web` + `/root/verify_dtext_entry` | `/Users/echoman/.codex/worktrees/dentry/SelfAlone`、`codex/m1-f2-d-text-entry@48e5f66` clean；26 files 仅 `main.ts` / library 入口、私有 reader / annotation / book-detail、测试与 acceptance；不改 server / contracts / schema / lock / Mini / 台账 | 作者定向 47 / 47、全仓 32 files / 179 tests、typecheck / build / diff-check；真实 TXT / EPUB、四入口、划线 / 想法 / 笔记 CRUD、409 保留重试、1440 / 1200 / 1024 / 768@DPR2、reduced-motion、rail / inert / overflow / console 通过。原生200%仍 VERIFY，chat / PPT fail-closed；非作者 PASS 前不集成 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY / FROZEN 2deb4f6 / SIZE REWORK ACTIVE / H3 BLOCKED / NOT IN MAIN` / `/root/mini_visual_rework_fix` | 11-file scoped commit 已关闭 Reader controls 省略与 320 scenery 几何；非作者已通过 Reader、四宽抽屉、font23 / safe-area 与 IMG-04 视觉 / Alpha。保护 project config / lock 与两张旧 EARLY 图未纳入 | 定向 13 files / 68 tests、typecheck / diff-check 通过；build dist 2236 KiB，未过 `<2 MiB`，不得集成。同一 writer 正做 indexed PNG + `PLTE` / `tRNS` 体积修复；冻结后必须重新 build、真实 DevTools 与非作者 FINAL。软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY / IMG-04 VISUAL+ALPHA PASS / RUNTIME SIZE REWORK ACTIVE` / `/root/mini_visual_rework_fix`；FINAL=`/root/mini_visual_final_review` | runtime `1024×737 RGBA`、SHA `875ebae56229a657a505d1071e76000078fd1250b4c0894b3f71ca7bb43d2e50`、无实底；320 / 360 / 390 / 430 实装图已通过，短屏 slot=100px。原 reviewer 已回收，等待同一 writer 的体积修复新 HEAD 后事件触发 FINAL；不新增图片生成调用 | indexed PNG 优化后仍须真实 Alpha、参考11构图 / 色彩 / 渐隐等价、四宽 / font23 / safe-area / 命中通过并使 build `<2 MiB`。真实多会话长列表、软件键盘与 PPT preview 继续精确保留 UNCOVERED / H3，不因静态或开发态数据升级 |
...
| `M2-F0-A` 当前候选 | `ff4767b → … → a0e98f9 → 2deb4f6`，状态 `VERIFY / VISUAL FROZEN / SIZE FAIL / H3 BLOCKED / NOT IN MAIN` | Reader controls、IMG-04 runtime 与四宽证据已冻结；新增 PNG 使 dist=2236 KiB，必须先按 UI/UX 合同做确定性体积优化并重验，不得只摘末提交。完整链仍不含 lock importer；保护 project config / lock 不得带入，软件键盘、PPT 资产 / 真实预览仍缺。 |
...
| `IMG-04` | `VISUAL+ALPHA PASS / RUNTIME SIZE REWORK ACTIVE` | OpenAI 内置 `image_gen`；具体模型 ID 未暴露 | 1 张，`1024×1536`；按参考生成小程序 drawer 底部低对比山亭 | 白底源 `redesign-v2/output/ui-design-preview/product-design/mobile-drawer/candidates/v1/mobile-drawer-landscape-white-bg-v1-candidate.png`，SHA-256 `37cbfb304a7b6a3d5f8c2013f84774c0e3339ab5474adb49e3de1993d7edb3ba`；透明裁切候选 `1024×737 RGBA`、SHA-256 `55a655177c84202c255786fefdb4752e2cb575ffcf6c11f6540ae51aa48e2a38`；水平镜像 runtime SHA-256 `875ebae56229a657a505d1071e76000078fd1250b4c0894b3f71ca7bb43d2e50` 已获非作者四宽视觉与 Alpha PASS，但 truecolor 版本导致 Mini 包超限，正做不改变构图的 indexed PNG 体积返工；无新增图片生成调用 | 已实际调用 `4 / 20`，剩余 `16` |
