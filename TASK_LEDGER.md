...
- 当前 Goal：`M1-F2-B` EPUB / TXT 桌面阅读闭环收口；`535b9ec` 的专注 toolbar 修正通过，但非作者复验发现移出屏幕的全局左栏仍进入 Tab 与可访问树，当前由原 B 实施任务做唯一 rail follow-up，之后由同一 UI/UX 任务复验。M2-F0-A 保持未合并 idle，不与 B 争用文件。
- 当前阶段：桌面 Web `M1-F2-A` 已在 main `DONE`；`M1-F2-B` 候选链 `04f03d2 → 47748fa → 535b9ec` 的代码门已通过，但 UI/UX 复验仍为 `FAIL`，不得集成。唯一 P1 是专注态全局导航未退出焦点与可访问树；原生 200% 仍是独立视觉 `DONE` 门。B 恢复 `ACTIVE` 做范围内修正，M2-F0-A H3 继续 `BLOCKED`。
...
- 当前执行波次：`M1-F2-B-FOCUS-RAIL-FIX`；A 已回收，M2-F0-A 未合并候选保持 idle，C 安全首包已完成集成并停止原任务。
- 当前活动项：实现任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 已从 `da89@535b9ec` 恢复，只写 `text-reader.css`、`text-reader.test.ts` 与必要的 B acceptance，使专注态 rail 退出视觉、命中、Tab 与可访问树并保持原地模式切换。审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 已完成 FAIL 报告并回到 idle，等待新 HEAD；M2-F0-A 在 `f312@91934f1` idle。
- 协调下一动作：B 追加最小 TDD 提交并一次性审计专注态完整焦点集合，证明 rail 及后代不在 accessibility snapshot、Tab / Shift+Tab 与命中路径，Esc 后全局导航、同一 Selection / Range、复制 / 对话、scrollTop 和焦点恢复；随后原 UI/UX 任务复验新 revision。复验通过前不得集成，原生 200% 未取得前不得标视觉 `DONE`。
...
| `M1-F2-B` | `ACTIVE / VERIFY` / 实施任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` | EPUB / TXT 正文、目录、复制、全书位置、专注模式和浅 / 深阅读背景的桌面阅读闭环 | `535b9ec` toolbar 修正通过但非作者复验仍 `FAIL`；rail 仅平移出屏仍可聚焦 / 可访问，修复前不集成。原生 200% 继续阻塞视觉 `DONE` | `da89` / `codex/m1-f2-b` 仅写 `text-reader.css`、`text-reader.test.ts` 与必要的 B acceptance；共享入口 / 样式、contracts、schema、锁文件、事实源和台账不得写 | 专注浅 / 深各验证 rail 的视觉 / 命中 / Tab / accessibility 隐藏与 Esc 恢复；并完整审计专注可聚焦集合、Selection / Range、scrollTop、console / overflow 和全仓回归 | 以 `535b9ec` 为恢复点追加提交；新 HEAD 交原 UI/UX 任务复验，不重跑已通过的无关状态 |
| `M1-F2-B-UX1` | `FAIL / 等待复验` / 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` idle | 已完成浅 / 深阅读私有侧栏、无固定桌宠、首帧、焦点、滚动、TXT、复制反馈与完整专注可聚焦集合的非作者合同；不实现 | `535b9ec` 唯一 P1 为屏外 rail 仍在可访问树与 Tab 序列；PDF 页面一致性只锁视觉合同、不冒充当前实现，原生 200% 缺口继续显式保留 | 审查任务保持只读；事实源由总控以 `8f40658` 同步，修正只归 B 当前路径所有者 | toolbar 四类隐藏、三个 44px 控件、Selection / Range、Esc 恢复、浅深层次、无桌宠、overflow、console 与全仓验证均通过；新候选只聚焦 rail 及完整焦点集合 | 报告 `/tmp/selfalone-b-ux1-focus-reaudit-20260825-Z7butU/REPORT.md` SHA-256 `896823be…5486e`；审查已回到 idle |
...
当前唯一实现写入卡为 B 专注 rail 修正：任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 从 `da89@535b9ec` 只写 `apps/web/src/text-reader.css`、`apps/web/src/text-reader.test.ts` 与必要的 B acceptance，不重建正文 DOM，不改共享入口、contracts、schema、锁文件、事实源或台账。原 UI/UX 任务 idle 等待新 HEAD；M2-F0-A `f312@91934f1` idle，C 原任务已停止。
...
| `M1-F2-B` 当前候选 | `44b81ea → 04f03d2 → 47748fa → 535b9ec`，状态 `ACTIVE / VERIFY / UIUX FAIL` | `535b9ec` 的 toolbar 修正已通过；唯一 P1 是 rail 只用 transform 出屏，品牌 / 对话 / 读书仍进入 accessibility snapshot 与 Tab。实现任务只做 rail 隐藏语义、测试与必要证据的最小 follow-up；原生 200% 继续阻塞视觉 `DONE`。 |
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1`，状态 `VERIFY / H3 BLOCKED / 不合并` | follow-up 20 文件、`+150/-37` 且仅 `apps/miniapp/**`；浅 / 暗两态、DevelopmentClient 运行内恢复、失败保留、filtered-empty 与 `libVersion 3.17.1` 已通过 25 项定向测试、根 48 项验证和独立 UI/UX 代码 / 证据复核。工作树另有预先存在的未暂存 `pnpm-lock.yaml` importer 差异，SHA 未变且候选未吸收；`design-qa.md` 把 RGB 头像误称 RGBA，后续 H3 包一并纠正。游客 console、真实失败态、最终宽高矩阵、键盘 / 字体和批准小程序山水资产仍阻塞 H3。 |
...
| 开发基线 | 本地 `main` 产品代码 @ `a306149`，台账治理父级 @ `f51d54e` | A 已完整闭合；`44b81ea` 冻结 B / C 接缝，`589a22b` 纳入 C 安全首包，`8f40658` 固定 B 阅读私有外壳与交互规则，`a306149` 释放 B 实施。B 候选 `535b9ec` 仍在 `da89` 做 rail follow-up，M2 H3 保持阻塞；主线未跟踪视觉候选不得纳入台账提交。 |
...
| `M1-F2-B-UX1` 阅读视觉 / 交互 brief | `FAIL / 等待复验` | `535b9ec` 的 toolbar 修正通过，但屏外 rail 仍在 Tab / accessibility；报告已登记，B 修正后由原审查任务聚焦完整焦点集合，原生 200% 仍为视觉 `DONE` 门 |
