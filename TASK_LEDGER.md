...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复、北京时间双时段计费、真实 Store 并发硬上限与失败后重试本地门已分别在 `main@b637023+81f5f1f+df32890+ba70ced` 闭合。真实 Provider H4 继续等待三项外部条件；项目级旁路 `M1-F5-A1` 虽已被并发事件提前集成到 `main@a220c2f`，但完整非作者审查随后确认 legacy requirements 回归、换书后幂等指纹漂移和读取快照撕裂三项 Important，现已重新打开；先在隔离修复分支完成同案 RED / GREEN 与复审，再恢复 Web 入口工作。
