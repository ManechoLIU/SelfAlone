...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。Desktop Conversation已在`main@dea90d7`完成真实Case FINAL PASS；Book Detail修复链已以`main@0cc5d46`完成EARLY与main真实Chrome scoped FINAL。F3B-C Selection私有候选`csel1@655b51f`已冻结并进入短时非作者实际diff审查；D-TEXT handoff已从clean `dpth@7317274`重新派发私有adapter writer，当前ACK PENDING，未冒充ACTIVE。Mini长列表`mlong@a9da90a → f312@5878400`代码门PASS，真实DevTools EARLY因Mac锁屏暂停。当前构成：controller / integrator 1、implementation writer 0 ACTIVE + 1 DISPATCHED ACK PENDING、code reviewer 1 ACTIVE。
- **下一用户可见检查点**：Selection实际diff审查PASS后顺序接入shared seam并开真实Chrome EARLY；D-TEXT writer完成preflight / RED后交不触碰`main.ts`的私有候选；Mac解锁后Mini长列表立即在唯一f312完成三宽 / 大字体真实EARLY。任一FAIL回原范围最小整改。
- **真实阻塞**：Mini Library真实runtime仍受M2-F1 session bridge与DevTools transport阻塞，`20d61d6`只保存未接线合同。Mini长列表代码门已闭合，但微信DevTools当前因Mac锁屏无法读取，解除条件是本机解锁后续跑唯一f312；不反向阻塞桌面writer。Selection shared seam等待当前非作者代码 verdict；D-TEXT shared `main.ts`接缝在Selection冻结后顺序集成，但私有adapter不等待。Mini父项另缺软件键盘、游客凭证化clean console与真实PPT preview。
- **当前可见入口 / revision**：旧稳定Desktop仍是`4127 / 4198@ab13310`，不得误认当前进度；当前Book Detail main预览为`5330 / 5331@0cc5d46`，schema `selfalone_book_detail_early_c91b691_20260826`、runtime `/tmp/selfalone-book-detail-main-0cc5d46/`，真实Chrome scoped FINAL PASS；旧候选`5320 / 5321@c91b691`仍可访问但不代表main。D-TEXT `5198 / 4318`已关闭，保留既有schema / runtime。仓库`main@0cc5d46`；唯一DevTools候选为`f312@5878400`，保护config / lock与两张QA图未变，长列表尚未合入main。
...
| `M1-F3B-C` | `VERIFY` · `CANDIDATE 655b51f / CODE REVIEW ACTIVE` / 项目总控 | 简单选择、显式确认、只读历史和失效问题的私有纵向切片；不含真实模型、图片、书籍上下文或shared UI seam | 17文件私有候选clean；78 files / 434 tests、typecheck / build / diff-check由writer通过，但shared seam与真实页面尚未产生；非作者正在核对幂等、刷新草稿与失效确认语义 | `csel1@655b51f`仅新增Server/Web `conversation-selection*` 17文件；shared `app.ts` / `index.ts` / `main.ts` / `styles.css`仍冻结 | ref36 SHA `1b6ec630…a5796b40`；下一步按实际diff verdict返修或顺序接线，再走1440 / 1024 / 1200 / 768真实EARLY |
...
| `M1-F2-D-TEXT-PPT-HANDOFF-FIX` | `READY` · `DISPATCHED / ACK PENDING / DPTH 7317274 CLEAN` / `/root/dtext_ppt_private_fix` | 当前书 PPT intent必须驱动对应workspace或明确fail-closed；恢复滚动时banner不得被sticky reader header遮挡 | Selection shared seam正在审查；本轮writer只做不触碰`main.ts`的私有adapter，不等待shared接缝。未回cwd / branch / status / RED前不得标ACTIVE | 唯一owned为新建`apps/web/src/text-reader-ppt-handoff.ts` / `.test.ts`；`main.ts` / 现有CSS / Book / Selection均冻结 | 下一步：writer回精确preflight即转ACTIVE并完成私有candidate；Selection冻结后由总控顺序接线，再跑当前书 / 固定seed、1440 / 768、banner、console / overflow |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader scoped FINAL；Conversation已至`main@dea90d7` FINAL；Book Detail已至`main@0cc5d46` scoped FINAL；Selection `655b51f` code review中；D-TEXT handoff私有writer已派发 | Selection按actual diff verdict返修或接线；D-TEXT私有adapter与其并行，shared `main.ts`顺序集成。旧`bdport`WIP只读保留；当前main预览为5330 / 5331。 |
...
| 开发基线 | 产品顺序集成到 `main@0cc5d46`；Desktop Auth / Settings、D-TEXT reader、Conversation与Book Detail限定范围PASS，Mini Reader / Drawer与send-retry限定范围PASS | main仅允许本次台账事件diff与三个受保护未跟踪视觉目录；Selection、D-TEXT handoff、Mini长列表与Library合同仍未完成，不能自动 `DONE`。 |
