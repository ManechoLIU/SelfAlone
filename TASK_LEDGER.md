...
- 当前 Goal：项目总控任务以 Goal 收口 `M1-F2-A` 候选验收、范围提交、本地 `main` 集成与复验这一独立里程碑；实现任务内旧 `blocked` Goal 不作为项目状态来源，执行仍以本台账为准。
- 当前阶段：桌面 Web `M1-F2-A` 候选保持 `VERIFY`，尚未合入 `main`；范围回归、真实长标题、键盘焦点、`1440×844` 视觉与 200% 重排已由项目总控和非作者审查通过，只剩 `1440×1024` 当前构图证据与范围提交。小程序客户端基础包已从 `main@ff4767b` 独立释放，不再整体等待 `M1-V1`。
- 当前拆分：M1 保持功能闭环粒度，A 合入并复验后先由项目总控冻结 B / C 共用的阅读定位、版本、状态、API 与 schema，再以互斥模块和隔离工作树并行释放文本与 PDF 阅读；M2 已把候选复用、原生运行壳、书架接入、连续阅读接入和最终双端验收按真实依赖展开，M3 仍保持模块级父项。
...
- 当前执行波次：`M1-F2-A-VISUAL-VERIFY` 与 `M2-F0-MINIAPP-SHELL` 并行；两者文件范围互斥。
- 当前活动项：`M1-F2-A` 由 Codex 任务 `01a0339a-3299-7150-91d9-f95a5331c627` 在 `47b4` 独占候选代码；`M2-F0-A` 由 Codex 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 从 `ff4767b` 开始，分支固定为 `codex/m2-miniapp-shell`，唯一写入 `apps/miniapp/**`。两项均不得写 `TASK_LEDGER.md`。
- 协调下一动作：A 只补明确标注为“构图检查、非真实 Chrome”的当前 `1440×1024` 同态证据并创建范围提交，随后由项目总控合入并复验 `main`；小程序任务以 `42d85fe → 925bbc1` 为只读种子重建当前视觉和自然连续滚动运行壳，用显式开发适配器隔离尚未进入主线的真实 API，并完成 360 / 390 / 430 与微信开发者工具 H3。A 合入后再接真实书架 / 导入，B / C / D / E 后接正文、笔记与跨端恢复，F1 后接身份，F3 / F4 后接会话与额度，F5 后接真实 PPTX。
...
| `M1-F2-A` | `VERIFY` / 任务 `01a0339a-3299-7150-91d9-f95a5331c627` | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 候选 `codex/m1-f2-a` @ `353b4fd` 尚未合入主线且有未提交改动；只缺当前 `1440×1024` 构图补证与范围提交；真实该高度受物理屏幕限制，不得伪造 | CORE、API、WORKER、DB、WEB、QA；实现任务拥有候选代码 / 证据，台账文件只由项目总控写 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名异源、作者缺失、刷新、账户隔离；Web 4 视口对照书架参考 | 总控独立复验定向 16 / 16 与 `pnpm verify`（13 文件 / 50 项、类型检查、构建）通过；22 号真实 `1440×844` 经非作者审查视觉 `PASS`，23 号真实 200% 为 3 列且书架与桌宠保留 6px 安全间距；整体仍为 `VERIFY` |
| `M1-F2-BC-S1` | `PENDING` / 项目总控 | 冻结文本 / PDF 共用的文件版本、内容状态、稳定定位、阅读位置、API、schema 与模块接缝；不实现阅读 UI | A 合入并由总控复验 `main`；生产解析 / 渲染依赖或安全边界需要新授权时暂停 | CONTRACT、DB、API 接缝；`contracts`、迁移、`app.ts` / `index.ts`、共享 Web 路由与样式只由总控写 | JSON 兼容类型、复合 owner 外键、条件更新 / `STALE_VERSION`、迁移回退、文本与 PDF 定位互不混用 | 通过后在隔离 worktree 并行释放 B / C；每项独占新增 `text-reader*` 或 `pdf-reader*` 模块 |
| `M1-F2-B` | `PENDING` / 下一波独立实现任务 | EPUB / TXT 正文、目录、复制、全书位置、专注模式和阅读背景的桌面阅读闭环 | A、BC-S1；解析定位不稳定或生产解析依赖未获授权时暂停；可与 C 并行 | 独占 `text-reader*` 的 DOMAIN、SERVER、WORKER、WEB、QA；不写共享入口 / schema | 真实 EPUB / TXT；章节顺序、按需目录、唯一进度、刷新恢复、浅 / 深背景、键盘、五类状态和 Web 4 视口 | A 合入复验且 BC-S1 冻结后创建独立任务；通过后释放 D |
| `M1-F2-C` | `PENDING` / 下一波独立实现任务 | 文本型 / 图片型 PDF 页面阅读、异常说明和独立笔记闭环；不 OCR、不伪造正文 | A、R2、BC-S1；必须先明确生产 PDF 渲染器、安全 / 缓存边界和真实样本；可与 B 并行 | 独占 `pdf-reader*`、PDF Worker / 缓存、WEB、QA；不写共享入口 / schema | 真实文本 / 扫描 / 加密 / 损坏 / 单页失败 PDF；页级恢复、重试、独立笔记、重启与旧版本防回写 | 当前 `ready_pages` 仅为检测摘要，不是页面渲染；满足释放门后创建独立任务，通过后释放 D |
...
| --- | --- | --- | --- | --- |
| `M2-R0` 候选复用审计 | `VERIFY` / 项目总控 + 只读审计 | 以 `42d85fe → 925bbc1` 为代码种子，按 `main@ff4767b` 的连续滚动和当前视觉逐模块裁决复用 / 重做 | 只读候选 `6f5c@925bbc1`，不继承其工作树状态 | 旧“连续滚动应废弃”结论失效；保留 HEAD / status、19 项测试和主线缺口证据，当前复验后闭合 |
| `M2-F0-A` 原生运行壳与客户端状态 | `ACTIVE` / 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` | 原生小程序运行壳、登录 / 书架 / 连续阅读 / PPT 状态边界、正式资产与五类状态；未就绪服务走显式开发适配器 | `f312`，`codex/m2-miniapp-shell` @ `ff4767b`；唯一写 `apps/miniapp/**`，锁文件和共享契约交总控 | 360 / 390 / 430、自然连续滚动、键盘 / 触控 / 安全区、微信开发者工具 H3；不得称真实账户或跨端闭环 |
| `M2-F4-A` 真实书架 / 导入接入 | `PENDING` / 后续小程序任务 | 将运行壳接入真实书籍摘要、搜索、导入与解析状态 | A 合入并稳定书架契约；共享 contracts / lockfile 由总控集成 | 真实 EPUB / TXT / PDF、五类状态、开发者工具与账户隔离 |
| `M2-F4-B` 连续正文 / 笔记 / 恢复接入 | `PENDING` / 后续小程序任务 | 自然连续纵向滚动正文、目录 / 设置 / 操作层、笔记和跨端位置恢复 | B / C / D / E 分别提供正文、PDF、笔记与 owner / 恢复契约；不强制分页或整页吸附 | 真实内容、刷新 / 重开恢复、动态字号无跳位、开发者工具与真机 |
...
| `M2-F4` 小程序书架与阅读 | `ACTIVE` | 统一书架、导入、介绍首屏、自然连续纵向滚动正文、操作层、书籍内容与跨端恢复 | 客户端运行壳先行；真实书架依赖 A，正文 / 笔记 / 恢复依赖 B～E，身份依赖 M2-F1，微信读书数据依赖 M2-F2 | 先验收 F0-A 的 360 / 390 / 430 与 H3；完整门覆盖真实导入、连续滚动、内容面板、DB 与真机 |
...
当前有两个互不重叠的执行卡。`M1-F2-A` 由任务 `01a0339a-3299-7150-91d9-f95a5331c627` 在 `47b4` 收口：22 号真实 `1440×844` 与 23 号真实 200% 已过功能、可访问性和非作者视觉门；物理屏幕无法提供真实 `1440×1024`，按 `DESIGN-WEB` 只补明确标注“构图检查、非真实 Chrome”的当前同态证据后创建范围提交，项目总控审查、合入并复验 `main`，再执行 BC-S1 并释放 B / C。`M2-F0-A` 由任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` / `codex/m2-miniapp-shell` 从 `ff4767b` 开始，只写 `apps/miniapp/**`；先交付客户端运行壳和 H3，不等待 A，也不把开发适配器称为真实跨端闭环。
...
| `M1-F2-A` 当前候选 | `codex/m1-f2-a` @ `353b4fd`，状态 `VERIFY` | 候选含功能 / 视觉提交及未提交代码、规范、验收图和旧台账 diff。22 号 `1440×844`、23 号 200% 与总控独立 16 / 16、全量 50 项验证通过；错误 PPT 图和中间态已移除。只缺当前 `1440×1024` 构图补证与范围提交，真实窗口缺失记录为环境限制；提交前后、合并前后均重验。 |
| 开发基线 | 本地 `main` @ `ff4767b` | 连续滚动事实源修正已提交；主线另有两组未跟踪视觉候选，必须保留且不得纳入台账提交。用户已授权候选通过门禁后本地合并；合并前仍须重验 HEAD / diff / 共享 PostgreSQL，并只纳入目标范围。 |
