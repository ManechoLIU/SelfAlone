...
| `M1-F4-V1-PRICING-SCHEDULE` | `READY / PENDING ASSIGNMENT` / 项目总控 | 根因已定界：`deepseek-v4-flash` 代码模型与官方当前模型一致，但 Server 只在启动时解析一组 cache-hit / cache-miss / output 价格，无法按北京时间工作日高峰窗口选择真实成本；官方事实源为 `https://api-docs.deepseek.com/zh-cn/quick_start/pricing/` 的 2026-09-01 当前页 | TDD 先锁工作日 11:59→12:00、14:00→18:00与周末边界的 RED；最小实现要求部署侧同时提供高峰 / 空闲六项人民币微元价格，调用开始时按 Asia/Shanghai 选档，旧单档或部分配置拒绝启动。仅改平台 DeepSeek adapter / composition 测试与 TECHNICAL 事实源，不读取 Key、不调用真实 Provider、不改 Web / Mini / 成本表 |
