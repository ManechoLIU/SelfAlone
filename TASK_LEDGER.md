...
- 当前执行波次：`CONTROL-RESET-1`；Web `47db65b` 的非作者终验已判 `FAIL / REWORK`：精确 1024px 断点使右工作区覆盖中栏约 96.7px、生成中锁定文案错误、消息 / 输入 / 右栏密度与纸白表面偏离 02；作者侧功能 / 路由证据仍保留但不能覆盖三个 P1，现已退回原作者。D-TEXT 私有 Web `e2a6ac4` 的非作者 verdict 为 `FAIL / REWORK`；原作者已在 11 个私有 Web 文件完成首轮实现，原 41 项回归全部转绿，仍需全仓与同一真实 Chrome Case 复验。小程序 Reader failure 已冻结至 `b28aa9c`，PPT failure 仍为活动 WIP，父项保持 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 为 D-TEXT 私有唯一写入者，状态 `ACTIVE / TARGETED GREEN / REAL CHROME PENDING`，共享入口继续排队。原 Web 作者 `/root/desktop_shell_m0_fix` 已收到三个 P1 的限定返工卡，当前 `DISPATCHED / ACK PENDING`；非作者已清理 Web 端口 / schema。小程序同一 writer 继续 PPT failure `ACTIVE / GREEN IN PROGRESS`；其 f312 工作树正在移动，UI/UX 已被要求停止审过期 `598e34f` 并保持 `QUEUED / CANDIDATE MOVING`。两个受保护差异保持未提交。
- 协调下一动作：收取 Web 作者失败测试 / 文件 ACK 后升级返工 ACTIVE；D-TEXT 完成全仓、真实 Chrome 与 scoped commit。小程序 PPT 子项冻结新 clean HEAD 后，总控复核并向 UI/UX 明确唯一最新 revision，再做全端四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验；禁止混用 `598e34f` 与后续链证据。
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / DISPATCHED / ACK PENDING` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main 旧 M0；`6afd@47db65b` 非作者已覆盖 requirements / outline / template / 真实 generating / failed / completed、四宽、路由 / 草稿 / scroll / focus / retry / AX / console 并清理环境 | PASS 子项：单一 shell / current step / 右工作区、失败保留、1440 / 1200 / 768 无横溢和 768 等价下移。P1：1024px compact 边界漏掉导致 96.7px 覆盖；generating 文案须精确“停止生成后可修改要求”；消息 / 输入 / 右栏须按 02 / reader token 收紧密度、圆角和纸白表面。原作者已收到限定文件卡但尚无失败测试 / 文件 ACK；完成态无独立定稿只可验结构 / 共享语言，禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / ELEVEN CHECKPOINTS COMMITTED / DESIGN-READY / STATE-MATRIX REWORK / PPT FAILURE GREEN IN PROGRESS / REVERIFY QUEUED / CANDIDATE MOVING` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `b28aa9c` 的 Reader failure checkpoint 已复核；PPT 后续子项仍在私有 page-state / index / WXML / WXSS 实现，未冻结新 HEAD。旧 `598e34f` 已过期，UI/UX 不得对其或移动工作树做 FINAL；待 clean scoped HEAD 返回后由总控明确最新 revision。软件键盘仍未实显，正式山亭缺失；全路由终验前不合并 |
