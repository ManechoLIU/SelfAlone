...
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@2437e00a10b1a1cfdc71c119750af12f9a2f265a` 开始，当前主线已由范围单一台账提交前进至 `d49e9e155a1c16fd0df00d62edab4f789d35f743`；主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。
- **活动实现 / 复验线**：Desktop `/root/desktop_quota_close` 在 `quota2@2437e00` 已证明历史 quota候选与 current-main等价、14项定向与隔离 A / B双账户Chrome主流程通过；非作者 `/root/quota_nonauthor_review` 随后给出 `P1` 延迟响应跨账户代际污染与 `P2` DOM替换后焦点丢失，原 writer 已提交真实 RED并按 TDD 回到 `ACTIVE REWORK`。Mini `/root/mini_drawer_rework_writer` 现于唯一 `f312@63782ba` `ACTIVE` 做四宽矩阵；controller + 两 writer占用 `3 / 4`，可用 `1` 留给 Mini非作者 EARLY，误中断 `0`。
- **READY 重算**：Desktop `M1-F4-A` 已派发且进入非作者 FAIL返工；`M1-F2-A`、`M1-F2-B` 与 `D-TEXT` 会触碰同一 `main.ts / styles.css` 接缝，待 quota候选冻结后顺序派发。`M1-F2-C` 受生产 PDF依赖 / 许可证门，`M1-F1-V1` 受真实邮件边界，F3A/F3B-B/D及F4-B仍有上游依赖。Mini recent-project记录与原生菜单均已只剩 f312，gitignored `long-list-early` 已恢复并真实出现会话 01～18；Drawer writer矩阵 `ACTIVE`，Settings / 长列表其他 FINAL复验与其共用唯一 DevTools而不并行，`M2-F4-A` 等 session bridge，`M2-F1` 等真实 AppID / 身份授权。
- **下一用户可见检查点**：Desktop 先交代际隔离与焦点恢复 RED→GREEN范围单一候选，再在 current main真实 Chrome / DB覆盖领取、刷新、并发、双账户、断服恢复和 activeElement；Mini直接交 `320 / 360 / 390 / 430` 同状态 writer自验，合格后使用释放的第4槽做非作者 EARLY。
- **真实阻塞**：本 Goal内 Project List / query阻塞已解除；Mini仍未把 tourist token / route旧日志、软件键盘或真实AppID冒充通过，但这些不阻塞当前确定性 Drawer视觉矩阵。远端、发布、真实邮件 / 微信 / 模型 / Presenton与付费调用继续保持原授权门。
- **当前可见入口 / revision**：真实 main 为 `d49e9e1`；当前主线 Web `127.0.0.1:4276` PID `4051` 通过代理连接 API `127.0.0.1:4212` PID `4652`，schema `selection_main_c858a06_20260826`，两端 `/api/v1/health/ready` 实测 ready。Mini唯一活动 DevTools路径为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、源码 HEAD `63782ba`；recent-project JSON与重启后的原生菜单均只保留 f312，`long-list-early` query已送达18项且保护 status未变。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `f312 63782ba / UNIQUE ENTRY + QUERY RESTORED / DRAWER WRITER ACTIVE` / 项目总控 + `/root/mini_drawer_rework_writer` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；本轮只重开 Drawer 同态与保留长列表 / Settings scoped NO-GO，不冒充完整Mini闭环 | Drawer v2四文件已在唯一f312；recent-project JSON与重启后原生菜单均只剩f312，`long-list-early`已真实显示18项，保护config / lock / QA原样 | 直接做非ImageGen Alpha、多背景、320 / 360 / 390 / 430 writer自验、EARLY与FINAL；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F0-A` 当前候选 | main已含`35bad75`长列表与`14b69dc` Settings；Drawer v2四文件`ae92912`已顺序放入唯一`f312@63782ba`，asset SHA `87cf9828…8876` | Settings / 长列表旧 scoped FINAL仍`FAIL / NO-GO`。唯一recent-project与原生菜单现只剩f312，`long-list-early`已出现18项；Drawer仍待本轮四宽EARLY，保护config / lock与既有QA不动。 |
...
| 当前 Mini 候选波次 | Reader / Conversation既有限定结论保持；长列表`35bad75`与Settings`14b69dc`已在main但scoped FINAL为NO-GO；Drawer v2`ae92912`在唯一`f312@63782ba`，旧整页PASS仍被用户截图否定 | recent-project JSON已范围单一保留f312并留可恢复五项备份；DevTools重启后原生菜单唯一、query 18项已送达，writer正做四宽同状态矩阵，保护现场与四个历史源码/worktree均未删除。 |
| 当前 Desktop main 预览 | `RUNNING / CURRENT MAIN d49e9e1 / HEALTH READY` | Web `127.0.0.1:4276` PID `4051`、API `127.0.0.1:4212` PID `4652`；schema `selection_main_c858a06_20260826`。旧`4173 / 4100`无监听；Selection收据继续有效，quota使用隔离环境验证但仍须在本轮候选集成后复验current main。 |
| 开发基线 | 本台账事件父为`main@d49e9e155a1c16fd0df00d62edab4f789d35f743`；本轮仍只写`TASK_LEDGER.md` | 6个受保护DOCX / Word锁文件与3组受保护视觉候选目录仍与冻结a233副本同字节，禁止暂存 / 修改 / 删除；Mini唯一f312的2个保护修改与5组既有QA未改变。 |
