...
- 当前 Goal：`M1-F2-A` 统一书架视觉纠偏与主线复验。
- 当前阶段：桌面 Web `M1-F2-A` 功能闭环保持通过；三张已批准本地书背景、稳定 ID 选图、真实元信息排印、中性色画布、44px 对话气泡与无描边封面已进入运行时并在真实 Chrome `1440×844 @ DPR 2` 复验；左栏透明山水仍无批准素材，且其余真实窗口矩阵未重拍，视觉结论保持 `VERIFY`。
- 当前拆分：M1 已展开为 31 个功能闭环与验收项；`M1-F2-A` 功能闭环保持通过、视觉纠偏正在复验，`M1-F2-B` 与 `M1-F2-C` 仍未开始；M2 / M3 保持 8 个模块级父项，进入对应阶段时再展开。
- 唯一主线：本地 `main`；本轮治理链已纳入主线，后续只从 `main` 当前 HEAD 续接并在开始前重验。其他工作树保持只读，不作为开发目录或当前功能证据。
- 当前执行波次：`M1-F2-A-VISUAL-VERIFY`。
- 当前活动项：`M1-F2-A`。
- 协调下一动作：保留既有功能，不合并、不推送；后续只为 184px 左栏接入另行批准的透明山水素材，并在同一用户 Chrome 窗口实际调整至 1440×1024、1200、1024、768 后补齐正常态截图，不用 viewport override 冒充实机矩阵。
- 当前授权边界：用户已授权 `M1-F2-A` 本地开发、范围单一提交与候选分支验收；最新明确禁止本轮合并、push、建 PR、发布或部署。不实现正文阅读、OCR、小程序 UI 或后续 M1 功能，不调用付费 / 未授权外部服务。
...
| `M1-F2` 桌面书库、阅读与笔记 | `IN_PROGRESS` | 本地书导入、阅读、记录、详情与账户隔离 | `M1-F2-A` 功能通过、视觉 `VERIFY`；B、C 不释放 |
...
| `M1-F2-A` | `VERIFY` / 主 Agent | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 功能无阻塞；三张正式封面、外壳密度与桌宠已复验；左栏透明山水无批准素材，真实窗口矩阵尚缺 1440×1024 / 1200 / 1024 / 768 新正常态证据 | CORE、API、WORKER、DB、WEB、QA；变更限定于 A 的契约、领域解析、Server、Web、视觉事实源与验收证据 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名、作者缺失、刷新、账户隔离；Web 4 视口与 02/03 绑定参考 | `09-real-chrome-covers-1440x844-dpr2.png`；视觉保持 `VERIFY`，等待新山水批准与真实窗口矩阵 |
...
`M1-F2-A` 候选收据（功能通过，视觉 `VERIFY`）：

- 负责人：主 Agent；本轮不使用子代理，同一文件只有一个写入者。
- 候选与回退：开发分支 `codex/m1-f2-a`；起点 / 回退点 `f89a4b77131c7e13676de15901cfd6712d504d10`。`codex/m1-f2-preflight` @ `ceff65c` 是当前主线祖先、没有独有实现；`67d77d7` 仅复用解析与持久化思路，不带入正文阅读 / 笔记，也不继承其拒绝图片型 PDF 的旧边界。
- 文件所有权：主 Agent 独占 `packages/contracts/`、`packages/domain/`、`apps/server/`、`apps/web/`、本完成收据、视觉事实源及 `redesign-v2/output/acceptance/m1-f2-a/`；未修改其他工作树。
- 隔离环境：PostgreSQL schema `m1_f2_a_47b4`；Server `4310`；Web `4380`；空态另用 `m1_f2_a_empty_47b4` / `4311` / `4381`；对象目录 `data/artifacts/m1-f2-a-47b4/`。共享 `public` schema 未用于能力证明。
- 功能证据：默认账户 10 个唯一对象键且全部账户前缀正确，状态 `ready_text` 8、`ready_pages` 1、`failed` 1；第二账户 0 本；真实 EPUB / TXT / 文本 PDF / 页面 PDF / 损坏 PDF、同名、缺作者、搜索、刷新与失败保留均通过。
- 视觉证据：用户本轮附图 SHA-256 `82fc9a7c4103d863a6115868f89bef16c35ef91c2f5885753ed30638b9b7a53e`，实际为旧 `1486×1058` 四列候选，不是最终 1440×1024 验收证据；当前实现截图 `02-normal-1440x1024.png` SHA-256 `8828fc0d188f9141ede59477296c6f7ee0c88956ca0f36111a3afe92686256c2`。尺寸、DPR、溢出、状态和键盘检查结果继续保留，但用户已否决当前视觉，且“正常内容态无桌宠”规则存在争议，不能据此宣称视觉通过。
- 本轮视觉证据：已拣取主仓 `1a6a91a` 的权威规范与三张正式 `1024×1434` 封面底图；运行时按稳定书籍 ID 哈希固定选择，并叠加真实书名 / 作者。`nav-surface` 为 `#E7EAE8`、`desktop-canvas` 为 `#F1F1EF`，没有绿色半透明遮罩；封面无描边，只保留一层轻阴影。用户当前 Chrome 未设置模拟视口，实测外窗 `1440×900`、CSS `1440×844`、zoom `1`、DPR `2`；左栏 `184px`、工具区 `52px`、按钮 `152px`、五列封面 `160×224px`、两排来源完整，桌宠 `104px`、气泡 `44px`，刷新前后十本书底图映射一致，控制台无 error/warning。截图 `09-real-chrome-covers-1440x844-dpr2.png` SHA-256 `2d20cb07a627fec22d8aecb1996eedbb0ed5702be83d78c7870b15e7bee5e755`。
- 用户逐点复核：当前内置浏览器 `902×904 @ DPR 1.8` 下，工具条响应式高度由 `52px` 收到 `48px`，连接行与工具条 / 书架分别保留 `12px / 22px`；导航选中面改为 `#D4E5DC`；桌宠素材右侧透明留白通过负边距补偿。气泡可见绿色圆面缩为 `38×38px`，实际点击目标保持 `44×44px`，相对居中位置上移 `4px` 落在人物肩部右侧。截图 `11-in-app-902x904-companion-shoulder.png` SHA-256 `a17e90a99c3d0a89070bf0423f7ab89f23fa74edc4a06db471a83256192339cb`；该证据只证明用户点名的工具区、导航与桌宠气泡修正，不升级左栏候选状态。
- 剩余视觉阻塞：透明山水 v4 在真实 `184px` 左栏中树木基本消失、亭子贴底被截，用户未批准且已排除在运行时和提交之外；需按 `184px×不同高度` 重新构图或另做候选。Chrome 控制接口不能无模拟地指定其余目标窗口尺寸，不能用 viewport override 替代 1440×1024 / 1200 / 1024 / 768 的真实窗口复验，因此视觉继续 `VERIFY`。
- 图标约束复核：导航、工具、来源、空态与右下气泡统一使用固定版本 `Heroicons Outline 24 v2.2.0`；气泡为白色线性圆角对话框和三个省略点，44px 圆形按钮有可访问名称。真实 Tab 从搜索框进入文件选择器时，导入按钮保持焦点轮廓。
- 自动证据：候选分支 `pnpm verify` 通过 13 个测试文件 / 45 项测试和生产构建；本轮气泡视觉合同定向回归 1 个文件 / 9 项通过；证据索引为 `redesign-v2/output/acceptance/m1-f2-a/README.md`。这些结果不替代待补的左栏视觉与真实窗口矩阵。
...
| `M1-F2` 共享外壳 | 旧左栏资产已失效 | `desktop-left-rail-landscape-approved-v1.png` 自 `1a6a91a` 起不再是有效运行时素材；透明 v4 仍未批准。新素材必须先在真实 184px 左栏完成构图验收。 |
...
- 当前外部授权门：本地 `main` 合并与复验、远端 push、PR、发布、部署、真实邮件 / 微信 / 模型 / 微信读书 / 联网 / Presenton 付费调用和小程序审核均未授权。
