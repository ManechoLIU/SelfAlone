...
- 下一可见检查点：`M1-F3B-D` 上游幂等链已顺序进入 `main@70488c8+ee7c837`，root实际diff审查及main定向31项、typecheck、build、diff-check均PASS；下一检查点为重审既有Server候选 `5110ed9` 在新上游上的真实缺口并恢复同一主路径。
...
- 容量 / READY：`/Users/echoman/.codex/config.toml` 配置上限为8，当前collaboration运行时实际硬上限为4个总槽（含root），现为1/4、可用3；新建 `luna_worker` 仍受本会话root推理档高于Luna运行上限约束，不静默覆盖模型 / 推理配置。活动状态与owner仅以下方任务表为权威。
...
| `M1-F3B-D` | `RECOVERING / UPSTREAM MAIN ee7c837 PASS / SERVER 5110ed9 REVIEW` / 项目总控 | 幂等链 `70488c8+ee7c837` 已顺序进main：账户作用域、失败迟到正文拒绝、删除后重放不复活；main定向4文件31项、typecheck / build / diff-check绿 | root立即重审既有Server候选 `5110ed9` 的实际diff与新上游关系；合格则恢复Server纵切，缺口明确则限定文件返原Writer，不把上游局部PASS冒充父闭环完成 |
...
| `M2-F4-B` | `VERIFY / CANDIDATE 6f7cc87 / UPSTREAM CANDIDATE 38aa297` / 项目总控 | root审Mini两文件diff与定向22项PASS；生产位置保存原受Server UTF-16单位缺口阻塞，现上游两文件candidate已冻结 | 集成队列先闭合当前Goal的 `M1-F3B-D` Server恢复；随后root审 `38aa297` 并顺序进main，再重审6f7cc87及唯一f312；AppID / GUI / 域名不阻塞代码集成 |
| `SHARED-TEXT-LOCATOR-UTF16` | `VERIFY / CANDIDATE 38aa297 / ROOT REVIEW READY` / 项目总控 | clean两文件候选以真实emoji PUT+DB RED锁定offset=4原返回400，改由JavaScript `String.length`按UTF-16校验；Writer现有text-reader 7项、Server typecheck、diff-check绿 | root审实际diff与查询成本 / owner语义；PASS后顺序进main并跑main回归，同事件释放 `M2-F4-B` 与clean worktree |
