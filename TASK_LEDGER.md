...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / STORES MAIN d16159a+c252cc7 / COMPOSITION RED NEXT` / 项目总控 | 账户隔离 AES-256-GCM 连接存储，以及账户内 requestId 幂等的书架 / 批注 run、单领取恢复、终态重放与 last-success 快照均已进入 main；失败 / 暂停保留上次完整快照，旧连接迟到结果受 fencing 拒绝 | Sync Store 定向3项、Server全量42文件227项、typecheck / build / visual-contract / diff-check通过，未调用真实腾讯。下一步把 validation、Gateway Adapter、stores、worker 与8个HTTP端点组成正式 runtime，并以 fake gateway 完成 API→DB→恢复 Case；真实 Key H4仍是独立授权门 |
