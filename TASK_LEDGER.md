...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-CODEX-02 / ROUND-2 NONAUTHOR FAIL / REMEDIATION` | `49712c5` 与增量 `e5d481f` 均未进入 main：首轮 3 个 Important 已补同案修复，第二轮又以真实隔离 PostgreSQL 复现 4 并发 snapshot 池自锁、默认恢复窗口遗留 running 且 ready 误绿、recover intent 遇 active drain 丢失共 3 个 Important；原 Writer 已从 clean worktree delivered ACK，最小扩展 owned files 至 runtime 与 sync-store 各自实现 / 测试及既有 index 接线 | 先补第二轮三项 RED，再改为同一 TransactionSql snapshot、封顶退避与可见 worker health、不会丢失的 recover intent；保持首轮旧快照 / requestId / 原子回滚回归后形成新增量候选并重新非作者复核。真实 Key H4仍是独立授权门，禁止读取凭证或调用真实 Provider |
