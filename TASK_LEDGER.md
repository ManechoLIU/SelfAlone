...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-CODEX-02 / ROUND-3 NONAUTHOR FAIL / REMEDIATION` | `49712c5→e5d481f→85bf8b0` 均未进入 main：前两轮 6 个 Important 已补同案修复；第三轮又以同一隔离 schema 的双 runtime 复现 A 留下 running 后由 B 恢复完成、A 仍永久 unhealthy 并无限重臂 1 个 Important。原 Writer 已从 clean worktree delivered ACK，owned files 仍限 runtime 与 sync-store 各自实现 / 测试及既有 index 接线 | 恢复状态改为跟踪失败的精确 `(accountId, runId)` 并读取其终态：他实例完成或 commit-ACK 丢失时清健康，仍 running 才退避，queued 继续 drain；保持前三轮全部反例后形成新增量候选并再次非作者复核。真实 Key H4仍是独立授权门，禁止读取凭证或调用真实 Provider |
