...
- 当前执行波次：`CONTROL-RESET-1`；Desktop `6afd@44e66af` 已获非作者 `PASS` 并合入 `main@deba325`。D-TEXT `dentry@df96ee3` 已合入 `main@9143638`，但独立 main Chrome FINAL 因本地标签被客户端占用而返回 `VERIFY / BLOCKED`，未取得页面证据。E-OWNER `owner@9512c40` 已获非作者 `PASS`，完整链已以 merge commit `main@f1334ad` 集成；main owner 定向 4 / 4、全仓 33 files / 187 tests、typecheck / build 通过。Mini DESIGN-READY 已返回，conversation / Reader / drawer 三个互斥 writer 均已创建；原样 ACK 前仍只登记 DISPATCHED。台账仍由项目总控独占。
- 当前活动项：Mini conversation 已冻结为 `f312@cb4752f`，6 个私有文件；总控定向复跑 9 / 9 PASS，writer 报 miniapp 78 / 78、typecheck / build / diff-check 通过，但未取得真实 DevTools，现由独立只读 `/root/verify_mini_conversation_final` 在 detached `cvre@cb4752f` 执行 FINAL。Reader `/root/mini_reader_sheet_rework` 使用独立 `rdrs@3fed4c7`；drawer `/root/mini_drawer_visual_rework` 使用独立 `drwr@3fed4c7`，两者仍在实现。三条 Mini 泳道文件互斥，均不得碰保护 config / lock、台账或对方页面。
- 协调下一动作：conversation reviewer须补 320 / 360 / 390 / 430、font23、safe-area、选择层 / 摘要 / 单一 PPT 入口 / 输入与趴宠的真实 DevTools verdict；Reader与drawer分别做EARLY、冻结提交与非作者FINAL。任一完成即回收并释放下一槽。D-TEXT须先释放或更换可控制的真实Chrome标签再重派FINAL。`M1-F1-A` 已READY，但当前4槽为总控、conversation reviewer、Reader writer、drawer writer，下一槽优先派发。保护差异与真实软件键盘等H3阻塞不变。
...
| `M2-F3` 小程序会话 | `VERIFY / FROZEN cb4752f / NONAUTHOR FINAL DISPATCHED / ACK PENDING` / `/root/verify_mini_conversation_final` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | `f312@cb4752f` 只含 conversation 私有 6 文件；4 / 4 RED 已闭合为定向 9 / 9，正常态内部文案、旧常驻任务卡 / 双动作已删除，半屏选择层、摘要 / 修改、单一 PPT 阶段入口、本地恢复与失败保留已实现；miniapp 78 / 78、typecheck / build / diff-check 由 writer报告，总控定向9 / 9复跑通过 | 独立 reviewer 固定 clean detached `cvre@cb4752f`，必须真实 DevTools 覆盖320 / 360 / 390 / 430、font23、safe-area、选择层生命周期、摘要 / 单入口、1 / 3 / 5行与附件、趴宠和正常文本扫描；软件键盘外部门单列 | PASS前不集成，不把测试替代视觉 |
| `M2-F4` 小程序书架与阅读 | `ACTIVE / READER VISUAL FAIL / DESIGN-READY / WRITER DISPATCHED / ACK PENDING` / `/root/mini_reader_sheet_rework` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | 独立 `rdrs` worktree / `codex/m2-reader-sheet-rework@3fed4c7` clean baseline，Reader 13 / 13；writer 仅拥有 Reader 私有 5 文件与测试，背景只实现 `light / dark`，IMG-05 只采纳结构关系 | 原样 ACK 后才升级 ACTIVE；先 RED 五态手势 / 状态保留 / 背景持久化 / 正常文本扫描，再做 320 / 360 / 390 / 430、font23、safe-area、reduced-motion EARLY / FINAL；软件键盘外部门单列 |
...
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION FROZEN cb4752f FINAL DISPATCHED / READER+DRAWER WRITERS RUNNING / IMG-04 ALPHA VERIFY / IMG-05 STRUCTURE-ONLY ADOPT` | conversation固定clean detached复验点`cvre@cb4752f`；Reader独立`rdrs@3fed4c7`；drawer独立`drwr@3fed4c7`。三者文件互斥，均以当前DESIGN-READY与绑定参考实现 | conversation代码门已绿但真实DevTools尚未裁决；Reader / drawer已有实现WIP，尚无冻结提交。IMG-04 runtime Alpha / size可复用但整体drawer仍失败；IMG-05仅采纳紧凑表面、浅暗纸面预览与选中反馈，不直接作为运行时 | 每个候选做独立FINAL；430、真实键盘、长列表、reduced-motion与未覆盖五态不得静默升级 |
