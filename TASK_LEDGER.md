...
- 当前执行波次：`M1-F2-B-MAIN-VERIFY`；A 已回收，B 实施与非作者审查任务在候选合入、主线复验和恢复点登记后均已归档，M2-F0-A 未合并候选保持 idle，C 安全首包已完成集成并停止原任务。
- 当前活动项：仅项目总控完成本次证据裁决与台账回写；B 实现任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 和非作者审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 已收到收口说明并归档，不再提示或轮询。M2-F0-A 在 `f312@91934f1` idle。
...
| `M1-F2-B-UX1` | `DONE / FOCUS PASS` / 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 已归档 | 浅 / 深阅读私有侧栏、无固定桌宠、首帧、焦点、滚动、TXT、复制反馈与完整专注可聚焦集合的非作者合同已闭合；不实现 | 固定 `804f952` 的专注合同已通过；PDF 页面一致性只锁视觉合同、不冒充当前实现，父项 B 的原生 200% / 精确视口门继续保留 | 审查任务只读且已停止；事实源由总控以 `8f40658` 同步，实施任务不再写入 | 浅 / 深专注中 rail / toolbar 从视觉、命中、Tab 与 AX tree 退出；三控件、Selection / Range、scrollTop、Esc 恢复、console / overflow 均通过 | 允许集成 `804f952`；完整视觉 verdict 为 `VERIFY`，后续只在取得缺失真实端证据时恢复 |
...
当前没有实现写入卡：B 候选与共享接缝已合入 main，实施和审查任务均在恢复点登记后归档；缺失的两项 Chrome 证据不需要代码写入。共享入口、contracts、schema、锁文件、事实源和台账仍由总控独占；M2-F0-A `f312@91934f1` idle，C 原任务已停止。
