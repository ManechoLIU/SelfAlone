...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已以 `6bcc4a4 → 99ef0ac` 顺序进入 current main并全门 PASS。`M1-F4-C-WEB-HANDOFF@a810c325` 非作者代码审查 FAIL 后已由原作者在同一 Assignment 取得 exhaustion 引导、完整会话/PPT hash 与设置 `returnTo` 应用的定向 `3 FAIL / 15 PASS`；下一检查点为最小 GREEN 候选重新交代码审查。
...
| `M1-F4-C-WEB-HANDOFF` | `RECOVERING / CANDIDATE a810c325 NONAUTHOR FAIL / Assignment M1-F4-C-WEB-HANDOFF-CODEX-01` / owner=`/root/web_handoff_codex_fallback` | 候选局部测试与构建通过，但实际合同审查确认两项 P1：`PLATFORM_EXHAUSTION` 应与未配置一致引导“配置自己的 AI 模型后继续”，不能并入 `UNAVAILABLE`；`parseSettingsRoute()` 取得的 `returnTo` 未同步进 `settingsState.textModel`，因此配置成功/返回会落到设置而非原会话或 PPT 阶段。作者 SELF 只到登录壳，未形成三错误态 PASS | 同一 canonical Assignment 已补六文件 recovery delivered ACK 且 lease allowed；定向测试已执行并得 `3 FAIL / 15 PASS`，锁住 exhaustion 引导、带阶段/书籍参数的完整 return hash、设置状态应用。恢复动作为按该 RED 做最小修复，checkpoint=首个 GREEN 候选；停止条件=不碰 Mini/Server/contracts/依赖、不调用付费模型并重交非作者审查 |
