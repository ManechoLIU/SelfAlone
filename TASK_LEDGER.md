...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已显式 `READY`：先用 TDD 把 current-main 已存在但未接线的 trial claim + cost ledger + 自有模型 responder 组合成 server-owned shared gate，并只先接 conversation；随后才释放 `M1-F4-C-WEB-HANDOFF`。Server 包不允许伪造 provider 实际成本；平台 adapter / cost meter 缺失时必须 fail-closed 并保留 draft，可测试注入 deterministic metered adapter。当前准备创建 current-main 隔离 worktree、过新 revision Assignment lease，再派 Grok 4.6 Writer。
...
| `M1-F4-C-SERVER-GATE` | `READY / Grok 4.6 BACKEND / TDD REQUIRED` | current-main 已有 trial claim、CostLedgerStore、用户自有 DeepSeek responder，但三者尚未接线且 cost ledger 未进 production composition。建立可复用平台文本能力门：自有已配置模型优先且不计平台额度；未配置时仅已 claim + ledger available 才可走 server-owned metered platform adapter；平台调用必须 request-scoped reserve→settle，失败 release；limit/external-unavailable 必须返回可区分失败并保留 ConversationStore retainedDraft；不得伪造实际成本或把平台凭证下发浏览器 | owned scope 限 `platform-text-capability.ts/.test.ts`（可新增）、`conversation-store.ts/.test.ts`、`conversation-responder.ts/.test.ts`、`index.ts` 及必要的同目录 composition test（最多8个Server文件）。现有 cost-ledger/trial/model-config/deepseek 实现只读复用，除非 Writer 明确 STOP 证明无法完成；第一步必须 RED，至少覆盖 claimed+available fallback、limit no-call、provider failure release、own-model no-charge；高风险 cost/external-call 后续 required review 必须 TDD causal evidence |
