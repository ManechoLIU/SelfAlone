...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：当前可见 main 仍是旧 M0；6afd 原 writer 已由 11 个文件的持续 WIP 证明恢复执行，当前 diff 已把 hash / history 事件收敛到单一 `renderRoute`，但 844 短高输入区、青瓷 / 纸白组件语言和首次 / 往返对话恢复尚无失败测试与真实 Case 证明，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，Reader 私有标注 / 笔记候选 5 文件 / 35 项定向测试通过但真实 API / Chrome 与冻结提交仍缺；小程序已冻结 `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28` 七个局部恢复点，分别保留系统视口、Reader、账户 / 书架、会话意图、PPT 视口与账户页字体证据，但整体仍为 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 已按实际 diff 确认为 `IMPLEMENTATION ACTIVE / ACK BY EVIDENCE`，继续独占 6afd 的 shared shell / conversation / main / styles；原非作者 UI/UX 已 ACK 并在 4174 / 4101 对 844 短高输入、共享视觉语言和 fresh→library→返回 / 刷新 / 前后退 / `?stage=` / 断网缓存恢复做同包 brief，4173 旧 main 已排除。`/root/reading_text_notes_web` 已进入 `PRIVATE WEB GREEN / CHROME PENDING`，继续独占 Reader 私有文件。小程序任务已提交 `6aba8ef` PPT viewport 局部点并继续系统返工；两个禁止差异保持。
- 协调下一动作：UI/UX 先回 6afd 844 短高输入 / 消息密度 / 青瓷纸白 / 路由同源 brief，Web 作者据此连同阶段持久化、失败动作与真实往返 Case 冻结新 HEAD，再按 02 / 04 / 05 / 06 / 33 逐阶段复验。D-TEXT 继续私有范围候选。小程序同一 writer 继续其余状态矩阵；冻结全端候选后做四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验。
...
| `M1-F2-D-TEXT` | `ACTIVE / PRIVATE WEB GREEN / CHROME PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | `dtxtui@996d8b1` / `codex/m1-f2-d-text-web` 私有 WIP；总控定向运行 text-reader / annotation / book-detail 5 文件 / 35 项通过，覆盖稳定键盘解绑、44px、聊天接缝 fail-closed 与非假标签 | 私有 writer 只写 `text-reader-*` 与新增 annotation / book-detail 模块；`main.ts` / shared shell / styles 等 Web shell 合入后由总控顺序接入；旧 `18c7522` 只读 | 真实 EPUB / TXT 选区→划线 / 想法 / 老己笔记 CRUD，失败保留、刷新恢复、owner / fileVersion 隔离、返回原文与绑定参考真实 Chrome通过 | 私有测试不证明真实 API / Chrome；全仓回归、候选提交、非作者复验与入口集成仍缺，不能称用户可见功能已实现 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28` 已形成七个局部恢复点；`6aba8ef` 通过 9 文件 / 53 项及 320 / 360 / 390 / 430 PPT 视口局部矩阵，`36aeb28` 只新增 6 张 login / settings 的 360 / 390 与字体 23 DevTools 收据 | `cdaa307` 只关闭开发适配器内 Reader→conversation→PPT 流程，`6aba8ef` 只关闭 PPT viewport 局部回归，`36aeb28` 只关闭 login / settings 标准字体局部门；均不证明服务端 / 跨端会话、软件键盘、全路由字体、H3 或视觉通过。两个禁止差异和游客 console / 资产门保持 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / SEVEN CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f`、`630c051`、`5e270b1`、`5e0188d`、flow-only `cdaa307`、PPT viewport `6aba8ef` 与 account evidence `36aeb28` 是七个局部恢复点；软件键盘仍未实显，继续其余状态 / 字体 / 视觉矩阵。全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 七个局部点已冻结；`cdaa307` 只关闭 DevelopmentClient Reader→conversation→PPT intent，`6aba8ef` 只关闭 PPT live viewport / safe-area / action clearance，`36aeb28` 只补 login / settings 四宽与字体 23 局部收据。其余状态、软件键盘 / 全路由字体矩阵与最终视觉仍未完成；既有禁止差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28` 是七个局部恢复点；全小程序六页 / 共享层四宽四高和状态矩阵、真实软件键盘 / 全路由字体、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
