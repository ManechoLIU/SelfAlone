...
- **活动实现线**：Auth / Settings与Mini Reader / Conversation既有main PASS保持。D-TEXT 768 `main@cc0cec0` scoped FINAL PASS。Desktop Conversation `87a598c`代码门PASS但真实1440 EARLY缺新建 / 搜索 / 新账户领取条；唯一writer `/root/mini_drawer_writer`已加载preflight，首个client RED由总控复跑为2 FAIL / 2 PASS，正在实现。Mini Drawer已从`f312@b6e7b66`顺序合入`main@b98c118`并获Drawer-only scoped FINAL PASS。释放槽已派`/root/mini_library_real_slice`做真实Library首包，另由`/root/book_detail_candidate_review`短时审查长期冻结候选。当前构成：controller / integrator 1、implementation writer 1 ACTIVE + 1 DISPATCHED / ACK PENDING、reviewer 1 DISPATCHED；三个子槽均有真实代码或候选审查事件。
- **下一用户可见检查点**：Conversation闭合quota / new / search后作者Chrome自验并冻结候选；Mini Library先交精确ACK + client / import首个RED，再产出可顺序进入唯一f312的候选；Book Detail审查给出私有提交可否先合入与shared seam最小计划。任何候选通过后由总控顺序集成main并复验，再按文件冲突释放Book Detail seam或D-TEXT PPT handoff。
- **真实阻塞**：Conversation真实EARLY缺ref02的新建 / 搜索与ref36的新账户一次性领取条；`M1-F4-A`只释放真实claim最小闭环，F4-B / C不扩。Mini Drawer局部缺陷已关闭，Mini父项仍缺长会话列表、软件键盘、游客凭证化clean console与真实PPT preview。D-TEXT reader子包无阻塞，PPT handoff / banner和父项继续开放；Book Detail与D-TEXT PPT handoff的shared seam仍受Conversation单writer约束，但私有候选审查不受阻。
- **当前可见入口 / revision**：Desktop候选入口`5188 / 4308`既有验收仍绑定`a9408ad`；Conversation EARLY隔离入口已清理。D-TEXT `5198 / 4318`已关闭，schema `selfalone_dtext_main_final_cc0cec0_20260826`与runtime `/tmp/selfalone-dtext-main-final-cc0cec0.0LEzLV`保留，未授权删除。稳定恢复点`4127 / 4198@ab13310`。Mini产品main为`b98c118`，唯一DevTools仍指`f312@b6e7b66`且与main Drawer source / dist等价；config / lock与两张QA图未变。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE` · `CANDIDATE 87a598c / CODE PASS / CHROME EARLY FAIL / REOPEN 5 RED CONFIRMED` / `/root/mini_drawer_writer` | 输入、发送、确定性回复、刷新恢复、错误保留与统一对话UI；代码竞态闭合。真实1440新账户状态缺新建 / 搜索和一次性领取条，需补真实行为而非静态视觉 | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、HEAD `87a598c`；preflight已加载，shared seam仍只此包 | 原25个Conversation文件 + 最小F4-A私有模块 / tests与shared app / index / main；禁止Book Detail / Settings私有 / Mini | 代码门22 / 155 PASS；EARLY FAIL `conversation-early-87a598c/`；client首个RED 2 FAIL / 2 PASS。下一步实现claim / new / search→作者Chrome→候选复审 |
| `M1-F3B-A1-CONVERSATION-CORE` | `VERIFY` · `CURRENT-MAIN CANDIDATE 87a598c / CODE PASS / VISUAL REOPEN` / `/root/mini_drawer_writer` | 确定性回复、PG会话 / 消息、API、Web、legacy、导航、幂等、发送锁与hydrate竞态代码门保留；本轮不重做已过范围 | 当前视觉 / 缺行为REOPEN不推翻代码局部PASS；owned外即停 | 同一25文件候选及F4-A最小接缝；禁止其它页面 | 首个真实FAIL止于1440 idle，后续发送 / 恢复Case未运行；补齐入口后重跑完整EARLY |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `ACTIVE` · `ACK RECEIVED / CLIENT RED CONFIRMED` / `/root/mini_drawer_writer` | 新账户一次性领取、按账户grant状态和桌面对话首页轻量领取条闭环；不建设券、余额、充值、成本账本或常驻记录 | Auth + F3B-A共享外壳已具备；不再等待F3B-V1，shared seam与A同一writer；外部 / 付费调用即停 | 最小CORE / API / DB / WEB / QA新增文件及A已登记shared seam | claim幂等 / 并发 / 隔离；成功短反馈后移除，失败保留聊天输入；ref36 / 38且无金额 / 余额 / 进度。`getTrialQuota`尚不存在的RED已确认，下一步实现与真实Chrome |
...
| `M2-F4-A` 真实书架 / 导入接入 | `READY` · `DISPATCHED / ACK PENDING` / `/root/mini_library_real_slice` | 将运行壳接入真实书籍摘要、搜索、导入与解析状态；不把开发适配器或静态五态冒充真实持久化 | worktree `/Users/echoman/.codex/worktrees/mlib1/SelfAlone@b98c118`、branch `codex/m2-f4-a-mini-library`；只写Mini private client / Library page / state / tests，真实账户仍停在M2-F1边界 | ref14；真实EPUB / TXT / PDF、五类状态、失败保留、刷新恢复、DevTools与账户隔离。下一步精确ACK + transport / import RED→candidate→唯一f312 EARLY |
...
| `M2-F4` 小程序书架与阅读 | `VERIFY` · `MAIN MERGED b98c118 / READER SCOPED FINAL / LIBRARY DISPATCHED` / 项目总控 | 统一书架、连续正文、内容面板、老己笔记与背景恢复；非目标：真实notes API / 跨设备。Reader视觉 / 手势范围已关闭，真实Library / API接入仍开放 | Reader已释放；`M2-F4-A`已派唯一Mini Library writer。真实账户、跨端持久化仍按M2-F1 / owner门 | refs 14 / 18～20；`mini-reader-final-db40e99/`与`mini-main-final-31462d9/`。下一步Library ACK + RED，不返工已通过Reader |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE` · `cvmain@87a598c / CHROME EARLY FAIL / REOPEN 5 / RED CONFIRMED` / `/root/mini_drawer_writer` | 既有功能 / 并发修复保留；本轮只闭合真实new / search与unclaimed claim条后重跑EARLY | preflight已闭合；shared seam与F4-A同writer，Book Detail / D-TEXT handoff继续等 | 原25文件 + F4-A最小新增模块；禁止其它私有页面 / Mini / ledger | ref02 / 36；FAIL图与DOM在`conversation-early-87a598c/`；client首个RED 2 FAIL / 2 PASS。下一步实现→作者Chrome→非作者EARLY |
| `M1-F2-BOOK-DETAIL-VISUAL` | `VERIFY` · `CANDIDATE f6c6d0e + 75e764c + 6c50a76 / NONAUTHOR REVIEW DISPATCHED` / `/root/book_detail_candidate_review` | Notes / Highlights / PPT empty局部PASS；Web runtime与account / book-scoped current / history / artifact / failed私有查询候选已冻结；非目标为自由改版、真实模型 / PPTX | shared `app.ts` / `index.ts` / `main.ts`禁写；审查只读且不与Conversation冲突；真实HTTP / 下载 / reload / generating / failed浏览器Case仍等shared seam顺序释放 | Web `book-detail-runtime{,.test}.ts`等8文件；Server `book-presentation{,.test}.ts`两文件；候选clean | 定向代码门既有PASS但仍不是EARLY；下一步审实际diff、冲突与私有提交可集成性，证据`book-detail-candidate-review-6c50a76/` |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader scoped FINAL；Conversation `cvmain@87a598c`代码门PASS、Chrome EARLY FAIL后REOPEN 5已确认首个RED；Book Detail候选进入短时非作者审查 | Conversation缺真实new / search / claim且唯一writer正在实现。D-TEXT端口关闭，schema / runtime保留；Book Detail shared seam仍等Conversation冻结。 |
| 当前 Mini 候选波次 | Reader + Conversation + Drawer在`main@b98c118`限定范围FINAL；Mini Library真实接入首包已派`mlib1@b98c118` | Drawer三轮FAIL与最终PASS证据均已保留；Library待ACK + RED，后续只顺序进入唯一f312。保护现场不变。 |
...
| 开发基线 | 产品顺序集成到 `main@b98c118`；Desktop Auth / Settings、D-TEXT reader、Mini Reader / Conversation / Drawer限定范围PASS | main仅有当前台账事件diff与三个受保护未跟踪视觉目录；Conversation EARLY FAIL、Mini Library仅DISPATCHED、Book Detail仅审查中，均不能自动 `DONE`。 |
