...
| `M1-F2-WEREAD-RUNTIME` | `VERIFY / STORES MAIN d16159a+c252cc7 / CHILD REWORK ACTIVE` / 项目总控 | 账户隔离 AES-256-GCM 连接存储，以及账户内 requestId 幂等的书架 / 批注 run、单领取恢复、终态重放与 last-success 快照均已进入 main；失败 / 暂停保留上次完整快照，旧连接迟到结果受 fencing 拒绝 | 先由 `M1-F2-WEREAD-SYNC-REWORK` 关闭连接替换后的快照身份混用与失败指纹顺序敏感缺口；随后把 validation、Gateway Adapter、stores、worker 与8个HTTP端点组成正式 runtime，并以 fake gateway 完成 API→DB→恢复 Case。真实 Key H4仍是独立授权门 |
| `M1-F2-WEREAD-SYNC-REWORK` | `ACTIVE / owner=grok46-weread-sync-writer / Assignment M1-F2-WEREAD-SYNC-REWORK-grok46-01` | 仅写 `apps/server/src/weread-sync-store.ts` 与对应测试；同一 execution lineage attempt 1 已在隔离工作树 `codex/m1-f2-weread-sync-rework@ac8f821` 启动，当前 lease / process healthy，尚无候选 | 先以 PostgreSQL RED 锁定连接替换后的跨连接快照身份混用、以及同语义失败对象键序变化导致冲突，再最小修复并冻结两文件候选；禁止真实腾讯调用、依赖安装、main / 台账写入 |
