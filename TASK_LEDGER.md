...
- 当前执行波次：`CONTROL-RESET-1`；Web 候选 `1727f95` 已按用户裁决判为 `FAIL / IA REOPEN`，原 6afd 唯一写入者正在按 02 / 04 / 05 / 06 / 33 matrix 整改；D-TEXT P1 修复候选 `5156b9c` 已冻结，等待原非作者复验同版本 metadata / 部分发布状态与锁序；小程序 `d4f587c` 已冻结 reader 短屏和非视觉定位修正，同一 UI/UX 任务正在按全端 `DESIGN-READY` 合同复验。`f312` 仍独占 `apps/miniapp/**`，后续全端视觉整改由同一写入者顺序承接；台账仍由项目总控独占。
- 当前活动项：`/root/desktop_shell_m0_fix` 在 `6afd@1727f95` 之上把四阶段收敛为右侧单一互斥工作区；`/root/reading_notes_integration_review` 只读复验 `dint@5156b9c`，修复作者已停止写入；UI/UX 任务 `01a03652-a5d4-76c1-b0fa-ff9303243664` 正在复验 `f312@d4f587c`，Web 新 HEAD 返回后切回阶段矩阵终验。小程序透明山亭与真实 PPT 模板 / 页面预览资产仍是局部资产阻塞；既有未暂存 `pnpm-lock.yaml` 禁止触碰。
...
| `M1-F2-D-TEXT-INTEGRATION` | `VERIFY / P1 FOLLOW-UP` / `/root/reading_notes_integration_review` | 固定 `dint@5156b9c`；parent `ed95d95`，follow-up 只改 `apps/server/src/text-reader.ts` 与直接测试；作者已停止 | metadata 漂移与 `section_count` 部分状态已由作者以真实 PostgreSQL 回归修复，`pnpm verify` 26 文件 / 123 项通过；原审查者复验 no-op、fail-closed、锁序和范围前仍禁止合并，P2 不混入本包 |
