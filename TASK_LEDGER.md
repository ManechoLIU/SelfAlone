...
| `M2-F5-A` | `ACTIVE / LOCAL SETTINGS SLICE` / `M2-F5-A-MINI-SETTINGS-KIMI-01` | Mini 账户邮箱/密码、文本模型配置与真实退出语义的本地/API 合同闭环；复用现有会话 Bearer 与 Server 已有 settings/model-credentials 路由 | Kimi K3 API Writer 已 canonical `assignment_started`，只改 Mini；图片模型明确排除，production AppID/apiBaseUrl/域名与真实邮件仍留父级 H3/H4，不阻塞本地/injected transport 代码闭环 |
