...
- 当前 Goal：`STAGE-READ-NOTES-V1-K / M2-F2 RECOVERING`；root实际审 `e4ed48e` 后确认NO-GO：Settings逻辑虽有editor / Key / save / delete状态，WXML却没有任何可操作配置入口，且旧测试反向断言不渲染editor。已由Kimi K3 API限定Settings四文件从RED恢复；候选修复后才可顺序放入唯一 `f312@4cdce97`，完成no-call连接、书架hydrate、失败保留、同requestId重试与刷新恢复真实DevTools Case，再顺序合main。真实WeRead Key / AppID / apiBaseUrl / 域名H3不在本Goal。
- 下一可见检查点：`M2-F2-REWORK-01` 在 `weread-mini@e4ed48e` 形成Settings配置入口scope-only candidate；`/root/m2_f2_uiux_acceptance` 已给出绑定ref22/ref39的EARLY合同，候选顺序进入唯一f312后执行真实端verdict。并行 `M2-F4-B@6f7cc87` 非作者审查已FAIL：源文本locator被直接用于trim后坐标系，含前置空白 / non-BMP时restore→save漂移；本事件落账后立即以原两文件范围恢复。`M1-F4-B@f53c930` 保持NONAUTHOR FAIL / RECOVERING，唯一f312候选 `4cdce97` 仅作为共享DevTools入口，不整体合main。
...
| `M2-F2` | `RECOVERING / e4ed48e NO-GO / KIMI ACTIVE` / 外部Kimi `kimi-k3 / api / high / os-keychain` | root审累计候选发现Settings WXML无editor / Key输入 / save / retry / disconnect，用户无法连接；现有测试还反向断言不渲染editor，定向绿灯无效。Kimi已对绝对cwd、clean HEAD、Settings四文件、首个RED与停止条件回完整ACK；只读验收任务 `/root/m2_f2_uiux_acceptance` 已完整ACK并冻结ref22/ref39、六状态与430/390/360/320 EARLY合同，不提前PASS | recovery动作：只改 `settings/index.ts/test.ts/wxml/wxss`，依据SPEC 112–128、DESIGN-MINIAPP 81 / 100–103与ref39派生全屏配置页；禁止真实Key / 调用、共享合同 / Server / f312 / main / lock / 资产写入。检查点=scope-only candidate + exact tests，随后root审diff并顺序进唯一f312，再由同一验收槽跑唯一DevTools Case |
...
| `M2-F4-B` | `RECOVERING / CANDIDATE 6f7cc87 NONAUTHOR FAIL` / 待新Assignment | `/root/m2_f4b_code_review` 已审实际两文件diff：HTTP adapter把Server源文本UTF-16 locator原样交给会trim正文的Mini reader，含前置空白 / non-BMP时restore与save坐标系漂移；候选测试仅覆盖无首尾空白样例。diff-check PASS但无候选worktree，Reviewer未把main旧实现测试冒充候选证据 | recovery动作：新Writer只在 `library-http.ts/test.ts` 锁 leading/trailing whitespace + emoji 的hydrate→restore→save RED并最小修复；形成scope-only candidate后交非作者复审。AppID / GUI / 域名不阻塞该恢复 |
...
- 当前 worktree 共4个：main、唯一f312、待恢复候选 `quota-cost@f53c930` 与待独立审查 / 集成的 `weread-mini@e4ed48e`；`weread-web@c2c009d` 合入后已同事件移除clean worktree。不得为 reviewer / 占位 / 纯消息任务新建 worktree；候选合入 / 废弃后同事件移除clean worktree，脏现场先形成scoped commit或带原路径+HEAD的恢复点。历史归档stash共12条，f312仅在唯一DevTools入口仍需时保留。
