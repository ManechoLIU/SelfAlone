...
- 当前阶段：桌面 Web `M1-F2-A` 候选保持 `VERIFY`，尚未合入 `main`；功能通过结论来自上一检查点，当前未提交视觉与状态改动的最新定向回归已出现 1 项失败，仍需修复范围越界并完成真实业务 Case、视觉证据、非作者 UI/UX 审查和主线复验。
...
- 协调下一动作：实现任务先恢复全局 `:root` Token 并把书架值限定到 `.library-shell`，再重跑定向测试和完整回归；随后补齐真实上传长标题 Case、正确书架 `1440×844` 与 `1440×1024` 真实 Chrome 同态证据、窄屏可访问性和非作者 UI/UX 结论。右下远山两次生图均失败，停止继续消耗额度并保持局部 `VERIFY` 阻塞，不得拖停其他收口。完整证据包交项目总控核验后才决定状态；全部门禁通过后方可范围提交、本地合并并复验 `main`，随后释放下一项。
...
| `M1-F2-A` | `VERIFY` / 任务 `01a0339a-3299-7150-91d9-f95a5331c627` | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 候选 `codex/m1-f2-a` @ `353b4fd` 尚未合入主线且有未提交改动；书架 Token 仍越界修改全局 `:root`，正确书架 `1440×844` / `1440×1024`、真实长标题 Case、窄屏可访问性和非作者 UI/UX 结论未闭合；两次远山候选均失败 | CORE、API、WORKER、DB、WEB、QA；实现任务拥有候选代码 / 证据，台账文件只由项目总控写 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名异源、作者缺失、刷新、账户隔离；Web 4 视口对照书架参考 | 上一 dirty 检查点的 `pnpm verify` 曾通过 13 文件 / 49 项，但其后测试与样式继续变化；总控最新定向回归为 15 项中 1 项失败，且名为 `16-real-chrome-1440x844-cover-status.png` 的文件实际是 PPT 完成页，旧通过结论和该截图均不得作为当前门禁；修复并合入 `main` 后才释放 B、C |
...
当前执行卡为 `M1-F2-A` 的 `VERIFY` 收口。实现任务 `01a0339a-3299-7150-91d9-f95a5331c627` 在 `47b4` 工作树继续功能、视觉和真实浏览器验收；项目总控下达边界、核验证据并写状态，不与实现任务并行修改候选代码。恢复点为 `codex/m1-f2-a` @ `353b4fd` 加当前未提交 diff；先修复书架 Token 对全局外壳的范围越界，再完成真实上传长标题 Case、正确书架 `1440×844` / `1440×1024`、窄屏检查、非作者 UI/UX 审查、完整回归和全 diff 复核。达到门禁后由实现任务创建范围提交，项目总控审查并按授权集成、复验本地 `main`；收到主线 HEAD 与复验证据后再释放 B、C。视觉偏差保持 `VERIFY`，局部素材问题只阻塞自身。
...
| `M1-F2-A` 当前候选 | `codex/m1-f2-a` @ `353b4fd`，状态 `VERIFY` | 与 `main` @ `592954a` 在 `f89a4b7` 后分叉；候选含功能 / 视觉提交及未提交代码、规范、验收图和旧台账 diff。最新定向测试因全局 Token 越界失败，且一张 `1440×844` 证据错拍为 PPT 页；不得把 dirty 工作树、过期测试或文件名当作已合入结果，候选提交前后、合并前后均重验。 |
| 开发基线 | 本地 `main` @ `592954a` | 主线当前另有其他任务的未跟踪视觉候选，必须保留且不得纳入台账提交。用户已授权候选通过门禁后本地合并；合并前仍须重验 HEAD / diff / 共享 PostgreSQL，并只纳入目标范围。 |
