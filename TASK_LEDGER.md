...
- 当前活动项：Mini 唯一 `/root/mini_visual_rework_fix` 正在修改 Reader controls 私有样式 / 测试与 drawer 私有组件 / 测试，并确定性派生一个镜像 RGBA runtime；不得触碰两项保护差异与旧 EARLY 图。`IMG-04` 为 `ASSET VERIFY / RUNTIME REWORK`，没有新增图片生成调用。D-TEXT 入口 `/Users/echoman/.codex/worktrees/dentry/SelfAlone@deba325` 已迁入 11 个私有 book-detail / annotation 新文件并修改 5 个 reader / library 私有文件；尚未写共享入口，owner 文本 ACK 仍待补，但不再是未启动状态。
...
| `M2-F0-A-VISUAL-REWORK` | `VERIFY / REWORK ACTIVE / IMG-04 ASSET VERIFY / RUNTIME WIP` / `/root/mini_visual_rework_fix` + `/root/mini_visual_final_review` | `IMG-04` 已核验为真实 RGBA / fractional alpha / 无实底；唯一 brief 要求完整源水平镜像一次输出 `1024×737 RGBA` Mini 私有 runtime、140px slot / opacity .82 / 线性 mask / 不入滚动与点击层。writer 已有 Reader / drawer / asset WIP；不重调色、不再次生成 | 新 HEAD 须定向 / miniapp verify / DevTools 320 / 360 / 390 / 430 + font23、320 深色 controls、列表滚动 / 命中 / safe-area、非作者 PASS。软件键盘、真实多会话列表与 PPT preview 继续精确保留 H3 / VERIFY 边界 |
