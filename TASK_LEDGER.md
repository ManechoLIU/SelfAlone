...
- 下一可见检查点：`M1-F4-C` 已显式 `READY`，目标是把一次性免费体验与真实成本台账接到 AI 对话 / PPT 的共同调用门：有可用平台额度时允许继续；达到硬上限且未配置自己的文本模型时，只在下一次需要模型的当前操作位置提示“配置自己的 AI 模型后继续”，并保留输入、会话上下文与 PPT 草稿；平台能力暂不可用/失败也保留现场可重试。先由总控按 current-main 事实拆 owned scope、过 Assignment lease，再派单一 Writer；不得把图片模型、M1-F5 全 PPT 生成链或 Mini production H3 门混入本 Goal。
...
| `M1-F4-C` | `READY / CURRENT GOAL / 待派 Writer` | AI / PPT 共用免费能力：平台额度可用时走受限平台能力；硬上限耗尽且未配置自己的文本模型时，在下一次需要模型的当前输入/任务位置引导配置并保留输入、会话上下文与 PPT 草稿；平台能力失败原位可重试 | A DONE；B DONE main@1bb0b76；文本模型真实H4与配置入口已存在；PPT draft/handoff合同已进main。下一事件先拆 current-main owned scope 与第一条可观察 RED，再派单一 Writer；不扩图片模型、完整PPT生成或Mini production H3 |
