...
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION+DRAWER DESIGN-READY DISPATCHED / ACK PENDING / IMG-04 ASSET ALPHA VERIFY` | 以 `main@3199945`、13 / 30–32 修正 conversation 的选择层→轻摘要→单一阶段入口、正常模式内部文案、消息 / 任务层级、空白和输入趴宠；按 11 修正 drawer 全层级尺度 / 留白 / 山亭关系 | `IMG-04` runtime 的真实 Alpha与 size `3fed4c7` 可复用，但整体实装视觉已被用户推翻。旧无证据 reviewer 已中断；替代 `/root/mini_visual_design_ready_v2` running 但尚无原样 ACK。conversation writer 不提前派发；drawer 与 conversation 保持单一 owner、互斥文件边界；320 / 360 / 390 / 430、font23、safe-area、EARLY / FINAL 必须由非作者真实 DevTools 复验 |
