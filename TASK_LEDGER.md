...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：当前可见 main 仍是旧 M0；6afd 原 writer 已由 11 个文件的持续 WIP 证明恢复执行，当前 diff 已把 hash / history 事件收敛到单一 `renderRoute`，但 844 短高输入区、青瓷 / 纸白组件语言和首次 / 往返对话恢复尚无失败测试与真实 Case 证明，当前禁止合并。D-TEXT 私有 Web 已冻结 `e2a6ac4`，全仓 30 文件 / 140 项、typecheck / build / diff-check 与作者侧真实 Chrome Case 通过，现转非作者视觉 / 交互复验，未接共享入口前不称用户可见闭环。小程序已冻结 `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53` 八个局部恢复点；`5d51c53` 只补全 360×640 / 字体 23 的代表性全路由证据，提交后 conversation / drawer 已继续产生视觉整改 WIP，整体仍为 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 已按实际 diff 确认为 `IMPLEMENTATION ACTIVE / ACK BY EVIDENCE`，继续独占 6afd 的 shared shell / conversation / main / styles；原非作者 UI/UX 已 ACK 并在 4174 / 4101 对 844 短高输入、共享视觉语言和 fresh→library→返回 / 刷新 / 前后退 / `?stage=` / 断网缓存恢复做同包 brief，4173 旧 main 已排除。`/root/reading_text_notes_web` 已冻结 clean 候选 `e2a6ac4` 并解除私有文件写入，等待同一非作者 UI/UX 顺序复验；共享入口仍由总控排队。小程序任务已提交 `5d51c53` 字体证据点并继续 conversation / drawer 系统视觉返工；两个受保护差异保持未提交。
- 协调下一动作：UI/UX 先回 6afd 844 短高输入 / 消息密度 / 青瓷纸白 / 路由同源 brief，Web 作者据此连同阶段持久化、失败动作与真实往返 Case 冻结新 HEAD，再按 02 / 04 / 05 / 06 / 33 逐阶段复验；随后对 clean 的 D-TEXT `e2a6ac4` 做非作者真实 Chrome 复验。小程序同一 writer 继续 conversation / drawer 与其余状态矩阵；冻结全端候选后做四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验。
...
| `M1-F2-D-TEXT` | `VERIFY / CANDIDATE FROZEN / NONAUTHOR PENDING / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | clean 候选 `dtxtui@e2a6ac4` / `codex/m1-f2-d-text-web`，29 文件 / `+1695 -2`；全仓 30 文件 / 140 项、typecheck / build / diff-check 通过，作者侧真实 Chrome 覆盖选区、划线 / 想法、笔记 CRUD、刷新恢复、失败保留、主题 / 专注 / Esc 与账户隔离 | 私有 writer 已停止写入；`main.ts` / shared shell / styles 等 Web shell 合入后由总控顺序接入；聊天 handoff 在共享路由接入前保持 disabled fail-closed | 真实 EPUB / TXT 选区→划线 / 想法 / 老己笔记 CRUD，失败保留、刷新恢复、owner / fileVersion 隔离、返回原文与绑定参考真实 Chrome通过 | 作者证据不替代非作者 UI/UX；等待同一审查任务复验 `e2a6ac4`。入口 / 账户 / quote intent 尚未接入，不能称用户可见闭环完成 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53` 已形成八个局部恢复点；`6aba8ef` 通过 9 文件 / 53 项及 320 / 360 / 390 / 430 PPT 视口局部矩阵，`36aeb28 + 5d51c53` 只补 login / conversation / drawer / library / reader / controls / PPT / settings 的 360×640、字体 23 代表性收据 | `5d51c53` 仅 6 张二进制证据且未重复源码验证；字体 23 下无明显横溢只关闭局部适配门。conversation 趴宠锚、drawer 纸白开放列表 / 正式山亭仍 FAIL，提交后已有同一 writer 的视觉 WIP；软件键盘仍为 0，游客 console / 凭证、正式资产、真实 PPT 预览与最终全路由 UI/UX 均未关闭 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / IMPLEMENTATION ACTIVE / ACK BY EVIDENCE / UIUX ACTIVE ACKED` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main@`4a94b4f` 旧 M0；`6afd@0e18846` 加 WIP 现为 11 个 tracked 文件、约 `+884 / -88`，新增 `conversationHref` / `lastConversationStage`、workspace / cache、requirements / outline draft 与双区 scroll 恢复，并已用单一 `renderRoute` 替代 hash / history 的整页 reload；`data/` 和 WIP 图不属于冻结候选 | UI/UX 已 ACK 同包覆盖 844 短高输入 / 密度 / 02 Token，以及 fresh #/conversation→library→返回、刷新、前后退、`?stage=`、断网缓存的一致 shell / stage / draft / scroll；代码调整不替代失败测试、真实往返 Case、brief 与冻结 HEAD，以上齐备前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / EIGHT CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f`、`630c051`、`5e270b1`、`5e0188d`、flow-only `cdaa307`、PPT viewport `6aba8ef`、account evidence `36aeb28` 与 full-route font evidence `5d51c53` 是八个局部恢复点；软件键盘仍未实显，conversation / drawer 视觉整改 WIP 继续。全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 八个局部点已冻结；`cdaa307` 只关闭 DevelopmentClient Reader→conversation→PPT intent，`6aba8ef` 只关闭 PPT live viewport / safe-area / action clearance，`36aeb28 + 5d51c53` 只补账户页与代表性全路由字体 23 收据。其余状态、软件键盘、最终视觉 / 资产门仍未完成；同一 writer 已在 `5d51c53` 后继续 conversation / drawer 视觉 WIP，既有受保护差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53` 是八个局部恢复点；360×640 / 字体 23 代表性全路由证据不替代全小程序六页 / 共享层四宽四高和状态矩阵，真实软件键盘、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
