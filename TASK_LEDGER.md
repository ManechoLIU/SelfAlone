...
- 协调下一动作：等待认证或 D-TEXT exact-contract ACK 事件，收到后先核对真实现场再局部升级 `ACTIVE`；无 ACK 时不再轮询，也不新建重复执行者。Mini 完成真实 DevTools EARLY 后按逐页 covered / uncovered 与同态证据裁决：FAIL 才释放对应唯一 writer，PASS 后才冻结 FINAL；总控不在此时把 Mini 候选合入 main。
