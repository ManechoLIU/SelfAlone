...
- 当前活动项：`M1-F2-D`
- 下一可见检查点：两项数据保留回归已真实 RED（fresh store 无 `claim`；hydrate 覆盖服务端 retry 草稿并清空 requestId）；原恢复 writer 在同一 worktree 最小修复，两项 GREEN 后冻结单一候选并重新交同状态 Chrome SELF。
...
- 容量 / READY：root + `reader_chat_handoff_recovery` 2/4、可用 2、待审 / 待集成候选 WIP 0。`M1-F2-D` 在原 worktree ACTIVE 返修；`M1-F4-B` 仍是下一可靠性 Goal候选，不抢占当前返修；其余开放包继续保留在 READY 宇宙。
...
| `M1-F2-D` | `ACTIVE / TWO RED LOCKED` / `reader_chat_handoff_recovery` | fresh store 恢复测试因缺 `claim` 失败；hydrate 现把服务端失败草稿替换为Reader引用且 `retryRequestId` 从 `request-failed` 变 null；定向2 failed / 13 passed | 同一 `readchat` worktree 最小修复并保持不自动发送 / 不重复预填；不得新建worktree，提交 / 废弃后由总控同事件移除 |
