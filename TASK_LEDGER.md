...
- 用户逐点复核：当前内置浏览器 `902×904 @ DPR 1.8` 下，工具条响应式高度由 `52px` 收到 `48px`，连接行与工具条 / 书架分别保留 `12px / 22px`；导航选中面改为 `#D4E5DC`；桌宠素材约 `20px` 的缩放后右侧透明留白通过按钮负边距补偿，按钮本体仍为 `44px`。截图 `10-in-app-902x904-toolbar-companion.png` SHA-256 `a72676e3cd9e0e3fbc487daabc244ef54e234ff51b9e074d49fbffb13c00053c`，只证明这三处修正，不升级左栏候选状态。
...
- 自动证据：候选分支 `pnpm verify` 通过 13 个测试文件 / 42 项测试和生产构建；定向封面、图标与视觉合同回归 3 个文件 / 9 项通过；证据索引为 `redesign-v2/output/acceptance/m1-f2-a/README.md`。这些结果不替代待补的左栏视觉与真实窗口矩阵。
