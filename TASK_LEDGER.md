...
- 当前执行波次：`CONTROL-RESET-1`；真实主线为 `main@5d89aa5`，状态仅三个既有未跟踪视觉候选目录。Desktop `6afd@44e66af` 已获非作者 `PASS` 并合入 `main@deba325`；D-TEXT `dentry@df96ee3` 已合入 `main@9143638`，但独立 main Chrome FINAL 仍为 `VERIFY / BLOCKED`；E-OWNER `owner@9512c40` 已获非作者 `PASS`，完整链已以 `main@f1334ad` 集成并通过 main 定向 4 / 4、全仓 33 files / 187 tests、typecheck / build。Mini conversation / Reader / drawer 三条互斥泳道继续运行，台账仍由项目总控独占。
- 当前活动项：Mini conversation 已冻结为 `f312@cb4752f`，总控复跑 9 / 9 PASS；独立只读 `/root/verify_mini_conversation_final` 固定 clean detached `cvre@cb4752f` 执行 FINAL，协作状态 running，但恢复后一次有界等待仍无原样 ACK。Reader `/root/mini_reader_sheet_rework` 在 `rdrs@3fed4c7` 已形成 5 个 Reader 私有文件 `+362/-29` 的状态与可见 WXML/WXSS WIP，误触 `project.config.json` 已恢复；drawer `/root/mini_drawer_visual_rework` 在 `drwr@3fed4c7` 已形成 4 个私有文件 `+263/-52` 的可见 WIP，另有禁止纳入的 `pnpm-lock.yaml` importer 副作用待恢复。两 writer 经中断恢复后协作状态 running，但一次有界等待仍无原样 ACK；因此只登记 worktree 证据，不把 ACK PENDING 冒充 ACTIVE / EARLY PASS。
- 协调下一动作：conversation reviewer补 320 / 360 / 390 / 430、font23、safe-area、选择层 / 摘要 / 单一 PPT 入口 / 输入与趴宠的真实 DevTools verdict；Reader / drawer 分别取得原样 ACK、清除保护文件副作用、做临时 DevTools EARLY、冻结提交与非作者 FINAL。DevTools 最近项目以 f312 为唯一常驻入口；rdrs / drwr 只在 EARLY / FINAL 临时打开并随后移除，不删除 6f5c dirty 工作树或用户脚手架。`M1-F1-A` 已 READY，但当前 4 槽为总控 + 三条 Mini 泳道，精确未派发原因是并发槽为 0；任一完成即回收并释放下一槽。
- main 可访问入口收据：`main@5d89aa5` 使用隔离 schema `control_main_5d89aa5_1205` 与隔离产物目录启动，API `http://127.0.0.1:4127/api/v1/health/ready` 返回 `200 {"status":"ready"}`，Web `http://127.0.0.1:4198/` 与 `#/conversation` 返回 200，代理 health 同为 200。原 4173 仍是返回 404 的既有错误进程，不作为候选证据；4198 / 4127 仅恢复稳定入口，不替代 D-TEXT Chrome FINAL。
...
| ID | 状态 / 负责人 | 可独立交付结果与边界 | 依赖 / 阻塞 / 暂停 | 预计读写 | 验收与验证 | 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `READY / SLOT HELD` / 待分配 | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | `E-OWNER` 已以 `main@f1334ad` 冻结共享 owner 接缝；候选 `658ba50` 仅作种子。当前 4 槽为总控 + 三个互斥 Mini writer，下一槽释放即派发；账户迁移冲突时暂停 | CORE、API、DB、WEB、QA；不得与后续共享 schema / app 集成并写 | 密码摘要、Cookie、安全错误、刷新会话、跨账户隔离；4 视口、键盘和认可视觉 | 本地假邮件与确定性会话无需外部凭证；通过后释放 B |
...
| `M2-F3` 小程序会话 | `VERIFY / FROZEN cb4752f / NONAUTHOR FINAL RUNNING / ACK PENDING` / `/root/verify_mini_conversation_final` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | `f312@cb4752f` 只含 conversation 私有 6 文件；4 / 4 RED 已闭合为定向 9 / 9，旧常驻任务卡 / 双动作已删除，半屏选择层、摘要 / 修改、单一 PPT 阶段入口、本地恢复与失败保留已实现；writer 报 miniapp 78 / 78、typecheck / build / diff-check，总控定向 9 / 9 复跑通过 | reviewer 固定 clean detached `cvre@cb4752f`；协作状态 running，恢复后有界等待未回原样 ACK，故不得写视觉已开工 / PASS。须真实 DevTools 覆盖 320 / 360 / 390 / 430、font23、safe-area、选择层生命周期、摘要 / 单入口、1 / 3 / 5 行与附件、趴宠和正常文本扫描 | PASS 前不集成，不把测试或 f312 当前页面替代非作者视觉裁决 |
| `M2-F4` 小程序书架与阅读 | `READER VISUAL FAIL / DESIGN-READY / RUNNING BY WORKTREE / ACK PENDING` / `/root/mini_reader_sheet_rework` | 统一书架、导入、介绍首屏、自然连续正文、操作层、书籍内容可展开面板、浅 / 深背景与跨端恢复 | 独立 `rdrs@3fed4c7`；真实 WIP 为 Reader 私有 5 文件 `+362/-29`，已含状态逻辑及背景 / 内容 sheet 的 WXML/WXSS，可见实现不再缺失；误触 project config 已恢复。未取得 writer 原样 ACK、DevTools EARLY 或冻结提交 | 原样 ACK 后才升级 ACTIVE；继续补五态手势 / 状态保留 / 背景持久化 / 正常文本扫描，做 320 / 360 / 390 / 430、font23、safe-area、reduced-motion 临时 DevTools EARLY，关闭临时最近入口后冻结并交非作者 FINAL；软件键盘外部门单列 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / MAIN MERGED 9143638 / MAIN FINAL BLOCKED BY CHROME CONTROL` / 项目总控 | `dentry@df96ee3` clean；焦点最小修复已合入，main `pnpm verify` 为 32 files / 183 tests、typecheck / build PASS。reviewer确认 4110 ready、隔离 schema 有真实书、4173 API target 正确，且 main 后续只变台账 / owner 接缝，不回退 Web 实现 | 独立 Chrome 访问被 `ERR_BLOCKED_BY_CLIENT`，父 Chrome tab 被会话占用，故 1440 / 768、DOM 焦点、PPT 空态 / handoff、console / overflow 与截图全部未覆盖；收据在仓库外 `selfalone-dtext-main-review/README.md` | 释放或更换可控制的真实 Chrome 标签后重派同范围 FINAL；不得改用 in-app Browser、旧图或测试冒充 | 保持 VERIFY；原生 200%、真实删除、真实 AI / PPTX 仍精确 VERIFY |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / SYSTEM VISUAL REWORK / SIZE PASS 3fed4c7 / DESIGN-READY RETURNED / THREE WRITERS DISPATCHED / H3 BLOCKED / NOT IN MAIN` | 用户最新 430×932 证据否决 conversation + drawer 整体视觉；旧 Reviewer 的 Reader / Alpha / 四宽机械 PASS 只保留为局部证据。`3fed4c7` 仅把 runtime 从 865,820 B 降至 165,998 B，并以 13 files / 69 tests、typecheck / build、dist=1552 KiB 关闭机械包体门；保护 project config / lock 与旧 EARLY 图未纳入 | 替代只读 `/root/mini_visual_final` 已完成并回收；conversation / Reader / drawer 三个互斥 writer 已创建，原样 ACK 前不升级 ACTIVE。软件键盘、真实多会话列表、游客凭证 / console、真实 PPT preview 继续 H3 / VERIFY |
| `M2-F0-A-VISUAL-REWORK` | `FAIL / CONVERSATION FROZEN cb4752f FINAL RUNNING / READER+DRAWER RUNNING BY WORKTREE / ACK PENDING / IMG-04 ALPHA VERIFY / IMG-05 STRUCTURE-ONLY ADOPT` | conversation 固定 clean detached `cvre@cb4752f`；Reader `rdrs@3fed4c7`；drawer `drwr@3fed4c7`。三者文件互斥；当前 4 / 4 协作槽已满 | conversation 代码门绿但真实 DevTools 未裁决；Reader 5 个私有文件已有可见 WIP；drawer 4 个私有文件已有可见 WIP，但 `pnpm-lock.yaml` importer 副作用必须在冻结前恢复。两 writer / reviewer 恢复后均 running，一次有界等待仍无原样 ACK，故不升级 EARLY / FINAL。IMG-04 runtime Alpha / size可复用但整体 drawer 仍失败；IMG-05 只采纳结构关系 | 各候选分别取得原样 ACK→临时 DevTools EARLY→范围单一提交→独立 FINAL；430、真实键盘、长列表、reduced-motion 与未覆盖五态不得静默升级。任一槽释放后优先派 READY `M1-F1-A`，除非同一冻结 Mini 候选已到非作者 FINAL 门 |
