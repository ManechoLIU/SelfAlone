...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-CODEX-02 / NATIVE FALLBACK` | 账户隔离 AES-256-GCM 连接存储，以及账户内 requestId 幂等的书架 / 批注 run、单领取恢复、终态重放与 last-success 快照均已进入 main；两次 Grok 启动均已终止并核对为零代码写入，当前唯一 Writer 已从 `main@f072889` 在 `codex/m1-f2-weread-runtime` delivered ACK，owned files 仅 `weread-runtime.ts`、`weread-runtime.test.ts`、`index.ts` | 首个 RED=正式 Server 尚未注册 WeRead runtime；按 fake adapter API→DB→恢复合同形成范围单一候选后交总控审查。真实 Key H4仍是独立授权门，禁止读取凭证或调用真实 Provider |
