...
- 当前阻塞：Runtime 发布门已解除，不再阻塞新 Assignment。`M1-F5-B` 的真实联网、真实文本模型调用与预算仍分别受既有授权边界约束；当前先实现不产生外部调用的本地合同、持久化、假适配器与编辑/自动保存闭环。六份历史 terminal receipt 已读取并消费：四份临时结果文件已不存在且 delivery 均 unresolved，只作为进程终态；Grok A1 与 Kimi book-seam 的历史范围均已由后续 main 事实吸收，不重复调用。`M2-F5-A` 的 Kimi attempt 1 在源码零写入、`side_effect=false / result_unknown=false` 下失败，但 canonical terminal `evidence=[]`，无法满足自动 safe-fallback 的机器证据门，且不得自动重试付费 Kimi；该旁路包等待 Runtime 产生可追溯 failure evidence 或等价的受验证路由修复，不阻塞当前 Goal。
- 规则版本：总控已重新加载 `AGENTS.md@076fb8184bf737acd286b53949287f4496542cad`（SHA-256 `418f8865…0949`）；该提交收敛 Controller 会话接续 / Web 强身份规则并移除已验证修复的 AI-Bridge `exit_code=78` 只读临时兼容。Adaptive Agent Runtime 安装副本仍为已 ACK revision `78285b066e37decd0836799f1ce875a7d24eeb2b`（manifest SHA-256 `03405911…9de`），handshake=`current / blocking=false`；本次仅项目治理文本变化，不触发 Runtime release handshake。canonical `desktop_codex` target `01a0670a-a055-7761-837e-bce4b6a1e1f1@generation=2` 复核为 `VERIFIED / controller_actions_allowed=true`，继续复用同一唯一 Controller。
...
| `M1-F5-B` | `ACTIVE / SERVER CONTRACT` / `M1-F5-B-SERVER-OUTLINE-GROK-01` | 公开资料补全、大纲生成、连续分层编辑与自动保存；当前先冻结 Server 本地合同、持久化与假适配器 | Grok 4.6 OAuth Writer 已 canonical `assignment_started`，worktree=`m1-f5-b-server-outline`，严格禁止真实联网 / 模型调用；Server candidate PASS 后做非作者审查并冻结 outline API，再释放 Web 连续分层编辑与自动保存 |
...
- 当前 worktree 共6个：main、唯一 `f312@f6a08f9`、已吸收 seam `m1-f5-a-web-book-seam@cfc8588c7`、已吸收 EARLY `m1-f5-a-web-requirements-early@88ab9c8`、运行中的 Server 基线 `m1-f5-b-server-outline@e202e82` 与失败冻结的 Mini 基线 `m2-f5-a-mini-settings@189cb6e`。`e202e82`、`189cb6e` 都只含用于 Assignment 路由校验的 `AGENTS.md` 声明，不含产品实现，不得合入 main；分别保留到对应 Writer 产生真实候选或 Runtime fallback 阻塞解除。两条 M1-F5-A 候选均已进入 main 祖先链并禁止继续写入；EARLY worktree 仅余未跟踪 `redesign-v2/output/visual-review/m1-f5-a-early-07/` 证据，回收前须先按保护现场规则归档，不以删除清场。f312 继续作为唯一 Mini DevTools / visual-qa 现场，代码已由 `main@43f1fc3` 等价吸收，其本机 AppID 配置、lockfile workspace 登记与205份 visual-qa 文件继续保持未提交 / 不推送，QA 集合指纹 `5fbd83a5…6b41`。`quota-cost@f47a990` 的完整成本链已等价顺序进入 `main@1bb0b76`，clean worktree 已于2026-09-01回收，恢复分支 `codex/m1-f4b-cost-ledger` 保留，禁止重复集成。旧 `m1-f4c-server@08b86254` 的两文件局部 WIP 已被 `main@99ef0ac` 完整实现取代，现以稳定 stash 对象 `3840b17546b1e592e7aa78389a562c88cd532a10`（标题含原路径与 HEAD）归档后移除 worktree；依赖安装产物未归档。不得为 reviewer / 占位 / 纯消息任务新建长期 worktree。历史归档 stash 共13条。
