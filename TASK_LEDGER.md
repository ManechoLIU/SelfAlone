...
- 当前执行波次：`CONTROL-RESET-1`；Desktop 共享壳已冻结为 `6afd@9638d12`，Mini drawer / Reader 返工已冻结为 `f312@a0e98f9`，两者均等待同一非作者 UI/UX FINAL，不得因作者测试或截图直接合并。D-TEXT `d65ab3b` 已获非作者 `PASS WITH RECEIPT CAVEATS`，排在 Desktop 释放 `main.ts` 后做 Web 入口集成；不代表父项 DONE 或完整视觉 PASS。台账仍由项目总控独占。
- 当前活动项：UI/UX 任务 `01a03652-a5d4-76c1-b0fa-ff9303243664` 已确认 `active`，本轮同时审 Desktop `9638d12`、Mini `a0e98f9` 与 `IMG-04` 候选。两个实现 writer 已完成并释放对应文件所有权；D-TEXT reviewer 已完成并释放。下一 READY writer 是 `M1-F2-D-TEXT-WEB-ENTRY`，但必须等 Desktop FINAL 通过并由总控完成候选差异审查、顺序集成与 main 复验后再创建，禁止抢写 `main.ts`。
- 协调下一动作：UI/UX 返回逐项 verdict 后，总控先审 `0969c33..9638d12` 与当前 main 的实际差异 / 冲突；Desktop 通过才按审查所得的安全方式集成并复验 main。随后释放唯一 D-TEXT Web 入口 writer。Mini 只按 `ff4767b..a0e98f9` 完整候选链审查 / 集成，保护差异与 H3 阻塞不变；不得只摘末提交或把 `IMG-04` 候选当运行时资产。
...
| `M1-WEB-SHELL-01` | `VERIFY / FROZEN 9638d12 / NONAUTHOR FINAL ACTIVE / NOT IN MAIN` / `/root/desktop_shell_unify` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | `6afd@9638d12` 的末提交（父 `221d5bb`）为范围单一 5 文件 `+107/-98`：conversation / library / settings 复用 `renderDesktopRail` 单一 DOM / active / token；仅既有 `?? data/` 未跟踪。完整 `0969c33..9638d12` 为 5 提交 / 33 文件；对 main@`7850b6c` 三方试合并无文本冲突，但仍须 merge 后行为复验，禁止只摘末提交 | 作者收据：`pnpm verify` 137 tests、typecheck / build 通过；Chrome 1440 / 1200 / 1024 / 768×844 覆盖 fresh、切换、刷新、前后退、`?stage=`，单 rail、输入可见、无横溢、console 0。API 未监听，因此仅证明缓存 / 断网保留态；UI/UX 正在对当前 HEAD 做 FINAL。PASS 后由总控合入完整链，并在 main 以在线隔离 API 补真实路由 / 恢复复验，通过前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `VERIFY / VISUAL REWORK FROZEN a0e98f9 / NONAUTHOR FINAL ACTIVE / H3 BLOCKED / NOT IN MAIN` / UI/UX + `/root/mini_visual_rework` | `f312@a0e98f9` 的末提交（父 `f81b463`）冻结 13 文件 `+81/-30`；Reader 标题三档 `28/36`、`26/34`、`22/28`、`max-width:12em`，drawer 唯一列表滚动、纸白开放行、140px 无图 scenery slot。完整 `ff4767b..a0e98f9` 为 21 提交 / 223 个 `apps/miniapp/**` 文件；对 main@`7850b6c` 三方试合并无文本冲突，但必须按完整链集成 | 作者收据：定向 14 / 14、miniapp 65 / 65、typecheck / build / diff-check 与 DevTools 320 / 360 / 390 / 430 + font23 通过；同一 UI/UX 正在 FINAL。候选未包含工作树中 `pnpm-lock.yaml` 的 `apps/miniapp: {}` importer；总控集成时须由唯一 writer 独立重建这项最小锁文件接缝并验证 frozen lock，禁止夹带受保护脏 diff。正式山亭、软件键盘、游客凭证 / console、真实 PPT preview 与 H3 均未闭合 |
| `M2-F0-A-VISUAL-REWORK` | `VERIFY / FROZEN a0e98f9 / NONAUTHOR FINAL ACTIVE / ASSET CANDIDATE / RUNTIME BLOCKED` / `/root/mini_visual_rework` | 末提交只改 drawer / Reader 与 8 张候选收据；工作树另有两张旧 EARLY 图未入提交。保护哈希仍为 project config `bb02f467…`、lock `9a0d8a10…`。`IMG-04` 未实装，透明山亭仍只是候选 | UI/UX 同时裁决页面与 `IMG-04` placement / size / opacity / crop；若页面 PASS 但资产仍未通过，只关闭无图骨架，不得称 drawer 最终视觉完成。总控必须审 `ff4767b..a0e98f9` 完整链及 main 冲突，禁止只摘 `a0e98f9` 冒充小程序闭环 |
...
| `M1-WEB-SHELL-01` 当前候选 | `0969c33 … 221d5bb → 9638d12`，状态 `VERIFY / NONAUTHOR FINAL ACTIVE / NOT IN MAIN` | 完整链 5 提交 / 33 文件；对 main@`7850b6c` 三方试合并干净，但作者在线 API 证据缺失，仍须 merge 后 main 真实服务复验。不能以末提交 5 文件或缓存态截图替代完整集成。 |
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4 → a952a49 → f81b463 → a0e98f9`，状态 `VERIFY / NONAUTHOR FINAL ACTIVE / H3 BLOCKED / NOT IN MAIN` | 完整链 21 提交 / 223 个 `apps/miniapp/**` 文件，对 main@`7850b6c` 三方试合并干净；候选不含验证现场所需的 lock importer，集成时须单独重建并验证。保护 project config EOF 差异不得带入；正式山亭、软件键盘、PPT 资产 / 真实预览仍缺，`IMG-04` 未实装。 |
