...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已以 `6bcc4a4 → 99ef0ac` 顺序进入 current main并全门 PASS。`M1-F4-C-WEB-HANDOFF` 的 Kimi API 首次请求因不支持 medium thinking effort 在零 tracked 写入前明确终止，未重试；Adaptive Delivery 安全失败解析已切到当前桌面宿主 Codex Terra/medium，Assignment `M1-F4-C-WEB-HANDOFF-CODEX-01` 的 delivered ACK、lease 与 canonical runtime 已闭合，下一检查点为三类平台错误提示 RED 与同 requestId+noteIntent 重试证据。
...
| `M1-F4-C-WEB-HANDOFF` | `ACTIVE / CODEX FALLBACK / SERVER MAIN CONTRACT 99ef0ac` / owner=`/root/web_handoff_codex_fallback` | Kimi K3 API Assignment 在请求参数门返回已知 400，tracked 零写入且不重试；安全失败 resolver 选择 desktop Codex `gpt-5.6-terra/medium`，Assignment `M1-F4-C-WEB-HANDOFF-CODEX-01` delivered ACK + lease + runtime receipt=`...:1:1` 已闭合；唯一四文件边界为 conversation view/test/controller test/CSS | 先 RED 锁三类平台错误就近提示；controller failed-result 复验 draft/context 与同 requestId+text+noteIntent 重试，已有行为若 GREEN 不改生产 controller。随后最小实现、定向回归、候选即停；不碰 Mini/Server/contracts/main/ledger/依赖 |
