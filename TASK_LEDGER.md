...
- 当前 Goal：`M1-F2-B` EPUB / TXT 桌面阅读闭环收口；实施任务已用 `804f952` 闭合专注态 rail 的焦点 / 可访问性 P1，当前由原 `B-UX1` 非作者任务对完整专注焦点集合做最终复验，通过后再进入 main 集成。M2-F0-A 保持未合并 idle，不与 B 争用文件。
- 当前阶段：桌面 Web `M1-F2-A` 已在 main `DONE`；`M1-F2-B` 候选链 `04f03d2 → 47748fa → 535b9ec → 804f952` 已重新通过总控范围、代码、定向与全仓验证。候选保持 `VERIFY` 等待非作者复验；原生 200% 仍是独立视觉 `DONE` 门。M2-F0-A H3 继续 `BLOCKED`。
...
- 当前活动项：审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 已被唤醒，只读复验 `da89@804f952` 的完整专注焦点集合、rail / toolbar 可访问性、Selection / Range、Esc 恢复、浅深层次与溢出；不得修改实现、事实源、验收证据或台账。实现任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 保持 idle，候选工作树 clean；M2-F0-A 在 `f312@91934f1` idle。
- 协调下一动作：原 UI/UX 任务对 `804f952` 给出 PASS / VERIFY / FAIL 与明确集成建议；若无新阻断，总控审查候选全链并合入 main，再以唯一写入者补共享入口 / contracts / schema / Worker 接缝和 main 真实回归。原生 200% 未取得前不得标视觉 `DONE`。
...
| `M1-F2-B` | `VERIFY` / 实施任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` idle | EPUB / TXT 正文、目录、复制、全书位置、专注模式和浅 / 深阅读背景的桌面阅读闭环 | `804f952` 已以两文件最小提交使专注 rail 退出焦点、可访问树与命中并通过总控代码门；等待非作者复验，复验前不集成。原生 200% 继续阻塞视觉 `DONE` | `da89` / `codex/m1-f2-b` 工作树 clean；本轮只改 `text-reader.css` 与 `text-reader.test.ts`，共享入口 / 样式、contracts、schema、锁文件、事实源和台账未改 | 总控新跑 1 文件 / 19 项、全仓 17 文件 / 77 项、typecheck / build / diff check 通过；真实 Chrome收据覆盖浅 / 深完整 Tab 循环、a11y snapshot、命中与 Esc 恢复 | 恢复点 `804f952`；原 UI/UX 任务最终复验后由总控裁决集成 |
| `M1-F2-B-UX1` | `ACTIVE / VERIFY` / 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` | 已完成浅 / 深阅读私有侧栏、无固定桌宠、首帧、焦点、滚动、TXT、复制反馈与完整专注可聚焦集合的非作者合同；当前只读复验、不实现 | 复验固定 `804f952`；PDF 页面一致性只锁视觉合同、不冒充当前实现，原生 200% 缺口继续显式保留 | 审查任务保持只读；事实源由总控以 `8f40658` 同步，实施任务不再写入 | 完整审计浅 / 深专注中 rail / toolbar 的视觉、命中、Tab、accessibility，三控件、Selection / Range、scrollTop、Esc 恢复、console / overflow 与既有通过项 | 本轮返回 PASS / VERIFY / FAIL 和集成建议后立即 idle，不重复轮询 |
...
当前没有实现写入卡：B 实施任务已在 `da89@804f952` 干净停止并保持 idle；唯一活动卡是原 UI/UX 任务对该 revision 做只读最终复验，不拥有任何文件。共享入口、contracts、schema、锁文件、事实源和台账仍由总控独占；M2-F0-A `f312@91934f1` idle，C 原任务已停止。
...
| `M1-F2-B` 当前候选 | `44b81ea → 04f03d2 → 47748fa → 535b9ec → 804f952`，状态 `VERIFY / UIUX review active` | 最后 follow-up 仅两文件、`+12/-1`：专注 rail 增加 `visibility:hidden` 与 `pointer-events:none`，保留原地模式切换和既有 toolbar 收敛。总控新跑 1 文件 / 19 项、全仓 17 文件 / 77 项、typecheck / build / diff check 通过；原生 200% 继续阻塞视觉 `DONE`，非作者复验前不集成。 |
...
| 开发基线 | 本地 `main` 产品代码 @ `a306149`，台账治理父级 @ `4f8a6df` | A 已完整闭合；`44b81ea` 冻结 B / C 接缝，`589a22b` 纳入 C 安全首包，`8f40658` 固定 B 阅读私有外壳与交互规则，`a306149` 释放 B 实施。B 候选 `804f952` 仍在 `da89` 等待最终复验，M2 H3 保持阻塞；主线未跟踪视觉候选不得纳入台账提交。 |
...
| `M1-F2-B-UX1` 阅读视觉 / 交互 brief | `ACTIVE / VERIFY` | `535b9ec` 的 rail P1 已由 `804f952` 最小修正；原审查任务正对完整专注焦点集合做最终复验，原生 200% 仍为视觉 `DONE` 门 |
