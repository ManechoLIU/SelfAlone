...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：缺真实可编辑的阶段返回、刷新丢未提交大纲、需求摘要状态错误、模板未按双列、生成中缺当前页 16:9 骨架、失败态缺重新生成 / 修改大纲，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，用户可见 Web 入口待 Web shell 返修终验 / 集成后释放；小程序候选链已冻结 `4fc422f → 630c051` 两个局部恢复点，但整体仍为六页与共享层 `FAIL / SYSTEM REWORK`，f312 继续独占 `apps/miniapp/**`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 在 `6afd@0e18846` 继续独占本包 Web 文件，并已获准在确需真实阶段回退保存时顺序修改 M0 runtime / route 与测试；原非作者 UI/UX 任务当前 idle，待新冻结 HEAD 后重验。D-TEXT Web 入口等待 Web shell 所有权释放。小程序任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 保持 `ACTIVE`：`4fc422f` 已冻结 conversation / drawer 四尺寸视口基线，`630c051` 已冻结 Reader 回顶 / 重进提示生命周期，当前连续推进 login / library / settings 响应式基线；局部资产门和 f312 两个既有禁止差异保持。
- 协调下一动作：Web 作者关闭阶段返回 / 持久化、需求摘要、模板、生成骨架与失败动作后冻结新 HEAD，由原非作者按 02 / 04 / 05 / 06 / 33 逐阶段复验；通过后总控集成 / main 复验并立即释放 D-TEXT Web 唯一入口写入者。小程序继续 login / library / settings 与其余路由基线；只有依赖精确视觉阈值或资产的局部文件等待设计门。冻结全端候选后再按四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 矩阵终验。
...
| `M1-F2-D-TEXT` | `READY / WEB ENTRY QUEUED` / 待 6afd 释放共享入口 | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | contracts / migration / API / publisher 已在 main@`f85d86c` 复验；当前缺真实 reader 选区入口、详情内容页、持久化交互与 Chrome 复验，不能称闭环 | 等 `M1-WEB-SHELL-01` 冻结后，由唯一写入者接 `main.ts` / 共享 shell 与新的 annotation Web 私有模块；旧 `18c7522` 仅作已被共享实现替代的只读种子 | 真实 EPUB / TXT 选区→划线 / 想法 / 老己笔记 CRUD，失败保留、刷新恢复、owner / fileVersion 隔离、返回原文与绑定参考真实 Chrome 通过 | 共享接缝合并 `f85d86c`；main 定向 5 文件 / 19 项、全仓 26 文件 / 123 项通过；用户可见 Web 仍未开始，等待唯一入口所有权 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051` 已形成两个范围单一恢复点：首包冻结共享 viewport / keyboard tracker、conversation / drawer 四尺寸视口基线；次包仅含 4 个 Reader 源 / 测试与 3 张 320×568 DevTools 图，证明正文激活后提示退出、回到介绍顶部恢复、离开 / 重进同书重新出现。验证分别为 8 文件 / 43 项与 8 文件 / 44 项、typecheck / build / diff-check 通过；两者均不是最终视觉通过 | 当前连续推进 login / library / settings 与其余页面 / 状态矩阵。游客 console、真实软件键盘、系统字体、透明山亭及真实 PPT 预览继续阻塞对应门，锁文件与 project config 既有差异禁止触碰 |
...
| `M1-F2-D-TEXT` | `READY / WEB ENTRY QUEUED` / 待分配 | `d4b2@18c7522` 已被 main 共享实现替代，仅作只读种子；新 Web 文件可准备，`main.ts` / shared shell 等 6afd 冻结后再由唯一写入者接入 | 仍缺用户可见选区三动作、划线 / 想法 / 老己笔记 CRUD、保存失败保留与真实 Chrome；不得以已合入 API 冒充功能完成 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / TWO CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f` 已冻结 conversation / drawer 320 / 360 / 390 / 430 视口基线；`630c051` 已冻结 Reader swipe lifecycle，8 文件 / 44 项、typecheck / build / diff-check 与 3 张 320×568 DevTools 交互图通过。当前推进 login / library / settings；全路由 / 状态矩阵前不合并 |
| `M2-F0-A-H3-REWORK` | `ACTIVE / KEYBOARD H3 UNCOVERED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 与系统适配共用同一 f312 writer，先做不涉及视觉裁决的 H3；既有 `project.config.json` / 锁文件禁止触碰 | `4fc422f` 证明四宽视口、安全区和键盘覆盖不重复扣减的状态合同；`630c051` 只增加 Reader 语义区间与回顶 / 重进恢复。各截图 keyboard 仍为 0，软件键盘 / 字体、游客 console 与凭证化环境仍为真实外部门，H3 总门不降级 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | `4fc422f` 冻结 conversation / drawer 四尺寸视口基线；`630c051` 仅冻结 Reader swipe lifecycle。login / library / settings、其余宽高 / 状态 / 安全区 / 键盘 / 字体矩阵与最终视觉仍未完成。既有锁文件 / project config SHA 保持且未进入提交。 |
...
| `M1-F2-D-TEXT` 共享集成 | `263d3e2 → 18c7522` 私有种子；`94056be → ed95d95 → 5156b9c → main@f85d86c` | 共享 contracts / migration / API / publisher 已通过原非作者与 main 复验；私有种子不再单独合并。用户可见 Web 仍 `READY / QUEUED`，等待 shared entry 所有权。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051` 分别是 conversation / drawer 四尺寸视口和 Reader swipe lifecycle 的已提交恢复点；全小程序六页、抽屉、输入 / 键盘与面板的真实四宽四高和状态矩阵、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
