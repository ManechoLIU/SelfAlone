...
- 下一可见检查点：`M1-F4-V1` 的最小平台文本 runtime 接线已在 `main@b637023` 形成代码门：正式 Server 从独立部署侧平台凭证与计费快照创建 DeepSeek 平台模型，保留领取、成本预占 / 按供应商 usage 结算 / 失败释放与用户模型优先级，不扩图片模型、完整 PPT 生成或新基础设施。下一步只在具备独立平台凭证与已核验人民币微元价格快照后执行真实 Provider H4、成本流水与浏览器成功路径；缺少这些输入时继续 fail-closed 为 `PLATFORM_UNAVAILABLE`，不把 fake HTTP 测试冒充真实能力。
...
| `M1-F4-V1` | `VERIFY / MAIN b637023 / PLATFORM RUNTIME CODE GATE` / 项目总控 | 领取、AI / PPT消耗、失败恢复和并发硬上限 Case；正式 Server 已接入独立平台环境配置，fake HTTP 证明用户模型未配置且领取后按 reserve→provider usage→settle 顺序成功，部分 / 非法配置在启动前拒绝，缺失配置保持 unavailable | Server 全量38文件213项、typecheck、build、visual-contract与diff-check通过；当前4174返回200、4322未登录入口返回401。真实 Provider H4、真实成本流水、浏览器成功路径与并发硬上限仍须独立平台 Key、当次价格快照及调用授权，不得由本代码门代替 |
