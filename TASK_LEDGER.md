...
| `M1-F2-A` | `VERIFY` / 任务 `01a0339a-3299-7150-91d9-f95a5331c627` | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 候选 `codex/m1-f2-a` @ `353b4fd` 尚未合入主线且有未提交改动；两次远山候选均失败，`1440×1024`、真实长标题 Case、窄屏可访问性和非作者 UI/UX 结论未闭合 | CORE、API、WORKER、DB、WEB、QA；实现任务拥有候选代码 / 证据，台账文件只由项目总控写 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名异源、作者缺失、刷新、账户隔离；Web 4 视口对照书架参考 | 已核验 `1440×844`、`1200×648`、`1024×648`、`768×648` 截图尺寸 / 文件；总控在当前 dirty 候选独立运行 `pnpm verify`，13 个文件 / 49 项、类型检查与构建通过，不替代待补门禁；通过并合入 `main` 后释放 B、C |
...
| `M1-F2-A` 当前候选 | `codex/m1-f2-a` @ `353b4fd`，状态 `VERIFY` | 与 `main` @ `f4cf52c` 在 `f89a4b7` 后分叉；候选含功能 / 视觉提交及未提交代码、规范、验收图和旧台账 diff。不得把 dirty 工作树或 Agent 报告当作已合入结果；候选提交前后、合并前后均重验。 |
| 开发基线 | 本地 `main` @ `f4cf52c` | 主线当前另有其他任务的未跟踪视觉候选，必须保留且不得纳入台账提交。用户已授权候选通过门禁后本地合并；合并前仍须重验 HEAD / diff / 共享 PostgreSQL，并只纳入目标范围。 |
