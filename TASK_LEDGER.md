...
- 当前活动项：Mini 非作者 EARLY 已确认 Reader controls / hint、drawer 360 / 390 / 430 与 IMG-04 Alpha 通过；唯一视觉阻断为 320×568 山亭上缘侵入当前会话行，长列表滚动仍未覆盖。唯一 `/root/mini_visual_rework_fix` 只做短屏 slot 几何最小修正、补证并冻结；两项保护差异、旧 EARLY 图与 diagnostic 不得纳入。D-TEXT `48e5f66` 为 26 files clean candidate，真实四宽 / TXT / EPUB / CRUD / 409 收据已齐；200% 保持 VERIFY，chat / PPT 入口 fail-closed，非作者结论前不集成。
...
| `M2-F0-A-VISUAL-REWORK` | `VERIFY / REWORK ACTIVE / EARLY FAIL(320) / IMG-04 ALPHA PASS` / `/root/mini_visual_rework_fix` + `/root/mini_visual_final_review` | Reader controls / hint 与 drawer 360 / 390 / 430 已通过 EARLY；runtime `1024×737 RGBA`、SHA `875ebae56229a657a505d1071e76000078fd1250b4c0894b3f71ca7bb43d2e50`、无实底。320 scenery slot 上缘侵入 current row，唯一 writer 只修短屏几何，不再次生成 | 320 装饰必须低于 current row 且不影响列表滚动 / 命中；冻结新 HEAD 后同一 reviewer 做四宽 + font23 + safe-area FINAL。真实多会话长列表、软件键盘与 PPT preview 继续精确保留 UNCOVERED / H3，不因静态或开发态数据升级 |
