...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。Desktop Conversation 12枚候选已顺序合入`main@dea90d7`，并完成main真实Case FINAL PASS。Book Detail `bdport2@af1091e`的9文件preflight已加载，唯一writer恢复私有checkpoint；D-TEXT PPT handoff已派发，待tracked-clean恢复后建隔离树。Mini长列表候选`mlong@a9da90a`已进入唯一`f312@5878400`并通过117项、typecheck / build，真实DevTools EARLY因当前Mac锁屏精确暂停。当前构成：controller / integrator 1、implementation writer 1 ACTIVE、implementation dispatch 1 ACK PENDING、reviewer 0；Mini真实端门由总控待解锁续验。
- **下一用户可见检查点**：Book Detail交当前main私有Web / Server范围单一候选；D-TEXT handoff交当前书→对应workspace与banner无遮挡的可运行EARLY；Mac解锁后Mini长列表立即在唯一f312检查390 / 360 / 320与font23的首段、中段、末段、搜索与固定底栏。任一FAIL回原范围最小整改。
...
| `M1-F2-BOOK-DETAIL-VISUAL` | `ACTIVE` · `V2 PREFLIGHT LOADED / UNIQUE PRIVATE WRITER` / `/root/desktop_book_detail_main_port` | 保留Notes / Highlights / PPT视觉候选，先移植当前main所需book-scoped私有Server / Web checkpoint；不重做已过视觉、不冒充shared endpoint | 旧`bdport@62f2ed1`的shared与untracked WIP原样保留；`bdport2@af1091e`before clean，main endpoint当前404；9文件边界已加载，任何shared seam需求即停 | `book-detail-state/view/css/runtime{,.test}`与`book-presentation{,.test}`共9文件；禁止`main.ts` / `styles.css` / server `app.ts` / `index.ts` | refs07 / 08 / 10精确SHA已ACK；下一步定向回归→scoped private candidate→总控审diff，再顺序接shared / Chrome EARLY |
| `M1-F2-D-TEXT-PPT-HANDOFF-FIX` | `READY` · `DISPATCHED / PREFLIGHT PAUSED ON LEDGER DIFF` / `/root/desktop_conversation_focus_fix` | 当前书 PPT intent 必须驱动对应workspace或明确fail-closed；恢复滚动时banner不得被sticky header遮挡 | Auth / Settings / Conversation shared seam已释放；main当前唯一tracked diff为总控台账事件，提交后立即创建隔离树并回preflight | 预计`apps/web/src/main.ts`与必要私有`text-reader-ppt-handoff{,.test}.ts`；禁止`styles.css`、Book Detail、Server / contracts / domain | 当前书 / 固定seed不混淆，1440 / 768刷新恢复、banner可见、PPT空态 / handoff、console / overflow；下一步tracked clean→ACK / RED→唯一writer |
