...
- 当前执行波次：`DESKTOP-QUOTA-MAIN-H3-MINI-DRAWER-V5-REWORK-PPT-EARLY`（Desktop quota六文件只读验收、Drawer四文件writer、PPT六文件只读EARLY互斥）
- 当前活动项：`M1-F4-A`、`M2-F0-A-MINI-DRAWER-VISUAL`、`M2-F6-MINI-PPT-DRAFT-HANDOFF`
- 协调下一动作：回收Quota当前main非作者Chrome / DB结论、Drawer原writer一次性候选和PPT候选EARLY；任一候选 / verdict先到即开启独立集成或状态事件，不让Mini DevTools阻塞Desktop。
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@7901b42469f54230b94046ef1119bfb27745df52` 复核；main无tracked diff，既有9组受保护未跟踪路径仍在。总控与三条live任务均已加载 `adaptive-delivery` 正式提交 `9967bb8a492a905799c6b5692625c454acf2e8ef`；全局 `SKILL.md`、`long-task-governance.md`、`methods.md` SHA-256依次为 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`、`a0f6abe6298e9063a151b3527dbf0636a66598144701921f79d28506a5cf09c5`、`109f5fabddb39dad44dd238624842baa1c094111cbc4818ebd16b7225a021e59`。主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`；Mini PPT产品 / 视觉 / 技术基线为`c06ec53`。
- **活动实现 / 复验线**：Quota替补非作者 `/root/desktop_quota_current_main_review_v2` 已ACK `main@7901b42`、9组保护status和规则，正只读核对`ed489a8`六文件等价与当前4276 / 4212 / PostgreSQL真实Case；首个reviewer经两次follow-up与消息 / 线程 / 工具 / 工作树四证均无新增后以`previous_status=running`回收1次，未产生项目diff。Drawer原writer `/root/mini_drawer_rework_writer` 在`mdrv2@d829969`、clean、四文件ownership与ref11 / brief / 规则ACK后ACTIVE，当前先写机械RED，不写f312。PPT候选`mppt1@d8e1e7` clean、六文件、55 / 55 + typecheck / build绿，非作者 `/root/mini_ppt_handoff_early_review` 已ACK并只读EARLY。当前总控+三任务`4 / 4`、可用`0`。
- **READY 重算**：三条当前项均ACTIVE且文件 / 环境边界明确。DeepSeek `c346772`非作者安全 / UI EARLY、Drawer候选后f312顺序集成、PPT候选后f312顺序集成均为READY或待直接依赖；当前因`4 / 4`无槽，首个reviewer释放即按先到事件补派。Mini notes真实save / edit / delete在PPT writer已冻结后不再有writer文件冲突，但仍需独立API / session包；Desktop D-TEXT真实删除复验不依赖Mini，排入下一Desktop验收波次。
- **下一用户可见检查点**：Desktop当前main再次完成注册→领取→status焦点→约1.2秒textarea恢复→刷新隐藏及DB grant完整性；Mini Drawer先产一次受控候选，再在唯一f312按320、360、390、430实看并交EARLY；Mini PPT先给出“无需新卡片”或精确派生卡片的唯一EARLY结论。
...
- **当前可见入口 / revision**：主线为 `7901b42`；Web `127.0.0.1:4276` 与API `127.0.0.1:4212` 均监听且`/api/v1/health/live=200`，默认responder仍fail-closed。Mini唯一常驻DevTools路径仍为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、当前HEAD `cbb63fa`，7项保护status不变；v5 320 SELF FAIL现场冻结，隔离writer / reviewer均禁止新增入口或直接写f312。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `ACTIVE` | 既有`MAIN MERGED ed489a8 / NONAUTHOR + H3 PASS`内容未被后续main改动；替补非作者`/root/desktop_quota_current_main_review_v2`正对当前`7901b42`复核六文件等价并执行真实Chrome / DB Case；不含F4-B/C、模型或付费调用 | 六文件从`ed489a8`到当前main实际diff为空，既有收据可作为种子但不能代替本目标要求的当前main实看；共享DB若无法安全区分则FAIL CLOSED，不污染数据 | 只读`conversation-chat-directory{,.test}.ts`、`conversation-chat-quota{,.test}.ts`、`conversation-chat.css`、`main.ts`与现有收据；不写源码 / 台账 | 当前main须再次证明注册→领取、status→textarea焦点、约1.2秒消隐、reload隐藏、console / overflow及grant owner / orphan / FK；PASS后恢复`DONE / CURRENT MAIN REVERIFIED`，不扩大AI或成本闭环 |
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `ACTIVE` | writer已交`mppt1@d8e1e7`六文件clean候选，55 / 55、Mini typecheck / build、diff-check绿；非作者`/root/mini_ppt_handoff_early_review`正审实际diff、反例和预发送态UI派生，不把测试自报或无WXML变更冒充EARLY PASS | 候选须证明versioned draft/context、当前conversation / 无会话回退、书籍元数据、精确可编辑预填、无auto-send / auto-sheet / task，以及发送后activate与失败 / 刷新保留。参考方向明确，禁止ImageGen | 候选六文件只读；现有conversation WXML/WXSS与refs23–32仅用于`NO NEW CARD`或`DERIVED CARD REQUIRED`判断；reviewer禁止写入 / 新DevTools入口 | EARLY PASS才由总控审diff并顺序集成唯一f312；随后真实SELF与非作者视觉 / 行为门。EARLY FAIL则原writer按六文件范围返工，不扩大卡片或阶段页 |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `ACTIVE` | 原writer`/root/mini_drawer_rework_writer`在`mdrv2@d829969`、clean起步，已加载ref11 / 根因brief / `9967bb8`并取得四文件ownership；当前先以test RED锁定同一v5的Alpha / slot / mask合同，不写f312 | 一次最小返工保留`widthFix`与scenery在scroll-view外；同一v5前52% Alpha≤8、52–68%渐入，亭子结构Alpha≥128且320 ROI约44×40；slot bottom距nav 8–12px、mask延后显影。失败即停，禁止机械v6 / ImageGen | `mobile-drawer-landscape-transparent-cropped-v5.png`、nav-drawer `index.{wxml,wxss,test.ts}`；禁止package / config / lock / QA / 台账 / f312 | 候选先交多背景 / Alpha / 机械证据与范围单一提交；总控顺序进唯一f312后writer先320 SELF（第1/2行无景、第三行后显影、亭子可读、导航/点击不遮），PASS才扩360 / 390 / 430并交非作者EARLY |
