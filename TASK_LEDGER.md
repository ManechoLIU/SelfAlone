...
- 规则版本：总控已从真实 `main@46ab1fd` 加载当前 AGENTS（SHA-256 `0cf10094…d0d`）；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 安装副本已加载并机器 ACK 精确 revision `8cd58338c0e77dcb1dc2aa6328b58f2c6d3db6bb`（manifest SHA-256 `3e6a6dfe…ef105`）：总控完整重载 SKILL / agent-model-routing / external-agent-auth / agent-delivery contract / long-task governance / control-event / lifecycle / rule-handshake，规则与 runtime 以 Git common-dir 握手、五状态投影、Assignment 启动前 ACK/lease、证据进展指纹与 recovery budget 为准；外部 Kimi/Grok 可见状态卡仍只能由 `run_external_agent.mjs --render-status-card` 生成。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控；已有工作包不因规则升级重做或改派。
