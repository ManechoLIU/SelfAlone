...
| `M1-F2-WEREAD-RUNTIME` | `VERIFY / STORES MAIN d16159a+c252cc7 / CHILD CANDIDATE VERIFY` / 项目总控 | 账户隔离 AES-256-GCM 连接存储，以及账户内 requestId 幂等的书架 / 批注 run、单领取恢复、终态重放与 last-success 快照均已进入 main；失败 / 暂停保留上次完整快照，旧连接迟到结果受 fencing 拒绝 | `M1-F2-WEREAD-SYNC-REWORK@3497f57` 已形成两文件候选，待独立非作者 verdict 后再决定是否顺序进 main；随后把 validation、Gateway Adapter、stores、worker 与8个HTTP端点组成正式 runtime，并以 fake gateway 完成 API→DB→恢复 Case。真实 Key H4仍是独立授权门 |
| `M1-F2-WEREAD-SYNC-REWORK` | `VERIFY / CANDIDATE 3497f57 / ROOT DIFF + FOCUSED 6/6 PASS / NONAUTHOR PENDING` / 项目总控 | 两文件候选已关闭连接替换后错误返回旧连接书籍 / 批注快照，以及同语义失败对象键序变化导致终态重放冲突；总控已审实际累计 `+233/-5` diff并复跑定向 PostgreSQL 6/6 PASS，Writer 收据另有 Server 42文件230项 PASS | 前序 `2ed6139` 的 Grok 非作者审查未在有界窗口内持久化正式 verdict；后续 Codex Writer 已交 clean `3497f57`，其 runtime delivery PASS 不等于非作者审查。最终候选保持 VERIFY、不合 main；下一次独立审查须作为新控制事件显式派发 |
