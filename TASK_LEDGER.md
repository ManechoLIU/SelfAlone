...
- 当前 Goal：无；`M1-F2-A` 已获开发授权，在当前书架视觉与真实浏览器门禁通过并完成主线 / 环境预检后创建和激活。
...
- 协调下一动作：当前书架任务通过后，重验 `main` 当前 HEAD / diff、相关工作树和共享 PostgreSQL，再创建 `M1-F2-A` Goal、补齐负责人及文件所有权并改为 `ACTIVE`；后续按依赖顺序连续推进，派生页面与组件继承确认参考和视觉规范，每页完成前由非作者 UI/UX Agent 独立审查，普通视觉取舍不重复请求用户确认。
- 当前授权边界：已获本地连续开发授权，以及最多 20 次外部图片生成调用额度；每次调用前记录模型 / 提供方、次数或张数、尺寸、用途与输出目录，生成物先为 `CANDIDATE`，只有实装和真实浏览器对照通过后才能升级。产品或视觉事实源发生无法消解的实质冲突、重大功能 / 信息架构 / 核心流程变化、大范围返工选择，以及超出 20 次额度、其他付费或外部服务与凭证、合并、push、PR、发布、部署、删除及其他难恢复动作仍须单独确认。
...
当前没有正式 `ACTIVE` 工作包；书架页面视觉与真实浏览器门禁正在任务 `01a0339a-3299-7150-91d9-f95a5331c627` 收口。用户已授权门禁通过后连续进入开发。下一张执行卡是 `M1-F2-A`：先固定 `codex/m1-f2-preflight` 的 HEAD / diff，登记隔离 PostgreSQL schema、端口与产物目录，再补齐负责人、文件所有权、回退点和真实文件验收矩阵并改为 `ACTIVE`。后续按依赖顺序推进；视觉偏差保持 `VERIFY`，局部素材问题只阻塞自身。
