...
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@ed489a805f98ae9eea5a9636eb01a2b7c9e524d6` 复核；跟踪文件干净且 9 组受保护未跟踪路径仍在。主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。
- **活动实现 / 复验线**：Desktop quota候选 `1d7d8b7` 已经非作者审查并顺序进入 `main@ed489a8`，main定向、typecheck、build、真实 Chrome领取 / 焦点 / reload与当前schema DB grant约束通过，writer / reviewer已释放。Mini `f312@23d6371` 的320同态经writer与非作者均判 `EARLY FAIL`；原writer `/root/mini_drawer_rework_writer` 已对 `mdrv2@d17bf52` clean现场、三源码 + 新v3 asset、旧 `scaleToFill` RED和首个runnable diff停止门给出delivered ACK，现 `ACTIVE REWORK`。Responder原候选 `10287b2` 的echo/canned Important已返工为 `chatresp@5982699`，定向25 / Server127、typecheck / build / diff-check通过，现 `READY FOR REVIEW`、未进main。controller + Mini writer为 `2 / 4`、可用 `2`；历史握手阶段曾对 `previous_status=running` 的 Mini / responder writer执行 `interrupt` 共 `2` 次、随后均在原任务恢复且候选结果未丢失，另对 Desktop reviewer握手执行 `interrupt` `1` 次、审查结果未丢失，不能记“误中断0”。
- **READY 重算**：`M1-F4-A` 已关闭，原 `main.ts` quota接缝冻结；当前 Desktop线的下一事件是总控审查 `5982699`、必要非作者复核并独占 `index.ts` 顺序组合，Mini线为已ACK的同四文件返工。其余可用槽位本回合不补派：responder候选刚回收，按短事件合同须先形成调度提交并yield，不能在同一turn继续集成；其他开放项分别仍受 `M1-F1-V1`、对象存储 / 模型目录、完整 `M1-F2-D`、平台成本适配器或唯一 DevTools互斥门约束，不创建重复writer。四个历史 recent-project入口清理已 `CLOSED`，只保留 f312。
- **下一用户可见检查点**：Desktop quota已可在current main真实领取并在reload后保持隐藏；下一事件审查 / 集成 responder非回显 development fake与fail-closed合同并做main API / Chrome Case（仍不称自由聊天）。Mini下一checkpoint是三源码 + v3资产首个runnable候选，由总控顺序放入唯一f312后只先做320 writer自验与新EARLY。
- **真实阻塞 / 授权门**：当前 main仍未接真实 text-model responder，用户不能自由聊天。用户已授权仅在 SelfAlone 当前开发 / 验收中做最小真实对话 Case，但服务商、模型与 Key仍 `credential pending`；执行前必须明确服务商、模型、预计调用次数和成本边界，Key不得要求贴入聊天，也不得进入 Git、命令参数、日志、截图或台账，优先产品内加密配置，未完成前只可用不回显本机输入 / 临时环境注入。此门不阻塞 responder contract、fake与无凭证 fail-closed开发；平台免费能力仍需 model adapter + cost ledger。远端、发布、真实邮件 / 微信 / Presenton与其他付费调用仍未授权。
- **当前可见入口 / revision**：事件复核主线为 `ed489a8`；主线 Web `127.0.0.1:4276` PID `4051` 通过代理连接 API `127.0.0.1:4212` PID `4652`，schema `selection_main_c858a06_20260826`，PostgreSQL `55432` 与两端 ready在线。当前入口真实完成 quota领取、状态焦点、约1.2秒消隐、reload持久隐藏；对话仍非真实模型自由聊天。Mini唯一活动 DevTools路径为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、源码 HEAD `23d6371`；recent-project只保留 f312，18项 query与保护 status未变。
...
| `M1-F2` 桌面书库、阅读与笔记 | `ACTIVE` | 本地书导入、阅读、记录、详情与账户隔离 | A仅功能完成；B `VERIFY`，C半成品；Book Detail与D-TEXT handoff scoped FINAL，D-PDF仍开放，E-OWNER已完成 |
...
| `M1-F3B` 桌面对话与 AI | `ACTIVE` | 会话、图文消息、选择与书籍上下文 | A已main FINAL；C私有候选进入实际diff审查；B、D仍按各自依赖 |
...
| `M1-F3A-V1` | `PENDING` / 待分配 | 完成模型配置本地闭环，并为至少一家真实文本模型取得脱敏 H4 收据 | A、B；用户已授权SelfAlone最小真实对话验收，但服务商 / 模型 / Key仍pending；调用前须声明次数与成本边界 | QA、DOC | 假适配器证明保存 / 失败 / 替换 / 撤销；真实Key只经产品内加密配置或不回显本机注入，禁止进入聊天 / Git / 命令参数 / 日志 / 截图 / 台账；H4仅记供应商、模型、次数和结果；本地门可先释放F3B |
...
| `M1-F3B-A2-RESPONDER-CONTRACT` | `CANDIDATE` · `5982699 / READY FOR REVIEW` / 项目总控 | 以显式 development fake证明完整顺序上下文进入窄 `ChatResponderPort`；无配置 / 空回复 / 调用失败必须失败关闭并复用草稿保留；不实现真实供应商、凭证、成本或额度 | 非作者对 `10287b2` 的echo/canned Important已由原writer修复：不再逐字回显，改用数量 / 角色计数 / SHA-256指纹的确定性派生摘要；仍不能替代 `M1-F3A-A/V1`、成本账本或真实自由聊天 | `chatresp@5982699`只含 `conversation-responder{,.test}.ts`、`conversation-store{,.test}.ts`；`index.ts`仍由总控顺序接线 | 旧echo断言RED→GREEN；定向5文件25项、Server29文件127项、typecheck / build / diff-check通过，worktree clean；下一事件先审实际diff / 必要非作者复核，再集成候选与main组合，禁止默认启用development fake |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `f312 23d6371 / UNIQUE ENTRY / DRAWER NONAUTHOR EARLY FAIL → ACTIVE REWORK` / 项目总控 + `/root/mini_drawer_rework_writer` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；320同态确认山水侵入第2/3行、底栏前山亭不可辨，不能以Alpha或测试绿灯关闭；不冒充完整Mini闭环 | `d17bf52`已顺序放入唯一f312，writer SELF FAIL后非作者在首宽判硬FAIL并停止扩宽；原writer对 `mdrv2@d17bf52` clean、三源码 + 新v3 asset、首个RED与移交门delivered ACK已闭合；recent-project清理`CLOSED`且只剩f312 | 禁止`scaleToFill`；比例保持、nav/safe-area派生slot、连续上缘渐隐、三背景合成和320前两行无景先形成runnable checkpoint，再由总控顺序放入唯一f312做writer自验与新EARLY；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `ACTIVE REWORK` · `f312 23d6371 / NONAUTHOR EARLY FAIL / mdrv2 ACK` / `/root/mini_drawer_rework_writer` | ref11与Drawer专用山亭asset的同态返工；320真实DevTools确认 `scaleToFill` 把1024×737资产压入120px slot，Alpha≥96约落在slot 28.7px，山水侵入第2/3行且山亭缺席 | 旧 `hasAlpha` 与整页PASS无效；reviewer在320硬FAIL后未扩360/390/430。writer独占nav-drawer test / WXML / WXSS + 新 `mobile-drawer-landscape-transparent-cropped-v3.png`，不写f312 / dist / config / lock / QA | delivered ACK：`mdrv2@d17bf52` clean，首个RED为旧WXML仍含 `mode="scaleToFill"`；`mini-drawer-early-review-23d6371/EARLY-VERDICT.md`绑定。先交runnable checkpoint再由总控回唯一f312，禁止writer自行导入或扩宽 |
...
| `M2-F0-A` 当前候选 | main已含`35bad75`长列表与`14b69dc` Settings；Drawer三源码候选`d17bf52`已顺序放入唯一`f312@23d6371`，现用v2 asset SHA `87cf9828…8876` | Settings / 长列表旧 scoped FINAL仍`FAIL / NO-GO`。recent-project清理`CLOSED`且唯一f312；320 writer SELF FAIL与非作者 EARLY FAIL均成立，已回原writer做三源码 + 新versioned asset最小返工，保护config / lock与既有QA不动。 |
...
| 当前 Mini 候选波次 | Reader / Conversation既有限定结论保持；长列表`35bad75`与Settings`14b69dc`已在main但scoped FINAL为NO-GO；Drawer `d17bf52`在唯一`f312@23d6371`，320非作者 EARLY FAIL | recent-project清理`CLOSED`、原生菜单唯一、query 18项已送达；reviewer已给5条可实现关系并停止扩宽，原writer回mdrv2做同范围返工，保护现场与历史源码/worktree均未删除。 |
| 当前 Desktop main 预览 | `RUNNING / EVENT BASELINE ed489a8 / QUOTA H3 PASS / FREE CHAT NOT WIRED` | Web `127.0.0.1:4276` PID `4051`、API `127.0.0.1:4212` PID `4652`；schema `selection_main_c858a06_20260826`。quota真实领取 / focus / reload / DB grant已通过；Server main仍未注入text-model responder，非回显返工候选`chatresp@5982699`待下一事件审查 / 顺序集成。 |
| 开发基线 | 本台账事件父为`main@ed489a805f98ae9eea5a9636eb01a2b7c9e524d6`；本轮只写`TASK_LEDGER.md` | 6个受保护DOCX / Word锁文件与3组受保护视觉候选目录仍与冻结a233副本同字节，禁止暂存 / 修改 / 删除；Mini唯一f312的2个保护修改与5组既有QA未改变。 |
...
- 当前外部授权门：验证合格后的本地 `main` 合并已授权；用户另已授权SelfAlone当前开发 / 验收的最小真实文本模型对话Case，但服务商、模型、预计调用次数 / 成本与安全Key录入未闭合前不得调用，也不扩展为生产、其他项目或无限调用。远端 push、PR、发布、部署、真实邮件 / 微信、微信读书 / Presenton及其他付费调用和小程序审核均未授权。外部图片生成仅限已授权的20次总额度并受上表逐次记录约束。
