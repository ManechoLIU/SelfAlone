...
- 当前执行波次：`CONTROL-RESET-1`；Web `47db65b` 的非作者终验已判 `FAIL / REWORK`：精确 1024px 断点使右工作区覆盖中栏约 96.7px、生成中锁定文案错误、消息 / 输入 / 右栏密度与纸白表面偏离 02；作者侧功能 / 路由证据仍保留但不能覆盖三个 P1，现已退回原作者。D-TEXT 私有 Web返工已扩为 13 个私有文件；总控定向 41 项与全仓 30 files / 145 tests、typecheck / build / diff-check 全绿，仍需同一真实 Chrome Case 与提交。小程序已冻结 `79c9d8b` PPT failure checkpoint，父项保持 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 为 D-TEXT 私有唯一写入者，状态 `ACTIVE / FULL VERIFY GREEN / REAL CHROME PENDING`，共享入口继续排队。原 Web 作者 `/root/desktop_shell_m0_fix` 已收到三个 P1 的限定返工卡，当前 `DISPATCHED / ACK PENDING`；非作者已清理 Web 端口 / schema。小程序同一 writer 已提交 `79c9d8b`，总控复核 8 文件、保护哈希、11 files / 61 tests 与两张 DevTools 收据通过；它只是第十二个局部恢复点，UI/UX 保持 `QUEUED / CANDIDATE MOVING`。两个受保护差异保持未提交。
- 协调下一动作：收取 Web 作者失败测试 / 文件 ACK 后升级返工 ACTIVE；D-TEXT 完成真实 Chrome 与 scoped commit。小程序继续完成剩余系统矩阵；只有同一 writer 明确冻结完整候选并回传唯一 clean scoped HEAD 后，总控才向 UI/UX 释放全端四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验；禁止混用 `598e34f` 与后续链证据。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b` 已形成十二个局部恢复点；`79c9d8b` 仅关闭 PPT later-load failure 保留 workspace / draft / stage / previews 与同工作区 retry | 总控核对 8 文件范围、保护哈希、commit diff-check、11 files / 61 tests 与两张 360×640 DevTools 收据；真实端仅覆盖 failure→retry，PPT 预览仍为开发适配器且不冒充真实产物。drawer 正式山亭仍 ASSET BLOCKED，软件键盘仍为 0；父项与 H3 均不升级 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / TWELVE CHECKPOINTS COMMITTED / DESIGN-READY / STATE-MATRIX REWORK / PPT FAILURE CHECKPOINT / REVERIFY QUEUED / CANDIDATE MOVING` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `79c9d8b` 经总控复核，只关闭 360×640 PPT failure→retry 局部门；它不是完整系统冻结候选。旧 `598e34f` 已过期，UI/UX 保持 `QUEUED / CANDIDATE MOVING`，不得混用快照；待完整 clean scoped HEAD 返回后由总控明确最新 revision。软件键盘仍未实显，正式山亭缺失；全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 十二个局部点已冻结；`79c9d8b` 只关闭 PPT later-load failure 稳定 workspace / context 与 retry。软件键盘、正式山亭 / PPT 资产、其余状态与最终全路由视觉均未完成；既有受保护差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b` 是十二个局部恢复点；Reader / PPT failure checkpoints 不替代全小程序六页 / 共享层四宽四高和状态矩阵，真实软件键盘、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
