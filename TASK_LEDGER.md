...
- 当前 Goal：`M1-F5-A / BOOK-TO-PPT REQUIREMENTS ENTRY`。关闭书籍 / 当前会话到 PPT 需求工作区的用户入口：书籍 CTA 先进入当前会话并给出可编辑 `帮我制作这本书PPT`，仅在用户消息成功持久化后创建 / 复用单书 workspace，再释放桌面需求面板；`M1-F4-V1` 保持外部 H4 等待项，不再把其外部阻塞映射为整个系统 Goal blocked。
- 下一可见检查点：`M1-F5-A-WEB-REQUIREMENTS-EARLY-07` 已形成未提交的9文件最小 GREEN，聚焦28/28、Web310/310、typecheck、visual-contract与diff-check通过；Writer真实 Chrome 只到达 `#/auth`，且其预期API `127.0.0.1:4100` 未监听，故未形成候选、截图或EARLY结论。当前由项目总控核对实际diff并恢复不触碰既有4174/4322现场的隔离身份/API路径；只有真实send→持久化→create/reuse→原子右栏与四宽通过后才冻结候选并派非作者EARLY，不扩到GET / 保存、大纲、模板、生成或Server / Mini。
- 当前阻塞：当前 Goal 无已确认的外部阻塞；`M1-F5-A-WEB-REQUIREMENTS-EARLY-07` 暂处内部恢复态，精确缺口是隔离Web未取得真实登录态且其目标API端口未启动，尚不能把测试GREEN升级为EARLY。`M1-F4-V1` 的本地实现与价格事实已清空，但真实成功路径仍缺独立平台 Key 与本次真实平台调用次数 / 金额上限；既有用户模型 Key 授权不静默扩展为平台凭证。其他既有外部门保持原边界：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal 未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
- 规则版本：总控已从真实 `main@018e084` 加载当前 AGENTS（SHA-256 `a850d7ba…c1b`），其 Fact-First 与 host-aware fallback 要求先核当前事实、证据不足保持未知，并使 Grok 安全失败后可先路由到当前宿主内部 GPT，不重置 execution lineage / recovery budget；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。Adaptive Agent Runtime 安装副本已完整加载并由 logical controller `01a03c61-5dd2-7553-968b-a3bc2f5777c9` 机器 ACK 精确 revision `0b79454b708508ceb9df6118fe63d2332d1684cf`（manifest SHA-256 `11164e07…a7b7`；`SKILL.md` SHA-256 `f1433e30…c061`；`control_event_guard.py` SHA-256 `874a1523…8c43`）。当前唯一 `desktop_codex` execution target 是本任务 `01a05c4b-5417-79e1-8b87-2b43c36068f4`、generation=`1`；旧 logical controller task 仅保留 lineage，向其 message / navigate / native resume 均 fail closed。registered controller 收据仍须使用 Hook 的同一解释器、安装副本内精确 Guard 与匹配当前 turn 的 machine trace；桌面 Hook 在完成 Codex 整体重启、exact definitions trusted 与该 revision 实机 Canary 前仍为 `degraded`，不把安装 / ACK 冒充宿主级完全强制，也不创建第二总控。
...
| `M1-F5-A` | `ACTIVE / SEAM MAIN a689a9b / DESKTOP UI RECOVERY` / 项目总控 | 书籍 CTA→当前会话可编辑 `帮我制作这本书PPT`→仅成功持久化的正向意图创建 / 复用单书 workspace 的十文件接缝已闭合 current-main 代码门；发送失败、否定 / 非意图、无 book context 均保持零 workspace 调用 | 当前只代表 state / controller / client 接缝，不代表可见右栏或真实用户入口。`M1-F5-A-WEB-REQUIREMENTS-EARLY-07` 已到测试GREEN但真实端被隔离身份/API路径阻断，项目总控正恢复后续EARLY；不扩 Server、Mini、大纲、模板或生成 |
...
| `M1-F5-A-WEB-REQUIREMENTS-EARLY-07` | `RECOVERING / TEST GREEN / REAL-END BLOCKED` / 项目总控 | 隔离工作树 `m1-f5-a-web-requirements-early@49fe0ce` 保留9个授权文件未提交diff；原Writer完成真实RED与聚焦28/28、Web310/310、typecheck、visual-contract、diff-check GREEN；项目总控已完成9文件只读复审验证，Critical/Important/Minor=`0/0/1`。Chrome `4173` 仅落到 `#/auth`，`4100` API拒绝连接，故无候选提交、截图或EARLY结论 | 当前继续恢复隔离身份/API路径，并补齐无book / 非正向零调用与失败后同requestId重试两项回归测试；Kimi前序失败已terminal、`result_unknown=false`、零源码写入且不重试。恢复后先由总控完成真实send→持久化→create/reuse→原子右栏、失败同requestId重试、普通会话零PPT调用与768/1024/1200/1440，再冻结候选并派非作者EARLY；不扩GET / 保存、大纲 / 模板 / 生成、Server / Mini |
