...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` Checkpoint B 已解除旧路由授权阻塞并进入 `ACTIVE`；Checkpoint A 保持 controller-verified `20/20 PASS`，不重做。旧 native Assignment `M1-F4-C-SERVER-GATE-B-CODEX-01` 已因宿主在总控窗口切换时卸载 spawned Agent 而 `disconnected / zero B writes` 正式终止；恢复子包 `M1-F4-C-SERVER-GATE-B-CODEX-02` 已完成 ACK-only、assignment lease 与 canonical runtime `assignment_started`，现转 `ACTIVE`；继承同一 Checkpoint B 策略与工作树，下一步直接执行 bounded RED。WEB-HANDOFF 继续等待 server contract 冻结。
- 当前阻塞：`M1-F4-C-SERVER-GATE` 无执行路由/授权阻塞。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
- 规则版本：总控已从真实 `main@207631b` 加载当前 AGENTS（SHA-256 `5cf99259…56582`），其 host-aware fallback 使 Grok 安全失败后可先路由到当前宿主内部 GPT，不重置 execution lineage / recovery budget；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 安装副本已加载并机器 ACK 精确 revision `12fffb193c841ca55ba3a1b51648529809f18623`（manifest SHA-256 `70f50c09…c6eb3`），当前规则与 runtime 以 Git common-dir 握手、五状态投影、Assignment 启动前 ACK/lease、证据进展指纹与 recovery budget 为准。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控。
...
| `M1-F4-C-SERVER-GATE` | `VERIFY / CHECKPOINT A ACCEPTED / CHECKPOINT B RECOVERY ACTIVE / HEAD 929d1aa` / 项目总控 | Checkpoint A accepted patch SHA=`f97579b…d9f` 保持 controller-verified conversation-store `20/20 PASS`；旧 `B-CODEX-01` 已以 host-disconnected、zero B writes 终止，Grok 越界草稿已回收 | 当前由显式恢复子包 `M1-F4-C-SERVER-GATE-B-CODEX-02` 接续同一工作树与策略；ACK/lease/runtime 已闭合，执行 bounded RED→GREEN；候选后必须非作者 TDD causal review、Integration Gate 与 current-main 验证 |
...
| `M1-F4-C-SERVER-GATE-B-CODEX-02` | `ACTIVE / CODEX NATIVE RECOVERY / HEAD 929d1aa` / owner=`codex-writer-b02` / session=`01a052c0-335a-7e52-97b9-589c3758d341` | ACK-only 已核 worktree/branch/HEAD/status 与 8-file ownership；assignment lease `allowed`，canonical runtime `assignment_started` 已写入，execution lineage=`c7bf5a0b…6235`、recovery_count=1；Checkpoint A 两文件 staged WIP 只继承不重做 | bounded RED → 最小实现 GREEN；禁止重做 A/第9文件/依赖安装/live paid provider call/TASK_LEDGER 或 main 写入；候选前停交总控 |
