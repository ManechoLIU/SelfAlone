...
- **活动实现线**：Settings `6c81a26` 已通过总控非作者 EARLY并冻结 `READY FOR FINAL`；Conversation `b8f34b4` 与 Auth `b7a6894` 已派发交叉 EARLY。Mini Reader 修复 `2c99ab3` 经总控审查后已顺序进入唯一 `f312@db40e99`，原 writer 已派发真实 DevTools重验；三个执行任务均待 exact preflight，未虚报 ACTIVE。
- **下一用户可见检查点**：先取得 Auth / Conversation 的真实 Chrome首门与 `f312@db40e99` Reader 的 390 + 320 精确失败 Case verdict；Auth PASS 后按 Auth → Settings 顺序合入本地 main，Conversation PASS 后再适配该 main；Reader 两首门 PASS 才扩360 / 430。
- **真实阻塞**：无全局串行阻塞。Settings 本范围已解除 P1，但集成仍依赖 Auth EARLY；Conversation `b8f34b4` 未经非作者真实端 verdict；Mini Reader 全屏 sheet 覆盖 refs 18～20 明确保留的共享底栏，修复未冻结。Mini Conversation 的软件键盘 / reduced-motion、Book Detail shared route seam及真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未闭合或未授权。
- **当前可见入口 / revision**：Settings 候选入口 `http://127.0.0.1:4299/` → `http://127.0.0.1:5178/#/settings`，revision `6c81a26`，API `4298` / Web `5178` / bootstrap `4299` 绑定隔离 schema `settings1_early_20260825`；真实 EARLY 收据 `settings-early-6c81a26/`。稳定 main Web `http://127.0.0.1:4127/` / API `http://127.0.0.1:4198/api/v1/workspace` 仍绑定旧产品 revision `ab13310`，只作恢复点；Mini 唯一 DevTools 项目仍为 `f312@891d26e`。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `VERIFY` · `CANDIDATE b8f34b4 / NONAUTHOR EARLY DISPATCHED / PREFLIGHT PENDING` / `/root/book_detail_visual` | 输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话 UI；非目标：真实模型、图片消息、Settings / Auth UI。两项 P1 已由原 writer 在 view / test 内修复 | shared seam 继续冻结；reviewer 只读并使用隔离端口 / schema；未经真实逐键 / IME 与无任务三栏 verdict 不升级 | 22 文件候选链；修复仅 `conversation-chat-view.ts/.test.ts` | 作者 219 项 / build / Chrome 只作候选收据；执行者回 preflight 后做 1440 首门，PASS 才扩 1200 / 1024 / 768与 reduced-motion |
...
| `M1-F1-A-AUTH-EARLY` | `VERIFY` · `CANDIDATE b7a6894 / NONAUTHOR DISPATCHED / PREFLIGHT PENDING` / `/root/desktop_conversation_slice` | `auth1@b7a6894` clean；仅四个认证 UI 文件；reviewer 只读 | 隔离端口 / schema；首条 P1/P0 停止，真实邮件 / 微信不执行 | 绑定 refs 01 / 02；默认登录、扫码弹窗、邮箱切换、失败保留、四宽、console / overflow / reduced-motion | exact preflight 后升级 ACTIVE；PASS 冻结 FINAL，FAIL 原 writer原范围返工 |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `VERIFY` · `CANDIDATE b8f34b4 / NONAUTHOR DISPATCHED / PREFLIGHT PENDING` / `/root/book_detail_visual` | 输入、发送、确定性回复、刷新恢复、错误保留、统一对话 UI；非目标：真实模型 / 图片 / Settings | shared seam 冻结；reviewer 隔离运行且只读 | 22 文件链；修复仅 `conversation-chat-view.ts/.test.ts` | 绑定 02 / 36；exact preflight 后做逐键 / IME / Enter / Shift+Enter / send / refresh / failure / 三栏，PASS 后适配 Settings main |
...
| 当前桌面候选波次 | Settings `6c81a26` EARLY PASS / READY FOR FINAL；Conversation `b8f34b4` 与 Auth `b7a6894` 已派交叉 EARLY；Book Detail `75e764c + 6c50a76` 私有候选冻结 | Settings 证据 `settings-early-6c81a26/`；Auth→Settings→Conversation shared seam 顺序不变。各候选未合入 main、不得写 DONE。 |
...
| 开发基线 | 台账事件写入前 `main@1fdf9b4`；所有当前产品候选未合入 main | main tracked clean，三个既有未跟踪视觉目录原样保留；候选提交、测试、作者自验或 EARLY PASS 不能自动 `DONE`。 |
