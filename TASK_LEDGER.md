...
- 当前阶段：桌面 Web `M1-F2-A` 功能、回归和主线集成收据继续有效；独立 UI/UX 首轮审查已以真实 `1440×844 @ DPR2` 页面判定 `FAIL`，范围内视觉 / 交互修正已交唯一实施任务。文本 / PDF 阅读共享接缝已在 `44b81ea` 冻结；B 非视觉首包已到可恢复候选，侧栏主题联动单独等待 UI/UX brief，C 与小程序客户端工作按各自门继续。
...
- 当前执行波次：`M1-F2-A-VISUAL-REOPEN`、`M1-F2-B-UX1` 与 `M2-F0-MINIAPP-SHELL` 并行；B 实现候选在等待 brief 期间保持 idle 可恢复，C 安全首包已完成集成并停止原任务。活动项文件范围互斥，共享 contracts / schema / 入口由项目总控单写入。
- 当前活动项：A 的唯一实施任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 在 `59ae` / `codex/m1-f2-a-visual-reopen` 从 `2298f63` 开始，独占书架 `main.ts` / `styles.css`、必要状态 / DOM 测试和 A 新证据。审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 现以只读方式形成 B 侧栏主题 brief，待 A 候选提交后仍由其复验 A。B 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` 保留五文件 `AM` 恢复点并已释放临时端口 / schema，不再轮询；M2-F0-A 由任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 独占 `apps/miniapp/**`。所有执行 / 审查任务均不得写 `TASK_LEDGER.md`。
- 协调下一动作：A 实施任务按既有审查 brief 收口并交原审查任务复验；不通过不得恢复 A=`DONE`。B 的其他阅读功能继续，已出现的侧栏配色、联动与相关测试差异只冻结、不提交、不合并且不回退，待 UI/UX 给出唯一推荐 brief 后由总控重新授权唯一写入者。C 后续须在生产依赖获授权后另包完成剩余生产能力；M2 按既有边界继续推进。
...
| `M1-F2-A` | `VERIFY`（首轮 UI/UX=`FAIL`）/ 实施任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` + 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` | 本地 EPUB / TXT / PDF 导入与桌面统一书架闭环；功能保持通过，本轮修正已集成书架的视觉与既有任务路径，不实现正文阅读 | 阻断为左右山水层次、状态带质感、搜索可发现性 / 失败语义、空态导入键盘模型；另修正网格节奏与同步说明层级，均无需改变信息架构 | `59ae` 实施任务独占 `apps/web` 书架视觉 / 交互、定向测试和 A 新证据；审查任务只读复验；台账仍只由总控写 | 真实三类文件与既有功能收据复用；新门要求 Chrome `1440×844 @ DPR2`、200%、五态、搜索乱序 / 恢复、双导入入口键盘、导入中断、对比度、无溢出与非作者复验 | 首轮接受截图 `07-accepted-chrome-4380-main-11bd41e-1440x844-dpr2.jpg` SHA-256 `33b511fb…abe90`，参考对照 `08-comparison-current-left-reference-top844-right.png` SHA-256 `87872937…dc7d`；原功能恢复点 `ed2cec6 → 38f9ec3` |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `ACTIVE` / 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` | 原生小程序运行壳、登录 / 书架 / 连续阅读 / PPT 状态边界、正式资产与五类状态；未就绪服务走显式开发适配器 | `f312`，`codex/m2-miniapp-shell` @ `ff4767b`；唯一写 `apps/miniapp/**`，锁文件和共享契约交总控 | 微信开发者工具真实渲染覆盖 360 / 390 / 430 与不同高度：单任务单列、顶部状态 / 导航、底部安全区、长标题 / 正文、连续滚动、输入法、弹层、44px 目标、系统字体和零横向溢出；保留关键页截图、交互 Case、控制台证据并由任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 复验。工具 / 凭证不可用时仅登记等价验证与真实端缺口，不得称完成 |
| `M2-F4-A` 真实书架 / 导入接入 | `READY` / F0-A 之后同端下一包 | 将运行壳接入真实书籍摘要、搜索、导入与解析状态 | A 已以 `38f9ec3` 合入；为避免与 F0-A 同写 `apps/miniapp/**`，等待其范围提交后续接，共享 contracts / lockfile 由总控集成 | 真实 EPUB / TXT / PDF、五类状态、开发者工具与账户隔离 |
...
当前有三个互不重叠的实施卡。A 任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 在 `59ae` 只写书架 Web 视觉 / 交互、定向测试与 A 证据，完成后回到 idle 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 复验；B 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` 只写 `text-reader*` 与 B 证据；M2-F0-A 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 只写 `apps/miniapp/**`。C 安全首包的原任务已完成停止，生产后续包在依赖授权后重新分配。总控独占 contracts、schema、共享入口、Worker 注册、台账和最终集成。
...
| `M1-F2-A` 已集成候选 | `ed2cec6 → 38f9ec3`，状态 `VERIFY / UIUX FAIL` | 功能、回归和主线集成收据保留；审查任务以 `782b@11bd41e` 的真实 Chrome `1440×844 @ DPR2`、五态和绑定参考完成首轮审查。实施任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 从 `2298f63` 修正，候选提交后回到原审查任务复验。 |
...
| 开发基线 | 本地 `main` @ `045434f` | `38f9ec3` 已集成 A，`44b81ea` 已冻结 B / C 接缝，`2298f63` 收紧小程序适配门，`589a22b` 纳入 C 安全首包；`6ce045e` 的侧栏配色条款因顺序错误已由 `045434f` 撤回，等待独立 UI/UX brief。主线两组未跟踪视觉候选继续保留且不得纳入台账提交。A / B 从各自冻结基线产出，集成时由总控对账。 |
...
| `M1-F2-A` 书架闭环 | `VERIFY / UIUX FAIL` | `ed2cec6 → 38f9ec3` 功能与主线恢复点有效；首轮审查证据固定，任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 修正并提交候选后，由任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 复验再裁决 |
