...
- **活动实现线**：Mini Conversation 的 scenery 修复 `09dce23` 已顺序进入唯一 f312，构建与保护哈希复核通过，原 writer 正在真实 DevTools 重验；Desktop Conversation 在 `f3bc@97e1c53` 续作私有 CORE / API / DB / WEB，shared seam 冻结待顺序集成；Book Detail `bdv1@f6c6d0e` 的 Notes / Highlights / PPT empty 局部 PASS，真实 PPT runtime / reload 返工已派发。Settings `set1@039cb596` 为 `READY` 候选，作者已释放。
- **下一用户可见检查点**：先取得 `f312@09dce23` 的 Conversation 真实 DevTools PASS / FAIL；并行取得 Book Detail runtime checkpoint 与 Conversation 首个可运行候选；Settings 通过短 EARLY 后按 shared seam 顺序进入本地 main，再以 main 真实 Chrome 复验。
- **真实阻塞**：无全局串行阻塞。Mini 上一轮 scenery FAIL 已有最小修复并进入复验，仍需真实端 verdict 才能解除；Book Detail 整体 EARLY 等真实 PPT list / state / reload 与后续 shared route seam；Conversation 与 Settings 的 `app.ts` / `index.ts` / `main.ts` 只允许顺序集成；真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未授权。
- **当前可见入口 / revision**：Settings 候选入口 `http://127.0.0.1:4299/` → `http://127.0.0.1:5178/#/settings`，revision `039cb596`，真实 Chrome normal 状态与 console clean；稳定 main Web `http://127.0.0.1:4127/` / API `http://127.0.0.1:4198/api/v1/workspace` 仍绑定旧产品 revision `ab13310`，只作恢复点；Mini 唯一 DevTools 项目为 `f312@09dce23`，上一轮 FAIL 证据在 `mini-conversation-early-43a99c7/`，当前重验进行中。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F1-A` | `READY` · `CANDIDATE b7a6894 / READY FOR EARLY / REOPEN 1` / 项目总控 | 邮箱注册、登录、退出、服务端会话和桌面登录 / 注册 UI 的完整闭环；微信入口保持主层级但未授权时 fail-closed | `auth1@b7a6894` clean；范围只含 `main.ts`、`styles.css`、`auth-page.ts`、`auth-page.test.ts`，原 EARLY 的 50/50 硬切与弹窗焦点缺陷已由原 writer返工 | CORE checkpoint + 上述四个 UI 文件；不得写 D-TEXT、Mini 或 Settings 私有实现 | writer 真实 Chrome 覆盖 1440 / 1200 / 1024 / 768、DPR2、1440×1024，横溢 / console clean，弹窗初始焦点、Tab trap、Escape、焦点恢复与失败保留通过；候选尚未非作者 EARLY、未合入 main；真实成功登录、200%、reduced-motion 与真实邮件 / 微信仍未覆盖 |
| `M1-F1-B` | `READY` / 项目总控（候选作者已释放） | 目标：邮箱验证 / 找回 / 修改与桌面设置闭环；非目标：真实邮件送达、微信绑定、Conversation 合并。`set1@039cb596` 已形成 16 文件范围单一候选 | 候选 clean；真实邮件保持 `EMAIL_DELIVERY_UNAVAILABLE` fail-closed；与 Conversation 重叠的 `app.ts` / `index.ts` / `main.ts` 只由总控顺序集成，候选未过 EARLY 时暂停合入 | `apps/server/src/account-settings{,-migration,-routes}{,.test}.ts`、`app.ts`、`index.ts`；`apps/web/src/{settings-page,settings-state}{,.test}.ts`、`settings-page.css`、`main.ts`、`ui/desktop-shell{,.test}.ts` | 定向 RED 2→15 / 15、全仓 229 / 229、typecheck / build / diff-check；作者真实 Chrome 1440 / 1200 / 1024 / 768 normal / failure / logout / keyboard / console / overflow PASS。候选入口 `4299 → 5178/#/settings`；下一步短非作者 EARLY，PASS 后顺序合入 main |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `READY` / `/root/desktop_conversation_slice`（已派发，preflight ACK pending） | 目标：输入、发送、确定性本地回复、刷新恢复、错误保留与统一对话 UI；非目标：真实模型、图片消息、Settings / Auth UI | `f3bc@97e1c53` 既有私有 WIP 可续；`app.ts` / `index.ts` / `main.ts` 的旧差异冻结，待 Settings 顺序集成；无 ACK 不升级 `ACTIVE` | conversation domain / runtime / migration / store / routes；Web chat client / state / controller / view / CSS / tests；不得新增 Settings / Book Detail / Mini 写入 | 既有 CORE / API / DB / controller / view 测试与隔离刷新恢复 Case 为恢复点；下一步回传绝对 cwd / branch / HEAD / status、owned files、首个 RED，再形成首个可运行 Chrome 候选 |
| `M1-F3B-A1-CONVERSATION-CORE` | `READY` / `/root/desktop_conversation_slice`（已派发，preflight ACK pending） | 目标：复用 `f3bc` 会话 CORE / API / DB / Web 私有纵切并闭合用户可见输入→回复→刷新；非目标：真实 AI 与其他页面 | Settings shared seam 已冻结；若继续需要改 shared 文件，先回传最小请求，由总控顺序裁决 | `f3bc` 当前 conversation 私有差异；现存 `app.ts` / `index.ts` / `main.ts` / domain index 差异只读冻结，不得继续扩写 | 既有测试与隔离恢复 Case不代表 EARLY；取得 preflight / RED 后完成 scoped candidate、真实 Chrome、console / overflow / persistence，再交非作者 EARLY |
...
| `M2-F3` 小程序会话 | `ACTIVE` / `/root/mini_conversation_visual`（EARLY FAIL reopen 1） | 目标：会话抽屉、文字 / 选择 / 附件状态、输入增长、失败保留；非目标：真实 AI / PPTX。`f312@43a99c7` 已关闭旧 parser 错但因 scenery 缺失 FAIL；修复 `09dce23` 已进入唯一候选 | `09dce23` 只把 `index.wxml` / `index.wxss` / `index.test.ts` 改为真实 `<image>`；不得碰 Reader / drawer / config / lock / QA 图。真实端未 verdict 前保持 ACTIVE | 13 / 13、TS、build、diff-check与保护哈希通过；上一轮 390×844 / font23 FAIL 证据在 `mini-conversation-early-43a99c7/`。当前从 `f312@09dce23` 重跑四宽 / 状态 / keyboard / console / overflow，下一步真实 PASS / FAIL |
| `M2-F4` 小程序书架与阅读 | `VERIFY` / 项目总控 | 目标：统一书架、连续正文、内容面板、老己笔记与背景恢复；非目标：真实 notes API / 跨设备。Reader 私有候选 `866f237` 已有作者真实 DevTools 390×844 收据 | 当前唯一候选已前进到 `f312@43a99c7`；Conversation 编译 / 视觉复验未闭合前不把整树称 EARLY PASS，Reader 代码与受保护 config / lock / QA 图不得改写 | 参考 18～20；证据 `mini-reader-early-866f237/`。下一步在同一 f312 DevTools 保留 Reader 局部收据并补当前 revision 的非作者 EARLY / 扩展视口；真实持久化、暗色 editor 仍未覆盖 |
...
| `M1-F1-A-AUTH-EARLY` | `READY` · `CANDIDATE b7a6894 / READY FOR EARLY / AUTHOR RELEASED` / 项目总控 | `/Users/echoman/.codex/worktrees/auth1/SelfAlone@b7a6894` clean；仅四个认证 UI 文件 | 按对应执行包的依赖与暂停条件 | writer Chrome 四宽 / DPR2、焦点闭环、失败保留、console / overflow 自验已完成 | 分派短非作者 EARLY；PASS 后冻结 FINAL，FAIL 原 writer原范围返工 |
| `M1-F1-B-SETTINGS` | `READY` / 项目总控（作者已释放） | 目标：桌面设置 normal / failure / logout / account 服务闭环；非目标：真实邮件、Conversation / Auth 扩围。候选 `set1@039cb596` clean | 未过短 EARLY 不合入；与 Conversation 重叠的 shared seam 按 Settings→Conversation 顺序集成 | 16 文件：account-settings migration / service / routes / tests，Server `app.ts` / `index.ts`，Web settings state / page / CSS / tests、`main.ts`、desktop-shell / test | RED 2→15 / 15、全仓 229 / 229、typecheck / build / diff-check；真实 Chrome 四宽 normal / failure / logout / keyboard、console / overflow PASS；预览 `4299 → 5178/#/settings`。下一步非作者 EARLY |
| `M1-F3B-A-CONVERSATION-SLICE` | `READY` / `/root/desktop_conversation_slice`（已派发，preflight ACK pending） | 目标：输入、发送、确定性回复、刷新恢复、错误保留、统一对话 UI；非目标：真实模型 / 图片 / Settings。复用 `f3bc@97e1c53` 既有 WIP | shared `app.ts` / `index.ts` / `main.ts` 旧差异冻结，Settings 顺序集成后再接；无 preflight 不升级 ACTIVE | conversation domain / runtime / migration / store / routes；Web client / state / controller / view / CSS / tests；不得写 Settings / Book Detail / Mini | 既有 CORE / API / DB / controller / view 测试及隔离刷新恢复是恢复点；下一步 exact ACK + RED，随后 scoped candidate 与真实 Chrome EARLY |
| `M1-F2-BOOK-DETAIL-VISUAL` | `READY` / `/root/book_detail_visual`（已派发，preflight ACK pending；reopen 1） | 目标：保持 Notes / Highlights / PPT empty 局部 PASS，并闭合真实 PPT list / loading / failed / reload / generating / completed；非目标：自由改版、真实模型 / PPTX | `f6c6d0e` 非作者结论为局部 PASS、整体 EARLY FAIL；shared `app.ts` / `index.ts` / `main.ts` 禁写，需接缝时停并回最小请求 | `book-detail-{state,view}{,.test}.ts`、`book-detail.css`、`text-annotation{,.test}.ts`；允许新增 `book-detail-runtime{,.test}.ts` 与私有命名 Server / contracts 模块 | 16 / 16、typecheck / diff-check及四宽 Notes / Highlights / PPT empty 局部 PASS；缺真实 PPT runtime / 下载 Case。下一步 preflight + RED→私有 scoped candidate；未接 shared seam 不得称 EARLY PASS |
...
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `VERIFY` / 项目总控 | 目标：参考 19 的老己笔记单列全宽流、sheet 手势与浅色背景重进保留；非目标：真实 notes API / 跨设备。Reader candidate `866f237` 已进入 f312 | 当前唯一候选为 `f312@43a99c7`；Conversation 重验未闭合前不扩大旧 Reader 局部收据；不得改保护 config / lock / QA 图 | Reader 四个私有源 / tests；绑定 refs 18～20，SHA `23d078e3…6077` | 作者真实 DevTools 390×844 / font23 / safe 47/34 覆盖初始 / 全屏 / 回落 / 重开、notes / editor / 草稿失败 / 背景恢复，console 0；证据 `mini-reader-early-866f237/`。下一步同一 f312 非作者 EARLY / 扩展视口 |
| `M2-F0-A-MINI-CONVERSATION-VISUAL` | `ACTIVE` / `/root/mini_conversation_visual`（EARLY FAIL reopen 1） | 目标：refs 13 / 30～32 的会话输入、选择、附件、错误恢复与同态矩阵；非目标：Reader、drawer、真实 AI / PPTX。`43a99c7` 正常态因 scenery 缺失 FAIL，最小修复已进入 `f312@09dce23` | `09dce23` 使用真实 `<image>`；当前只做同一 DevTools 重验，首条新错误即停止 | `pages/conversation/index.{test.ts,wxml,wxss}`；不得修改 nav-drawer / Reader / project config / lock / QA 图 | 旧 `513:22` 与本地 background 根因均有 scoped commit；13 / 13、TS、build、diff-check及保护哈希通过。上一轮证据 `mini-conversation-early-43a99c7/`；下一步当前 revision 真实四宽 / 状态矩阵 PASS / FAIL |
...
| 当前桌面候选波次 | Settings `set1@039cb596` READY；Conversation `f3bc@97e1c53` 已派发续作；Book Detail `bdv1@f6c6d0e` 局部 PASS / 整体 EARLY FAIL 并已派 runtime 返工 | Settings clean、测试与作者 Chrome PASS，候选预览 `4299 → 5178/#/settings`；Conversation 等 preflight ACK / RED；Book Detail 缺真实 PPT list / state / reload 与 shared seam。任一 revision / 状态事件发生即增量回写。 |
| 当前 Mini 候选波次 | Reader `866f237` 保留作者局部收据；Conversation `09dce23` 已顺序进入唯一 f312，原 writer ACTIVE 重验 | 旧 parser 与本地 background 两个根因均有 scoped commit；13 / 13、TS、build、diff-check与 project config / lock 保护哈希通过。上一轮 FAIL 证据 `mini-conversation-early-43a99c7/`；当前 revision 仍须真实 DevTools verdict。 |
...
| 开发基线 | 台账事件写入前 `main@22c3ef4`；所有当前候选未合入 main | main tracked clean，三个既有未跟踪视觉目录原样保留；候选提交、测试或作者自验不能自动 `DONE`。 |
