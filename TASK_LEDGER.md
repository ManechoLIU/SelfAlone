...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` Checkpoint B 已解除旧路由授权阻塞并进入 `ACTIVE`；Checkpoint A 保持 controller-verified `20/20 PASS`，不重做。旧 native Assignment `M1-F4-C-SERVER-GATE-B-CODEX-01` 已因宿主在总控窗口切换时卸载 spawned Agent 而 `disconnected / zero B writes` 正式终止；恢复子包 `M1-F4-C-SERVER-GATE-B-CODEX-02` 已转 `READY`，继承同一 Checkpoint B 策略与工作树，先 ACK 后 RED。WEB-HANDOFF 继续等待 server contract 冻结。
...
| `M1-F4-C-SERVER-GATE` | `VERIFY / CHECKPOINT A ACCEPTED / CHECKPOINT B RECOVERY READY / HEAD 929d1aa` / 项目总控 | Checkpoint A accepted patch SHA=`f97579b…d9f` 保持 controller-verified conversation-store `20/20 PASS`；旧 `B-CODEX-01` 已以 host-disconnected、zero B writes 终止，Grok 越界草稿已回收 | 当前由显式恢复子包 `M1-F4-C-SERVER-GATE-B-CODEX-02` 接续同一工作树与策略；候选后必须非作者 TDD causal review、Integration Gate 与 current-main 验证 |
...
| `M1-F4-C-SERVER-GATE-B-CODEX-02` | `READY / CODEX NATIVE RECOVERY / HEAD 929d1aa` / 待 ACK | 继承 Checkpoint A 两文件 staged WIP 与 Checkpoint B 既定 8-file owned scope；旧 -01 已 terminal，当前无同范围 live Writer | ACK-only → lease/runtime → bounded RED → 最小实现 GREEN；禁止重做 A/第9文件/依赖安装/live paid provider call/TASK_LEDGER 或 main 写入 |
