...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef` 已形成六个局部恢复点；`6aba8ef` 仅 11 个 `apps/miniapp/**` 文件，9 文件 / 53 项、typecheck / build `1368 KiB` / diff-check 通过，320 / 360 / 390 / 430 DevTools 证明 PPT requirements 视口、安全区与操作区局部几何成立 | `cdaa307` 只关闭开发适配器内 Reader→conversation→PPT 流程，`6aba8ef` 只关闭 PPT viewport 局部回归；均不证明服务端 / 跨端会话、软件键盘、H3 或视觉通过。两个禁止差异和真实字体 / 游客 console / 资产门保持 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / SIX CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f`、`630c051`、`5e270b1`、`5e0188d`、flow-only `cdaa307` 与 PPT viewport `6aba8ef` 是六个局部恢复点；软件键盘仍未实显，继续其余状态 / 字体 / 视觉矩阵。全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 六个局部点已冻结；`cdaa307` 只关闭 DevelopmentClient Reader→conversation→PPT intent，`6aba8ef` 只关闭 PPT live viewport / safe-area / action clearance。其余状态、软件键盘 / 字体矩阵与最终视觉仍未完成；既有禁止差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef` 是六个局部恢复点；全小程序六页 / 共享层四宽四高和状态矩阵、真实软件键盘 / 字体、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
