...
- 当前 Goal：`STAGE-READ-NOTES-V1-K / M2-F2 INTERNAL RECOVERING`；`c60ce3a` 已通过非作者代码复审并顺序进入唯一 `f312@1c9d445`，代码门为定向49、Mini全量234、typecheck / build / diff-check PASS。真实 DevTools 已在430 / 360 / 320完成未连接→连接编辑器→no-call失败保留 / UI重试的受控 EARLY PASS；内部缺口是 `MiniappGlobalData` 未合成 development-only `wereadClient`，故成功、A→B隔离、断开保留与页面重进恢复尚无可操作真实端证据。真实WeRead Key / AppID / apiBaseUrl / 域名H3不在本Goal，也不得用它们绕过该内部缺口。
- 下一可见检查点：`M2-F2` 的 `M2-F2-DEV-WEREAD-QA-01` 因 install 越界触碰禁写 `pnpm-lock.yaml` 已终止；总控已恢复 lock、保留两份合法测试 WIP 并用本地 vitest 证明 development 模块缺失 / `wereadClient` 未接线的真实 RED。替补 `M2-F2-DEV-WEREAD-QA-02` 已冻结 clean 四文件 candidate `08d9597`：targeted 18/18、Mini 242/242、typecheck、local-tool build/package-size 与 diff-check PASS；独立非作者 `M2-F2-DEV-WEREAD-QA-REVIEW-01` 已对 `08d9597` clean HEAD、四文件、首检与停止条件完整 ACK 并进入 ACTIVE。只有 reviewer PASS 才顺序进唯一f312并复用 `M2-F2-UIUX-REALEND-03` 的430主门与390 / 360 / 320矩阵补齐成功、同requestId重试、A→B隔离、断开保留和页面重进恢复。并行 `M1-F4-B-REWORK-03` 的 Grok 会话已终止但真实 Git 仍为 clean `f53c930` / 无新提交，且执行日志存在 npm cache EPERM 与被管道掩盖的失败；其“已修改/测试全绿”报告判为 INVALID RESULT / NO CANDIDATE，不得集成或据此宣称修复。
...
| `M1-F4-B` | `RECOVERING / CANDIDATE f53c930 NONAUTHOR FAIL / GROK REWORK-03 INVALID RESULT` / 项目总控 | 两个cost-ledger migration文件的并发 / 回滚局部证据保留；FAIL仍为运行期可直接插入终态reservation且audit反序历史未被拒绝，`f53c930` 不得合main。`M1-F4-B-REWORK-03` 虽最终文本声称已修复，但真实 worktree clean、HEAD/reflog 无新增，日志同时证明 npm cache EPERM、错误内联命令与被管道到 cat 掩盖的失败，且未在首个工具前形成合规 delivered ACK | 恢复检查点：本轮无候选可审；按外部调用失败门停止自动重试。后续只在形成新的可验证执行路由/授权事件后再恢复该两文件包，不影响无文件冲突的 Mini 主线继续推进 |
...
| `M2-F2` | `VERIFY / F312 1c9d445 / DEV-WEREAD CANDIDATE 08d9597 / NONAUTHOR REVIEW ACTIVE` / 外部 Kimi `M2-F2-DEV-WEREAD-QA-REVIEW-01 / kimi-k3 / api / high` | 原 `7667456` 两项产品合同FAIL已由同四文件 `c60ce3a` 恢复并通过外部Kimi非作者复审；提交链已顺序进入唯一f312，定向49、Mini234、typecheck / build / diff-check PASS。真实DevTools 430 / 360 / 320的未连接、全屏编辑、失败输入保留、重复点击与无横溢通过；01 因依赖 install 越界已终止且 lock 已恢复；02 从控制器验证 RED 续作后冻结 clean `08d9597`（四文件 +653），targeted18、Mini242、typecheck、local-tool build/package-size、diff-check PASS；Reviewer 已对 clean cwd / branch / HEAD / 四文件 / 首检 / 停止条件完整 ACK，lease guard PASS | 检查点：非作者实际 diff / release隔离 / idempotency / A→B / sync requestId 语义 verdict；PASS 后由总控顺序进唯一f312补成功 / A→B / disconnect / 页面重进真实端门，FAIL 只返原四文件恢复；禁止网络、真实Key / storage、release注入或页面自由补丁 |
