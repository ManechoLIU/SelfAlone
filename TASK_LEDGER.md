...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY / F312 cf6a8c5 / SELF PASS / NONAUTHOR ACTIVE` | 累计实现为 `0fa1447 + cf6a8c5`：携带bookId / 书名，可编辑预填；发送前不auto-send / sheet / task；失败保留草稿与上下文 | 由 `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` 做非作者最终门；PASS 后只进入总控 current-main 集成 / 回归，不重写实现；证据 `mini-ppt-f312-self-cf6a8c5/` |
| `M2-F6-MINI-PPT-DRAFT-HANDOFF-REVIEW` | `ACTIVE / NONAUTHOR READ-ONLY / Kimi K3 / DELIVERED ACK f01b184` | 只读审累计 `0fa1447 + cf6a8c5` 的6文件 handoff 边界，对照 SPEC 143–145 与 TECHNICAL handoff 契约；ACK 已在首个 repo tool 前送达，owned files 为该6文件且 observed modified files=0 | 当前动作：完成实际累计 diff、targeted test 与 current-main drift 审查；禁止写文件、安装依赖、改 refs/config/lockfiles 或创建 reviewer worktree。输出 PASS/FAIL 后由总控接管候选 |
