...
- **Goal**：当前有终点里程碑为“Mini 从书籍详情 / Reader 点击制作PPT后，以版本化 `draft/context handoff` 保留当前会话、书籍身份与可编辑预填，直到用户真实发送前不打开范围sheet、不建task或阶段；并行让Drawer v5在320 SELF FAIL后取得根因brief，让DeepSeek加密配置候选进入非作者EARLY”。Desktop quota既有H3与responder默认fail-closed保持，不把fake provider、未验收删除、Mini本地文案或PPT阶段壳冒充真实自由聊天 / notes CRUD / PPTX闭环。唯一项目总控任务仍为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`。
- 当前执行波次：`MINI-PPT-DRAFT-HANDOFF-DISPATCH-DRAWER-REDIAGNOSIS`（Mini PPT的reader/conversation/core与Drawer的nav-drawer/asset、Desktop DeepSeek文件互斥）
- 当前活动项：`M2-F6-MINI-PPT-DRAFT-HANDOFF`
- 协调下一动作：回收`/root/mini_ppt_handoff_writer_v2`的`LOADED ACK c06ec53 + 9967bb8`与首个真实RED；此前只允许规则加载、不允许源码写入。下一事件以两个空槽并行派Drawer原writer按已冻结根因brief返工，以及DeepSeek `c346772`非作者安全 / UI EARLY。
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@c06ec535a516f0b0034dbd23f2a3dc6b976be461` 复核；main仅有本台账tracked控制diff，既有9组受保护未跟踪路径仍在。总控已加载 `adaptive-delivery` 正式提交 `9967bb8a492a905799c6b5692625c454acf2e8ef`；全局 `SKILL.md`、`long-task-governance.md`、`methods.md` SHA-256依次为 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`、`a0f6abe6298e9063a151b3527dbf0636a66598144701921f79d28506a5cf09c5`、`109f5fabddb39dad44dd238624842baa1c094111cbc4818ebd16b7225a021e59`。主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。Mini PPT产品 / 视觉 / 技术事实源增量已提交为`c06ec53`；替补writer已完成delivered调度ACK，精确loaded ACK仍待回，未ACK前暂停受影响源码写入。
- **活动实现 / 复验线**：Mini PPT替补writer `/root/mini_ppt_handoff_writer_v2` 位于`/Users/echoman/.codex/worktrees/mppt1/SelfAlone`、分支`codex/m2-mini-ppt-draft-handoff`、`HEAD c06ec53`、clean，六文件ownership与首个RED / 停止条件delivered ACK已闭合；前任writer在初建worktree后连续缺ACK，四证无后续增量后以`previous_status=running`回收1次，未产生或丢失源码diff。Drawer v5在唯一`f312@cbb63fa`关闭静态体积门后320 SELF FAIL；非作者已返回`ASSET COMPOSITION + SLOT/PLACEMENT`根因brief并释放。DeepSeek writer已交clean候选`dsk1@c346772`并释放，安全授权delta已loaded；候选仍只用假master key / fake HTTP，不含真实调用。当前总控+Mini PPT writer `2 / 4`、可用`2`。
- **READY 重算**：Mini PPT draft/context handoff已派发并取得delivered ACK，loaded ACK前只做规则加载。Drawer原writer按“同一v5局部Alpha + slot/mask一次修复”brief `READY`，DeepSeek `c346772`为`VERIFY / READY FOR NONAUTHOR EARLY`，二者文件 / 环境互斥；按用户指定本原子事件在Mini PPT writer ACK后立即yield，不在本回合展开实现 / 集成 / 等待，下一事件以两个空槽并行派发。Mini PPT预发送态“无需新卡片或仅忠实派生”的只读UI判断在writer首次WXML需求前派发。Mini notes真实save / edit / delete与本writer的`reader/index.ts`重叠，须在handoff候选冻结或ownership收缩后再派；Desktop D-TEXT真实删除复验不受Mini阻塞，进入后续Desktop独立验收事件。
- **下一用户可见检查点**：先看到Mini点“制作PPT”后仍停在当前会话，输入框只预填可编辑“帮我制作这本书PPT”，不自动发送或弹范围sheet；该候选须先过测试、唯一f312真实端SELF与非作者EARLY。并行的下一检查点是Drawer根因brief与DeepSeek设置页候选EARLY，不把任一静态绿灯称为业务完成。
...
- **当前可见入口 / revision**：主线为 `c06ec53`；Web `127.0.0.1:4276` 与API `127.0.0.1:4212` 保留默认fail-closed，quota既有Chrome / DB收据保持。Mini唯一常驻DevTools路径仍为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、当前HEAD `cbb63fa`；v5 320 SELF FAIL现场冻结，PPT writer禁止新增DevTools入口或直接写f312，既有project.config / lock / QA保护现场不变。
...
| `M1-F2-D-TEXT` | `VERIFY` · `MAIN ENTRY + HANDOFF SCOPED FINAL / PARENT OPEN` / 项目总控 | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | 私有`dtxtui@d65ab3b`已集成；reader 768为`cc0cec0`，当前书handoff为`1e7763a`。1024 / 1200 / 原生200% / DPR2、真实删除仍未覆盖 | main text-annotation / book-detail / text-reader及handoff接缝；后续缺口按独立包分配 | 入口与handoff限定缺陷已真实Chrome闭合；`dtext-main-final-cc0cec0/`、`dtext-handoff-early-101789e/FINAL-RECEIPT.md`；不扩大为D或F2父项DONE |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3A-A` | `VERIFY` · `CANDIDATE c346772 / READY FOR NONAUTHOR EARLY` / 项目总控 | `dsk1@c346772`已交24文件范围单一候选：固定模型目录、AES-256-GCM信封加密、验证后原子替换 / 撤销 / 脱敏及桌面“检测并保存”；只用fake key / fake HTTP，不开放自定义端点或真实聊天 | 状态`DIRECT RETRIEVAL AUTHORIZED / LOCATION UNRESOLVED`；安全授权delta已loaded。provider / 入口 / 精确模型 / 官方计费 / 三次上限未闭合前禁止扩大安全存储 / 浏览器探测、摄入或真实调用 | domain / contracts、Server model-config / migration / injected DeepSeek adapter / app组合、Web API / model-config / settings / main；候选未改`.env`、Mini、台账且无秘密落盘 | 9 suites / 38 tests、typecheck、build、diff-check与1440 / 1024 fake Chrome SELF通过；下一事件派非作者安全 / UI EARLY，未合入main，不是自由聊天 |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `DONE` · `MAIN MERGED ed489a8 / NONAUTHOR + MAIN H3 PASS` / 项目总控 | 新账户一次性领取、账户grant与桌面对话轻量领取条；并发幂等、accounts外键、成功约1.2秒brief后消失；不含F4-B/C、模型或付费调用 | delayed A success / 503 / dismiss均受route generation、account与conversation guard；领取状态焦点及消隐后textarea恢复已关闭；本包不证明AI消耗或自由聊天 | main含候选 `1d7d8b7` 的6个quota Web文件；writer / reviewer已释放 | main定向6文件24项、typecheck / build / diff-check；真实Chrome注册→领取、status焦点、约1.2秒后textarea焦点、reload后隐藏，console / overflow通过；当前schema grants=1 / owners=1 / orphaned=0 / FK=1；`main-quota-ed489a8/FINAL-RECEIPT.md` |
...
| `M2-F6` 小程序 PPT | `VERIFY` · `DRAFT HANDOFF ACTIVE / FULL FLOW BLOCKED` / 项目总控 | `c06ec53`先释放书籍→当前会话的draft/context handoff：携带书籍身份与可编辑预填，用户发送前不自动开sheet、不建task / requirements-ready或跳阶段；发送后才按既有合同进入必要确认与四阶段 | handoff私有六文件已派替补writer；完整真实AI / PPTX仍依赖M2-F3、F4、M1-F5、下载域名与真机授权。不得以内嵌常驻双按钮卡代替后续阶段 | 先验当前会话保留、无会话回退、草稿 / 书籍上下文失败恢复、无auto-send / auto-sheet；再验四阶段、320 / 360 / 390 / 430、PPTX与iOS / Android，开发适配器不等于完成 |
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `ACTIVE` | 替补writer `/root/mini_ppt_handoff_writer_v2` 在`mppt1@c06ec53`以失败测试锁定书籍→当前会话的versioned draft/context handoff；至少bookId / title，真实已有则带author / source / cover，精确预填可编辑“帮我制作这本书PPT”，不自动发送 / 开sheet / 建task或跳阶段 | cwd / branch / HEAD / clean、六文件ownership、首个RED与停止条件delivered ACK已闭合；`LOADED ACK c06ec53 + 9967bb8`仍待回，回前暂停源码写入。书籍卡先走非作者只读判断，明确参考足够时不调用ImageGen | `core/ppt-intent{,.test}.ts`、`pages/reader/index.ts` / `page-state.test.ts`、`pages/conversation/index.ts` / `index.test.ts`；无另行授权不写WXML，禁止WXSS / asset / config / lock / QA / f312 | 首个RED须证明不建task / requirements phase、保留current conversation / 无会话回退、draft与book context恢复、无auto-send / auto-sheet；首个runnable提交后停止为CANDIDATE / READY FOR EARLY，再顺序进唯一f312做SELF与非作者EARLY |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `ROOT CAUSE BRIEF READY / REWORK READY` / 项目总控 | v5候选与build seam已进入唯一`f312@cbb63fa`；320 SELF确认山景第2行过早、亭子不可辨。非作者已判`ASSET COMPOSITION + SLOT/PLACEMENT`，不是视觉PASS | 静态门仍有效但不能覆盖FAIL。一次最小返工须保留`widthFix`与scenery在scroll-view外；同一v5前52% Alpha≤8、52–68%渐入，亭子结构Alpha≥128且320 ROI约44×40；slot bottom距nav 8–12px、mask延后显影。失败即停，禁止机械v6 / ImageGen | 原writer可恢复asset + nav-drawer WXML / WXSS / test原范围；与PPT writer六文件、DeepSeek候选互斥，唯一f312仍由总控顺序集成且保护七项status | 证据`mini-drawer-v5-f312-self-cbb63fa/SELF-VERDICT.md`与非作者根因回执；先320门（第1/2行无景、第三行后显影、亭子可读、导航/点击不遮），PASS才扩360 / 390 / 430与EARLY |
