...
| `M1-F2-WEREAD-RUNTIME` | `READY / ENCRYPTED CONNECTION + SYNC STORE` / 待分配 | 建立账户隔离、加密凭证、CAS替换 / 断开、sync run与last-success快照的 PostgreSQL runtime，并把 Gateway Adapter 与HTTP接缝注入正式Server；保持 Key 不出 callback / 日志 / 响应 | 依赖 HTTP代码门、H4 Adapter与共享API；先做 migration/store RED，不调用真实腾讯。完成本地 fake-gateway Case 后仍需独立真实Key H4，禁止由存储测试代替 |
