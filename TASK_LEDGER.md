...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。Desktop Conversation已冻结`cvmain@c88c424`的new / search / quota实现，代码门定向18项PASS；总控diff审查发现失败保留、领取成功brief和并发Case三个EARLY前缺口，原writer正在同范围最小修复并准备真实Chrome。Mini Library私有HTTP合同`20d61d6`已冻结但因M2-F1 session bridge明确BLOCKED，未接线也未冒充EARLY；其槽已轮转到`mcmsg1@279bdbc`的Mini Conversation本地确定性发送 / 失败重试，preflight ACK与首个sending RED均由总控复跑确认后ACTIVE。Book Detail短reviewer两次无收据已回收，总控完成实际审查并判`CODE PASS / REAL ENDPOINT FAIL`。当前构成：controller / integrator 1、implementation writer 2 ACTIVE、reviewer 0，第三子槽保留给首个候选的短审查。
- **下一用户可见检查点**：Desktop Conversation修复三项后完成作者Chrome new / search / filtered-empty / claim brief / 刷新 / 发送恢复 / 失败保留并冻结新候选；Mini Conversation先交发送RED，再形成normal / sending / failed-retry / attachment-only / restored候选，顺序进入唯一f312做三视口EARLY。首个候选冻结即启用第三槽短审查，PASS后总控顺序合入main并复验。
- **真实阻塞**：Mini Library的真实runtime受M2-F1小程序session bridge与DevTools transport阻塞，`20d61d6`只保存未接线合同。Book Detail候选Web仍请求忽略`bookId`的旧`/api/v1/workspace`，新Server service未注册且响应形状不一致，故保持冻结；解除条件是Conversation shared seam冻结后由唯一writer闭合真实book-scoped route / Web contract / Chrome。Mini父项仍缺长列表、软件键盘、游客凭证化clean console与真实PPT preview；D-TEXT PPT handoff / banner继续开放。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `ACTIVE` · `CANDIDATE c88c424 / CODE PASS / PRE-EARLY FIX` / `/root/mini_drawer_writer` | 输入、发送、确定性回复、刷新恢复、错误保留与统一对话UI；`c88c424`已补new / search / quota，但总控diff审查要求关闭create/open失败保留、claim成功短反馈和并发claim Case | cwd `/Users/echoman/.codex/worktrees/cvmain/SelfAlone`、branch `codex/m1-f3b-a1-conversation-main`、HEAD `c88c424`；shared seam仍只此包 | 21文件候选 + 同范围最小修复；禁止Book Detail / Settings私有 / Mini | 新候选定向6文件18项PASS，尚未EARLY。下一步三项修复commit→作者隔离Chrome→非作者EARLY |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `ACTIVE` · `CANDIDATE c88c424 / CODE PASS / PRE-EARLY FIX` / `/root/mini_drawer_writer` | 新账户一次性领取、按账户grant状态和桌面对话首页轻量领取条闭环；不建设券、余额、充值、成本账本或常驻记录 | Auth + F3B-A共享外壳已具备；shared seam与A同一writer；外部 / 付费调用即停 | trial grant migration / store / route、Web quota与A shared seam | 顺序幂等 / 账户隔离已有代码门；补并发Case、成功brief后消失和真实Chrome，未过前不写EARLY |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `ACTIVE` · `cvmain@c88c424 / CODE PASS / PRE-EARLY FIX` / `/root/mini_drawer_writer` | 既有功能 / 并发修复保留；new / search / quota候选已提交，本轮只补失败保留、claim成功brief、并发Case后重跑EARLY | shared seam与F4-A同writer，Book Detail / D-TEXT handoff继续等 | 21文件候选 + 最小修复；禁止其它私有页面 / Mini / ledger | ref02 / 36；定向18项PASS，尚无新Chrome收据。下一步修复commit→作者Chrome→非作者EARLY |
| `M1-F2-BOOK-DETAIL-VISUAL` | `VERIFY` · `CANDIDATE f6c6d0e + 75e764c + 6c50a76 / REAL ENDPOINT FAIL` / 项目总控 | Notes / Highlights / PPT empty视觉候选保留；代码门与文本合并树通过，但Web继续读固定M0 workspace，新Server book service未注册且形状不一致 | Conversation shared seam冻结前不派重复writer；之后须同一writer闭合book-scoped route / Web contract / page entry，禁止先合dead code | 候选10文件、967 / 70；恢复点`bdv1@6c50a76` clean | 4文件30项、typecheck / build / diff-checkPASS但非EARLY；证据`book-detail-candidate-review-6c50a76/REVIEW-RECEIPT.md` |
...
| `M2-F3-MINI-CONVERSATION-SEND-RETRY` | `ACTIVE` · `PREFLIGHT ACK / SENDING RED CONFIRMED` / `/root/mini_library_real_slice` | 在已过normal / selection上补本地确定性发送、单列user / assistant消息、发送锁、失败原位重试、刷新恢复和纯附件发送；不含真实AI/API | `mcmsg1@279bdbc`；只写Conversation page/state六文件与对应dist，需非作者EARLY后才可进main | refs13 / 31 / 32；首RED 2文件15项中1 FAIL（`sending`缺失）。下一步完整实现→candidate→唯一f312三视口EARLY |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader scoped FINAL；Conversation `cvmain@c88c424`代码门PASS且在EARLY前最小修复；Book Detail审查为real endpoint FAIL | Conversation真实Chrome收据仍缺；Book Detail shared seam等Conversation冻结。D-TEXT端口关闭，schema / runtime保留。 |
| 当前 Mini 候选波次 | Reader + Conversation + Drawer在`main@b98c118`限定范围FINAL；Library私有合同`20d61d6`因auth BLOCKED；新Mini Conversation send/retry已ACTIVE | send/retry待首个RED和候选；后续只顺序进入唯一f312。保护现场不变。 |
...
| 开发基线 | 产品顺序集成到 `main@b98c118`；Desktop Auth / Settings、D-TEXT reader、Mini Reader / Conversation / Drawer限定范围PASS | main仅有当前台账事件diff与三个受保护未跟踪视觉目录；Desktop Conversation、Mini send/retry、Library合同和Book Detail均未进main，不能自动 `DONE`。 |
