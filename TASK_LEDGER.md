...
- 当前 Goal：`M1-F2-B` EPUB / TXT 桌面阅读闭环收口；`B-UX1` 已给出并由总控批准唯一视觉 / 交互 brief，现由原 B 实施任务在五个冻结 Web 文件内完成整改、真实业务 Case、范围提交与非作者复验。M2-F0-A 保持未合并 idle，不与 B 争用文件。
- 当前阶段：桌面 Web `M1-F2-A` 已在 main `DONE`；`M1-F2-B` 恢复 `ACTIVE`。当前冻结 WIP 经非作者审查为 `FAIL`，不是候选通过；阅读私有左栏浅 / 深联动、首帧与偏好隔离、焦点 / 滚动恢复、TXT 标题和复制失败整改已由 `8f40658` 写入 Web 视觉事实源。M2-F0-A H3 保持 `BLOCKED`，不合并、不升级最终视觉。
...
- 当前活动项：实现任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 独占 `da89` 的 `text-reader-state.ts / text-reader-view.ts / text-reader.css / text-reader.ts / text-reader.test.ts` 五个 `AM` 文件，保留 staged 原候选并在 working WIP 上按批准 brief 收口，不得 reset、回退或扩写共享文件。审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 进入 idle，等待干净候选 revision 后只读复验。M2-F0-A 在 `f312@91934f1` 已释放资源并 idle。所有执行 / 审查任务均不得写 `TASK_LEDGER.md`。
- 协调下一动作：B 完成五文件整改、定向 / 全仓回归和真实 EPUB / TXT Chrome Case 后创建范围提交；原 UI/UX 任务复验浅 / 深、首帧、焦点、100% / 200% 与响应式证据，总控再裁决候选集成。若运行入口拿不到完整账户与文件版本，偏好缓存必须 fail closed 到浅色默认，不能继续按 `bookId` 单键复用。
...
| `M1-F2-B` | `ACTIVE` / 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` | EPUB / TXT 正文、目录、复制、全书位置、专注模式和浅 / 深阅读背景的桌面阅读闭环 | `44b81ea`；五文件 WIP 已解除设计冻结但当前仍为 `FAIL`，必须按 `8f40658` 与 B-UX1 brief 收口；解析定位、缓存身份或真实样本不成立时转 `VERIFY` | `da89` / `codex/m1-f2-b` 独占五个 `text-reader*` Web 文件及既有 B 范围；共享入口 / 样式、contracts、schema、Server 新范围和台账不得写 | 真实 EPUB / TXT；章节顺序、目录 / 焦点、复制失败、位置与偏好隔离 / 保存、刷新 / 重进、五态、100% / 原生 200% 与 Web 4 视口；再由 `B-UX1` 非作者复验 | 保留 index 中原候选与 working WIP，不 reset；完成整改、真实 Chrome、全仓验证和范围提交后回传干净 revision |
| `M1-F2-B-UX1` | `DONE / 等待复验` / 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` idle | 已完成浅 / 深阅读私有侧栏、普通态桌宠、首帧、焦点、滚动、TXT 与复制反馈的非作者 brief；不实现 | 当前 WIP 结论 `FAIL`；B 未交新 revision 前不重复轮询，PDF 页面一致性只锁视觉合同、不冒充当前实现 | 审查任务保持只读；事实源由总控以 `8f40658` 同步，实施只归 B 五文件所有者 | 新候选须覆盖无缓存 / 有缓存 / 存储拒绝、账户与 fileVersion 隔离、浅 / 深、目录 / 专注焦点、TXT 去重、复制失败、100% / 200% 和规定视口 | 真实 Chrome 初审为无 override `1440×900 @ DPR2`；精确 `1440×844` 与 `768` 仅 DPR1 override，原生 200% 和 PDF 页面仍缺，候选复验不得静默降门 |
...
当前唯一实现写入卡为 B：任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89@44b81ea` 独占五个 `AM` Web 文件，按 `8f40658` 与 B-UX1 brief 整改并保留既有 staged / working 双层恢复点；禁止 reset、覆盖原候选或扩到共享入口、contracts、schema、Server 新范围和台账。审查任务仅在候选提交后只读复验；M2-F0-A `f312@91934f1` idle，C 原任务已停止。总控继续独占共享接缝、台账和最终集成。
...
| `M1-F2-B` 当前候选 | `da89@44b81ea`，状态 `ACTIVE / 五文件整改` | staged 候选在主题 WIP 前通过定向 3 文件 / 14 项、全仓 16 文件 / 64 项、typecheck / build；冻结后 domain / server 2 文件 / 5 项再通过，真实私有账户读取他人 book 返回 404，cached / working diff check 通过。五个 `AM` Web 文件的 working 层同时含被否决的旧色值与可复用交互修正；B 须原地拆账并以 `8f40658` 为准，不回退 staged 候选。初审当前 WIP 为 `FAIL`，复验前不得提交完成结论。 |
...
| 开发基线 | 本地 `main` @ `8f40658` | A 已完整闭合；`44b81ea` 冻结 B / C 接缝，`589a22b` 纳入 C 安全首包，`8f40658` 固定 B 阅读私有外壳与交互规则。B 从 `da89` 恢复，M2 H3 保持阻塞；主线未跟踪视觉候选不得纳入台账提交。 |
...
| `M1-F2-B-UX1` 阅读视觉 / 交互 brief | `DONE / 等待复验` | 非作者初审判当前 WIP `FAIL`；总控以 `8f40658` 批准私有左栏浅 / 深联动、无固定桌宠、首帧与焦点规则，B 五文件恢复写入，审查任务 idle 等待新 revision |
