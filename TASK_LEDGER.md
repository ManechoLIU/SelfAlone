...
- 当前阶段：桌面 Web `M1-F2-A` 候选保持 `VERIFY`，尚未合入 `main`；范围回归、真实长标题和键盘焦点已由项目总控复验通过，非作者审查无 P0，但右下主画布远山、真实 Chrome `1440×1024` 与 200% 缩放仍未闭合。
...
- 协调下一动作：实现任务复用已批准左栏透明素材的远山段，以低透明度、裁切和 mask 补齐右下主画布落款，不再调用图片生成；同时把状态带字号对齐共享 Meta `12/18`，补正确书架 `1440×844`、真实 Chrome `1440×1024` 和 200% 缩放证据。完整证据包交项目总控核验后才决定状态；全部门禁通过后方可范围提交、本地合并并复验 `main`，随后释放下一项。
...
| `M1-F2-A` | `VERIFY` / 任务 `01a0339a-3299-7150-91d9-f95a5331c627` | 本地 EPUB / TXT / PDF 从真实上传到账户归属、对象保存、解析状态、元信息、动态默认封面及桌面统一书架的完整闭环；不实现正文阅读 | 候选 `codex/m1-f2-a` @ `353b4fd` 尚未合入主线且有未提交改动；非作者审查无 P0，右下主画布远山、正确书架 `1440×1024` 和 200% 缩放未闭合；名为 `16-real-chrome-1440x844-cover-status.png` 的文件实际是 PPT 页，不得作为书架证据 | CORE、API、WORKER、DB、WEB、QA；实现任务拥有候选代码 / 证据，台账文件只由项目总控写 | 真实三类文件；加载 / 真空 / 筛选空 / 失败 / 正常；搜索、同名异源、作者缺失、刷新、账户隔离；Web 4 视口对照书架参考 | 总控独立复验定向 15 / 15 与 `pnpm verify`（13 文件 / 49 项、类型检查、构建）通过；真实 Chrome `1440×844 @ DPR 2` 中 11 本书、5 列、无横向溢出和控制台警告 / 错误，真实上传长标题的标题 / 作者 / 24px 状态带无碰撞且完整语义保留，搜索与导入键盘往返及 3px 焦点轮廓通过；视觉保持 `VERIFY` |
...
当前执行卡为 `M1-F2-A` 的 `VERIFY` 收口。实现任务 `01a0339a-3299-7150-91d9-f95a5331c627` 在 `47b4` 工作树继续视觉和真实浏览器验收；项目总控下达边界、核验证据并写状态，不与实现任务并行修改候选代码。恢复点为 `codex/m1-f2-a` @ `353b4fd` 加当前未提交 diff；范围回归、真实长标题和键盘焦点已通过，下一步只收口右下主画布远山、共享 Meta 字号、正确书架 `1440×844` / `1440×1024`、200% 缩放及全 diff。达到门禁后由实现任务创建范围提交，项目总控审查并按授权集成、复验本地 `main`；收到主线 HEAD 与复验证据后再释放 B、C。
...
| `M1-F2-A` 当前候选 | `codex/m1-f2-a` @ `353b4fd`，状态 `VERIFY` | 与 `main` @ `8010af7` 在 `f89a4b7` 后分叉；候选含功能 / 视觉提交及未提交代码、规范、验收图和旧台账 diff。Token 范围回归已修复，真实长标题和键盘焦点已通过；右下远山、`1440×1024`、200% 缩放及错误 PPT 截图处理仍缺。不得把 dirty 工作树、过期测试或文件名当作已合入结果，候选提交前后、合并前后均重验。 |
| 开发基线 | 本地 `main` @ `8010af7` | 主线当前另有其他任务的未跟踪视觉候选，必须保留且不得纳入台账提交。用户已授权候选通过门禁后本地合并；合并前仍须重验 HEAD / diff / 共享 PostgreSQL，并只纳入目标范围。 |
