...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已以 `6bcc4a4 → 99ef0ac` 顺序进入 current main并全门 PASS。`M1-F4-C-WEB-HANDOFF` 已从 `a810c325` FAIL 恢复为六文件候选 `f5e1b71`：定向 `18/18`、controller `11/11`、Web `296/296`、typecheck/build/视觉契约/diff-check PASS；`/root/web_handoff_code_review` 已对该 revision delivered ACK，下一检查点为非作者 verdict 后顺序进入 main 或返原作者。
...
| `M1-F4-C-WEB-HANDOFF` | `VERIFY / CANDIDATE f5e1b71 / REVIEW-02 ACKED` / 项目总控 | 原候选 `a810c325` 的两项 P1 已由同一 Codex Assignment 以 `3 FAIL / 15 PASS` 锁定并恢复：EXHAUSTION/未配置都引导配置，UNAVAILABLE 才稍后重试；完整 conversation/PPT hash 进入设置 state，loading error 与 success/back 保留，不自动发送。候选链恰好六个 Web 文件，fresh Web `296/296`、typecheck、build、视觉契约、diff-check PASS | `/root/web_handoff_code_review` 已对 `f5e1b71` delivered ACK，复审完整六文件链与 return/error/controller 合同；PASS 后顺序进入 main 并 current-main 复验，FAIL 返原作者同范围恢复。真实 Chrome 三错误态仍须独立入口证据，不把登录壳当 PASS |
