...
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已以 `6bcc4a4 → 99ef0ac` 顺序进入 current main，非作者 noteIntent 平台耗尽反例 PASS；主线精确 Case 1/1、conversation-store23/23、平台能力+组合6/6、Server206/206、直接 typecheck、diff-check 全 PASS。`M1-F4-C-WEB-HANDOFF` 依赖已解除，下一检查点是 Web 当前输入/PPT草稿位置对可区分 exhaustion/config-required/unavailable 的保留与原位重试纵向候选。
...
| `M1-F4-C-WEB-HANDOFF` | `READY / SERVER MAIN CONTRACT 99ef0ac` | Web conversation 与当前 PPT 草稿位置消费 server 的可区分 exhaustion/config-required/unavailable 状态：保留输入、会话上下文/PPT draft，在当前操作位置显示“配置自己的 AI 模型后继续”与配置入口；失败可原位重试，不常驻余额/金额 | 派互斥 Web Writer 先 RED 锁定当前输入/PPT draft 保留与同 requestId 原位重试；绑定现有会话视觉合同，不碰 Mini production 或 Server 已冻结合同 |
