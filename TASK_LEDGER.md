...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-CODEX-02 / NONAUTHOR FAIL / REMEDIATION` | 首版候选 `49712c5` 未进入 main：独立 Reviewer 以真实 Fastify→隔离 PostgreSQL 复现旧连接快照泄露、客户端预占内部 requestId 复用旧 run、短暂 DB 异常后 running 不自恢复共 3 个 Important；当前原 Writer 已从同一 clean worktree delivered ACK，严格只修 `weread-runtime.ts`、`weread-runtime.test.ts`、`index.ts` | 先补三项同案 RED，再完成连接替换与初始 run 原子结果、当前连接快照 fencing、有界退避与运行期 stale recovery；形成增量候选后重新走非作者复核。真实 Key H4仍是独立授权门，禁止读取凭证或调用真实 Provider |
