...
- 当前阶段：桌面 Web `M1-F2-A` 功能、回归和主线集成收据继续有效；独立 UI/UX 首轮审查已以真实 `1440×844 @ DPR2` 页面判定 `FAIL`，范围内视觉 / 交互修正已交唯一实施任务。文本 / PDF 阅读共享接缝已在 `44b81ea` 冻结，B、C 与小程序客户端基础包继续并行。
...
- 当前执行波次：`M1-F2-A-VISUAL-REOPEN`、`M1-F2-B-TEXT-READER`、`M1-F2-C-PDF-FIRST-PACK` 与 `M2-F0-MINIAPP-SHELL` 并行；A 已从只读审查切到唯一实施者修正，四项文件范围互斥，共享 contracts / schema / 入口由项目总控单写入。
- 当前活动项：A 的唯一实施任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 在 `59ae` / `codex/m1-f2-a-visual-reopen` 从 `2298f63` 开始，独占书架 `main.ts` / `styles.css`、必要状态 / DOM 测试和 A 新证据；审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 已转 idle，候选提交后由其复验。B 由任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` 独占 `text-reader*`；C 由任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` 在 `7ef0` 独占 `pdf-reader*`；M2-F0-A 由任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 独占 `apps/miniapp/**`。所有执行 / 审查任务均不得写 `TASK_LEDGER.md`。
- 协调下一动作：A 实施任务按审查 brief 完成山水 / 状态带 / 栅格 / 服务行、搜索防抖与乱序保护、搜索失败恢复和空态导入键盘模型，交付真实 Chrome 与导入 Case 后由原审查任务复验；不通过不得恢复 A=`DONE`。C 候选先修复 `retryPage` 前置异常的租约泄漏再复审；B、M2 按既有边界继续推进。
...
| `M1-F2-B` | `ACTIVE` / 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` | EPUB / TXT 正文、目录、复制、全书位置、专注模式和阅读背景的桌面阅读闭环 | `44b81ea`；解析定位或真实样本不成立时保持 `VERIFY`；可与 C 并行 | `da89` / `codex/m1-f2-b` 独占 `text-reader*` 的 DOMAIN、SERVER、WORKER、WEB、QA 与 B 证据；不写共享入口 / schema | 真实 EPUB / TXT；章节顺序、按需目录、唯一进度、刷新 / 重启恢复、浅 / 深背景、键盘、五类状态、Web 4 视口和非作者视觉审查 | 当前从冻结接缝开工；范围候选通过后交项目总控集成，通过后释放 D |
| `M1-F2-C` | `ACTIVE / VERIFY`（安全首包）/ 任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` | 文本型 / 图片型 PDF 页面阅读的解析 / 渲染、安全、缓存、恢复与页锚点基础；不 OCR、不伪造正文 | 候选 `7856a7a` 的范围与 16 文件 / 65 项回归通过，但 `retryPage` 前置校验异常可能遗留租约，修复前不集成；真实页面渲染仍需依赖、许可证和 Worker 资源授权 | `7ef0` / `codex/m1-f2-c` 独占 `pdf-reader*`、PDF Worker / 缓存、QA 与 C 证据；不写共享入口 / schema / Web | 首包覆盖真实文本 / 扫描 / 加密 / 损坏样本元数据、部分页失败、过期租约重启、缓存键隔离、旧版本拒绝；完整门另需续租 / fencing、真实渲染、DB / API / Web 与浏览器 | 非作者审查结论 `VERIFY`；先补租约泄漏回归与最小修复。`pdfjs-dist@6.2.108` + `@napi-rs/canvas@1.0.8` 仅为待授权推荐，不得称生产能力 |
...
当前有四个互不重叠的实施卡。A 任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 在 `59ae` 只写书架 Web 视觉 / 交互、定向测试与 A 证据，完成后回到 idle 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 复验；B 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` 只写 `text-reader*` 与 B 证据；C 任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` 在 `7ef0` 只写 `pdf-reader*`、PDF Worker / 缓存与 C 证据；M2-F0-A 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 只写 `apps/miniapp/**`。总控独占 contracts、schema、共享入口、Worker 注册、台账和最终集成。
...
| `M1-F2-C` 安全首包 | `7856a7a`，状态 `VERIFY` | 只新增 `pdf-reader*`、四个真实 fixture 与 C 证据，补丁可应用于当前 main；15 项定向与全仓 65 项通过。`retryPage` 前置异常租约泄漏须先修复；续租 / fencing、真实 PDF.js / Canvas、DB / API / Web 与生产依赖仍未完成。 |
| 开发基线 | 本地 `main` @ `2298f63` | `38f9ec3` 已集成 A，`44b81ea` 已冻结 B / C 共享接缝，`c68b9eb` 重开 A 视觉门，`2298f63` 收紧小程序屏幕适配门；主线两组未跟踪视觉候选继续保留且不得纳入台账提交。A 视觉修正必须从当前 main 新建隔离范围任务。 |
