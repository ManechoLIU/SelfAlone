...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复、北京时间双时段计费、真实 Store 并发硬上限与失败后重试本地门已分别在 `main@b637023+81f5f1f+df32890+ba70ced` 闭合。2026-09-02 已从 DeepSeek 官方当前价格页核到 `deepseek-v4-flash` 六项人民币快照：峰值缓存命中 / 未命中 / 输出=`100000 / 3000000 / 9000000` 微元/百万 token，谷值=`50000 / 1500000 / 4500000`；北京时间周一至周五 `09:00–12:00、14:00–18:00` 为峰值。下一可见门是独立平台凭证安全注入与本次真实平台调用授权；旁路 `M1-F5-A1-REWORK-04` 保持独立恢复，不改变本 Goal 完成条件。
- 当前阻塞：`M1-F4-V1` 的本地实现与价格事实已清空，真实成功路径仍缺外部 H4。当前 shell 七项 `PLATFORM_DEEPSEEK_*` 变量均缺失，4322 / 4174 无监听；Keychain 预期 service `SelfAlone DeepSeek API`、`api.deepseek.com`、`platform.deepseek.com` 仅做不回显元数据查询均未找到。既有用户模型 Key 授权不静默扩展为平台凭证；唯一唤醒条件是机主创建并安全保存一枚独立平台 Key，并明确本次平台免费额度真实调用的次数 / 金额上限，之后才验证真实领取→Provider→成本流水→硬上限→浏览器恢复。其他既有外部门：Mini production 受 AppID / apiBaseUrl / 域名与非游客 DevTools 门约束；Mini Drawer clean-console 与软件键盘只受外部 H3 约束；Desktop Auth 专用背景 / Logo seal未绑定；真实 PDF 页面能力受生产依赖 / 许可证授权门约束。
- 规则版本：总控已从真实 `main@018e084` 加载当前 AGENTS（SHA-256 `a850d7ba…c1b`），其 Fact-First 与 host-aware fallback 要求先核当前事实、证据不足保持未知，并使 Grok 安全失败后可先路由到当前宿主内部 GPT，不重置 execution lineage / recovery budget；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。Adaptive Agent Runtime 安装副本已完整加载并由 canonical controller session `01a03c61-5dd2-7553-968b-a3bc2f5777c9` 机器 ACK 精确 revision `eba8cdd8ace679944190090f7d77191b8d19a35d`（manifest SHA-256 `e3a5c224…238fa`）；registered controller 收据必须使用 Hook 的同一解释器、安装副本内精确 Guard、匹配当前 turn 的 machine trace，成功收据后同一 turn 强制 yield。桌面 Canary 仍按安装 manifest 标记 `degraded`，不得把 hooks 配置存在冒充实机生效。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控。
...
| `M1-F5-A` | `VERIFY / A1 BACKEND REOPENED / WEB ENTRY HELD` / 项目总控 | 对话 / 书籍双入口、单书来源、范围 / 需求与任务工作区 | `M1-F5-A1` 完整非作者审查未通过，Web 入口不得基于错误完成态启动；已闭合 Mini handoff 不重复开发、不触碰 f312 现场 |
| `M1-F5-A1` | `ACTIVE / MAIN a220c2f / FULL NONAUTHOR FAIL` / 项目总控 | 真实账号 + 已发送用户消息门；当前会话内创建独立 PPT 草稿；来源关系按集合建模且 MVP 恰好一本书；固定字段仅用途、受众、页数范围、补充要求；换书保留通用要求 | 初版与 int4 修复已提前顺序进入 main `e79e91f+9eabb8b+ec0866d+ae11a07+dd365cf`，但完整审查对 `301026d` 判定 Critical 0 / Important 3 / Minor 3、`FAIL_TO_CONTROLLER`：必须关闭 legacy body 400、不可变创建指纹、单快照读取后再复审；`a220c2f` 的 DONE 结论已撤销 |
| `M1-F5-A1-REWORK-04` | `ACTIVE / GROK 4.6 OAUTH / DELIVERED ACK` / `grok-build-m1f5-a1-rework-04` | 基于 `main@a220c2f` 的单一隔离修复分支；Assignment `M1-F5-A1-REWORK-GROK-04` 已先回传 cwd / branch / HEAD / clean / 10 owned files / stop condition 后才读取仓库 | route=`grok-build/grok-4.6/oauth/high`，session `01a0614f-426d-75c2-b0b9-d6894fde4b02`，无调用预检与 delivered ACK 均闭合；现按真实 RED→GREEN 修三项 Important 及同范围版本自增、reset 外键、认证映射 Minor；候选提交后必须同案非作者复审，不得改 Web / Mini / TASK_LEDGER / 真实 Provider |
