...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。Mini send / retry已由唯一DevTools候选`f312@854c577`完成三宽、失败恢复、retry与attachment-only，并合入`main@11f480a`完成scoped FINAL。Desktop Conversation `cvfix@48edb7e`非作者EARLY PASS，12枚候选已顺序合入`main@dea90d7`，当前main真实Case FINAL复验中。Book Detail已保留旧`bdport`未提交恢复点并改派`bdport2`；Mini长会话列表已派发独立writer。当前构成：controller / integrator 1、implementation dispatch 2 ACK PENDING、reviewer 1 ACTIVE。
- **下一用户可见检查点**：Desktop Conversation在main完成注册→发送 / 回复→刷新恢复→断服保留与两宽focus FINAL；Book Detail先交当前main私有Web / Server checkpoint再补共享入口；Mini长列表交可运行checkpoint后顺序放入唯一f312，检查390 / 360 / 320与font23的首屏、中段、末项及固定底栏。任一FAIL回原范围最小整改。
...
- **当前可见入口 / revision**：Desktop候选入口`5188 / 4308`既有验收仍绑定`a9408ad`；Conversation main FINAL使用`dea90d7`隔离入口，未过Case前不替代stable。D-TEXT `5198 / 4318`已关闭，schema `selfalone_dtext_main_final_cc0cec0_20260826`与runtime `/tmp/selfalone-dtext-main-final-cc0cec0.0LEzLV`保留，未授权删除。稳定恢复点`4127 / 4198@ab13310`。仓库main为`dea90d7`，其中Mini产品提交至`11f480a`；唯一DevTools保持`f312@854c577`且Mini source / dist与main逐字节一致，保护差异未变。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F3B-A` | `VERIFY` · `MAIN MERGED dea90d7 / EARLY PASS / MAIN FINAL VERIFY` / 项目总控 | 输入、发送、确定性回复、刷新恢复、错误保留、new / search / quota与统一对话UI；owner key、错误文案、空task列和focus ring均已进入main | 12枚候选路径与`cvfix@48edb7e`逐字节一致；main真实Case未完成前不标DONE，Book Detail不得并发写`styles.css` | main新增Conversation CORE / API / DB / Web与shared `main.ts` / `styles.css`等38文件 | EARLY两宽focus / console / overflow PASS；main `pnpm verify` 70文件 / 411项通过。下一步main注册→发送 / 恢复 / 断服保留 FINAL；证据`desktop-conversation-early-48edb7e/` |
| `M1-F3B-A1-CONVERSATION-CORE` | `VERIFY` · `MAIN MERGED dea90d7 / CODE PASS / MAIN CASE VERIFY` / 项目总控 | 确定性回复、PG会话 / 消息、API、Web、legacy、导航、幂等、发送锁与hydrate竞态代码门保留；布局 / 错误 / focus EARLY已通过 | main真实业务Case与隔离持久化仍在复验，未完成前不扩大为父包DONE | main Conversation CORE / API / DB / Web与F4-A最小接缝 | main `pnpm verify` 70文件 / 411项；下一步真实注册、发送 / 回复、刷新无重复与断服草稿保留 |
...
| `M1-F3B-A-CONVERSATION-SLICE` | `VERIFY` · `MAIN MERGED dea90d7 / EARLY PASS / MAIN FINAL VERIFY` / 项目总控 | new / search / quota、确定性发送 / 恢复、失败保留、owner key、错误文案、三宽布局与focus已进入main | writer / EARLY reviewer均已释放；main FINAL reviewer只读；Book Detail仍不得碰`styles.css` | main Conversation纵切38文件；与`cvfix@48edb7e`对应路径逐字节一致 | refs02 / 36；两宽focus、console / overflow EARLY PASS；main70文件 / 411项。下一步main真实Case verdict |
| `M1-F2-BOOK-DETAIL-VISUAL` | `READY` · `OLD CANDIDATE f6c6d0e + 75e764c + 6c50a76 / V2 DISPATCHED / ACK PENDING` / `/root/desktop_book_detail_main_port` | 保留Notes / Highlights / PPT视觉候选，移植到当前main并闭合book-scoped Server / Web / Chrome；不重做已过视觉 | 旧`bdport@62f2ed1`的`app.ts` / `index.ts`与3个untracked WIP原样保留；新`bdport2`必须从`af1091e`创建，先只做私有checkpoint，禁止shared seam / `styles.css` / Conversation / Mini | `book-detail-state/view/css/runtime{,.test}`与`book-presentation{,.test}`；shared `main.ts` / `app.ts` / `index.ts`后续由总控顺序接缝 | refs07 / 08 / 10；旧证据只证明代码门。下一步cwd / RED ACK→私有candidate→共享接缝 / Chrome EARLY |
...
| `M2-F3-MINI-CONVERSATION-LONG-LIST` | `READY` · `DISPATCHED / ACK PENDING` / `/root/desktop_conversation_focus_fix` | 以developmentAdapter确定性数据闭合会话抽屉长列表、搜索与末项露出；不冒充生产会话API，不处理软件键盘 / 凭证 / AI | 基于`main@11f480a`独立`mlong`；唯一DevTools仍f312，writer只交checkpoint；若需越界文件即停 | `nav-drawer/index{,.test,.wxml,.wxss}`与必要`conversation/index{,.test}.ts`，精确范围待ACK | ref11 SHA `9e764e…c36e`；390 / 360 / 320、font16 / 23，首屏 / 中段 / 末项+固定底栏、搜索命中 / filtered-empty；下一步ACK→candidate→f312 EARLY |
