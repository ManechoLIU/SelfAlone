...
- **Goal**：当前有终点里程碑为“Desktop `M1-F4-A` 免费体验领取在当前 main 完成候选审查、真实 Chrome / DB Case、必要集成与状态收口；并行将 Mini Drawer v2 在唯一 `f312` 恢复后推进至 writer 四宽自验与非作者 EARLY”；唯一项目总控任务仍为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`。Mini 环境不得阻塞 Desktop，任一端只在真实外部授权门或已登记环境门前暂停。
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@2522c30704621b0363c8fee135667efbba5121bc` 复核；`2437e00..2522c30` 仅有 `TASK_LEDGER.md` 增量，当前业务代码仍停在 `c858a06`，不能据台账提交宣称新增页面或自由聊天。主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。
- **活动实现 / 复验线**：Desktop `/root/desktop_quota_close` 在 `quota2@2437e00` 已将 `P1` 延迟响应代际隔离与 `P2` 焦点恢复从 RED 推到 helper GREEN / mutation proof，仍 `ACTIVE REWORK` 完成真实 A→B 延迟 Case与候选提交。Mini `/root/mini_drawer_rework_writer` 在唯一 f312 的 320 / 390 fresh compile 给出同态 `SELF FAIL` 后，已 ACK 回 `mdrv2@ae92912` 原四文件做 `ACTIVE REWORK`。Server `/root/conversation_responder_contract` 已在 `chatresp@2522c30` ACK `ACTIVE TDD`，首个 RED 是“未配置 responder 必须 fail-closed并保留草稿，而非成功返回 `我先记下：原文`”。controller + 三 writer占用 `4 / 4`、可用 `0`；初次两个新 handshake 在两次有界等待后仍 clean且无 delivered ACK，已原任务回收并在专用 handshake turn闭合，active writer误中断 `0`。
- **READY 重算**：`M1-F4-A`、Mini Drawer返工和新拆 `M1-F3B-A2` responder contract均已派发并取得 delivered ACK。完整 `M1-F3A-A` 仍依赖 `M1-F1-V1`，`M1-F3A-V1` 仍依赖 A / B且真实收据需用户Key与调用授权；`M1-F3B-B` 仍等对象存储 / 模型能力目录，`M1-F3B-D` 仍等完整 `M1-F2-D`，`M1-F3B-V1` 不能由本次假适配器单独关闭，`M1-F4-B` 仍等平台适配器且真实费用受授权门。`M1-F2-A`、`M1-F2-B` 与 `D-TEXT` 触碰 quota writer的 `main.ts / styles.css` 接缝，待其冻结；Mini Settings / 长列表 FINAL与Drawer共用唯一 DevTools而不并行。四个历史 recent-project入口清理已 `CLOSED`，现只保留 f312，不再作为阻塞。
- **下一用户可见检查点**：预计先后为 Desktop quota交 A→B代际 / focus候选并在current main真实Chrome / DB复验；Server responder交显式开发假适配器的上下文响应与无配置失败保留候选（只证明接线合同，不冒充真实模型自由聊天）；Mini交修复候选，由总控顺序放入唯一f312后做 `320 / 360 / 390 / 430` 同状态自验与非作者 EARLY。
- **真实阻塞**：当前 main的 `ConversationStore` 仍使用 canned default responder且 `index.ts` 未注入真实模型，因此用户不能自由聊天；本轮不调用真实或付费模型。完整平台免费能力仍需 model adapter + cost ledger，用户自有模型仍需 `M1-F3A-A/V1` 与凭证 / 调用授权。Mini Project List / query阻塞和四历史入口清理均已关闭；tourist token、软件键盘与真实AppID仍保持原边界。远端、发布、真实邮件 / 微信 / 模型 / Presenton与付费调用继续受授权门。
- **当前可见入口 / revision**：事件复核主线为 `2522c30`、业务代码仍为 `c858a06`；主线 Web `127.0.0.1:4276` PID `4051` 通过代理连接 API `127.0.0.1:4212` PID `4652`，schema `selection_main_c858a06_20260826`，PostgreSQL `55432` 与两端 `/api/v1/health/ready` 实测在线。该入口可见统一对话壳，但当前回复仍是开发 canned responder，不能称自由聊天。Mini唯一活动 DevTools路径为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、源码 HEAD `63782ba`；recent-project JSON与重启后原生菜单已目视只保留 f312，`long-list-early` query已送达18项且保护 status未变。
...
| `M1-F3B-A2-RESPONDER-CONTRACT` | `ACTIVE TDD` / `/root/conversation_responder_contract` | 以显式 development fake证明对话上下文进入窄 `TextModelAdapter.chat` 相容接缝；无配置必须失败关闭并复用草稿保留；不实现真实供应商、凭证、成本或额度 | Server-only，可独立于F3A凭证UI先验；不能替代 `M1-F3A-A/V1`、平台成本账本、H4真实模型收据或完整自由聊天 | `chatresp@2522c30`仅新 `conversation-responder{,.test}.ts` 与 `conversation-store{,.test}.ts`；`index.ts`由总控顺序接线 | delivered ACK已闭合；首个 RED为 current default仍完成`我先记下：原文`，目标是未配置返回既有`CONVERSATION_REPLY_FAILED`并保留草稿 / 上下文。候选须定向tests / typecheck / build / diff-check，主线接线后再做真实API / Chrome上下文Case |
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `ACTIVE REWORK` · `NONAUTHOR P1 + P2 FAIL` / `/root/desktop_quota_close` | 新账户一次性领取、账户grant与桌面对话轻量领取条；并发幂等、accounts外键、成功约1.2秒brief后消失；不含F4-B/C、模型或付费调用 | current-main已含历史等价实现且服务端 / 主流程绿，但延迟A响应可污染B前端状态，领取阶段DOM替换会丢键盘焦点；未修复前不得VERIFY/DONE | `quota2@2437e00`只写quota Web状态 / view / helper / 直接测试与必要样式；不写Server、conversation migration/store/chat-view | writer已新增两项真实RED；下一步最小guard / focus恢复、定向、A/B延迟、失败 / success / 1.2s dismiss activeElement与真实Chrome。交范围单一候选后由总控顺序集成并在main复验 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY` · `f312 63782ba / UNIQUE ENTRY + QUERY RESTORED / DRAWER SELF FAIL → ACTIVE REWORK` / 项目总控 + `/root/mini_drawer_rework_writer` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；320 / 390 fresh compile确认山水过早进入列表、底栏前山亭不可辨、上缘渐隐 / 列表节奏偏离ref11；不冒充完整Mini闭环 | 原 writer已ACK回`mdrv2@ae92912`四文件最小修复；唯一f312不直接写。recent-project与原生菜单清理`CLOSED`且只剩f312，`long-list-early`真实显示18项，保护config / lock / QA原样 | 首个runnable diff即checkpoint；候选由总控顺序放入唯一f312后做非ImageGen Alpha / 多背景、320 / 360 / 390 / 430 writer自验、EARLY与FINAL；软件键盘、tourist凭证与真实PPT preview继续开放 |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `ACTIVE` · `f312 63782ba / V2 CANDIDATE / UNIQUE ENTRY + QUERY PASS` / `/root/mini_drawer_rework_writer` | ref11与既有山亭asset的同态返工；用户截图确认横向矩形接缝、山亭位置 / 尺度 / 上缘渐隐、列表 / 底栏节奏偏离，旧 `hasAlpha` 与旧整页 PASS 不再有效 | 四文件候选`ae92912`已顺序放入唯一f312，v2 asset SHA `87cf9828…8876`；唯一入口与18项query门已闭合，writer正在同一实例四宽自验，禁止新增导入 | 完成normal / open / current、single fallback、filtered-empty、底栏 / 44px / 安全区 / console / overflow与接缝、山亭、节奏；通过只交READY FOR NONAUTHOR EARLY，偏差回原mdrv2四文件最小修复 |
...
| `M2-F0-A` 当前候选 | main已含`35bad75`长列表与`14b69dc` Settings；Drawer v2四文件`ae92912`已顺序放入唯一`f312@63782ba`，asset SHA `87cf9828…8876` | Settings / 长列表旧 scoped FINAL仍`FAIL / NO-GO`。历史recent-project清理已`CLOSED`且唯一f312；320 / 390同态`SELF FAIL`已重开原writer四文件返工，尚无新source候选，保护config / lock与既有QA不动。 |
...
| 当前 Mini 候选波次 | Reader / Conversation既有限定结论保持；长列表`35bad75`与Settings`14b69dc`已在main但scoped FINAL为NO-GO；Drawer v2`ae92912`在唯一`f312@63782ba`，旧整页PASS仍被用户截图否定 | recent-project JSON已范围单一保留f312并留可恢复五项备份；清理`CLOSED`、原生菜单唯一、query 18项已送达。320 / 390 SELF FAIL后原writer回mdrv2返工，保护现场与四个历史源码/worktree均未删除。 |
| 当前 Desktop main 预览 | `RUNNING / EVENT BASELINE 2522c30 / BUSINESS CODE c858a06 / HEALTH READY / FREE CHAT NOT WIRED` | Web `127.0.0.1:4276` PID `4051`、API `127.0.0.1:4212` PID `4652`；schema `selection_main_c858a06_20260826`。统一壳可见，但Server仍用canned default responder且`index.ts`未注入模型；quota和responder均只在隔离候选推进，必须顺序集成与main Case后再更新。 |
| 开发基线 | 本台账事件父为`main@2522c30704621b0363c8fee135667efbba5121bc`；本轮仍只写`TASK_LEDGER.md` | 6个受保护DOCX / Word锁文件与3组受保护视觉候选目录仍与冻结a233副本同字节，禁止暂存 / 修改 / 删除；Mini唯一f312的2个保护修改与5组既有QA未改变。 |
