...
- **活动实现线**：Auth / Settings、D-TEXT reader与Mini Reader / Conversation / Drawer既有main PASS保持。Desktop Conversation已在`main@dea90d7`完成真实Case FINAL PASS；Book Detail修复链已以产品`main@0cc5d46`完成EARLY与main真实Chrome scoped FINAL。F3B-C Selection私有候选`csel1@655b51f`实际diff审查发现三项P1，已退回原writer做REOPEN 1；D-TEXT私有候选`dpth@1bdded7`实际diff审查确认banner方案未覆盖真实768根因，也已退回原writer做REOPEN 1。两名writer进程live但均等待本轮preflight / RED ACK，不冒充ACTIVE；短时reviewer已释放。Mini长列表`mlong@a9da90a → f312@5878400`代码门PASS，真实DevTools EARLY再次实测因Mac锁屏暂停。当前构成：controller / integrator 1、implementation writer 2 live（正式状态均DISPATCHED / ACK PENDING）、reviewer 0；余1槽无另一可执行READY，禁止重复writer填槽。
- **下一用户可见检查点**：Selection原writer先闭合pending幂等、自由输入刷新恢复与失效单选显式确认，再交新私有candidate；D-TEXT原writer用新book intent重置旧conversation scroll并对旧书workspace显式fail-closed，再交新私有candidate。两者actual diff PASS后按`main.ts`互斥顺序接线并开真实Chrome EARLY；Mac解锁后Mini长列表立即在唯一f312完成三宽 / 大字体真实EARLY。
- **真实阻塞**：Mini Library真实runtime仍受M2-F1 session bridge与DevTools transport阻塞，`20d61d6`只保存未接线合同。Mini长列表代码门已闭合，但本轮Computer Use再次返回“Mac is locked”，解除条件是本机解锁后续跑唯一f312；不反向阻塞桌面writer。Selection与D-TEXT各自私有REOPEN不冲突；它们的shared `main.ts`接缝必须候选通过后顺序集成。Mini父项另缺软件键盘、游客凭证化clean console与真实PPT preview。
- **当前可见入口 / revision**：旧稳定Desktop仍是`4127 / 4198@ab13310`，不得误认当前进度；当前Book Detail main预览为`5330 / 5331@0cc5d46`，schema `selfalone_book_detail_early_c91b691_20260826`、runtime `/tmp/selfalone-book-detail-main-0cc5d46/`，真实Chrome scoped FINAL PASS；旧候选`5320 / 5321@c91b691`仍可访问但不代表main。D-TEXT `5198 / 4318`已关闭，保留既有schema / runtime。本台账事件提交基于父`main@b0983bf`，产品revision仍为`0cc5d46`；唯一DevTools候选为`f312@5878400`，保护config / lock与两张QA图未变，长列表尚未合入main。
...
| `M1-F2` 桌面书库、阅读与笔记 | `ACTIVE` | 本地书导入、阅读、记录、详情与账户隔离 | A仅功能完成；B `VERIFY`，C半成品；Book Detail scoped FINAL，D-TEXT handoff / banner与D-PDF仍开放，E-OWNER已完成 |
...
| `M1-F2-D-ENTRY` | `VERIFY` · `BOOK DETAIL SCOPED FINAL PASS / MAIN 0cc5d46 / PARENT OPEN` / 项目总控 | 书籍详情、笔记 / 作品入口和五类状态；不实现 PDF 阅读或 AI | Book Detail修复链`584299f → 7bac9da → 0cc5d46`已接入book-scoped presentation route；真实删除、原生200%、D-TEXT handoff / banner与父项其余入口仍未覆盖，故不标父项 DONE | main Book Detail 12文件与Server shared seam；后续缺口继续按互斥私有包 | main 72 files / 431 tests、1440 / 1200 / 1024 / 768、真实笔记保存 / 刷新、16:9作品、范围失败保留旧作品 / 恢复、console / overflow PASS；证据`book-detail-early-c91b691/`与`book-detail-main-final-0cc5d46/` |
...
| `M1-F3B-C` | `VERIFY` · `CANDIDATE 655b51f CODE FAIL / REOPEN 1 DISPATCHED / ACK PENDING` / `/root/mini_conversation_devtools_fix` | 简单选择、显式确认、只读历史和失效问题的私有纵向切片；不含真实模型、图片、书籍上下文或shared UI seam | actual diff确认三项P1：pending多选同request replay会STALE、自由输入草稿刷新丢失、会使既有内容失效的单选仍直接提交；原writer已恢复但未回本轮preflight / RED前不标ACTIVE | `csel1@655b51f`仅新增Server/Web `conversation-selection*` 17文件且worktree clean；REOPEN仍独占同17文件，shared `app.ts` / `index.ts` / `main.ts` / `styles.css`冻结 | ref36 SHA `1b6ec630…a5796b40`；下一步逐项RED→GREEN后交新scoped candidate，actual diff复审PASS才顺序接线并走1440 / 1024 / 1200 / 768真实EARLY |
...
| `M1-F2-BOOK-DETAIL-VISUAL` | `DONE` · `MAIN MERGED 0cc5d46 / EARLY + MAIN FINAL PASS / REOPEN 1 CLOSED` / 项目总控 | book-scoped presentation、titleless Notes / Highlights与最多两列16:9作品面完成；不扩大为D / F2父项 | 跨书/坏响应fail-closed保留旧作品，多non-stale current歧义拒绝；真实删除、原生200%、D-TEXT handoff / banner仍独立开放 | main `book-detail*`、`book-presentation*`与Server `app.ts` / `index.ts`接缝共12文件 | refs07 / 08 / 10；main72 / 431、四宽、真实笔记持久化、范围失败保留 / 恢复、console / overflow PASS；`book-detail-main-final-0cc5d46/FINAL-RECEIPT.md` |
| `M1-F2-D-TEXT-PPT-HANDOFF-FIX` | `VERIFY` · `CANDIDATE 1bdded7 CODE FAIL / REOPEN 1 DISPATCHED / ACK PENDING` / `/root/dtext_ppt_private_fix` | 当前书 PPT intent必须驱动对应workspace或明确fail-closed；恢复滚动时真实`.book-ppt-intent`不得被conversation header遮挡 | candidate的严格book/workspace匹配可保留，但新`.desktop-connection-banner` + inline z-index未覆盖旧`scrollTop=90.5`使intent top=-6.5的真实768根因；原writer已恢复，未回preflight / RED前不标ACTIVE | `dpth@1bdded7`与REOPEN仍唯一owned新建`apps/web/src/text-reader-ppt-handoff.ts` / `.test.ts`；`main.ts` / 现有CSS / Book / Selection冻结 | 下一步补“非零旧scroll + 新book intent必须reset”与旧书workspace mismatch fail-closed RED→GREEN后交新candidate；actual diff PASS且Selection seam释放后顺序接线，再跑当前书 / 固定seed、1440 / 768、banner、console / overflow |
...
| 当前桌面候选波次 | Auth / Settings保持main PASS；D-TEXT reader scoped FINAL；Conversation已至`main@dea90d7` FINAL；Book Detail已至产品`main@0cc5d46` scoped FINAL；Selection `655b51f`与D-TEXT `1bdded7`均actual diff FAIL并由原writer并行REOPEN 1 | 两包先回preflight / RED再交新私有candidate；通过后shared `main.ts`顺序集成。旧`bdport`WIP只读保留；当前main预览为5330 / 5331。 |
...
| 开发基线 | 本台账事件提交的父为`main@b0983bf`，产品顺序集成至`0cc5d46`；Desktop Auth / Settings、D-TEXT reader、Conversation与Book Detail限定范围PASS，Mini Reader / Drawer与send-retry限定范围PASS | main仅允许本次台账事件diff与三个受保护未跟踪视觉目录；Selection、D-TEXT handoff、Mini长列表与Library合同仍未完成，不能自动 `DONE`。 |
