...
- 当前 Goal：`M1-F4-C / FREE EXPERIENCE EXHAUSTION HANDOFF`。上一 `M1-F4-B / COST LEDGER INTEGRITY RECOVERY` 已以原始四提交链 `695f6b0 → b2b1e8f → cb2a51e → f47a990` 经 ACK-first Kimi K3 TDD 因果审查 PASS 后顺序进入 `main@1bb0b76`，current-main migration+store `16/16`、Server 全量 `196/196`、typecheck、diff-check 全 PASS。项目级依赖重算后 C 成为当前最直接纵向后继：A 已完成、B 已闭合、文本模型真实 H4 已通过，PPT draft/handoff 合同已存在；图片模型不是无图 PPT 前置。
- 下一可见检查点：`M1-F4-C-SERVER-GATE` 已由 Grok 4.6 Writer 在隔离 `codex/m1-f4c-server-gate@f7219e3` ACTIVE：RESERVED lease、Grok OAuth route check、ACK-only `tool_call=0` 与新 revision ACTIVE lease 全 PASS。第一硬门=production edit 前先在 `conversation-store.test.ts` 取得 current-code behavioral RED：平台能力 exhaustion/config-required 错误必须可区分且 retainedDraft / request 可原样重试；随后才可实现 shared gate 与 deterministic metered tests。平台实际 cost 不得伪造，live paid call 禁止；WEB-HANDOFF 继续等待 server error contract 冻结。
...
- 规则版本：总控已从真实 `main@46ab1fd` 加载当前 AGENTS（SHA-256 `0cf10094…d0d`）；后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为事实源，Web / Mini 前端 Kimi K3 API 项目级路由授权以 `ba3a7af` 为事实源，普通已验证范围单一 `main` 提交的非强制快进 push 授权以 `46ab1fd` 为事实源。`adaptive-delivery` 已从真实源仓 `/Users/echoman/Documents/Codex/adaptive-delivery` 的 clean HEAD `4960b98130459467ee05b0c434197dcd7868f0fd` 同步到真实安装副本并完成 loaded ACK：52/52 tracked 文件逐字节一致、missing=0、mismatched=0，并重新加载 SKILL / agent-delivery contract / long-task governance / control-event / lifecycle 关键规则。新规则继续保留 `absorbed / parked` retained semantics；同时把 Bug 修复及 migration / money-cost / authorization / state-machine / external-call / release-critical required review 升级为风险裁剪 TDD 因果证据审查：`tdd_required=true` 时必须绑定 `red_evidence + candidate_revision + green_evidence + red_green_same_case=true + reviewer_counterexample + verdict=PASS|FAIL`，低风险机械/文案修改不强造 RED/GREEN。后续每次 adaptive-delivery 更新固定执行“精确 revision 通知 → loaded ACK → 台账同步”。唯一 controller 仍为现有 SelfAlone 总控，不创建第二总控；已有工作包不因规则升级重做或改派。
...
| `M1-F4-C` | `VERIFY / CURRENT GOAL / SPLIT SERVER→WEB` / 项目总控 | AI / PPT 共用免费能力：平台额度可用时走受限平台能力；硬上限耗尽且未配置自己的文本模型时，在下一次需要模型的当前输入/任务位置引导配置并保留输入、会话上下文与 PPT 草稿；平台能力失败原位可重试 | A DONE；B DONE main@1bb0b76；文本模型真实H4与配置入口已存在；PPT draft/handoff合同已进main。先完成 `M1-F4-C-SERVER-GATE`，再释放 `M1-F4-C-WEB-HANDOFF`；不扩图片模型、完整PPT生成或Mini production H3 |
| `M1-F4-C-SERVER-GATE` | `ACTIVE / Grok 4.6 BACKEND / DELIVERED ACK PASS / TDD REQUIRED` | current-main 已有 trial claim、CostLedgerStore、用户自有 DeepSeek responder但未接线；Writer 在 `codex/m1-f4c-server-gate@f7219e3`，ACK-only `tool_call=0` 后完整 delivered ACK，ACTIVE lease PASS。建立可复用平台文本能力门：自有已配置模型优先且不计平台额度；未配置时仅已 claim + ledger available 才可走 server-owned metered platform adapter；request-scoped reserve→settle，失败 release；limit/external-unavailable 返回可区分失败并保留 retainedDraft；不得伪造实际成本或下发平台凭证 | owned files 仅 `platform-text-capability.ts/.test.ts`、`conversation-store.ts/.test.ts`、`conversation-responder.ts/.test.ts`、`index.ts`、`platform-text-capability-composition.test.ts`；cost-ledger/trial/model-config/deepseek 只读。第一步必须 current-code behavioral RED，production edit 前不得越门；候选后因 cost/external-call 必须新的 `tdd_required=true` 非作者 review |
| `M1-F4-C-WEB-HANDOFF` | `PENDING / DEPENDS SERVER-GATE` | Web conversation 与当前 PPT 草稿位置消费 server 的可区分 exhaustion/config-required/unavailable 状态：保留输入、会话上下文/PPT draft，在当前操作位置显示“配置自己的 AI 模型后继续”与配置入口；失败可原位重试，不常驻余额/金额 | 仅在 SERVER-GATE candidate 集成并冻结错误合同后转 READY；不提前并行猜 API，不碰 Mini production |
...
- 当前 worktree 共4个：main、唯一 `f312@f6a08f9`、`quota-cost@f47a990` 与当前 Writer `m1-f4c-server@f7219e3`；本轮临时 `/private/tmp/selfalone-m1f4b-review-f47a990` 与 `/private/tmp/selfalone-m1f4b-integration-audit` 在 review / scratch proof 完成且 tracked-clean 后已移除。f312 继续作为唯一 Mini DevTools / visual-qa 现场，代码已由 `main@43f1fc3` 等价吸收；`quota-cost@f47a990` 的完整成本链已等价顺序进入 `main@1bb0b76`，本事件终态收据将其标 `absorbed` 后仅保留恢复/审计现场；`m1-f4c-server` 仅服务当前 C Server Gate，不作为第二总控。不得为 reviewer / 占位 / 纯消息任务新建长期 worktree。历史归档 stash 共12条。
