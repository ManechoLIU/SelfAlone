...
- **Goal**：已创建并激活有终点的“控制面恢复与并行启动”里程碑；唯一项目总控任务为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`，本 Goal 只闭合真实主线 / 规则 / 服务 / 候选对账、台账纠正与 Desktop + Mini 首波 delivered ACK，不吞并其余开放 backlog。
- **已加载规则基线**：真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@18f709d0218f3abaf387f63a98940658962b2be4`；主线 `AGENTS.md` 来自 `19d44285aed7aa70711514a3a81fa25da4070fda`（SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`）；`adaptive-delivery/SKILL.md` SHA-256 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`；全局 context-governance 已完整加载五层 + Raw Sources 属于 Wiki + 证据链贯穿五层版本，SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`。
- **活动实现 / 复验线**：Desktop shared seam writer `01a03c67-e0b2-7de2-a458-c43192cc38e0` 已在 `csseam@6feede9` 固定 7 个接缝文件、ref36 与 EOF RED 后进入隔离 Chrome / API 自验，dispatch→完整 ACK `75.9s`；Mini 非作者 FINAL `01a03c68-f9c0-7423-9717-7883e84622ac` 已固定 `main@18f709d`、唯一 `f312@db14fed`、9 源文件 SHA SAME、ref11 / ref22 与旧证据 `5 errors / 8或11 warnings` RED 后进入 fresh compile / DevTools，dispatch→完整 ACK `95.1s`。两项均 `ACTIVE`；当前 controller 1 + writer 1 + reviewer 1，占用 `3 / 4`，可用 `1`；本波误中断 active writer `0`、clean-but-live 误判 `0`。
- **READY 重算**：本 Goal 两个 READY 均已派发并闭合 ACK；当前无第三个文件 / 环境互斥且不依赖本波结果的 READY 执行包，剩余槽保留给 Desktop 非作者 EARLY 或 Mini verdict 后的定向修复。`M1-F1` 路线图 READY 是父级导航，不是可直接派发卡；外部邮箱 / 微信 / 模型与 Mini session / keyboard / credential 仍受现有依赖或授权门。
- **下一用户可见检查点**：Desktop 交 shared-seam 范围单一候选并在真实 Chrome 完成 Selection EARLY；Mini 对 main 已含的 Settings / 长列表给出唯一 DevTools scoped FINAL PASS / FAIL 及 fresh console / overflow 证据。
- **真实阻塞**：Selection 私有链已在 main 但缺 shared route / UI seam 与真实 Chrome 证据；Mini 旧截图可证明状态矩阵但红色 DevTools 计数未闭合。Mini Library 仍受 M2-F1 session bridge / transport 阻塞，软件键盘与游客凭证仍留在 H3；远端、发布与真实外部调用继续走授权门。
- **当前可见入口 / revision**：Web `127.0.0.1:4173/#/auth`（PID `29697`）与 API `127.0.0.1:4100`（PID `29534`）仍从真实 main 目录运行，但进程启动来源为旧 `main@6e9e4c4`，schema 为 `selfalone_main_preview_6e9e4c4_20260826`；Web 可达、`/health` 为 `401 AUTH_REQUIRED`，只作入口 / 服务存活证据，不代表 `main@18f709d` 当前 Selection 或 Mini 验收。
...
| `M1-F3B-C` | `ACTIVE` · `PRIVATE MAIN MERGED / SHARED-SEAM ACTIVE / VERIFY` / 项目总控 + `01a03c67-e0b2-7de2-a458-c43192cc38e0` | 简单选择、显式确认、只读历史和失效问题的纵向切片；不含真实模型、图片或书籍上下文 | 私有链 `a08fbbc → 30e0423 → 6e21cad` 已在 main，但 main 的 route / conversation UI 未接线；种子 seam `6feede9` 仅有 EOF diff-check RED 且无真实 Chrome 证据 | writer 仅写 Server `app.ts` / `index.ts` / seam test 与 Web chat / selection view / `main.ts` / seam test 共7文件；不碰 main / 台账 / styles | ref36 SHA `1b6ec630…a5796b40`只绑定通用会话外壳，Selection本体按SPEC 155～158、DESIGN 140～145、DESIGN-WEB 132～139派生；共享 seam 候选须先真实1440 / 1024 / 1200 / 768 EARLY，再由非作者审查与总控顺序集成 |
...
| 当前桌面候选波次 | Auth / Settings、D-TEXT、Conversation与Book Detail既有限定结论保持；Selection私有链已至`main@a08fbbc → 30e0423 → 6e21cad`，shared seam `csseam@6feede9` writer ACTIVE | 三提交actual diff / `git show --check`已核，main fresh 8文件 / 44项通过；当前main仍无route / UI seam，Desktop任务固定7文件并在隔离Chrome / API自验，交付只可CANDIDATE / READY FOR EARLY。 |
...
| 当前 Desktop main 预览 | `RUNNING / PROCESS PROVENANCE 6e9e4c4 / CURRENT MAIN 18f709d NOT ACCEPTED` | Web `127.0.0.1:4173/#/auth` PID `29697`、API `127.0.0.1:4100` PID `29534`；schema `selfalone_main_preview_6e9e4c4_20260826`。Web可达、`/health`为`401 AUTH_REQUIRED`；旧进程不证明当前Selection接缝。 |
| 开发基线 | 本台账事件提交父为`main@18f709d0218f3abaf387f63a98940658962b2be4`；Selection三提交、Mini长列表 / Settings的实际diff与定向测试已核 | main只允许本次`TASK_LEDGER.md`事件diff；另有6个受保护DOCX / Word锁文件与3组受保护视觉候选目录，均与冻结a233副本同字节，禁止暂存 / 修改 / 删除。 |
