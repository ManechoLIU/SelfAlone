...
- 下一可见检查点：`M1-F4-V1` 的平台文本 runtime、结算 ACK 丢失恢复、北京时间双时段计费、真实 Store 并发硬上限与失败后重试本地门已分别在 `main@b637023+81f5f1f+df32890+ba70ced` 闭合。下一步只在外部条件同时满足时执行真实 Provider H4；等待期间总控重算不依赖该外部门的项目级 `READY` 旁路，不让整项目停在凭证 / 费用门上。
