...
| `M1-F2-WEREAD-RUNTIME` | `ACTIVE / M1-F2-WEREAD-RUNTIME-WRITER-01 / GROK 4.6 OAUTH` | 账户隔离 AES-256-GCM 连接存储，以及账户内 requestId 幂等的书架 / 批注 run、单领取恢复、终态重放与 last-success 快照均已进入 main；Writer 已从 `main@f072889` 在 `codex/m1-f2-weread-runtime` 启动正式 composition，owned files 仅 `weread-runtime.ts`、`weread-runtime.test.ts`、`index.ts` | 首个 RED=正式 Server 尚未注册 WeRead runtime；停止于 fake gateway API→DB→恢复的范围单一候选或精确越界依赖。真实 Key H4仍是独立授权门，禁止读取凭证或调用真实 Provider |
