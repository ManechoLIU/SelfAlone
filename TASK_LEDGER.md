...
- 当前阶段：桌面 Web `M1-F2-A` 功能、回归和主线集成收据继续有效，但用户基于当前真实截图指出山水几乎不可见、封面状态带遮罩过重，原视觉 `PASS` 已失效并回退为 `VERIFY`。文本 / PDF 阅读共享接缝已在 `44b81ea` 冻结，B、C 与小程序客户端基础包继续并行。
...
- 当前执行波次：`M1-F2-A-VISUAL-REOPEN`、`M1-F2-B-TEXT-READER`、`M1-F2-C-PDF-FIRST-PACK` 与 `M2-F0-MINIAPP-SHELL` 并行；A 审查者只读，其余执行任务文件范围互斥，共享 contracts / schema / 入口由项目总控单写入。
- 当前活动项：A 的长期非作者 UI/UX 审查由任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 负责，只捕获 / 对照截图、出具 brief 和复验，不写实现；项目总控收到 brief 后创建一个新的视觉修正任务，独占 A 的 Web 视觉代码、定向视觉合同和新证据。B 由任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` / `codex/m1-f2-b` 独占 `text-reader*`；C 由任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` 在 `7ef0` / `codex/m1-f2-c` 独占 `pdf-reader*`；M2-F0-A 由任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 独占 `apps/miniapp/**`。所有执行 / 审查任务均不得写 `TASK_LEDGER.md`。
- 协调下一动作：UI/UX 审查任务用本次新捕获的 `1440×844 @ DPR2` 真实页面与 `03-reading-library.png` 同状态对照，主动发现问题并给出山水、mask、状态带及整体层级的推荐范围；总控把结论交唯一实现任务修正，再由原审查任务复验，不通过不得恢复 A=`DONE`。B、C、M2 按既有边界继续推进。
...
| `M1-F2-A` | `VERIFY` / 项目总控 + UI/UX 审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` | 本地 EPUB / TXT / PDF 导入与桌面统一书架闭环；功能保持通过，本轮只重开已集成书架的视觉质量，不实现正文阅读 | 当前真实截图中山水层次几乎不可见、封面状态带遮罩过重；审查者必须独立对照绑定参考，现场 CSS 数值不是最终答案 | 审查者只读；brief 后由新视觉修正任务独占 `apps/web` 书架视觉、定向合同和 A 新证据；台账仍只由总控写 | 真实三类文件与既有功能回归收据复用；新门要求同态 Chrome `1440×844 @ DPR2`、200%、山水 / 状态带 / 网格 / 桌宠 / 对比度主动审美复核与非作者复验 | 功能候选 `ed2cec6 → 38f9ec3` 保留为恢复点；当前起点 `/tmp/selfalone-ui-audit-20260825/03-chrome-4380-current.png`。审查 → 唯一实现者修正 → 原审查者复验前不得恢复 `DONE` |
...
| `M1-F2-C` | `ACTIVE`（安全首包）/ 任务 `01a034f9-193d-7740-9f73-e5dd88f9c64d` | 文本型 / 图片型 PDF 页面阅读的解析 / 渲染、安全、缓存、恢复与页锚点基础；不 OCR、不伪造正文 | `44b81ea`；真实页面渲染需先明确生产依赖、许可证、体积和 Worker 资源边界 | `7ef0` / `codex/m1-f2-c` 独占 `pdf-reader*`、PDF Worker / 缓存、WEB、QA 与 C 证据；不写共享入口 / schema | 首包覆盖真实文本 / 扫描 / 加密 / 损坏 / 单页失败样本、租约重启、缓存隔离、旧版本拒绝与 fail-closed；完整门再加真实页面、笔记和浏览器 | `ready_pages` 仍只作检测摘要；首包必须回传依赖推荐，未获授权不得把占位或本机工具升级为完成 |
...
| `M1-F2-A` 已集成候选 | `ed2cec6 → 38f9ec3`，状态 `VERIFY` | 功能、回归和主线集成收据保留；原视觉通过结论被用户当前真实截图推翻。审查起点为 `/tmp/selfalone-ui-audit-20260825/03-chrome-4380-current.png`，绑定参考为 `redesign-v2/design-reference/03-reading-library.png`；需新审查、新实现候选与新复验。 |
| 开发基线 | 本地 `main` @ `11bd41e` | `38f9ec3` 已集成 A，`44b81ea` 已冻结 B / C 共享接缝，`11bd41e` 已释放并行阅读任务；主线两组未跟踪视觉候选继续保留且不得纳入台账提交。A 视觉修正必须从当前 main 新建隔离范围任务。 |
...
| `M1-F2-A` 书架闭环 | `VERIFY` | `ed2cec6 → 38f9ec3` 功能与主线恢复点有效；当前视觉因山水层次与状态带质感问题重新打开，任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 审查、唯一实现者修正和同任务复验后再裁决 |
