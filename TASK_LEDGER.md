...
- 当前 Goal：`STAGE-READ-NOTES-V1-J / M1-F2-WEREAD RECOVERING`；Desktop 候选 `dfb0574` 经 root 与非作者实际 diff 审查判 FAIL，7项 RED 已冻结；共享 books snapshot 缺 local opaque `bookId` 使 annotations 下游无法在不猜 provider ID 的前提下闭合，先由 `SHARED-WEREAD-BOOK-ID` 冻结上游 HTTP 投影，再同事件释放原 Writer。默认 no-call，不读取真实 WeRead Key，真实 WeRead H4 仍为独立授权门。Mini `M2-F2` 保持独立旁路，不改变本 Goal 完成条件。
- 下一可见检查点：`SHARED-WEREAD-BOOK-ID` 先把 API books snapshot 的 local `bookId` / provider `externalId` 双身份、multi-record 与 annotation round-trip写成失败合同并经 main验证；随后立即恢复 `M1-F2-WEREAD-WEB-REWORK-01` 的4测试文件 RED WIP。旁路 `M2-F2` 已 root FAIL且 Writer在同10文件恢复；唯一 f312 仍为 `4cdce97` 与既有保护现场，未写入候选。
...
- 规则版本：总控已从真实 `main@087b552` 加载当前 AGENTS；SelfAlone 后端 Grok OAuth 项目级路由授权以 `dfd83c7` 为唯一职责源。已加载 `adaptive-delivery@5b7ad46841bf61a059641cea2bb97f9ea94a1aee`；installed SKILL / agent-model-routing / external-agent-auth SHA-256 为 `d3b6b8f1…de73` / `61f611f5…7236` / `887de425…eb2`。外部执行可见状态只从下一次真实 Kimi / Grok 路由生效，当前原生 Writer不重跑 / 不改派；controller生命周期仍以 `0f0feb9` 注册，`hooks.json` 已配置但尚未由 `/hooks` 信任，故继续人工 `control_event_guard`，不得宣称自动 Hook 生效。
...
| `M1-F2-WEREAD` | `RECOVERING / dfb0574 FAIL / 4-TEST RED WIP` / 项目总控 | 原 Writer在既有4个测试文件新增71行并复现fresh假连接、unknown local bookId、空成功快照、常态手动同步、账户隔离、同requestId重试与annotation N+1；生产代码未改、diff-check PASS | 当前恢复负责人=`SHARED-WEREAD-BOOK-ID`；先冻结API books snapshot的local ID，再将 main合同提交传播给原 Writer并从该WIP继续，不丢弃 / 不提交为候选。其余6项仍在原10文件范围修复 |
...
| `SHARED-WEREAD-BOOK-ID` | `ACTIVE / ROOT EXTERNAL PREFLIGHT` / 项目总控 | 只修共享 HTTP books snapshot投影缺 local opaque `bookId` 的失真：保留 provider `externalId`、opaque cursor、multi-record、last-success与annotation request / response双身份；不改 adapter provider对象、不建HTTP路由、不读Key / 调真实WeRead | 路由=SelfAlone backend Grok Build `grok-4.6` / OAuth / high；owned=`packages/contracts/src/weread-api.ts`+`weread-api.test.ts`，先no-call preflight，候选经root实际diff / 定向 / typecheck / diff-check / main回归后才释放Web与Mini |
| `M2-F2` | `ACTIVE / 73a0f11 ROOT FAIL / REWORK ACKED` / `/root/m2_f2_weread_mini_writer` | no-call port默认 fail-closed保持有效；但 actual diff 新增无WXSS / 无 DESIGN-READY 的内嵌凭证表单、缺 disconnect、失败重试生成新 requestId且无请求 fencing、annotation local / external ID单位漂移；原候选不得进入唯一 f312 | `M2-F2-WEREAD-MINI-REWORK-01` 已在原 clean worktree / 同10文件完整 ACK并开始行为 RED；恢复动作=先删除自由派生内嵌UI并补行为回归，视觉需全屏配置页则冻结并拆 UI/UX+独立Writer。f312保持 `4cdce97` 与原 project.config / lock / QA保护差异，未写入 |
...
| `M2-F4-B` | `VERIFY / CANDIDATE 6f7cc87 / UPSTREAM MAIN PASS / ROOT REVIEW READY` / 项目总控 | root审Mini两文件diff与定向22项PASS；生产位置保存所需 Server UTF-16 单位已以 `main@ef10a9d+5fcd559` 关闭 | 下一独立候选事件重审`6f7cc87`并顺序集成 / 唯一f312；AppID / GUI / 域名不阻塞代码集成 |
