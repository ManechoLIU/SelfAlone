...
- 当前 Goal：`STAGE-READ-NOTES-V1-K / M2-F2 REAL-END VERIFY`；develop-only `wereadClient` 内部缺口已由 `f6a08f9` 闭合并通过独立非作者语义复审，随后顺序 fast-forward 进入唯一 `f312@f6a08f9`。f312 代码门为定向22、Mini全量246、typecheck、真实 build / package-size 与 diff-check PASS，既有 `project.config.json` / `pnpm-lock.yaml` / `visual-qa` 验收现场哈希保持不变。真实 DevTools 已在同一运行时分别验证 A、B 凭证首次 deterministic fail-once 后原位同 requestId 重试成功；后续 build 自动重载按设计清空内存 port，因此不得把重载前证据冒充页面重进 / 断开闭环。真实 WeRead Key / AppID / apiBaseUrl / 域名 H3 不在本 Goal，也不得用它们绕过 develop-only QA 边界。
- 下一可见检查点：`M2-F2` 只剩同一未重载 DevTools 运行时内的 B 书架隔离、页面重进与断开后已水合内容保留 / 新调用 fail-closed 三项真实端证据；当前 DevTools CLI service port 按既有安全设置关闭，Compute Helper 对模拟器的稀疏辅助树 / 坐标映射暂不稳定，因此这些证据仍记为未完成而不是产品 FAIL。只有获得稳定 GUI 或人工辅助的直接可见证据后才允许从 VERIFY 升级并进入 main；不得为验收改页面、开启 DevTools 安全端口或引入真实 Key。并行 `M1-F4-B` 继续保持 BLOCKED，不影响 Mini 主线。
...
| `M2-F2` | `VERIFY / F312 f6a08f9 / DEV-WEREAD NONAUTHOR PASS / REAL-END PARTIAL` / 项目总控 | `08d9597` 的 F1/F2 已由两文件恢复提交 `f6a08f9` 修复；独立非作者复审确认同语义 / 同 live connection replay 保持幂等，跨 operation / book / account 与 disconnect / replace 后陈旧 replay / run lookup 均 fail closed，无新错误码、网络、storage 或真实 Key seam。`f6a08f9` 已顺序进入唯一 f312，定向22、Mini246、typecheck、真实 build/package-size、diff-check PASS。真实 DevTools 同一运行时已验证 A 与 B 各自首次 fail-once、输入保留、第二次原位同 requestId 成功；随后 build 自动重载清空内存 port，故未把重载前状态冒充页面重进证据 | 检查点：只补同一未重载运行时的 B 书架隔离、页面重进、disconnect 后已水合内容保留 / 新调用 fail-closed 直接证据；当前 CLI service port 保持关闭且 Compute Helper 模拟器定位不稳定，未完成部分记 evidence gap，不得据此宣称 M2-F2 REAL-END PASS 或进入 main，也不得为验收修改页面 / 合同 / 安全设置 |
| `M2-F2-MAIN-INTEGRATION-CHAIN` | `READY / READ-ONLY DEPENDENCY AUDIT` / 待派 Kimi | 识别 `f6a08f9` 从唯一 f312 顺序进入 main 所需的最小已审 prerequisite 提交链；禁止修改源码、禁止新实现、禁止扩大到整个 Mini 历史 | 输出 exact prerequisite revisions、patch 等价性、main 已有/缺失 seam、推荐 cherry-pick 顺序与每步验证命令；如无法证明最小链则 BLOCKED，不得猜测 |
...
- 当前 worktree 共4个：main、唯一 `f312@1c9d445`、待恢复候选 `quota-cost@f53c930` 与已通过代码复审、等待f312真实端门后顺序进main的 `weread-mini@c60ce3a`；`weread-web@c2c009d` 合入后已同事件移除clean worktree。不得为 reviewer / 占位 / 纯消息任务新建 worktree；候选合入 / 废弃后同事件移除clean worktree，脏现场先形成scoped commit或带原路径+HEAD的恢复点。历史归档stash共12条，f312仅在唯一DevTools入口仍需时保留。
