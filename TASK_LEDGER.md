...
- 当前执行波次：`CONTROL-RESET-1`；Web 候选 `1727f95` 已按用户裁决判为 `FAIL / IA REOPEN`，非作者 02 / 04 / 05 / 06 / 33 structure matrix 与主线事实源 `f1b48b0` 已就绪，原 6afd 唯一写入者正在整改；D-TEXT 共享候选 `ed95d95` 正在独立代码 / PostgreSQL 复审；小程序 `d4f587c` 已冻结 reader 短屏和非视觉定位修正，等待同一 UI/UX 任务按全端 `DESIGN-READY` 合同复验。`f312` 仍独占 `apps/miniapp/**`，后续全端视觉整改由同一写入者顺序承接；台账仍由项目总控独占。
- 当前活动项：`/root/desktop_shell_m0_fix` 在 `6afd@1727f95` 之上把四阶段收敛为右侧单一互斥工作区；`/root/reading_notes_integration_review` 只读复审 `dint@ed95d95`；UI/UX 任务 `01a03652-a5d4-76c1-b0fa-ff9303243664` 在新 Web 候选返回后做阶段矩阵终验，随后复验 `f312@d4f587c`。`M2-UX-CONTRACT-ALL` 的逐页合同已完成；小程序透明山亭与真实 PPT 模板 / 页面预览资产仍是局部资产阻塞。既有未暂存 `pnpm-lock.yaml` 禁止触碰。
- 协调下一动作：6afd 只实现单一右侧当前任务工作区，中栏仅保留会话、轻量阶段摘要 / 系统消息和输入；新候选逐阶段覆盖前进 / 返回 / 刷新 / 断网与四宽度后重交非作者。D-TEXT 与小程序候选分别取得独立裁决后，总控审查实际 diff、合并并复验 main。小程序后续按 `b45a7a8` 与 4.1 合同顺序处理不依赖新资产的整改；山亭与真实 PPT 预览只在正式运行时资产登记后接入。每个局部包关闭后仍有安全 `READY` 项就立即切换。
...
| `M1-WEB-SHELL-01` | `ACTIVE / IA REWORK` / `/root/desktop_shell_m0_fix` | `6afd@1727f95` 之上仍限共享 shell / conversation / main / styles / app-state、测试与证据，不改 reader / Server /台账 / 事实源 / 锁文件 | 02 / 04 / 05 / 06 / 33 structure matrix 已完成并由 `f1b48b0` 固化；整改四阶段为右侧唯一互斥视图，中栏无完整阶段控件，标题 / current step / 正文原子一致，覆盖 1440 / 1200 / 1024 / 768、前进 / 返回 / 刷新 / 断网。冻结后再交原非作者终验 |
