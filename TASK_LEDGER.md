...
- **活动实现线**：Auth 已顺序合入 `main@b6b77ee` 并完成本地注册 / 会话 Main Case；Settings 已合入 `main@d9a3b46`，但真实 FINAL发现密码草稿落 `localStorage` 的 `P0`，唯一 writer已派发且待 exact preflight。Conversation `b8f34b4` 私有 Case通过，shared seam继续等 Settings P0关闭。Mini Reader `f312@db40e99` 作者四宽 DevTools `EARLY PASS`，非作者 FINAL已派发。当前构成：controller / integrator 1、implementation writer 0（Settings dispatch ACK pending）、reviewer 1。
- **下一用户可见检查点**：Settings writer先 RED并冻结不落盘密码的最小候选，总控审查 / EARLY后修复前进 main并重跑真实 refresh / new-tab Case；随后释放 Conversation shared seam。Mini Reader并行取得非作者 FINAL，PASS只关闭 Reader范围；Mini Conversation仍保留 H3门。
- **真实阻塞**：Settings `P0` 根因已定位 `apps/web/src/settings-state.ts` / `main.ts`：三个密码字段被序列化并跨刷新 / 新标签恢复，当前 main不得作为稳定入口。Conversation `desktop-shell.ts` 又与该 Settings修复顺序冲突，解除条件是 P0 candidate冻结并前进 main。Mini Conversation的软件键盘 / reduced-motion、Book Detail shared route seam及真实邮件、微信、模型、Presenton、push / PR / 发布 / 部署仍未闭合或未授权。
- **当前可见入口 / revision**：当前 main候选 Web `http://127.0.0.1:5188/` / API `http://127.0.0.1:4308` 绑定 `d9a3b46`、schema `selfalone_main_auth_settings_20260826` 与 `/tmp/selfalone-main-auth-settings.XE5Dfd/`；只作 Auth Main Case PASS与 Settings P0复现入口，不是稳定 PASS。稳定恢复点 `4127 / 4198` 仍绑定旧产品 revision `ab13310`。Mini唯一 DevTools为 `f312@db40e99`，保护 config / lock与两张既有 QA图未变。
...
| `M1-F1-B` | `VERIFY` · `MAIN MERGED d9a3b46 / FINAL FAIL(P0) / FIX DISPATCHED` / `/root/desktop_conversation_slice` | 邮箱 / 密码修改与桌面设置闭环；真实 main已复现敏感草稿跨刷新 / 新标签恢复，EARLY PASS被重开 | writer exact preflight前不得写；P0关闭前不释放 Conversation shared seam。非目标仍是真实邮件送达、微信绑定 | owned `settings-state{,.test}.ts`、`main.ts`，测试证明确需才含 `settings-page.test.ts`；禁止其它文件 | main全仓43文件/230项、typecheck/build通过但不抵消P0；证据 `main-auth-settings-final-d9a3b46/`。下一步RED→最小candidate→非作者 EARLY→main重验 |
...
| `M1-F1-B-SETTINGS` | `VERIFY` · `MAIN MERGED d9a3b46 / FINAL FAIL(P0) / REOPEN 1 / FIX DISPATCHED` / `/root/desktop_conversation_slice` | 设置 normal / failure / logout / account服务已集成，但三个密码草稿明文落 localStorage且跨刷新恢复 | exact preflight前停写；修复只保持同页内存保留，持久化仅非敏感email并清洗旧记录 | `settings-state{,.test}.ts`、`main.ts`，必要时`settings-page.test.ts` | EARLY旧证据保留未受影响局部；FINAL P0证据 `main-auth-settings-final-d9a3b46/`。新candidate必须重跑 refresh / new-tab / email-only / focus |
...
| `M2-F0-A-VISUAL-REWORK` | `VERIFY` · `READER f312@db40e99 AUTHOR EARLY PASS / FINAL DISPATCHED; CONVERSATION 891d26e PARTIAL` / 项目总控 | Reader两文件修复已进入唯一f312并在390 / 320 / 360 / 430真实DevTools通过；Conversation同态局部通过但H3未覆。保护差异不变 | Reader非作者 FINAL首个P0/P1即停；Conversation不因Reader PASS自动升级 | Reader `pages/reader/index.wxss` / `page-state.test.ts`；Conversation仍为其私有两文件 | Reader证据 `mini-reader-early-db40e99/`；FINAL派发待preflight。keyboard=0 / reduced-motion边界保留，drawer独立 |
| `M2-F0-A-MINI-NOTES-VISUAL-REWORK` | `VERIFY` · `CANDIDATE db40e99 / AUTHOR EARLY PASS / NONAUTHOR FINAL DISPATCHED` / `/root/book_detail_visual` | refs 18～20老己笔记单列流、sheet手势、背景保留与共享底栏；390 / 320精确FAIL Case、360 / 430均通过；非目标：真实notes API / 跨设备 | reviewer exact preflight前不虚报ACTIVE；只读唯一f312，禁止源码 / config / lock / QA写入 | `pages/reader/index.wxss` / `page-state.test.ts`候选已冻结；保护 config / lock / 两QA图不变 | 24项/typecheck/build与作者DevTools只作EARLY收据；证据 `mini-reader-early-db40e99/`。下一步非作者同态FINAL，keyboard / reduced-motion仍明确未覆 |
...
| 当前桌面候选波次 | Auth已进main并完成 scoped Main Case；Settings已进main但FINAL P0重开且fix dispatched；Conversation私有Case PASS / shared seam等待；Book Detail私有候选冻结 | main候选 `5188 / 4308@d9a3b46`；证据 `main-auth-settings-final-d9a3b46/`、`conversation-early-b8f34b4/`。当前稳定入口仍不替换。 |
| 当前 Mini 候选波次 | Reader `f312@db40e99` 作者四宽EARLY PASS、非作者FINAL已派发；Conversation `891d26e`局部通过但H3未覆 | Reader `mini-reader-early-db40e99/`；唯一DevTools与project config / lock保护现场未变。PASS不自动代表Mini父项或Conversation完成。 |
...
| 开发基线 | 产品顺序集成到 `main@d9a3b46`；Auth scoped Main Case PASS，Settings FINAL P0重开 | main tracked clean，三个既有未跟踪视觉目录原样保留；当前main候选不能替换稳定入口，修复候选 / 测试 / 自验也不能自动 `DONE`。 |
