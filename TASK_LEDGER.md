...
| `M1-F4-V1-PRICING-SCHEDULE` | `ACTIVE / M1-F4-V1-PRICING-SCHEDULE-GROK-01 / DELIVERED ACK PRE-TOOL=0` / 项目总控 | 根因已定界：`deepseek-v4-flash` 代码模型与官方当前模型一致，但 Server 只在启动时解析一组 cache-hit / cache-miss / output 价格，无法按北京时间工作日高峰窗口选择真实成本；Grok 4.6 OAuth Writer 已在任何仓库工具前对 worktree、base、四个 owned files、规则 revision、路由与停止条件显式 ACK，首回合无写入且 transport completed / delivery unresolved | TDD 先锁工作日 11:59→12:00、14:00→18:00与周末边界的 RED，再继续同一 Assignment 实现；最小实现要求部署侧同时提供高峰 / 空闲六项人民币微元价格，调用开始时按 Asia/Shanghai 选档，旧单档或部分配置拒绝启动。禁止读取 Key、调用真实 Provider或修改 Web / Mini / 成本表 |
