...
| `M2-F3` 小程序会话 | `FAIL / DESIGN-READY ACTIVE / WRITE HELD` | 会话抽屉、文字 / 图片、结构化选择层、确认摘要、停止、模型与额度恢复 | 当前 `2deb4f6` 的常驻 PPT intent 双按钮卡、工程诊断文案与会话层级违反 `main@3199945`；UI/UX brief 返回前不释放 conversation 私有写入 | 正常模式内部文案扫描；选择层打开 / 关闭 / 保留 / 确认摘要；单一阶段入口；320 / 360 / 390 / 430、font23、safe-area 真实 DevTools EARLY / FINAL；软件键盘外部门单列 |
...
| `M1-F2-D-TEXT-WEB-ENTRY` | `VERIFY / FROZEN 6e62153 / NONAUTHOR FINAL ACTIVE BY RUNTIME` / `/root/verify_dtext_entry_recovery` | `/Users/echoman/.codex/worktrees/dentry/SelfAlone`、`codex/m1-f2-d-text-entry@6e62153` clean；失效 reviewer 已回收，替代只读 reviewer已启动隔离 API `127.0.0.1:4127` 与 Web `127.0.0.1:4387`，仍待原样 ACK / verdict | 作者 `pnpm verify` 32 files / 182 tests、typecheck / build / diff-check 与 1440 / 1200 / 1024 / 768×844 DPR2 真实 TXT / EPUB / 409 / owner / PPT handoff / overflow / console通过；替代 reviewer 按同一 Case FINAL。PASS 前不集成；原生200%、真实删除、真实AI/PPTX继续精确 VERIFY |
