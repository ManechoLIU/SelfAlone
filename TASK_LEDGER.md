...
- 当前执行波次：`CONTROL-RESET-1`；Web 已冻结 clean tracked 候选 `47db65b`，16 文件 / `+929 -89`，单一右侧阶段工作区、hash / history 单一 `renderRoute`、阶段草稿 / scroll / focus 与失败恢复已有测试和作者侧真实 Chrome 收据；总控新鲜定向 4 文件 / 30 项及全仓 24 文件 / 131 项、typecheck / build 通过。当前可见 main 仍是旧 M0，候选只剩未跟踪本地产物 `data/`，视觉 / 路由终验尚待非作者，当前禁止合并。D-TEXT 私有 Web 已冻结 `e2a6ac4`，全仓 30 文件 / 140 项、typecheck / build / diff-check 与作者侧真实 Chrome Case 通过，现转非作者视觉 / 交互复验，未接共享入口前不称用户可见闭环。小程序已冻结 `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f` 九个局部恢复点；`598e34f` 只关闭 conversation 趴宠锚与 drawer 纸白开放列表局部返工，正式山亭、软件键盘与最终全路由 UI/UX 仍缺，整体保持 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：`/root/reading_text_notes_web` 已冻结 clean 候选 `e2a6ac4` 并解除私有文件写入；同一非作者 UI/UX 当前已真实进入该候选的隔离 Chrome / schema 复验，共享入口仍由总控排队。原 Web 实现者 `/root/desktop_shell_m0_fix` 已冻结 `47db65b` 并解除 shared shell / conversation / main / styles 写入，FINAL-VISUAL-VERIFY 排在 D-TEXT 清理之后；4173 旧 main 已排除。小程序任务已提交 `598e34f` 视觉返工局部点，9 文件 / 53 项、typecheck / build / diff-check 通过，四宽各两张真实 DevTools 收据已冻结；其二次非作者复验排在 Web 之后。两个受保护差异保持未提交。
- 协调下一动作：UI/UX 顺序完成 `e2a6ac4` D-TEXT → `47db65b` 桌面对话 → `598e34f` 小程序局部视觉三项复验，每项独立清理端口 / schema / 进程后再换项；未轮到的候选只记 QUEUED。作者只在明确 FAIL 时恢复最小返工。小程序同一 writer 可继续不与审查冲突的其余状态矩阵；全端候选冻结后再做四宽四高、五态 / 安全区 / 键盘 / 字体 / reduced-motion 终验。
...
| `M1-F2-D-TEXT` | `VERIFY / CANDIDATE FROZEN / NONAUTHOR ACTIVE / ENTRY QUEUED` / `/root/reading_text_notes_web` | EPUB / TXT 可靠 locator 上的划线、想法与无标题手工老己笔记 CRUD；保存失败保留草稿 | clean 候选 `dtxtui@e2a6ac4` / `codex/m1-f2-d-text-web`，29 文件 / `+1695 -2`；全仓 30 文件 / 140 项、typecheck / build / diff-check 通过，作者侧真实 Chrome 覆盖选区、划线 / 想法、笔记 CRUD、刷新恢复、失败保留、主题 / 专注 / Esc 与账户隔离 | 私有 writer 已停止写入；非作者已固定 HEAD / clean 并启动隔离 Chrome / schema 复验；`main.ts` / shared shell / styles 等 Web shell 合入后由总控顺序接入；聊天 handoff 在共享路由接入前保持 disabled fail-closed | 真实 EPUB / TXT 选区→划线 / 想法 / 老己笔记 CRUD，失败保留、刷新恢复、owner / fileVersion 隔离、返回原文与绑定参考真实 Chrome通过 | 作者证据不替代非作者 UI/UX；等待当前复验 verdict 与清理收据。入口 / 账户 / quote intent 尚未接入，不能称用户可见闭环完成 |
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f` 已形成九个局部恢复点；`598e34f` 为 12 个 `apps/miniapp/**` 文件，关闭 conversation 可见下缘贴输入上沿与 drawer 纸白开放列表局部实现。后续非可见 state route 已真实到达 library filtered-empty 与 reader failed | library filtered-empty 的查询 / 清除和无横溢为局部绿；reader failed 新鲜 360 DevTools 证据为 RED：整页丢失书名、Reader shell、背景 / 位置上下文，只剩错误与重试。同一 writer 已 ACK 并仅在 Reader 私有范围开始 TDD；总控定向复现为 12 tests / 2 failed / 10 passed，缺 `preserveReaderFailureContext` 且 WXML 仍走整页失败态，子项升级为 `ACTIVE / TDD RED CONFIRMED`。drawer 正式山亭仍 ASSET BLOCKED，软件键盘仍为 0；其余总门未关闭 |
...
| `M1-F2-D-TEXT` | `VERIFY / CANDIDATE FROZEN / NONAUTHOR ACTIVE / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@e2a6ac4`、`codex/m1-f2-d-text-web`；29 个私有 Web / acceptance 文件，worktree clean，不碰 6afd 的 `main.ts` / shared shell / styles 或共享 contracts / Server | 全仓 30 文件 / 140 项、typecheck / build / diff-check 与作者侧真实 Chrome Case 通过；非作者已启动选择工具、10 号参考内容节奏、失败保留、浅深 / 专注 / 键盘 / 44px 的隔离真实 Chrome 复验。全局路由仍在 Web shell 合入后由总控顺序接入 |
...
| `M1-WEB-SHELL-01` | `VERIFY / CANDIDATE FROZEN / NONAUTHOR QUEUED` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main 旧 M0；`6afd@47db65b` 为 16 文件 / `+929 -89` 冻结提交，新增单一阶段工作区、`conversationHref` / `lastConversationStage`、workspace / cache、requirements / outline draft、双区 scroll 与单一 `renderRoute`；acceptance README 已纳入提交，未跟踪 `data/` 不属于候选 | 总控定向 4 文件 / 30 项、全仓 24 文件 / 131 项、typecheck / build / diff-check 通过。作者收据声明 768×844 全阶段 composer 可见、fresh #/conversation→library→返回、reader→conversation、刷新 / 前后退 / `?stage=`、断网草稿 / scroll / focus 与真实 retry 恢复；D-TEXT 清理后再由同一非作者真实 Chrome 重验消息密度、02 Token、桌宠锚、四宽与路由同源，结论前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / NINE CHECKPOINTS COMMITTED / DESIGN-READY / STATE-MATRIX REWORK / REVERIFY QUEUED` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | 九个局部恢复点保持；新增非可见开发 state routes 只用于真实端取证，不进入 Settings UI。Reader failure 修复已由同一 writer 在 `page-state.test.ts` 先写红灯，后续写入限定 `page-state.ts / index.ts / index.wxml / index.wxss` 与既有测试；总控复现 12 tests / 2 failed / 10 passed，状态为 `ACTIVE / TDD RED CONFIRMED`。非作者按 D-TEXT→Web→Mini 顺序执行；软件键盘仍未实显，正式山亭缺失。全路由终验前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 九个局部点已冻结；`598e34f` 只关闭 conversation 趴宠锚与 drawer 纸白开放列表局部实现并补四宽收据，仍等待非作者复验。软件键盘、正式山亭 / PPT 资产、其余状态与最终全路由视觉均未完成；既有受保护差异保持未提交。 |
...
| `M1-F2-D-TEXT` 共享集成与私有候选 | `263d3e2 → 18c7522` 私有种子；`94056be → ed95d95 → 5156b9c → main@f85d86c`；私有 Web `e2a6ac4` | 共享 contracts / migration / API / publisher 已通过原非作者与 main 复验；私有种子不再单独合并。`dtxtui@e2a6ac4` 已冻结并排入非作者复验，shared entry 继续等待 Web shell 所有权释放。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f` 是九个局部恢复点；conversation / drawer 局部视觉候选不替代全小程序六页 / 共享层四宽四高和状态矩阵，真实软件键盘、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
