...
| `M2-F0-A` 原生运行壳与客户端状态 | `VERIFY / 集成修正` / 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` | 原生小程序运行壳、登录 / 书架 / 连续阅读 / PPT 状态边界、正式资产与五类状态；未就绪服务走显式开发适配器 | `f312@142b6f2` 范围提交已形成；集成审查发现阅读背景错误实现为五种且未持久化，另需把漂移的 `libVersion: trial` 固定到实际验收 `3.17.1`；仍只写 `apps/miniapp/**` | 修正为浅 / 暗两种并在开发适配边界恢复 / 保存；根 `pnpm verify` 已通过 12 文件 / 44 项。H3 仍缺凭证化 clean console、最终 360 / 390 / 430 + 高度、键盘、系统字体、批准山水资产和非作者复验，不得标绿 |
...
当前有两个实际写入卡与一个只读审查卡。A 任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 在 `59ae` 继续非右山整改，右山 hunk 冻结等待审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 的聚焦 brief；M2-F0-A 任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 只修 `apps/miniapp/**` 的两背景 / 持久化与基础库固定。B 任务在 `da89` idle 保留五文件恢复点，C 原任务已停止。总控独占 contracts、schema、共享入口、Worker 注册、台账和最终集成。
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2`，状态 `VERIFY / 不合并` | 106 文件、3439 行且仅 `apps/miniapp/**`，工作树干净；开发适配器 fail-closed、范围和根 `pnpm verify` 通过。集成审查拒绝五种未持久化阅读背景与 `libVersion: trial` 漂移；H3 的游客 console、最终矩阵 / 键盘 / 字体、批准山水资产和独立复验继续缺失。 |
