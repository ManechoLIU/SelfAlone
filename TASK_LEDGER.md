...
- 当前执行波次：`CONTROL-RESET-1`；Web `221d5bb` 的阶段、草稿、滚动与焦点证据仍有效，但 UI/UX 已因书架 `.library-rail` 与对话 `.desktop-rail` 两套组件树撤销 FINAL 前提，改判 `STRUCTURAL FAIL / REWORK` 并先补共享外壳 brief。D-TEXT 已冻结 `d65ab3b`，总控新鲜复跑 30 files / 146 tests、typecheck / build / diff-check 全绿，等待非作者复验。小程序 `f81b463` 全矩阵审查已因 drawer 与 Reader 视觉偏差判 `VISUAL FAIL / REWORK`；旧全端合同只保留历史。台账仍由项目总控独占。
- 当前活动项：Web 新唯一 writer `/root/desktop_shell_unify` 已从 `6afd@221d5bb` 派发但尚无首个 ACK，状态 `DISPATCHED / ACK PENDING`；所有权仅限 `apps/web/src/main.ts`、`apps/web/src/ui/desktop-shell.ts`、`apps/web/src/styles.css` 与必要测试。UI/UX 当前 `active`，只读形成 02 / 03 共享左栏 brief，不再继续旧候选 FINAL。D-TEXT `d65ab3b` 保持冻结待非作者裁决。Mini `/root/mini_visual_rework` 同样待真实 ACK；`IMG-04` 仍只是候选，正式运行时资产未通过。
- 协调下一动作：UI/UX 先交共享壳 brief，Desktop writer 再以单一 shell / nav / brand / rail 组件闭合 fresh conversation、library 往返与状态恢复，冻结后重新 FINAL；收到 desktop / mini 的实际 cwd、HEAD、status、首组文件与 RED/GREEN 回执前不升级 ACTIVE。D-TEXT 可只读复验但不得抢共享入口；Mini 必须重跑当前 WIP 并完成 EARLY→FINAL。
...
| `M1-F2-D-TEXT` | `VERIFY / FROZEN d65ab3b / NONAUTHOR QUEUED / ENTRY QUEUED` / `/root/reading_text_notes_web` | `/Users/echoman/.codex/worktrees/dtxtui/SelfAlone@d65ab3b`、`codex/m1-f2-d-text-web`；只含私有 text-annotation / book-detail / text-reader 与 acceptance，作者 worktree clean | 定向 42 / 42；总控新鲜全仓 30 files / 146 tests、typecheck / build / diff-check 通过；作者真实 Chrome 覆盖 TXT / EPUB、1440 / 768×844 DPR2、Selection 清理、独立笔记、tab / AX / focus、失败 / 409、浅深、overflow / console。待 Web 终验后由非作者复验；共享路由 / chat seam 仍未接入 |
...
| `M1-WEB-SHELL-01` | `STRUCTURAL FAIL / REWORK / DESIGN BRIEF ACTIVE / IMPLEMENTATION DISPATCHED / ACK PENDING` / `/root/desktop_shell_unify` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | `6afd@221d5bb` 是返工基线；`main.ts:libraryShell()` 仍生成 `.library-rail`，conversation 才使用 `renderDesktopAppShell()` / `.desktop-rail`。唯一 writer 只可写 `main.ts`、`ui/desktop-shell.ts`、`styles.css` 与必要测试；不得碰 Reader / D-TEXT 私有文件 | 保留 `221d5bb` 已通过的阶段、草稿、滚动、焦点与四宽子证据，但先按 02 / 03 brief 抽成单一 shell / nav / brand / rail 组件。fresh `#/conversation`、conversation→library→return、刷新 / 前后退 / `?stage=` / 断网恢复必须落同一组件树与当前会话 / 阶段 / 草稿；首个真实 Chrome 同状态比较→scoped commit→非作者 FINAL，通过前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `FAIL / VISUAL REWORK / CANDIDATE MOVING / EARLY-VERIFY PENDING / H3 BLOCKED` / UI/UX + `/root/mini_visual_rework` | `f81b463` 后仅 drawer / Reader 视觉文件允许新唯一 writer 顺序写入；原 f312 writer已停止这两处，台账与保护文件禁止触碰 | UI/UX 已基于真实 360×640/font23 判 drawer / Reader FAIL；现 WIP 尚未重跑，2 张图只作 EARLY 线索。writer ACK→RED/GREEN与DevTools→EARLY-VERIFY→scoped commit→FINAL-VERIFY 后才可重新冻结；正式山亭资产缺失保持独立阻塞 |
| `M2-F0-A-VISUAL-REWORK` | `DISPATCHED / ACK PENDING / ASSET CANDIDATE / RUNTIME BLOCKED` / `/root/mini_visual_rework` | 当前仅 `nav-drawer/index.wxml|wxss|test.ts`、`reader/index.wxss|page-state.test.ts` 与对应新收据；不得改其他六页或保护文件 | 重跑当前最终 WIP；320 / 360 / 390 / 430 与 font23 实测 drawer 列表滚动 / surface、Reader 标题与提示生命周期。`IMG-04` 只可作为候选交 UI/UX、实装与真实 DevTools 对照，未通过前不得升级运行时资产。首屏可运行即交 UI/UX EARLY，冻结后再 FINAL |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307 → 6aba8ef → 36aeb28 → 5d51c53 → 598e34f → 6586bca → b28aa9c → 79c9d8b → e4d94b4 → a952a49 → f81b463 + visual WIP`，状态 `FAIL / VISUAL REWORK / CANDIDATE MOVING / H3 BLOCKED / 不合并` | `f81b463` 仍是确定性功能恢复点，但当前 drawer / Reader WIP 使其失去 FINAL 候选身份。UI/UX 已给短 brief；正式山亭、软件键盘、PPT 资产 / 真实预览仍缺，保护差异保持未提交。 |
...
| 开发基线 | 产品集成恢复点 `main@f85d86c`；当前控制面续接于 `main@f216fba` | A 功能闭合但视觉重开；B / C 共享接缝和 C 安全首包已进入 main，D-TEXT 共享接缝已合并但 Web 闭环返工中；Web shell / miniapp 候选未集成。主线三个未跟踪视觉候选目录仍属他人现场，不得纳入台账提交。 |
