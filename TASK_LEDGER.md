...
- **Goal**：当前有终点里程碑为“Desktop `M1-F4-A` 在当前main完成内容等价复核与真实Chrome / DB H3收据；Mini Drawer v5按冻结根因brief完成一次原writer返工、唯一f312的320→360→390→430 SELF与非作者EARLY；并行对Mini PPT draft/context候选完成行为与UI派生EARLY”。不把旧收据、静态门、开发适配器或fake provider冒充当前用户可见闭环。唯一项目总控任务仍为 `01a03c61-5dd2-7553-968b-a3bc2f5777c9`。
- 当前执行波次：`DESKTOP-QUOTA-PASS-MINI-DRAWER-CANDIDATE-PPT-INTEGRATION-READY`（三项候选 / verdict已回收，下一事件按共享入口顺序处理；不把候选冒充main或唯一f312完成）
- 当前活动项：无
- 协调下一动作：下一独立事件先由总控审Mini PPT实际两提交并顺序放入唯一f312，完成点击制作PPT→当前会话预填→不自动发送 / 开sheet及失败刷新保留SELF；Quota PASS回写、Drawer候选集成与DeepSeek非作者EARLY各自保持独立事件，不混入本次控制提交。
- **已加载规则基线**：本事件从真实仓库 `/Users/echoman/Documents/SelfAlone`、`main@44078ed5119137a6ef866c4bc2919a76b4f8e863` 复核；main无tracked diff，既有9组受保护未跟踪路径仍在。总控与本波次任务均已加载 `adaptive-delivery` 正式提交 `9967bb8a492a905799c6b5692625c454acf2e8ef`；全局 `SKILL.md`、`long-task-governance.md`、`methods.md` SHA-256依次为 `5b441a744dd3250d78879caf734ca23725ba4a7775e93f02ee9eb9714849e1ec`、`a0f6abe6298e9063a151b3527dbf0636a66598144701921f79d28506a5cf09c5`、`109f5fabddb39dad44dd238624842baa1c094111cbc4818ebd16b7225a021e59`。主线 `AGENTS.md` SHA-256 `fa2f1eef816c137841fe5ed53b73550ee796573acd5910b1896e4d83ff241850`，五层 context-governance SHA-256 `21b4a05a0ca6f5582cb92f8c76aa9eba9939f6c0a9a78e8aa7d26b6011ee3919`；Mini PPT事实源提交`c06ec53`的SPEC / DESIGN-MINIAPP / TECHNICAL三份实际diff已重新核对，无需重复修改。
- **活动实现 / 复验线**：Mini PPT替补writer `/root/mini_ppt_handoff_writer_v2` 的delivered ACK保留：绝对cwd `/Users/echoman/.codex/worktrees/mppt1/SelfAlone`、分支`codex/m2-mini-ppt-draft-handoff`、基线`c06ec53`、初始clean、六文件ownership、核心5项RED与runnable候选停止条件均闭合；已交`d8e1e7`六文件候选及`49e0e5e`一文件失败→刷新恢复回归，当前`mppt1@49e0e5e` clean。非作者`/root/mini_ppt_handoff_early_review`判`EARLY PASS / NO NEW CARD`：refs30～32足以承接现有composer与发送后选择，refs23～29只属于后续阶段，不需WXML / WXSS或ImageGen。Quota与Drawer候选回执也已到达但留待各自事件处理；当前仅总控live，`1 / 4`、可用`3`。
- **READY 重算**：Mini PPT为`VERIFY / READY FOR CONTROLLER DIFF REVIEW + UNIQUE f312 INTEGRATION`；释放条件已经满足，不再等待DeepSeek或Drawer。Drawer `1138669`为`VERIFY / READY FOR f312 INTEGRATION`，但与PPT共用唯一f312，须等PPT本次顺序集成 / SELF释放入口；Quota为`VERIFY / CURRENT-MAIN PASS RECEIVED`，下一Desktop控制事件可直接关闭；DeepSeek `c346772`非作者安全 / UI EARLY与Desktop D-TEXT真实删除复验均文件互斥且READY，本次因用户明确限定Mini PPT原子控制事件后立即yield而未派发。Mini notes真实save / edit / delete仍与PPT的`reader/index.ts`集成边界重叠，精确释放条件为PPT进入f312并冻结该文件。
- **下一用户可见检查点**：唯一f312中从书籍详情 / Reader点击“制作PPT”后仍进入当前会话，输入框只预填可编辑“帮我制作这本书PPT”，不自动发送、不弹范围sheet、不建task；失败后刷新仍保留草稿与书籍上下文。该SELF通过后再交非作者真实端门，不把代码EARLY称为用户可见完成。
...
- **当前可见入口 / revision**：主线事件基线为 `44078ed`；Web `127.0.0.1:4276` 与API `127.0.0.1:4212` 保持现有入口，默认responder仍fail-closed。Mini唯一常驻DevTools路径仍为 `/Users/echoman/.codex/worktrees/f312/SelfAlone/apps/miniapp`、当前HEAD `cbb63fa`，7项保护status不变；本事件未写f312或新增DevTools入口。
...
| ID | 状态 / 唯一 owner | 目标与非目标 / 交付结果 | 依赖 / 阻塞 / 暂停 | 文件 / 模块范围 | 验收 / 证据 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| `M1-F4-A` | `VERIFY` · `CURRENT-MAIN H3 PASS RECEIVED / LEDGER CLOSE PENDING` / 项目总控 | 替补非作者已在`main@44078ed`复核六文件相对`ed489a8`内容不变，并交真实Chrome注册→领取→焦点恢复→刷新隐藏及可区分DB grant完整性PASS；不含F4-B/C、模型或付费调用 | 候选回执已到，但本Mini PPT原子事件不顺带关闭Desktop包；下一Desktop控制事件由总控核对实际收据后回写最终状态 | 只读`conversation-chat-directory{,.test}.ts`、`conversation-chat-quota{,.test}.ts`、`conversation-chat.css`、`main.ts`与`main-quota-ed489a8/FINAL-RECEIPT.md` | 回执记录1370×642@DPR2、1302ms后textarea恢复、reload隐藏、console / overflow空及grants2 / owners2 / orphan0 / FK1；总控未复核前保持VERIFY，不扩大AI或成本闭环 |
...
| `M2-F6` 小程序 PPT | `VERIFY` · `DRAFT HANDOFF EARLY PASS / f312 PENDING / FULL FLOW BLOCKED` / 项目总控 | `c06ec53`定义的书籍→当前会话draft/context handoff已有`d8e1e7 + 49e0e5e`候选：携带书籍身份与可编辑预填，用户发送前不自动开sheet、不建task / requirements-ready或跳阶段；代码合同EARLY不代表真实端完成 | 非作者判`NO NEW CARD`且补充失败→刷新组合回归已交；下一步由总控顺序放入唯一f312。完整真实AI / PPTX仍依赖M2-F3、F4、M1-F5、下载域名与真机授权 | 先在唯一f312验当前会话保留、无会话回退、草稿 / 书籍上下文失败恢复、无auto-send / auto-sheet；再验后续四阶段、PPTX与iOS / Android，开发适配器不等于完成 |
...
| `M2-F6-MINI-PPT-DRAFT-HANDOFF` | `VERIFY` · `CANDIDATE d8e1e7 + 49e0e5e / EARLY PASS / f312 PENDING` / 项目总控 | writer已交六文件行为候选与一文件失败→刷新组合回归；非作者实际diff / 反例审查判预发送合同通过，且现有composer足够、`NO NEW CARD`，不把测试自报或无WXML变更冒充真实端完成 | cwd / branch / HEAD / 初始status、六文件ownership、5项首个RED、停止条件及`c06ec53 + 9967bb8` loaded ACK均已闭合；完整AI / PPTX与生产适配器不在本包 | `core/ppt-intent{,.test}.ts`、`pages/reader/index.ts` / `page-state.test.ts`、`pages/conversation/index.ts` / `index.test.ts`；候选未改WXML / WXSS / asset / config / lock / QA / f312 | writer：55 / 55 + Mini typecheck / build；补充后Conversation26 / 26、Mini16文件 / 125项、typecheck、diff-check。下一事件由总控审两个实际提交并顺序放入唯一f312做SELF，PASS后交非作者真实端门；当前不得标DONE |
...
| `M2-F0-A-MINI-DRAWER-VISUAL` | `VERIFY` · `CANDIDATE 1138669 / f312 PENDING` / 项目总控 | 原writer已交同一v5的asset composition + slot / mask最小候选，实际只改PNG、WXSS与test；多背景 / Alpha机械证据、Mini16文件 / 122项、typecheck / build绿，但未进入唯一f312，不是视觉PASS | 与PPT源码文件互斥，但两者共用唯一f312验收入口；按本事件顺序，先等PPT集成 / SELF释放入口，再处理Drawer。320失败即停，禁止机械v6 / ImageGen | `mobile-drawer-landscape-transparent-cropped-v5.png`、nav-drawer `index.wxss` / `index.test.ts`；WXML无diff，未写package / config / lock / QA / 台账 / f312 | 候选`1138669`、资产SHA`c0086fab…29fdb`；总控顺序进唯一f312后先320 SELF（第1/2行无景、第三行后显影、亭子可读、导航/点击不遮），PASS才扩360 / 390 / 430并交非作者EARLY |
