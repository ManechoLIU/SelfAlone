...
- 当前执行波次：`CONTROL-RESET-1`；Web `0e18846` 四阶段结构主干通过但候选仍 `FAIL / REWORK`：除阶段数据 / 失败动作外，当前可见 main 仍是旧 M0，6afd WIP 的 844 短高输入区、青瓷 / 纸白组件语言及首次 / 往返对话恢复一致性均未通过，当前禁止合并。D-TEXT 共享接缝已合入 main@`f85d86c`，Reader 私有标注 / 笔记候选正在隔离 TDD；小程序已冻结 `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307` 五个局部恢复点，最新包关闭 DevelopmentClient 内书籍制作 PPT 绕过会话的流程回归，但整体仍为 `FAIL / SYSTEM REWORK`。台账仍由项目总控独占。
- 当前活动项：原 Web 实现者 `/root/desktop_shell_m0_fix` 已重新触发但仍无文本 ACK，当前只记 `DISPATCHED / ACK PENDING`；原非作者 UI/UX 已 ACK 并在 4174 / 4101 对 844 短高输入、共享视觉语言和 fresh→library→返回 / 刷新 / 前后退 / `?stage=` / 断网缓存恢复做同包审查，4173 旧 main 已排除。`/root/reading_text_notes_web` 继续私有 Web TDD。小程序任务已提交 `cdaa307` flow-only 恢复点并恢复 PPT viewport WIP；两个禁止差异保持。
...
| `M2-F0-A` 原生运行壳与客户端状态 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` / `01a034e4-12d5-7b31-9104-4d7431c12110` | 六页、抽屉、共享输入 / 键盘层、底部面板与客户端五态；未就绪服务走显式开发适配器 | `f312@4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307` 已形成五个局部恢复点；`cdaa307` 仅 11 个 `apps/miniapp/**` 文件，9 文件 / 51 项、typecheck / build `1368 KiB` / diff-check 通过，320×568 DevTools 证明 Reader 先进入同一 Development conversation、重进恢复预选书且仅确认后进入 PPT | flow-only 包只关闭开发适配器内流程回归，不证明服务端 / 跨端会话、H3 或视觉通过；当前恢复 PPT viewport / 键盘 / 操作安全区 WIP。两个禁止差异和真实软件键盘 / 字体 / 游客 console / 资产门保持 |
...
| `M1-WEB-SHELL-01` | `FAIL / REWORK / IMPLEMENTATION ACK PENDING / UIUX ACTIVE ACKED` / `/root/desktop_shell_m0_fix` + `01a03652-a5d4-76c1-b0fa-ff9303243664` | 当前 4173 来自 main@`4a94b4f` 旧 M0；`6afd@0e18846` 加 WIP 在 4174 / 4101 运行。原 writer 未在限定窗口回 ACK，现场和唯一所有权保留，不另建重复任务 | UI/UX 已 ACK 同包覆盖 844 短高输入 / 密度 / 02 Token，以及 fresh #/conversation→library→返回、刷新、前后退、`?stage=`、断网缓存的一致 shell / stage / draft / scroll；核对 `location.reload()` 根因后给 route dispatcher brief。作者 ACK、brief 与新 HEAD 前禁止合并 |
...
| `M2-F0-A-SCREEN-ADAPT-ALL` | `ACTIVE / FIVE CHECKPOINTS COMMITTED / DESIGN-READY` / UI/UX + `01a034e4-12d5-7b31-9104-4d7431c12110` | 既有 f312 任务顺序独占 `apps/miniapp/**` 六页、drawer、输入 / 键盘与底部面板；台账只读，既有 `project.config.json` / 锁文件 diff 禁止触碰 | `4fc422f`、`630c051`、`5e270b1`、`5e0188d` 与 flow-only `cdaa307` 是五个局部恢复点；当前继续 PPT viewport / 键盘 / 操作安全区。全路由 / 状态矩阵前不合并 |
...
| `M2-F0-A` 当前候选 | `ff4767b → 142b6f2 → 91934f1 → 76227b9 → d90d0fb → d4f587c → 4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307`，状态 `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED / 不合并` | 五个局部点已冻结，`cdaa307` 只关闭 DevelopmentClient Reader→conversation→PPT intent 流程。PPT、其余状态 / 键盘 / 字体矩阵与最终视觉仍未完成；既有禁止差异保持未提交。 |
...
| `M2-F0-A` 原生运行壳 | `FAIL / SYSTEM REWORK / ACTIVE / H3 BLOCKED` | `4fc422f → 630c051 → 5e270b1 → 5e0188d → cdaa307` 是五个局部恢复点；全小程序六页 / 共享层四宽四高和状态矩阵、凭证化 clean console、山亭 / PPT 资产门通过前不合并 |
