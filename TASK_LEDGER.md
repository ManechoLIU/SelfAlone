...
- 当前阶段：桌面 Web `M1-F2-A` 功能、回归和主线集成收据继续有效；独立 UI/UX 首轮审查已以真实 `1440×844 @ DPR2` 页面判定 `FAIL`，范围内视觉 / 交互修正已交唯一实施任务。文本 / PDF 阅读共享接缝已在 `44b81ea` 冻结；B 的侧栏主题联动单独等待 UI/UX brief，其他 B、C 与小程序客户端工作继续并行。
...
- 当前执行波次：`M1-F2-A-VISUAL-REOPEN`、`M1-F2-B-TEXT-READER` 与 `M2-F0-MINIAPP-SHELL` 并行；C 安全首包已完成集成并停止原任务，后续生产包等待依赖授权和重新分配。三项活动任务文件范围互斥，共享 contracts / schema / 入口由项目总控单写入。
- 当前活动项：A 的唯一实施任务 `01a03515-113f-7ab2-bc87-38c14cd3a702` 在 `59ae` / `codex/m1-f2-a-visual-reopen` 从 `2298f63` 开始，独占书架 `main.ts` / `styles.css`、必要状态 / DOM 测试和 A 新证据。审查任务 `01a03500-6fc0-73f1-8d7a-5ea4b886f793` 现以只读方式形成 B 侧栏主题 brief，待 A 候选提交后仍由其复验 A。B 由任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` 在 `da89` 独占 `text-reader*`；M2-F0-A 由任务 `01a034e4-12d5-7b31-9104-4d7431c12110` 在 `f312` 独占 `apps/miniapp/**`。所有执行 / 审查任务均不得写 `TASK_LEDGER.md`。
...
| `M1-F2-B` | `ACTIVE` / 任务 `01a034f9-193d-7740-9f73-e5fa6b172fd4` | EPUB / TXT 正文、目录、复制、全书位置、专注模式和浅 / 深阅读背景的桌面阅读闭环 | `44b81ea`；解析定位或真实样本不成立时保持 `VERIFY`；`M1-F2-B-UX1` 只冻结侧栏主题视觉，不暂停其他阅读功能 | `da89` / `codex/m1-f2-b` 独占 `text-reader*` 的 DOMAIN、SERVER、WORKER、WEB、QA 与 B 证据；不写共享入口 / schema / `DESIGN-WEB` | 真实 EPUB / TXT；章节顺序、目录、复制、位置与偏好保存、刷新 / 重启恢复、键盘、五态、Web 4 视口；视觉结论由 `B-UX1` 单独给门 | B 保持当前候选继续非视觉收口；范围候选交总控审查，主题视觉差异不得先进入提交 |
...
| `M1-F2-B` 当前候选 | `da89@44b81ea`，状态 `ACTIVE / 五文件 WIP 冻结` | 原 staged 候选在主题 WIP 前通过定向 14 项、全仓 16 文件 / 64 项与 build；未暂存的 `text-reader-state.ts`、`text-reader-view.ts`、`text-reader.css`、`text-reader.ts`、`text-reader.test.ts` 同时混有未批准主题方案与非主题审查修正，均保持 `AM`、不回退也不提交。主题 TDD 实现前为 4 fail / 8 pass，部分实现后未复跑，不构成通过证据。 |
