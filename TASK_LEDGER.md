...
| `M1-F5-A1-REWORK-04` | `READY / GROK 4.6 OAUTH PREFLIGHT PASS / ACK PENDING` / 项目总控 | 基于 `main@a220c2f` 的单一隔离修复分支；owned files 仅 Server PPT workspace、M0 reset、对应测试与 A1 计划证据 | route=`grok-build/grok-4.6/oauth/high`，无调用预检已确认 CLI 与既有会话；先锁真实 RED，再最小修复三项 Important 及同范围版本自增、reset 外键、认证映射 Minor；候选提交后必须同案非作者复审，不得改 Web / Mini / TASK_LEDGER / 真实 Provider |
