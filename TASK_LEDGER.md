...
| `M1-F4-B-REWORK-03-REVIEW-TDD` | `READY / NONAUTHOR TDD CAUSAL REVIEW / Kimi K3 / candidate f47a990` | 高风险 `migration + cost` required review，`tdd_required=true`；必须绑定修复前同案 RED（direct terminal INSERT + reverse audit chronology）、精确 `f47a99044693210d738126aeffee8dd30675d3cc`、修复后同案 GREEN、`red_green_same_case=true`、至少一组 reviewer counterexample、`verdict=PASS|FAIL`；首个 repo tool 前必须 delivered ACK | 派 Kimi K3 独立 reviewer；不得修改 candidate；FAIL 返同一实现包，PASS 才进入 main integration |
